...
use crate::runtime_types::{ArrayElementType, ConcreteType, LatticeType, ValueType};
use crate::StructRegistry;
use subset_julia_vm_types::inference_core::CorePrimitive;
use subset_julia_vm_types::inference_core::{CoreAbstract, CoreType};
...
/// `Core(Primitive(p)) => value_type_from_core_primitive(p)`. The match is exhaustive over
...
pub fn value_type_from_core_primitive(primitive: &CorePrimitive) -> ValueType {
    match primitive {
...
pub fn value_type_to_lattice(value_type: &ValueType) -> LatticeType {
    match value_type {
        ...
        ValueType::Generator => LatticeType::Concrete(ConcreteType::generator(ConcreteType::Core(
            CoreType::Primitive(CorePrimitive::Float64),
        ))),
        ...
        ValueType::Union(types) => {
            ...
            let concrete_types: Vec<ConcreteType> = types
                .iter()
                .filter_map(|vt| match value_type_to_lattice(vt) {
        ...
        // Memory type: bare `Memory` has unknown element type (Any);
        // `MemoryOf(elem)` carries element-type information and maps to
        // the new `ConcreteType::Memory` lattice variant (Issue #9009).
        ValueType::Memory => LatticeType::Concrete(ConcreteType::Memory {
            element: Box::new(ConcreteType::Core(CoreType::Any)),
            ndims: None,
        }),
        ValueType::MemoryOf(elem) => LatticeType::Concrete(ConcreteType::Memory {
            element: Box::new(convert_array_element_type(elem)),
            ndims: None,
        }),
...
///
/// This function should be used instead of `value_type_to_lattice(&ValueType)` when
...
pub fn value_type_to_lattice_with_struct_table(
    ...
    match value_type {
        ...
        _ => value_type_to_lattice(value_type),
    }
}
...
pub fn lattice_to_value_type(lattice_type: &LatticeType) -> ValueType {
    match lattice_type {
        ...
        LatticeType::Const(cv) => {
            ...
            lattice_to_value_type(&LatticeType::Concrete(concrete))
        }

        LatticeType::Concrete(concrete) => match concrete {
            // Issue #6916: every nullary primitive routes through the
            // single-source-of-truth `value_type_from_core_primitive`
            ...
            ConcreteType::Core(CoreType::Primitive(p)) => value_type_from_core_primitive(p),
            ...
            // Memory types (Issue #9009): now tracked in the lattice.
            ...
            | ConcreteType::Core(CoreType::Abstract(CoreAbstract::AbstractFloat)) => ValueType::Any,

            // Union types (element type unions) - convert to ValueType::Union
            ConcreteType::UnionOf(types) => {
                let value_types: Vec<ValueType> = types
                    .iter()
                    .map(|ct| lattice_to_value_type(&LatticeType::Concrete(ct.clone())))
                    .collect();
        ...
        // PartialStruct — codegen sees the widened struct type; the
        // per-field facts are an inference-only refinement (Issue #8544).
        ps @ LatticeType::PartialStruct { .. } => lattice_to_value_type(&ps.widen_partial_struct()),
    }
}
...
fn convert_array_element_type(elem_type: &ArrayElementType) -> ConcreteType {
    match elem_type {
        ...
        ArrayElementType::StructInlineOf(type_id, _field_count)
        | ArrayElementType::StructInlineF64(type_id, _field_count) => ConcreteType::Struct {
            name: format!("Struct#{}", type_id),
            type_id: *type_id,
        },
        ...
        // Abstract/UnionAll logical tags are runtime type-object evidence, not
        // proof that an indexed value has one concrete optimizer type.
        ArrayElementType::Abstract(_) | ArrayElementType::Structured(_) => {
            ConcreteType::Core(CoreType::Any)
        }
...
fn convert_union_array_element_members(members: &[JuliaType]) -> ConcreteType {
    match subset_julia_vm_types::types::canonicalize_union(members.to_vec()) {
...
pub(crate) fn julia_type_to_concrete_type_lossy(ty: &JuliaType) -> ConcreteType {
    ...
    ConcreteType::from(&subset_julia_vm_types::inference_core::type_core::CoreType::from(ty))
}
...
pub fn julia_type_to_lattice_with_struct_resolver(
    ...
    match ty {
        ...
        // `Type{C}` over a concrete `C` (a type object used as a value, e.g. a
        // method returning a `where`-bound type parameter it was called with)
        // is the `Type{name}` singleton lattice element (Issue #9955). An
        // unbound `Type{T}` where `T` is itself a `where`-parameter (no
        // concrete substitution reached this conversion) carries no concrete
        // name and widens to `Top`, same as before.
        JuliaType::TypeOf(inner) if !matches!(inner.as_ref(), JuliaType::TypeVar(_, _)) => {
            let name = inner.name();
            if name.is_empty() {
                LatticeType::Top
            } else {
        ...
        // Everything else (unbound typevars, `Type{T}` still parameterized by
        // one, abstract families without a dedicated `ConcreteType` marker,
        // metaprogramming nodes) widens to `Top` to preserve dynamic-dispatch
        // compatibility.
        _ => LatticeType::Top,
    }
}
...
mod tests {
    use super::*;
    use crate::runtime_types::StructInfo;
    ...
    fn test_valuetype_to_latticetype_integers() {
        // Signed integers
        assert_eq!(
            value_type_to_lattice(&ValueType::I8),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(CorePrimitive::Int8)))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::I16),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Int16
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::I32),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Int32
            )))
        );
        ...
        assert_eq!(
            value_type_to_lattice(&ValueType::I128),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Int128
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::BigInt),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::BigInt
            )))
        );
        ...
        // Unsigned integers
        assert_eq!(
            value_type_to_lattice(&ValueType::U8),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::UInt8
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::U16),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::UInt16
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::U32),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::UInt32
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::U64),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::UInt64
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::U128),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::UInt128
            )))
        );
    ...
    fn test_valuetype_to_latticetype_floats() {
        assert_eq!(
            value_type_to_lattice(&ValueType::F32),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Float32
            )))
        );
        ...
        assert_eq!(
            value_type_to_lattice(&ValueType::BigFloat),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::BigFloat
            )))
        );
    ...
    fn test_issue_4270_union_array_element_type_is_preserved() {
        let result = value_type_to_lattice(&ValueType::ArrayOf(
    ...
    fn test_valuetype_to_latticetype_strings() {
        ...
        assert_eq!(
            value_type_to_lattice(&ValueType::Char),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(CorePrimitive::Char)))
        );
    }
    ...
    fn test_valuetype_to_latticetype_special() {
        assert_eq!(
            value_type_to_lattice(&ValueType::Nothing),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Nothing
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::Missing),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Missing
            )))
        );
        assert_eq!(
            value_type_to_lattice(&ValueType::Symbol),
            LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Symbol
            )))
        );
    ...
    fn test_valuetype_to_latticetype_arrays() {
        // Legacy array type
        assert_eq!(
            value_type_to_lattice(&ValueType::Array),
            LatticeType::Concrete(ConcreteType::Array {
                element: Box::new(ConcreteType::Core(CoreType::Any)),
                ndims: None
            })
        ...
        // Typed array
        assert_eq!(
            value_type_to_lattice(&ValueType::ArrayOf(ArrayElementType::I64, None)),
            LatticeType::Concrete(ConcreteType::Array {
                element: Box::new(ConcreteType::Core(CoreType::Primitive(
                    CorePrimitive::Int64
                ))),
        ...
        assert_eq!(
            value_type_to_lattice(&ValueType::ArrayOf(ArrayElementType::F32, None)),
            LatticeType::Concrete(ConcreteType::Array {
                element: Box::new(ConcreteType::Core(CoreType::Primitive(
                    CorePrimitive::Float32
                ))),
        ...
        assert_eq!(
            value_type_to_lattice(&ValueType::ArrayOf(ArrayElementType::Any, None)),
            LatticeType::Concrete(ConcreteType::Array {
                element: Box::new(ConcreteType::Core(CoreType::Any)),
                ndims: None
            })
    ...
    fn test_valuetype_to_latticetype_struct() {
        let type_id = 42;
        let result = value_type_to_lattice(&ValueType::Struct(type_id));
        assert!(
    ...
    fn test_valuetype_to_latticetype_top() {
        assert_eq!(value_type_to_lattice(&ValueType::Any), LatticeType::Top);
        // Range, Dict, Set now have concrete type representations
        assert!(matches!(
            value_type_to_lattice(&ValueType::Range),
            LatticeType::Concrete(ConcreteType::Range { .. })
        ));
        assert!(matches!(
            value_type_to_lattice(&ValueType::Dict),
            LatticeType::Concrete(ConcreteType::Dict { .. })
        ));
        assert!(matches!(
            value_type_to_lattice(&ValueType::Set),
    ...
    fn test_latticetype_to_valuetype_integers() {
        // Signed integers
        assert_eq!(
            lattice_to_value_type(&LatticeType::Concrete(ConcreteType::Core(
    ...
    fn test_latticetype_to_valuetype_special() {
        ...
        assert_eq!(
            lattice_to_value_type(&LatticeType::Concrete(ConcreteType::Core(CoreType::Any))),
            ValueType::Any
        );
    }
    ...
    fn test_latticetype_to_valuetype_arrays() {
        assert_eq!(
            lattice_to_value_type(&LatticeType::Concrete(ConcreteType::Array {
                element: Box::new(ConcreteType::Core(CoreType::Primitive(
                    CorePrimitive::Int64
                ))),
                ndims: None
    ...
    fn test_latticetype_to_valuetype_struct() {
        let type_id = 42;
        assert_eq!(
            lattice_to_value_type(&LatticeType::Concrete(ConcreteType::Struct {
                name: "Test".to_string(),
                type_id
            })),
            ValueType::Struct(type_id)
    ...
    fn test_latticetype_to_valuetype_top_bottom() {
        assert_eq!(lattice_to_value_type(&LatticeType::Top), ValueType::Any);
        ...
        assert_eq!(lattice_to_value_type(&LatticeType::Bottom), ValueType::Any);
    }
    ...
    fn test_empty_union_value_type_is_bottom_issue_5916() {
        ...
        assert_eq!(value_type_to_lattice(&empty_union), LatticeType::Bottom);
        ...
        assert_eq!(
            value_type_to_lattice(&lattice_to_value_type(&LatticeType::Top)),
            LatticeType::Top
        );
        ...
        assert_eq!(
            value_type_to_lattice(&lattice_to_value_type(&LatticeType::Bottom)),
            LatticeType::Top
        );
    }
    ...
    fn test_join_meet_laws_at_conversion_boundary_issue_5916() {
        let bottom_image = value_type_to_lattice(&ValueType::Union(Vec::new()));
        ...
        assert!(matches!(
            value_type_to_lattice(&union_vt),
            LatticeType::Union(_)
        ));
    }
    ...
    /// Issue #9914: a named `DataType` lattice value is a specific type object
    /// for reflection, even though the storage-level `ValueType` remains the
    /// generic `DataType`.
    #[test]
    fn lattice_to_parametric_julia_type_preserves_named_datatype_issue_9914() {
        assert_eq!(
            lattice_to_parametric_julia_type(&LatticeType::Concrete(ConcreteType::DataType {
    ...
    fn test_latticetype_to_valuetype_union() {
        ...
        let result = lattice_to_value_type(&LatticeType::Union(types));
        assert!(
    ...
    fn test_latticetype_to_valuetype_conditional() {
        ...
        assert_eq!(lattice_to_value_type(&conditional), ValueType::Any);
    }
    ...
    fn test_round_trip_basic_types() {
        ...
        for vt in value_types {
            let lattice = value_type_to_lattice(&vt);
            let back = lattice_to_value_type(&lattice);
    ...
    fn test_all_valuetype_variants_to_lattice() {
        ...
        for vt in &all_variants {
            let _lattice = value_type_to_lattice(vt);
        ...
        for vt in &all_variants {
            let lattice = value_type_to_lattice(vt);
            let _back = lattice_to_value_type(&lattice);
            // If this doesn't panic, the variant is properly handled
        }
    }

    #[test]
    fn test_memory_values_map_to_concrete_lattice_issue_9034() {
        use crate::ArrayElementType;

        // Issue #9034 (tracking option, PR #9052): `Memory{T}` is now a
        // `ConcreteType::Memory` lattice carrier mirroring `Array`, so Memory
        // values map to a concrete lattice value instead of widening to `Top`.
        // Bare `Memory` (unknown element type) carries `Any` as its element.
        assert_eq!(
            value_type_to_lattice(&ValueType::Memory),
            LatticeType::Concrete(ConcreteType::Memory {
                element: Box::new(ConcreteType::Core(CoreType::Any)),
                ndims: None,
            })
        );
        // `MemoryOf(I64)` carries the element type into the lattice carrier.
        assert!(matches!(
            value_type_to_lattice(&ValueType::MemoryOf(ArrayElementType::I64)),
            LatticeType::Concrete(ConcreteType::Memory { .. })
        ));
        // Round-trip preserves the element type via `MemoryOf`.
        assert_eq!(
            lattice_to_value_type(&value_type_to_lattice(&ValueType::MemoryOf(
    ...
    fn test_julia_type_to_lattice_struct_table_resolution_issue_5916() {
        ...
        let mut table: StructRegistry = StructRegistry::new();
        table.insert(
            "Foo".to_string(),
            StructInfo {
                type_id: 42,
    ...
    fn concrete_type_to_julia_type_routes_primitives_through_core_hub_issue_6916() {
        use subset_julia_vm_types::inference_core::core_type_to_julia_type;
    ...
    /// behaviour). The mapping is a bijection with `value_type_from_core_primitive`
    /// being
    ...
    fn value_type_from_core_primitive_is_reverse_bridge_source_of_truth_issue_6916() {
        ...
        for (p, expected) in cases {
            ...
            assert_eq!(
                value_type_from_core_primitive(&p),
                expected,
...
pub fn lattice_to_parametric_julia_type(lattice_type: &LatticeType) -> Option<JuliaType> {
    match lattice_type {
        ...
        LatticeType::Concrete(ConcreteType::DataType { name }) if !name.is_empty() => Some(
            JuliaType::TypeOf(Box::new(JuliaType::from_name_or_struct(name))),
        ),
        LatticeType::Concrete(ConcreteType::Struct { name, .. }) if name.contains('{') => {
            Some(JuliaType::from_name_or_struct(name))
        }
        // PartialStruct behaves exactly like its widened `Concrete(Struct)`
        // for parametric-name recovery (Issue #8544).
        ps @ LatticeType::PartialStruct { .. } => {
            lattice_to_parametric_julia_type(&ps.widen_partial_struct())
        }
...
pub fn lattice_to_julia_type(lattice_type: &LatticeType) -> JuliaType {
    match lattice_type {
        ...
        // PartialStruct widens to its struct type — dispatch and reflection
        // see the same `JuliaType` as `Concrete(Struct)` (Issue #8544).
        ps @ LatticeType::PartialStruct { .. } => lattice_to_julia_type(&ps.widen_partial_struct()),
        _ => JuliaType::Any,
    }
}
