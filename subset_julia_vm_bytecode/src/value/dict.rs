...
pub enum DictKey {
    ...
    /// WeakRef keys compare by WeakRef object identity, not by their current
    /// `.value` target. This supports `WeakKeyDict`'s `Dict{WeakRef,V}` backing
    /// table without making ordinary dict lookup weak-reference-aware.
    WeakRef(WeakRefDictKey),
}

/// Identity payload for a `Value::WeakRef` dictionary key.
#[derive(Debug, Clone)]
pub struct WeakRefDictKey {
    identity: usize,
    original: Box<Value>,
}

impl PartialEq for WeakRefDictKey {
    fn eq(&self, other: &Self) -> bool {
        self.identity == other.identity
    }
}

impl Eq for WeakRefDictKey {}

impl WeakRefDictKey {
    fn from_value(value: &Value) -> Option<Self> {
        let Value::WeakRef(cell) = value else {
            return None;
        };
        Some(Self {
            identity: std::rc::Rc::as_ptr(cell) as *const () as usize,
...
impl TypeDictKey {
    ...
    fn from_type_value(value: &Value) -> Option<Self> {
        use subset_julia_vm_types::inference_core::CoreType;
...
impl DictKey {
    pub fn from_value(v: &Value) -> Result<Self, VmError> {
        match v {
            ...
            Value::WeakRef(_) => WeakRefDictKey::from_value(v)
                .map(DictKey::WeakRef)
                .ok_or_else(|| VmError::InvalidDictKey(format!("{:?}", v))),
    ...
    pub fn to_value(&self) -> Value {
        match self {
            DictKey::Str(s) => Value::str_new(s.clone()),
            ...
            DictKey::WeakRef(w) => (*w.original).clone(),
        }
    }
...
impl DictKey {
    ...
    fn key_shape(&self) -> Option<KeyShape<'_>> {
        match self {
            ...
            DictKey::WeakRef(w) => Some(KeyShape::WeakRef(w.identity)),
            _ => None,
        }
    }
}
...
impl PartialEq for DictKey {
    fn eq(&self, other: &Self) -> bool {
        match (self, other) {
            ...
            (DictKey::WeakRef(a), DictKey::WeakRef(b)) => a == b,
            (DictKey::WeakRef(_), _) | (_, DictKey::WeakRef(_)) => false,
...
impl<'a> KeyRef<'a> {
    ...
    fn from_value(value: &'a Value) -> Option<Self> {
        match value {
            ...
            Value::WeakRef(_) => WeakRefDictKey::from_value(value).map(KeyRef::WeakRef),
            _ => None,
        }
    }
    ...
    fn hash_into<H: Hasher>(&self, state: &mut H) {
        match self {
            ...
            KeyRef::WeakRef(w) => hash_key_shape(&KeyShape::WeakRef(w.identity), state),
    ...
    fn matches_dict_key(&self, key: &DictKey) -> bool {
        match self {
            ...
            KeyRef::WeakRef(w) => matches!(key, DictKey::WeakRef(k) if k.identity == w.identity),
...
impl std::fmt::Display for DictKey {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            DictKey::WeakRef(_) => write!(f, "WeakRef"),
        }
    }
}
...
mod tests {
    ...
    fn test_dict_insert_and_get() {
        ...
        d.insert(DictKey::I64(42), Value::str_new("hello"));
        ...
        assert!(matches!(d.get(&DictKey::I64(42)), Some(Value::Str(s)) if s.as_ref() == "hello"));
    ...
    fn test_dict_contains_key() {
        let mut d = DictValue::new();
        d.insert(DictKey::I64(1), Value::str_new("one"));
    ...
    fn test_dict_keys_values() {
        let mut d = DictValue::new();
        d.insert(DictKey::I64(1), Value::str_new("a"));
        d.insert(DictKey::I64(2), Value::str_new("b"));
    ...
    fn test_keyref_hash_parity_with_dictkey_issue_5187() {
        let cases: Vec<(DictKey, Value)> = vec![
            (DictKey::Str("hello".into()), Value::str_new("hello")),
            (DictKey::Str(String::new()), Value::str_new(String::new())),
            (
    ...
    fn test_dict_get_by_value_parity_issue_5187() {
        ...
        for i in 0..50 {
            let probe = Value::str_new(format!("k{i}"));
        ...
        // Misses
        assert!(d.get_by_value(&Value::str_new("missing")).is_none());
        assert!(!d.contains_key_by_value(&Value::str_new("missing")));
        assert!(d
        ...
        assert!(d2.get_by_value(&Value::str_new("x")).is_none());
