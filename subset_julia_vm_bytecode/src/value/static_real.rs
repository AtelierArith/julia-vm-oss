...
impl StaticArrayInlineData {
    ...
    pub fn julia_type_name_static(&self) -> &'static str {
        ...
        match (rows, cols, et) {
            ...
            // Issue #11432: SMatrix carries the upstream fourth length
            // parameter L == rows*cols.
            (2, 2, "Float64") => "SMatrix{2, 2, Float64, 4}",
            (3, 3, "Float64") => "SMatrix{3, 3, Float64, 9}",
            (4, 4, "Float64") => "SMatrix{4, 4, Float64, 16}",
            (2, 3, "Float64") => "SMatrix{2, 3, Float64, 6}",
            (3, 2, "Float64") => "SMatrix{3, 2, Float64, 6}",
            (2, 2, "Float32") => "SMatrix{2, 2, Float32, 4}",
            (3, 3, "Float32") => "SMatrix{3, 3, Float32, 9}",
            (2, 2, "Int64") => "SMatrix{2, 2, Int64, 4}",
            (3, 3, "Int64") => "SMatrix{3, 3, Int64, 9}",
    ...
    pub fn julia_type_name_owned(&self) -> Box<str> {
        ...
        if !s.is_empty() {
            ...
            if self.is_vector() {
                ...
                // Issue #11432: report the upstream fourth length parameter.
                format!(
                    "SMatrix{{{}, {}, {}, {}}}",
                    self.rows,
                    self.cols,
    ...
    /// Materialise the inline carrier as the heap-backed static-array carrier.
    ///
    /// Arithmetic can legitimately mix the two representations: a vector of
    /// up to four elements is inline while a 3×3 or 4×4 matrix is heap-backed.
    /// Keeping this conversion on the representation itself lets those mixed
    /// operations reuse the same typed kernels without widening the result to
    /// an ordinary dynamic `Vector` (Issue #11240).
    pub fn to_static_real_value(&self) -> StaticRealValue {
        let n = self.n as usize;
        let elems = match self.tag {
            InlineElemTag::F64 => StaticElem::F64(
                self.data[..n]
                    .iter()
                    .map(|raw| f64::from_bits(*raw))
                    .collect(),
            ),
            InlineElemTag::F32 => StaticElem::F32(
                self.data[..n]
                    .iter()
            ...
            InlineElemTag::I64 => {
                StaticElem::I64(self.data[..n].iter().map(|raw| *raw as i64).collect())
            }
            InlineElemTag::I32 => {
                StaticElem::I32(self.data[..n].iter().map(|raw| *raw as i32).collect())
            }
            InlineElemTag::I16 => {
                StaticElem::I16(self.data[..n].iter().map(|raw| *raw as i16).collect())
            }
            InlineElemTag::I8 => {
                StaticElem::I8(self.data[..n].iter().map(|raw| *raw as i8).collect())
            }
            InlineElemTag::U64 => StaticElem::U64(self.data[..n].to_vec()),
            InlineElemTag::U32 => {
                StaticElem::U32(self.data[..n].iter().map(|raw| *raw as u32).collect())
            }
            InlineElemTag::U16 => {
                StaticElem::U16(self.data[..n].iter().map(|raw| *raw as u16).collect())
            }
            InlineElemTag::U8 => {
                StaticElem::U8(self.data[..n].iter().map(|raw| *raw as u8).collect())
            }
            InlineElemTag::Bool => {
                StaticElem::Bool(self.data[..n].iter().map(|raw| *raw != 0).collect())
            }
        };
        let type_name = self.julia_type_name_owned();
        if self.is_vector() {
            StaticRealValue::new_vector(type_name, elems)
        } else {
            StaticRealValue::new_matrix(type_name, self.rows(), self.cols(), elems)
        }
    }
...
/// `"SMatrix{2, 2, Float64, 4}"` → `"Float64"` (Issue #11432: the trailing
/// `L` length parameter is a plain integer, so skip trailing comma-separated
/// segments that parse as a number and return the first one that doesn't —
/// that generically covers both SVector's `T`-last shape and SMatrix's
/// `T`-then-`L` shape without hardcoding either arity).
fn elem_type_str(type_name: &str) -> &str {
    for candidate in type_name.rsplit(", ") {
...
fn smatrix_type_name(rows: usize, cols: usize, et: &str) -> Box<str> {
    // Issue #11432: SMatrix carries the upstream fourth length parameter
    // L == rows*cols.
    let s: &str = match (rows, cols, et) {
        ...
        (2, 2, "Int64") => "SMatrix{2, 2, Int64, 4}",
        (3, 3, "Int64") => "SMatrix{3, 3, Int64, 9}",
        _ => return format!("SMatrix{{{}, {}, {}, {}}}", rows, cols, et, rows * cols).into(),
    };
    Box::from(s)
}
