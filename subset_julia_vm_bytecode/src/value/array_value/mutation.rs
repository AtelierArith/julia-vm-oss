...
pub(crate) fn real_scalar_as_f64(value: &Value) -> Option<f64> {
    match value {
        Value::F64(x) => Some(*x),
    ...
    if let Some(elem_type) = element_type_override {
        match elem_type {
            ArrayElementType::ComplexF64 => {
                ...
                if let ArrayData::StructF64(v) = data {
                    v.push(re);
                    v.push(im);
                    return Ok(v.len() / 2);
                }
            ...
            ArrayElementType::StructInlineF64(_, field_count) => {
                // Pack the all-`Float64` isbits struct's fields as raw f64 into
                // the contiguous StructF64 buffer (Issue #9198 S4).
                let fields = match &value {
                    Value::Struct(s) if s.values.len() == *field_count => &s.values,
                    _ => {
                        return Err(VmError::TypeError(format!(
                            "Cannot push {:?} to isbits struct array (expected {}-field struct)",
                            value.value_type(),
                ...
                let mut packed = Vec::with_capacity(*field_count);
                for field in fields {
                    match real_scalar_as_f64(field) {
                        Some(x) => packed.push(x),
                        None => {
                            return Err(VmError::TypeError(format!(
                                "Cannot push {:?} field to Float64 isbits struct array",
                                field.value_type()
                            )))
                        }
                    }
                ...
                return Err(VmError::TypeError(
                    "Invalid array data for Float64 isbits struct array".to_string(),
                ));
            }
...
impl ArrayValue {
    ...
    pub fn set_linear_inbounds(&mut self, linear: usize, value: Value) -> Result<(), VmError> {
        ...
        if let Some(ref elem_type) = self.element_type_override {
            match elem_type {
                ArrayElementType::ComplexF64 => {
                    ...
                    // Write to interleaved storage (Issue #9198 S5: contiguous
                    // `StructF64`, shared with user 2×f64 isbits structs).
                ...
                ArrayElementType::StructInlineF64(_, field_count) => {
                    // Pack the all-`Float64` isbits struct's fields into the
                    // contiguous StructF64 buffer at `linear` (Issue #9198 S4).
                        ...
                        Value::Struct(s) if s.values.len() == field_count => {
                            ...
                            for field in &s.values {
                                match real_scalar_as_f64(field) {
                                    ...
                                    None => {
                                        return Err(VmError::TypeError(format!(
                                        "Cannot store {:?} field in Float64 isbits struct array",
                                        field.value_type()
                                    )))
                                    }
                                }
                    ...
                    if let ArrayData::StructF64(v) = &mut self.data {
                        if raw_idx + field_count <= v.len() {
                            v[raw_idx..raw_idx + field_count].copy_from_slice(&packed);
                            return Ok(());
                        }
                    }
                    return Err(VmError::IndexOutOfBounds {
        ...
        if let Some(ref elem_type) = self.element_type_override {
            match elem_type {
                ArrayElementType::ComplexF64 => {
                    ...
                    // Issue #9198 S5: contiguous `StructF64` interleaved buffer.
                    if let ArrayData::StructF64(v) = &mut self.data {
                        if v.len() >= 2 {
                ...
                ArrayElementType::StructInlineF64(type_id, field_count) => {
                    // Pop field_count raw f64 slots and reconstruct the
                    // all-`Float64` isbits struct (Issue #9198 S4).
                    if let ArrayData::StructF64(v) = &mut self.data {
                        if v.len() >= *field_count {
                            let mut fields = Vec::with_capacity(*field_count);
                            for _ in 0..*field_count {
                                fields.push(Value::F64(v.pop().ok_or(VmError::EmptyArrayPop)?));
                            }
                            fields.reverse();
                            self.shape[0] = v.len() / field_count;
                            return Ok(Value::Struct(StructInstance::new(*type_id, fields)));
    ...
    pub fn push_first(&mut self, value: Value) -> Result<(), VmError> {
        ...
        match (&mut self.data, value) {
            ...
            (ArrayData::String(v), val @ (Value::Str(_) | Value::StrBytes(_))) => v.insert(0, val),
    ...
    pub fn pop_first(&mut self) -> Result<Value, VmError> {
        ...
        let val = match &mut self.data {
            ...
            ArrayData::String(v) => v.remove(0),
            ArrayData::Char(v) => Value::Char(v.remove(0)),
            ArrayData::StructRefs(v) => Value::StructRef(v.remove(0)),
            ArrayData::Any(v) => v.remove(0),
            // Raw-slot removal, matching the boxed-AoS `Any` path above; whole
            // struct-element `pushfirst!`/`popfirst!` grouping is out of scope
            // for the inline-f64 array (Issue #9198 S4).
            ArrayData::StructF64(v) => Value::F64(v.remove(0)),
        };
        self.shape[0] = self.data.raw_len();
        Ok(val)
    }
    ...
    pub fn insert_at(&mut self, index: usize, value: Value) -> Result<(), VmError> {
        ...
        match (&mut self.data, value) {
            ...
            (ArrayData::String(v), val @ (Value::Str(_) | Value::StrBytes(_))) => {
                v.insert(rust_index, val)
            }
    ...
    pub fn delete_at(&mut self, index: usize) -> Result<Value, VmError> {
        ...
        let val = match &mut self.data {
            ...
            ArrayData::String(v) => v.remove(rust_index),
            ...
            // Raw-slot removal, matching the boxed-AoS `Any` path (Issue #9198 S4).
            ArrayData::StructF64(v) => Value::F64(v.remove(rust_index)),
        };
        self.shape[0] = self.data.raw_len();
        Ok(val)
    }
    ...
    fn set_complex_linear_value(
        ...
        if let Some(ref elem_type) = self.element_type_override {
            match elem_type {
                ArrayElementType::ComplexF64 => {
                    // Issue #9198 S5: contiguous `StructF64` interleaved buffer.
                    if let ArrayData::StructF64(ref mut data) = self.data {
                        let offset = linear * 2;
