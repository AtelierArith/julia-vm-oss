...
use crate::vm::value::{ArrayElementType, ArrayRef, IORef, Value};

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum RedirectStreamKind {
    Stdout,
    Stderr,
}
...
pub(crate) struct RedirectState {
    pub kind: RedirectStreamKind,
    pub old_stream: IORef,
...
pub(crate) struct BroadcastState {
    ...
    /// For a runtime-callable `FilterMap` (Issue #9271): the map callable value,
    /// paired with `runtime_callable` holding the predicate. When set, the
    /// FilterMap driver calls these via `call_runtime_callable_value` instead of
    /// the `func_index` / `reduce_func_index` function-table entries. `None` for
    /// the function-index FilterMap and every other op-kind.
    pub runtime_reduce_callable: Option<Value>,
    /// Statically-inferred element type of the collected result (Issue #9127).
    /// Threaded through so a filtered generator whose predicate removes *every*
    /// element at runtime (`collect(x^2 for x in 1:4 if x > 100)`) still reports
    /// the inferred `eltype` (`Int64[]`) instead of collapsing to `Union{}[]`.
    /// Only the `FilterMap` op-kind populates it; other op-kinds leave it `None`.
...
pub(crate) enum GeneratorIterateKind {
    ...
    /// Runtime-callable analogue of `FilterPredicate` (Issue #9271): the map and
    /// predicate are callable `Value`s (function/closure) driven via
    /// `call_runtime_callable_value`, so a function-scope / capturing lifted
    /// `__gen_body_N` / `__gen_pred_N` iterates lazily too.
    FilterPredicateRuntime {
        map: Value,
        predicate: Value,
        iter: Value,
        input_value: Value,
