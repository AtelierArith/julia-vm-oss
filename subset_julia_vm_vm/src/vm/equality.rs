...
fn range_struct_element_type(name: &str) -> value::RangeElementType {
    let params = name
        .split_once('{')
        .map(|(_, rest)| rest)
        .unwrap_or_default();
    if params.contains("BigInt") {
        value::RangeElementType::BigInt
    } else if params.contains("Char") {
        value::RangeElementType::Char
    } else if params.contains("UInt8") {
        value::RangeElementType::UInt8
    } else if params.contains("UInt16") {
        value::RangeElementType::UInt16
    } else if params.contains("UInt32") {
        value::RangeElementType::UInt32
    } else if params.contains("UInt64") {
        value::RangeElementType::UInt64
    } else if params.contains("Int8") {
        value::RangeElementType::Int8
    } else if params.contains("Int16") {
        value::RangeElementType::Int16
    } else if params.contains("Int32") {
        value::RangeElementType::Int32
    } else {
        value::RangeElementType::Default
    }
}

fn range_struct_step_type(name: &str, step: Option<&Value>) -> value::RangeElementType {
    let params = name
        .split_once('{')
        .map(|(_, rest)| rest)
        .unwrap_or_default();
    if let Some((_, step_param)) = params.split_once(',') {
        return range_struct_element_type(step_param.trim_end_matches('}').trim());
    }
    match step {
        Some(Value::BigInt(_)) => value::RangeElementType::BigInt,
        Some(Value::Char(_)) => value::RangeElementType::Char,
        Some(Value::U8(_)) => value::RangeElementType::UInt8,
        Some(Value::U16(_)) => value::RangeElementType::UInt16,
        Some(Value::U32(_)) => value::RangeElementType::UInt32,
        Some(Value::U64(_)) => value::RangeElementType::UInt64,
        Some(Value::I8(_)) => value::RangeElementType::Int8,
        Some(Value::I16(_)) => value::RangeElementType::Int16,
        Some(Value::I32(_)) => value::RangeElementType::Int32,
        _ => value::RangeElementType::Default,
    }
}

fn range_struct_value_to_bigint(value: &Value) -> Option<value::RustBigInt> {
    match value {
        Value::BigInt(v) => Some(v.clone()),
        Value::Char(c) => Some(value::RustBigInt::from(i64::from(u32::from(*c)))),
        Value::I8(v) => Some(value::RustBigInt::from(i64::from(*v))),
        Value::I16(v) => Some(value::RustBigInt::from(i64::from(*v))),
        Value::I32(v) => Some(value::RustBigInt::from(i64::from(*v))),
        Value::I64(v) => Some(value::RustBigInt::from(*v)),
        Value::U8(v) => Some(value::RustBigInt::from(u64::from(*v))),
        Value::U16(v) => Some(value::RustBigInt::from(u64::from(*v))),
        Value::U32(v) => Some(value::RustBigInt::from(u64::from(*v))),
        Value::U64(v) => Some(value::RustBigInt::from(*v)),
        _ => None,
    }
}
...
fn struct_instance_as_range_value(instance: &StructInstance) -> Option<RangeValue> {
    ...
    if !matches!(base, "UnitRange" | "StepRange") {
        return None;
    }

    let is_step_range = base == "StepRange";
    let unit_step = Value::I64(1);
    let start = instance.values.first()?;
    let step = if is_step_range {
        instance.values.get(1)?
    } else {
        &unit_step
    };
    let stop = if is_step_range {
    ...
    if matches!(element_type, value::RangeElementType::BigInt)
        || matches!(step_type, value::RangeElementType::BigInt)
    {
...
impl<R: RngLike> Vm<R> {
    ...
    pub(super) fn compare_struct_fields(&self, left: &Value, right: &Value) -> bool {
        ...
        if let (Some(left_range), Some(right_range)) = (
            self.value_as_struct_backed_range(left),
            self.value_as_struct_backed_range(right),
        ) {
            return left_range.elements_equal(&right_range);
        }
    ...
    fn value_as_struct_backed_range(&self, value: &Value) -> Option<RangeValue> {
        match value {
    ...
    pub(super) fn compare_memory_values_with_shape_isequal(
        &self,
        left_mem: &MemoryRef,
        left_shape: &[usize],
        right_mem: &MemoryRef,
        ...
        left.len() == right.len()
            && (0..left.len()).all(|i| {
                self.compare_boundary_values_isequal(
                    left.data.get_value(i),
                    right.data.get_value(i),
                )
            })
    }

    fn compare_array_values_logical_with_mode(
        &self,
        left: &ArrayValue,
        right: &ArrayValue,
        mode: BoundaryEqualityMode,
    ) -> bool {
        ...
        left.shape == right.shape
            ...
            && left_values
                ...
                .zip(right_values)
                .all(|(left, right)| {
                    self.compare_boundary_values_with_mode(Some(left), Some(right), mode)
                })
    }
    ...
    fn compare_struct_like_values_with_mode(
        &self,
        left: &Value,
        right: &Value,
        mode: BoundaryEqualityMode,
            ...
            (Some((ln, lv)), Some((rn, rv))) => {
                crate::vm::type_utils::normalize_struct_name(&ln)
                    ...
                    && lv.iter().zip(rv.iter()).all(|(xv, yv)| {
                        self.compare_boundary_values_with_mode(
                            Some(xv.clone()),
                            Some(yv.clone()),
                            mode,
                        )
    ...
    pub(super) fn compare_boundary_values_equal(
        ...
        self.compare_boundary_values_with_mode(left, right, BoundaryEqualityMode::Equal)
    }

    pub(super) fn compare_boundary_values_isequal(
        &self,
        left: Option<Value>,
        right: Option<Value>,
    ) -> bool {
        self.compare_boundary_values_with_mode(left, right, BoundaryEqualityMode::Isequal)
    }

    fn compare_boundary_values_with_mode(
        &self,
        left: Option<Value>,
        right: Option<Value>,
        mode: BoundaryEqualityMode,
        ...
        match (left, right) {
            ...
            (Some(ref x), Some(ref y))
                if matches!(mode, BoundaryEqualityMode::Equal)
                    && super::numeric_identity::mixed_bigint_values_equal(x, y).is_some() =>
            {
                super::numeric_identity::mixed_bigint_values_equal(x, y).unwrap_or(false)
            }
            (Some(ref x), Some(ref y))
                if matches!(mode, BoundaryEqualityMode::Isequal)
                    && super::numeric_identity::mixed_bigint_values_isequal(x, y).is_some() =>
            {
                super::numeric_identity::mixed_bigint_values_isequal(x, y).unwrap_or(false)
            }
            (Some(Value::I64(x)), Some(Value::I64(y))) => x == y,
            (Some(Value::F16(x)), Some(Value::F16(y))) => match mode {
                BoundaryEqualityMode::Equal => x == y,
                BoundaryEqualityMode::Isequal => {
                    x.to_bits() == y.to_bits() || (x.is_nan() && y.is_nan())
                }
            },
            (Some(Value::F32(x)), Some(Value::F32(y))) => match mode {
                BoundaryEqualityMode::Equal => x == y,
                BoundaryEqualityMode::Isequal => {
                    x.to_bits() == y.to_bits() || (x.is_nan() && y.is_nan())
                }
            },
            (Some(Value::F64(x)), Some(Value::F64(y))) => match mode {
                BoundaryEqualityMode::Equal => x == y,
                BoundaryEqualityMode::Isequal => {
                    x.to_bits() == y.to_bits() || (x.is_nan() && y.is_nan())
                }
                ...
                if super::numeric_identity::mixed_int_float_values_equal(x, y).is_some() =>
            {
                ...
                match mode {
                    BoundaryEqualityMode::Equal => equal,
                    BoundaryEqualityMode::Isequal => {
                        equal
                            && !super::numeric_identity::is_negative_zero_fixed_float(x)
                            && !super::numeric_identity::is_negative_zero_fixed_float(y)
                    }
                }
            }
            ...
            {
                self.compare_struct_like_values_with_mode(x, y, mode)
            }
            (Some(x), Some(y)) => {
                if let (Some(left_range), Some(right_range)) = (
                    self.value_as_struct_backed_range(&x),
                    self.value_as_struct_backed_range(&y),
                ) {
                    left_range.elements_equal(&right_range)
                } else {
                    format!("{:?}", x) == format!("{:?}", y)
                }
            }
    ...
    pub(super) fn compare_array_wrapper_boundary_values_equal(
        ...
        self.compare_array_wrapper_boundary_values_with_mode(
            left,
            right,
            BoundaryEqualityMode::Equal,
        )
    }

    pub(super) fn compare_array_wrapper_boundary_values_isequal(
        &self,
        left: &Value,
        right: &Value,
    ) -> Option<bool> {
    ...
    fn compare_array_wrapper_boundary_values_with_mode(
        &self,
        left: &Value,
        right: &Value,
        mode: BoundaryEqualityMode,
        ...
        match (left_array_for_equality, right_array_for_equality) {
            (Some(left_arr), Some(right_arr)) => {
                return Some(
                    self.compare_array_values_logical_with_mode(&left_arr, &right_arr, mode),
                );
            }
            (Some(left_arr), None) => {
                if let Value::Memory(right_mem) = right {
                    let right_len = right_mem.borrow().len();
                    return Some(self.compare_array_values_logical_with_mode(
                        &left_arr,
        ...
        match (left_wrapper, right_wrapper) {
            (Some((left_mem, left_shape)), Some((right_mem, right_shape))) => match mode {
                BoundaryEqualityMode::Equal => Some(self.compare_memory_values_with_shape(
                    &left_mem,
                    &left_shape,
                    &right_mem,
                    &right_shape,
                ...
                BoundaryEqualityMode::Isequal => {
                    Some(self.compare_memory_values_with_shape_isequal(
                        &left_mem,
                        &left_shape,
                        &right_mem,
                        &right_shape,
            ...
            (Some((mem, shape)), None) => {
                self.compare_array_wrapper_against_other_with_mode(&mem, &shape, right, mode)
            }
            (None, Some((mem, shape))) => {
                self.compare_array_wrapper_against_other_with_mode(&mem, &shape, left, mode)
            }
            _ => None,
        }
    }
    ...
    fn compare_array_wrapper_against_other_with_mode(
        &self,
