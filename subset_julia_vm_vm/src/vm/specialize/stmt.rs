...
use super::{
    CallableRegistry, FunctionSpecializer, SpecializationError, SpecializationRecursionGuard,
};
...
impl<'a> FunctionSpecializer<'a> {
    ...
    pub(super) fn new(
        ...
        recursion_guard: &'a std::cell::RefCell<SpecializationRecursionGuard>,
    ) -> Self {
        Self {
            code: Vec::new(),
            locals,
            current_return_type: ValueType::Nothing,
            return_type_seen: false,
            return_type_conflict: false,
    ...
    pub(super) fn new_for_tests(
        ...
        // Each test gets its own leaked (test-only, never freed) empty
        // registry and recursion guard: `RefCell` is not `Sync`, so a single
        // shared `static` cannot hold one, and tests may run concurrently.
        let registry: &'static CallableRegistry = Box::leak(Box::new(CallableRegistry::new()));
        let guard: &'static std::cell::RefCell<SpecializationRecursionGuard> = Box::leak(Box::new(
            std::cell::RefCell::new(SpecializationRecursionGuard::new()),
        ));
        Self::new(
            locals,
            struct_defs,
    ...
    /// Record one observed return-value type for the function being specialized.
    ///
    /// `current_return_type` is LAST-WRITE-WINS: a body with several return
    /// sites of different types (`x > 0 ? x * 2 : "neg"`) ends up reporting
    /// only the type of the *last* one compiled. That was harmless while the
    /// reported type merely annotated the installed body (each return site
    /// emits its OWN typed `Return*`), but Issue #10749 makes a CALLER trust
    /// this type to type its own downstream instructions — so a caller could
    /// treat a value that is sometimes an `Int64` as if it were always a
    /// `String`. `return_type_conflict` records that heterogeneity so
    /// `SpecializationResult::return_type_consistent` can tell a cross-function
    /// caller "do not trust this", and that call falls back to the generic path.
    ///
    /// Kept as an additive flag (rather than widening `current_return_type` to
    /// `Any` on conflict) so the pre-existing single-function consumers of the
    /// reported type keep byte-identical behavior.
    pub(super) fn record_return_type(&mut self, ty: ValueType) {
        if self.return_type_seen && self.current_return_type != ty {
            self.return_type_conflict = true;
        }
        self.return_type_seen = true;
    ...
    pub(super) fn compile_function_body(
        ...
        if stmts.is_empty() {
            ...
            self.record_return_type(ValueType::Nothing);
        ...
        match last_stmt {
            Stmt::Return {
                ...
                self.record_return_type(ty.clone());
                self.emit_return(ty);
            }
            ...
            Stmt::IndexAssign { .. } | Stmt::FieldAssign { .. } => {
                // Same rule as `Stmt::IndexAssign`/`Stmt::FieldAssign` in
                // `compile_block_with_implicit_return` below — see that arm
                // for the full explanation (Issue #10431).
                self.compile_assign_stmt_tail_via_temp(last_stmt)?;
            }
            Stmt::DestructuringAssign { targets, value, .. } => {
                self.compile_destructuring_assign(targets, value, true)?;
            }
            _ => {
    ...
    pub(super) fn compile_block_with_implicit_return(
        ...
        match last_stmt {
            ...
            Stmt::IndexAssign { .. } | Stmt::FieldAssign { .. } => {
                // Julia: `v[i] = x` / `obj.field = x` are each an expression
                // whose value is the assigned (RHS) value, matching the
                // interpreter/legacy-compiler fix for the same statement
                // shapes (Issue #10431). Without this arm, the specializer's
                // `_` catch-all below forced a `Nothing` return even when the
                // enclosing function's declared return type (computed by the
                // now-fixed abstract-interp inference) was the RHS's real
                // type — a declared-type vs. specialized-body mismatch that
                // crashes the caller's type-specific instruction (e.g.
                // `PrintI64NoNewline` fed a `Nothing`). `Stmt::DictAssign`
                // has no arm here because this specializer's `compile_stmt`
                // does not support it at all (falls to
                // `SpecializationError`, which the caller of
                // `CallSpecialize` handles by falling back to the legacy
                // compiled path).
                self.compile_assign_stmt_tail_via_temp(last_stmt)?;
            }
            Stmt::DestructuringAssign { targets, value, .. } => {
    ...
    /// Compile an indexed/field assignment (`Stmt::IndexAssign`/
    /// `Stmt::FieldAssign`) sitting in tail position, then return the value
    /// that was assigned. Reuses `split_assign_stmt_via_temp` (shared with
    /// the lowering-layer/legacy-compiler fix, Issue #10431): bind the RHS to
    /// a fresh compiler-internal temporary, perform the store using the
    /// temporary, then reload and return the temporary's value.
    fn compile_assign_stmt_tail_via_temp(
        &mut self,
        last_stmt: &Stmt,
    ) -> Result<(), SpecializationError> {
        match crate::lowering::expr::split_assign_stmt_via_temp(last_stmt.clone()) {
            Some((tmp, init, store)) => {
                self.compile_stmt(&init)?;
                self.compile_stmt(&store)?;
                let ty = self.compile_var(&tmp)?;
                self.record_return_type(ty.clone());
                self.emit_return(ty);
            }
            None => {
                // Unreachable given both call sites only match
                // IndexAssign/FieldAssign, which `split_assign_stmt_via_temp`
                // handles; stay defensive rather than panicking.
                self.compile_stmt(last_stmt)?;
                self.emit(Instr::PushNothing);
                self.record_return_type(ValueType::Nothing);
                self.emit(Instr::ReturnNothing);
    ...
    /// Compile explicit flat-identifier destructuring IR. The literal path
    /// retains its per-element typed slots; a nonliteral RHS is evaluated once
    /// into one tuple slot and that same tuple is returned in tail position.
    fn compile_destructuring_assign(
        &mut self,
        targets: &[String],
        value: &Expr,
        tail: bool,
    ) -> Result<(), SpecializationError> {
        let Expr::TupleLiteral { elements, .. } = value else {
            let ty = self.compile_expr(value)?;
            if ty != ValueType::Tuple {
                return Err(SpecializationError::Unsupported(
                    "Destructuring assignment requires a tuple".to_string(),
                ));
            }
            let temp = format!("#destructure_tuple_{}", self.code.len());
            self.locals.insert(temp.clone(), ValueType::Tuple);
            ...
            if tail {
                ...
                self.record_return_type(ValueType::Tuple);
                self.emit_return(ValueType::Tuple);
            }
            return Ok(());
        };
        if targets.len() != elements.len() {
            return Err(SpecializationError::Unsupported(
                "Destructuring assignment arity mismatch".to_string(),
            ));
        }

        let id = self.code.len();
        let mut temps = Vec::with_capacity(elements.len());
        for (index, element) in elements.iter().enumerate() {
            let temp = format!("#destructure_{id}_{index}");
            let ty = self.compile_expr(element)?;
            self.locals.insert(temp.clone(), ty.clone());
            self.emit_store(&temp, ty.clone());
            temps.push((temp, ty));
        }
        for (target, (temp, ty)) in targets.iter().zip(&temps) {
            self.compile_var(temp)?;
            self.locals.insert(target.clone(), ty.clone());
            self.emit_store(target, ty.clone());
        }
        ...
        if tail {
            ...
            self.emit(Instr::NewTuple(temps.len()));
            self.record_return_type(ValueType::Tuple);
            self.emit_return(ValueType::Tuple);
        }
        Ok(())
    ...
    pub(super) fn compile_stmt(&mut self, stmt: &Stmt) -> Result<(), SpecializationError> {
        match stmt {
            ...
            Stmt::DestructuringAssign { targets, value, .. } => {
                self.compile_destructuring_assign(targets, value, false)?;
            }
            Stmt::Return {
                value: Some(expr), ..
            } => {
            ...
            Stmt::ForEach {
                ...
                // The counted specializer is valid only for proven indexable
                // carriers. Generator and struct/unknown iterators use Julia's
                // `iterate` protocol; compiling them as length + IndexLoad can
                // freeze the first call's indexability assumption onto a later
                // wrapper such as Iterators.Flatten (Issue #9403).
                if self.iterable_requires_iterate_protocol(iterable) {
                    return Err(SpecializationError::Unsupported(
                        "ForEach over a protocol iterator falls back to iterate (Issues #9353/#9403)"
                            .to_string(),
                    ));
                }
                // `compile_foreach` emits a counted `length(itr)` + `itr[idx]`
                // loop and stores the length in an I64 slot. BigInt-backed
                // range structs intentionally report BigInt lengths, so the
                // counted-loop specialization rejects otherwise-valid
                // `UnitRange{BigInt}` iteration with "expected I64, got BigInt".
                // Fall back to the generic iterate protocol, which already
                // preserves BigInt elements and state (Issue #9420).
                if self.iterable_has_bigint_range_length(iterable) {
                    return Err(SpecializationError::Unsupported(
                        "ForEach over a BigInt range falls back to the iterate protocol (Issue #9420)"
                            .to_string(),
                    ));
                }
    ...
    pub(super) fn compile_for(
        ...
        // Compile condition.
        // For the I64-bound form we emit the directional `var > end` jump that
        // the peephole fuser and typed-loop recognizer expect
        // (`LoadSlotI64; LoadSlotI64; JumpIfGtI64` -> `JumpIfGtI64Slots` plus
        // `AddConstI64SlotAndJumpIfLe` back-edge).  The F64 path keeps the old
        // `<=` compare + false-branch shape.
        ...
        if end_ty == ValueType::F64 {
            ...
            // Jump if false (var > end)
            self.emit(Instr::JumpIfZero(0)); // Will be patched
        } else {
            // I64 bound: exit when var > end.  This directional shape is the one
            // the peephole fuser turns into `JumpIfGtI64Slots` and then pairs
            // with a counted-loop back-edge (`AddConstI64SlotAndJumpIfLe`) for
            // the typed-loop fast path.
            self.emit(Instr::JumpIfGtI64(0)); // Will be patched
        }
        ...
        // Patch jump to end
        let end_pos = self.code.len();
        match self.code[jump_end_pos] {
            Instr::JumpIfZero(_) => self.code[jump_end_pos] = Instr::JumpIfZero(end_pos),
            Instr::JumpIfGtI64(_) => self.code[jump_end_pos] = Instr::JumpIfGtI64(end_pos),
            ref other => {
                return Err(SpecializationError::CompileFailed(format!(
                    "unexpected loop guard instruction: {:?}",
                    other
                )))
            }
        }
    ...
    pub(super) fn emit_store(&mut self, var: &str, ty: ValueType) {
        match ty {
            ...
            ValueType::ComplexF64 => {
                self.ensure_complex_split(var);
                self.emit_store_complex_split(var);
            }
    ...
    /// Whether a for-loop iterable must stay on the Julia `iterate` protocol.
    /// Struct and unknown values are not proof of public `length` + `getindex`;
    /// only the dedicated indexable carrier paths may enter `compile_foreach`.
    fn iterable_requires_iterate_protocol(&self, iterable: &Expr) -> bool {
        matches!(
            iterable,
            Expr::Var(name, _)
                if matches!(
                    self.locals.get(name.as_str()),
                    Some(ValueType::Generator | ValueType::Struct(_) | ValueType::Any)
                )
        ) || matches!(
            self.infer_literal_type(iterable),
            Some(ValueType::Generator | ValueType::Struct(_) | ValueType::Any)
        ) || Self::expr_constructs_generator(iterable)
    }

    fn iterable_has_bigint_range_length(&self, iterable: &Expr) -> bool {
        let Some(ValueType::Struct(type_id)) = self.infer_literal_type(iterable) else {
            return false;
        };
        ...
        let base = def
            ...
            .next()
            .unwrap_or(def.name.as_str());
        matches!(base, "UnitRange" | "StepRange") && def.name.contains("BigInt")
    }

    fn expr_constructs_generator(expr: &Expr) -> bool {
        match expr {
            Expr::Generator { .. } => true,
            Expr::Call { function, .. } => {
                matches!(function.as_str(), "Generator" | "Base.Generator")
            }
            Expr::LetBlock { body, .. } => match body.stmts.last() {
                Some(Stmt::Expr { expr, .. }) => Self::expr_constructs_generator(expr),
                _ => false,
            },
            _ => false,
        }
