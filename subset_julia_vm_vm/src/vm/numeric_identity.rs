...
use super::value::{get_bigfloat_precision, BigFloatConsts, RustBigFloat};
use super::Value;
...
fn bigint_from_numeric_integer(value: NumericInteger) -> num_bigint::BigInt {
    match value {
        NumericInteger::NonNegative(value) => num_bigint::BigInt::from(value),
        NumericInteger::Negative(value) => num_bigint::BigInt::from(value),
    }
}

fn fixed_float_integral_bigint(value: &Value) -> Option<num_bigint::BigInt> {
    use num_traits::FromPrimitive;

    let value = fixed_float_as_f64(value)?;
    if !value.is_finite() || value.fract() != 0.0 {
        return None;
    }
    num_bigint::BigInt::from_f64(value)
}

/// Value-based `==` for mixed `BigInt` / fixed-width integer or fixed IEEE-float
/// pairs. Returns `None` when neither side is a `BigInt` mixed with one of those
/// numeric families so callers can keep their existing dispatch path.
pub(crate) fn mixed_bigint_values_equal(left: &Value, right: &Value) -> Option<bool> {
    match (left, right) {
        (Value::BigInt(left), right) => {
            if let Some(right) = numeric_integer_value(right).map(bigint_from_numeric_integer) {
                return Some(left.as_inner() == &right);
            }
            if fixed_float_as_f64(right).is_some() {
                return Some(
                    fixed_float_integral_bigint(right)
                        .as_ref()
                        .is_some_and(|right| left.as_inner() == right),
                );
            }
            None
        }
        (left, Value::BigInt(right)) => {
            if let Some(left) = numeric_integer_value(left).map(bigint_from_numeric_integer) {
                return Some(&left == right.as_inner());
            }
            if fixed_float_as_f64(left).is_some() {
                return Some(
                    fixed_float_integral_bigint(left)
                        .as_ref()
                        .is_some_and(|left| left == right.as_inner()),
                );
            }
            None
        }
...
/// `isequal` for mixed `BigInt` / fixed integer or fixed IEEE-float pairs.
/// This matches `==` except for signed zero: integers are always positive zero,
/// so `isequal(big(0), -0.0)` is false.
pub(crate) fn mixed_bigint_values_isequal(left: &Value, right: &Value) -> Option<bool> {
    mixed_bigint_values_equal(left, right).map(|equal| {
        equal && !is_negative_zero_fixed_float(left) && !is_negative_zero_fixed_float(right)
    })
}

fn numeric_value_to_bigfloat(value: &Value) -> Option<RustBigFloat> {
    let precision = get_bigfloat_precision();
    let from_f64 = |value: f64| Some(RustBigFloat::from_f64(value, precision));
    let from_integer = |value: String| {
        let mut consts = BigFloatConsts::new().ok()?;
        Some(RustBigFloat::parse_integer_exact_decimal(
            &value,
            precision,
            &mut consts,
        ))
    ...
    match value {
        Value::BigFloat(value) => Some(value.clone()),
        Value::F64(value) => from_f64(*value),
        Value::F32(value) => from_f64(f64::from(*value)),
        Value::F16(value) => from_f64(value.to_f64()),
        Value::Bool(value) => from_f64(if *value { 1.0 } else { 0.0 }),
        Value::I8(value) => from_integer(value.to_string()),
        Value::I16(value) => from_integer(value.to_string()),
        Value::I32(value) => from_integer(value.to_string()),
        Value::I64(value) => from_integer(value.to_string()),
        Value::I128(value) => from_integer(value.to_string()),
        Value::U8(value) => from_integer(value.to_string()),
        Value::U16(value) => from_integer(value.to_string()),
        Value::U32(value) => from_integer(value.to_string()),
        Value::U64(value) => from_integer(value.to_string()),
        Value::U128(value) => from_integer(value.to_string()),
        Value::BigInt(value) => from_integer(value.to_string()),
        _ => None,
    }
}

/// Value-based `==` for a `BigFloat` mixed with any supported numeric value.
/// Returns `None` unless at least one side is a `BigFloat`.
pub(crate) fn mixed_bigfloat_numeric_values_equal(left: &Value, right: &Value) -> Option<bool> {
    if !matches!(left, Value::BigFloat(_)) && !matches!(right, Value::BigFloat(_)) {
        return None;
    }
    Some(matches!(
        numeric_value_to_bigfloat(left)?.cmp(&numeric_value_to_bigfloat(right)?),
        Some(0)
    ))
}
...
mod mixed_int_float_tests {
    use super::{
        cmp_integer_to_f64, mixed_bigfloat_numeric_values_equal, mixed_bigint_values_equal,
        mixed_int_float_ordering, NumericInteger,
    };
    use crate::vm::value::RustBigInt;
    use crate::vm::Value;
    use std::cmp::Ordering;
    ...
    #[test]
    fn mixed_bigint_values_equal_without_float_rounding() {
        assert_eq!(
            mixed_bigint_values_equal(&Value::BigInt(RustBigInt::from(1)), &Value::I64(1)),
            Some(true)
        );
        assert_eq!(
            mixed_bigint_values_equal(&Value::I64(1), &Value::BigInt(RustBigInt::from(2))),
            Some(false)
        );
        assert_eq!(
            mixed_bigint_values_equal(&Value::BigInt(RustBigInt::from(0)), &Value::F64(-0.0)),
            Some(true)
        );
        assert_eq!(
            mixed_bigint_values_equal(
                &Value::BigInt(RustBigInt::from(9_007_199_254_740_993_i64)),
        ...
        assert_eq!(
            mixed_bigint_values_equal(
                &Value::BigInt(RustBigInt::from(9_007_199_254_740_992_i64)),
                &Value::F64(9_007_199_254_740_992.0),
            ),
            Some(true)
        );
    ...
    #[test]
    fn mixed_bigfloat_numeric_values_equal_uses_numeric_value() {
        assert_eq!(
            mixed_bigfloat_numeric_values_equal(
                &Value::BigInt(RustBigInt::from(1)),
                &Value::bigfloat_from_f64(1.0),
            ),
            Some(true)
        );
        assert_eq!(
            mixed_bigfloat_numeric_values_equal(
                &Value::BigInt(RustBigInt::from(2)),
                &Value::bigfloat_from_f64(1.0),
            ),
            Some(false)
        );
