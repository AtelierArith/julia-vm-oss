...
pub(crate) fn array_element_type_from_julia_type(jt: &JuliaType) -> ArrayElementType {
    // Runtime TypeVar identity is structural regardless of the outer shape:
    // `Vector{a}`, tuples containing `a`, and direct RuntimeParametric nodes
    // need the same carrier as a RuntimeUnionAll (Issue #11236).
    if jt.contains_runtime_typevar() {
        return ArrayElementType::Structured(Box::new(jt.clone()));
    }
    ...
    match jt {
        ...
        JuliaType::BigInt => ArrayElementType::Abstract("BigInt".to_string()),
        // Issue #9301: Float16 has no inline storage variant (boxed Any + tag),
        // mirroring I128/U128 above so `Vector{Float16}`/`Float16[...]`/`collect`
        // report `Vector{Float16}` instead of `Vector{Any}`.
        JuliaType::Float16 => ArrayElementType::F16,
        JuliaType::Float32 => ArrayElementType::F32,
        JuliaType::Float64 => ArrayElementType::F64,
        JuliaType::BigFloat => ArrayElementType::Abstract("BigFloat".to_string()),
        ...
        // A partially applied parametric family is a valid array element type
        // when its fixed prefix is concrete (for example `SubArray{Int8}`).
        // Runtime-TypeVar-aware construction represents that family with a
        // structured UnionAll instead of the legacy rendered `Struct` image;
        // retain the same logical element tag for typed allocation (Issue #10861).
        JuliaType::UnionAll { .. } | JuliaType::RuntimeUnionAll { .. } => {
            ArrayElementType::Structured(Box::new(jt.clone()))
        }
        ...
        JuliaType::Struct(name) => match name.as_str() {
            ...
            _ if name.starts_with("Complex{") => ArrayElementType::Abstract(name.clone()),
            "Pair" => ArrayElementType::Abstract("Pair".to_string()),
            "NamedTuple" => ArrayElementType::Abstract("NamedTuple".to_string()),
            ...
            _ if name.starts_with("@NamedTuple{") || name.starts_with("NamedTuple{") => {
                ArrayElementType::Abstract(name.clone())
            }
...
pub(crate) fn array_element_type_from_julia_type_resolved(
    ...
    // Partially applied types are valid Julia array element types. They use
    // boxed storage, but their logical UnionAll must survive allocation so
    // `Memory{W{T}}` / `Array{W{T}}` do not widen to `Any` (Issue #11232).
    if matches!(
        jt,
        JuliaType::UnionAll { .. } | JuliaType::RuntimeUnionAll { .. }
    ) {
    ...
    if matches!(base, ArrayElementType::Any) {
        if let JuliaType::Struct(name) = jt {
            ...
            if let Some(type_id) = struct_defs.iter().position(|def| {
                def.name == *name || struct_def_name_matches_base(&def.name, base_name)
            }) {
                if name.contains('{') && !crate::vm::value::is_rational_type_name(name) {
                    return ArrayElementType::Abstract(name.clone());
                }
                // Issue #9198 S4: an all-`Float64` isbits immutable struct is
                // stored byte-contiguously (unboxed) rather than as boxed
                // `StructRefs`. The decision is purely structural (Principle
                // 8/10): field types + isbits come from the `StructDefInfo`.
                if let Some(field_count) = struct_defs[type_id].inline_f64_field_count(struct_defs)
                {
                    return ArrayElementType::StructInlineF64(type_id, field_count);
                }
...
fn struct_def_name_matches_base(def_name: &str, base_name: &str) -> bool {
    def_name == base_name
        || def_name
    ...
    pub(super) fn execute_array_basic(&mut self, instr: &Instr) -> Result<DispatchAction, VmError> {
        match instr {
            Instr::NewArray(capacity) => {
                ...
                if self.memory_allocation_exceeds_budget(&ArrayElementType::F64, *capacity) {
            ...
            Instr::ReserveArray => {
                ...
                if additional > 0 {
                    let element_type = match self.stack.last() {
                        Some(Value::Memory(memref)) => Some(memref.borrow().element_type().clone()),
                        _ => None,
                    };
                    if let Some(element_type) = element_type {
                        if self.memory_allocation_exceeds_budget(&element_type, additional) {
                            self.raise(VmError::OutOfMemory)?;
                            return Ok(DispatchAction::Continue);
                        }
                    }
...
mod typed_array_push_tests {
    use super::{
        array_element_type_from_julia_type, array_element_type_from_julia_type_resolved,
        typed_array_element_push_value,
    };
    use crate::types::JuliaType;
    use crate::vm::value::{StructInstance, Value};
    use subset_julia_vm_bytecode::{ArrayElementType, StructDefInfo, ValueType};

    fn struct_def(name: &str) -> StructDefInfo {
        StructDefInfo {
            name: name.to_string(),
            is_mutable: false,
            fields: Vec::new(),
    ...
    #[test]
    fn parametric_struct_array_eltype_preserves_concrete_params_issue_8288() {
        let defs = vec![struct_def("QuadGK.Segment{Tx, Nothing, Nothing}")];
        let jt = JuliaType::Struct("QuadGK.Segment{Float64, Nothing, Nothing}".to_string());

        assert_eq!(
            array_element_type_from_julia_type_resolved(&jt, &defs),
            ArrayElementType::Abstract("QuadGK.Segment{Float64, Nothing, Nothing}".to_string())
        );
    }

    #[test]
    fn partial_unionall_array_eltype_keeps_logical_type_issue_11232() {
        ...
        let partial = JuliaType::RuntimeUnionAll {
            ...
            body: Box::new(JuliaType::RuntimeParametric {
                ...
                params: vec![JuliaType::Int64, trailing],
            }),
        };

        assert_eq!(
            array_element_type_from_julia_type_resolved(&partial, &[]),
            ArrayElementType::Structured(Box::new(partial))
        );
    }

    #[test]
    fn nested_runtime_typevar_eltype_uses_structured_carrier_issue_11236() {
        let runtime_var = JuliaType::RuntimeTypeVar {
            id: 11236,
            name: "T".to_string(),
            lower_bound: Box::new(JuliaType::Bottom),
            upper_bound: Box::new(JuliaType::Any),
        };
        let nested = JuliaType::VectorOf(Box::new(runtime_var));

        assert_eq!(
            array_element_type_from_julia_type(&nested),
            ArrayElementType::Structured(Box::new(nested))
        );
    }

    #[test]
    fn bare_struct_array_eltype_still_uses_struct_storage() {
        let defs = vec![struct_def("Point")];

        assert_eq!(
            array_element_type_from_julia_type_resolved(
                &JuliaType::Struct("Point".to_string()),
                &defs,
            ),
            ArrayElementType::StructOf(0)
        );
    }

    #[test]
    fn rational_parametric_array_eltype_keeps_existing_struct_storage_issue_9773() {
        let defs = vec![StructDefInfo {
            fields: vec![
                ("num".to_string(), ValueType::I64),
                ("den".to_string(), ValueType::I64),
            ],
            ..struct_def("Rational{T}")
        }];
