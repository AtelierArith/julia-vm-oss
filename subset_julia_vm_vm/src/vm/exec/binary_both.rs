...
use crate::inference_core::dispatch_resolver::{
    resolve_runtime_core_signature_candidates, runtime_core_pattern_score, RuntimeCoreCandidate,
};
...
fn try_static_array_binary_op(left: &Value, right: &Value, op: &Intrinsic) -> Option<Value> {
    ...
    match (left, right) {
        ...
        (Value::StaticArrayInline(a), Value::StaticArrayInline(b)) => match op {
            Intrinsic::DynamicAdd | Intrinsic::AddInt => a.inline_add(b),
            Intrinsic::DynamicSub | Intrinsic::SubInt => a.inline_sub(b),
            Intrinsic::DynamicMul | Intrinsic::MulInt => {
        ...
        (Value::StaticArray(a), Value::StaticArrayInline(b)) => {
            let b = b.to_static_real_value();
            match op {
                Intrinsic::DynamicAdd | Intrinsic::AddInt => static_add(a, &b),
                Intrinsic::DynamicSub | Intrinsic::SubInt => static_sub(a, &b),
                Intrinsic::DynamicMul | Intrinsic::MulInt => {
                    if !a.is_vector() && b.is_vector() {
                        static_matvec(a, &b)
                    } else if !a.is_vector() && !b.is_vector() {
        ...
        (Value::StaticArrayInline(a), Value::StaticArray(b)) => {
            let a = a.to_static_real_value();
            match op {
                Intrinsic::DynamicAdd | Intrinsic::AddInt => static_add(&a, b),
                Intrinsic::DynamicSub | Intrinsic::SubInt => static_sub(&a, b),
                Intrinsic::DynamicMul | Intrinsic::MulInt => {
        ...
        (scalar, Value::StaticArrayInline(sv))
            if matches!(op, Intrinsic::DynamicMul | Intrinsic::MulInt) =>
        {
...
pub(super) fn try_matrix_diagonal_mul<R: RngLike>(
    ...
    if left_is_diag && right_is_diag {
        // This bridge handles exactly one dense array and one Diagonal.
        // Diagonal * Diagonal is ordinary Julia method dispatch; treating
        // either struct as the dense operand raises a spurious Array TypeError
        // before the matching method can run (Issue #11240).
        return Ok(None);
    }
    ...
    if ndims == 1 {
        if mat_arr.shape[0] != n {
            return Err(VmError::DimensionMismatchMsg(format!(
                "Diagonal matrix has {n} columns, but vector has {} elements",
                mat_arr.shape[0]
            )));
        }
    ...
    if ndims != 2 {
        return Err(VmError::DimensionMismatchMsg(format!(
            "Diagonal * A requires A to be 1D or 2D, got {ndims}D"
        )));
    }
    ...
    if matrix_on_left {
        if ncols != n {
            return Err(VmError::DimensionMismatchMsg(format!(
                "Diagonal matrix has {n} columns, but A has {ncols} columns"
            )));
        }
        ...
        if nrows != n {
            return Err(VmError::DimensionMismatchMsg(format!(
                "Diagonal matrix has {n} rows, but A has {nrows} rows"
            )));
        }
        for j in 0..ncols {
...
fn values_equal_for_operator(left: &Value, right: &Value) -> bool {
    ...
    if let Some(result) = crate::vm::numeric_identity::mixed_bigint_values_equal(left, right) {
        return result;
    }
    if let Some(result) =
        crate::vm::numeric_identity::mixed_bigfloat_numeric_values_equal(left, right)
    {
        return result;
    }
    match (left, right) {
        ...
        (x, y) if x.string_bytes().is_some() && y.string_bytes().is_some() => {
            x.string_bytes() == y.string_bytes()
        }
        (Value::Char(x), Value::Char(y)) => x == y,
        (Value::Nothing, Value::Nothing) => true,
        (Value::Missing, Value::Missing) => true,
        (Value::Symbol(x), Value::Symbol(y)) => x == y,
        (Value::Function(x), Value::Function(y)) => x.same_generic_function(y),
...
fn complex_scalar_instance(
    value: &Value,
    struct_heap: &[StructInstance],
) -> Option<StructInstance> {
    let instance = match value {
        Value::Struct(s) => s.clone(),
        Value::StructRef(idx) => struct_heap.get(*idx)?.clone(),
        _ => return None,
    };
    instance.is_complex().then_some(instance)
}
...
fn bigint_fallback_intrinsic(fallback_intrinsic: &Intrinsic) -> Intrinsic {
    match fallback_intrinsic {
        Intrinsic::DynamicAdd => Intrinsic::AddBigInt,
        Intrinsic::DynamicSub => Intrinsic::SubBigInt,
        Intrinsic::DynamicMul => Intrinsic::MulBigInt,
        // Issue #8900: BigInt `/` (DynamicDiv) returns BigFloat (float division),
        // matching upstream Julia where `big(a) / big(b)` always yields BigFloat.
        // `÷` (SdivInt) stays as DivBigInt for integer-truncating division.
        // With DivBigInt for both, `big(x) / big(0)` incorrectly raised
        // DivisionByZero instead of returning BigFloat Inf.
        Intrinsic::DynamicDiv => Intrinsic::DivBigFloat,
        Intrinsic::SdivInt => Intrinsic::DivBigInt,
...
fn bigfloat_fallback_intrinsic(fallback_intrinsic: &Intrinsic) -> Intrinsic {
    match fallback_intrinsic {
        Intrinsic::DynamicAdd => Intrinsic::AddBigFloat,
        Intrinsic::DynamicSub => Intrinsic::SubBigFloat,
        Intrinsic::DynamicMul => Intrinsic::MulBigFloat,
        Intrinsic::DynamicDiv | Intrinsic::SdivInt => Intrinsic::DivBigFloat,
...
fn unqualified_struct_base_name<'a>(
    value: &'a Value,
    struct_heap: &'a [StructInstance],
) -> Option<&'a str> {
    let name = match value {
        Value::Struct(s) => s.struct_name.as_ref(),
        Value::StructRef(idx) => struct_heap.get(*idx)?.struct_name.as_ref(),
        _ => return None,
    };
...
fn dedicated_bare_container_arg_mismatch(
    func: &FunctionInfo,
    arg_idx: usize,
    value: &Value,
    struct_heap: &[StructInstance],
) -> bool {
    let expected = match func.param_julia_types.get(arg_idx) {
        Some(crate::types::JuliaType::Dict) => "Dict",
        Some(crate::types::JuliaType::Set) => "Set",
        _ => return false,
    };

    unqualified_struct_base_name(value, struct_heap) != Some(expected)
}

/// Issue #9343: Bool "strong zero" multiply. Upstream `base/bool.jl` defines
/// `*(x::Bool, y::T) where {T<:AbstractFloat} = ifelse(x, y, copysign(zero(y), y))`
/// (and the symmetric `*(y::AbstractFloat, x::Bool)`). This is stronger than IEEE
/// NaN propagation: `false * Inf == 0.0`, `false * -Inf == -0.0`, `false * NaN ==
/// 0.0`. The generic Bool→Int normalization in `execute_binary_both` would instead
/// widen `false` to `0.0` and compute `0.0 * Inf == NaN`, so this pair must be
/// intercepted before that normalization. Returns `None` for anything that is not
/// a `DynamicMul` of a `Bool` with a primitive float (either order); BigFloat and
/// other `AbstractFloat` structs are handled by the pure-Julia method in bool.jl.
fn bool_float_strong_zero_mul(
    left: &Value,
    right: &Value,
    fallback_intrinsic: &Intrinsic,
) -> Option<Value> {
    if !matches!(fallback_intrinsic, Intrinsic::DynamicMul) {
        return None;
    }
    let (b, fv) = match (left, right) {
        (Value::Bool(b), fv @ (Value::F64(_) | Value::F32(_) | Value::F16(_))) => (*b, fv),
        (fv @ (Value::F64(_) | Value::F32(_) | Value::F16(_)), Value::Bool(b)) => (*b, fv),
        _ => return None,
    };
    Some(match fv {
        Value::F64(y) => Value::F64(if b {
            *y
        ...
        Value::F16(y) => {
            if b {
                ...
                Value::F16(half::f16::from_f32(if y.is_sign_negative() {
                    -0.0
                } else {
                    0.0
                }))
        ...
        // Pair was already narrowed to a primitive float above.
        _ => return None,
    })
}

fn bool_float_add_preserve_zero(
    left: &Value,
    right: &Value,
    fallback_intrinsic: &Intrinsic,
) -> Option<Value> {
    if !matches!(fallback_intrinsic, Intrinsic::DynamicAdd) {
        return None;
    }
    match (left, right) {
        (Value::Bool(false), Value::F64(y)) | (Value::F64(y), Value::Bool(false)) => {
            Some(Value::F64(*y))
        }
        (Value::Bool(true), Value::F64(y)) | (Value::F64(y), Value::Bool(true)) => {
            Some(Value::F64(1.0 + *y))
        }
        (Value::Bool(false), Value::F32(y)) | (Value::F32(y), Value::Bool(false)) => {
            Some(Value::F32(*y))
        }
        (Value::Bool(true), Value::F32(y)) | (Value::F32(y), Value::Bool(true)) => {
            Some(Value::F32(1.0 + *y))
        }
        (Value::Bool(false), Value::F16(y)) | (Value::F16(y), Value::Bool(false)) => {
            Some(Value::F16(*y))
        }
        (Value::Bool(true), Value::F16(y)) | (Value::F16(y), Value::Bool(true)) => {
            Some(Value::F16(half::f16::from_f32(1.0 + y.to_f32())))
        }
        _ => None,
    }
}
    ...
    match (left, right) {
        (Value::F64(a), Value::F64(b)) => {
            let value = match fallback_intrinsic {
                Intrinsic::DynamicAdd => Value::F64(a + b),
                Intrinsic::DynamicSub => Value::F64(a - b),
                Intrinsic::DynamicMul => Value::F64(a * b),
                Intrinsic::DynamicDiv => Value::F64(a / b),
        ...
        (Value::I64(a), Value::I64(b)) => {
            let value = match fallback_intrinsic {
                Intrinsic::DynamicAdd | Intrinsic::AddInt => Value::I64(a.wrapping_add(*b)),
                Intrinsic::DynamicSub | Intrinsic::SubInt => Value::I64(a.wrapping_sub(*b)),
                Intrinsic::DynamicMul | Intrinsic::MulInt => Value::I64(a.wrapping_mul(*b)),
                Intrinsic::DynamicDiv => Value::F64(*a as f64 / *b as f64),
                Intrinsic::SdivInt => {
                    // checked_div: div(typemin(Int64), -1) throws DivideError in
                    // Julia; a plain `/` panics on the i64::MIN / -1 overflow
                    // (Issue #9429). None covers both b == 0 and that overflow.
                    match a.checked_div(*b) {
                        Some(q) => Value::I64(q),
                        None => return Some(Err(VmError::DivisionByZero)),
                    }
                }
                Intrinsic::SremInt => {
                    if *b == 0 {
...
fn missing_dynamic_binary_result(
    left: &Value,
    right: &Value,
    fallback_intrinsic: &Intrinsic,
) -> Option<Value> {
    ...
    match fallback_intrinsic {
        ...
        | Intrinsic::SleInt
        | Intrinsic::SgtInt
        | Intrinsic::SgeInt => Some(Value::Missing),
        _ => None,
    }
}

/// Extract `(struct_name_rc, type_id, re, im)` from a `Value` that is a
/// genuine `Complex{Float64}` (both fields `Value::F64`).  Handles both owned
/// (`Value::Struct`) and heap-indexed (`Value::StructRef`) representations.
///
/// Uses the strict [`StructInstance::complex_f64_parts`] (Issue #9167), NOT the
/// widening `as_complex_parts`: a `Complex{Int}`/`{Bool}`/`{Float32}` operand
/// must fall through to pure-Julia dispatch rather than be re-tagged with
/// `F64` fields. Returns `None` for non-Complex values, non-F64 fields, or heap
/// misses.
#[inline]
pub(crate) fn extract_complex_f64<'a>(
    value: &'a Value,
    heap: &'a [StructInstance],
) -> Option<(std::rc::Rc<str>, usize, f64, f64)> {
    match value {
        Value::Struct(s) => {
            let (re, im) = s.complex_f64_parts()?;
            Some((std::rc::Rc::clone(&s.struct_name), s.type_id, re, im))
        }
        Value::StructRef(idx) => {
            let s = heap.get(*idx)?;
            let (re, im) = s.complex_f64_parts()?;
...
/// Rust-level fast path for `Complex{Float64}` binary arithmetic (Issue #9125).
///
/// Intercepts `-`, `/`, `==`, `!=` before the 23-candidate Julia method
/// resolver fires.  Both operands must be `Complex{Float64}` (or a `Complex`
/// whose fields are `F64`); any other pair falls through to the existing
/// dispatch path.
///
/// Issue #10645: the `+`/`*` arms were retired here. Concrete
/// `Complex{Float64}, Complex{Float64}` overloads were added to
/// `Base.:+`/`Base.:*` (`subset_julia_vm/src/julia/base/complex.jl`) using
/// direct `z.re`/`z.im` field access and the concrete `Complex{Float64}(r, i)`
/// constructor, so those method bodies now predecode into a frame-less
/// `TypedScalarFunctionBlock` (static `GetField`/`AddF64`/`MulF64`/`SubF64` +
/// `NewStruct`, the same shape `abs2(z::Complex{Float64})` already used).
/// `execute_binary_both`'s resolved-method call site
/// (`try_execute_typed_scalar_function_call_from_values`) now runs that block
/// directly — no frame, no boxing round trip through `real`/`imag` dispatch —
/// so the dedicated Rust arithmetic here is redundant for `+`/`*` and would
/// only add a second, harder-to-audit implementation of the same formula to
/// keep bit-identical.
///
/// The result `struct_name` is cloned via `Rc::clone` from the left operand
/// — a reference-count bump, not a heap allocation — giving the hot loop
/// a per-iteration cost of exactly 1 allocation (the field `Vec`) instead of
/// the prior 6+ (method lookup + `StructInstance` name + Vec on each operand
/// load + result).
///
/// Arithmetic follows Julia's `julia/base/complex.jl`:
/// - `-`: component-wise
/// - `/`: standard `(ac+bd, bc-ad) / (c²+d²)` (correct for finite floats)
/// - `==`/`!=`: both components must be equal
pub(crate) fn try_complex_f64_binary_op(
    left: &Value,
    right: &Value,
    op: &Intrinsic,
    struct_heap: &[StructInstance],
) -> Option<Result<Value, VmError>> {
    let (name, type_id, re1, im1) = extract_complex_f64(left, struct_heap)?;
    let (_, _, re2, im2) = extract_complex_f64(right, struct_heap)?;

    let result = match op {
        Intrinsic::DynamicAdd => Value::Struct(StructInstance::complex_with_shared_name(
            type_id,
            name,
            re1 + re2,
            im1 + im2,
        )),
        Intrinsic::DynamicSub => Value::Struct(StructInstance::complex_with_shared_name(
            type_id,
            name,
            re1 - re2,
            im1 - im2,
        ...
        Intrinsic::DynamicMul => {
            // Julia: muladd(real(z), real(w), -imag(z)*imag(w)), muladd(real(z), imag(w), imag(z)*real(w))
            let re = re1 * re2 - im1 * im2;
            let im = re1 * im2 + im1 * re2;
            Value::Struct(StructInstance::complex_with_shared_name(
                type_id, name, re, im,
            ))
        }
        Intrinsic::DynamicDiv => {
            // Julia: (ac+bd)/(c²+d²), (bc-ad)/(c²+d²)
        ...
        Intrinsic::EqFloat => Value::Bool(re1 == re2 && im1 == im2),
        Intrinsic::NeFloat => Value::Bool(re1 != re2 || im1 != im2),
        _ => return None,
    };
    Some(Ok(result))
}
...
/// table with the `%`/`div` semantics that the promoted mixed-type pairs need
/// (`%` is truncated `rem` via fmod (`a % b`, sign of the dividend — NOT
/// floor-mod, Issues #9507/#9450); `div` is `floor(a/b)`). It also owns the
/// Float32×Float32 and
...
fn same_type_fast_path(
    ...
    match (left, right) {
        (Value::F64(a), Value::F64(b)) => match op {
            // `%` / `rem` for floats: truncated remainder (sign of the dividend).
            // Rust's `%` is fmod, exactly Julia's float `rem` — including the
            // Inf/NaN/signed-zero edges (`rem(1.0, Inf) == 1.0`, where the
            // `a - floor(a/b)*b` / `a - trunc(a/b)*b` formulas give NaN). NOT
            // floor-mod: that is `mod`, which base/math.jl derives from `%` with
            // its own sign adjustment (Issues #9507/#9450).
            Intrinsic::SremInt => Some(Ok(Value::F64(a % b))),
            // Julia's div for floats: floor division.
            Intrinsic::SdivInt => Some(Ok(Value::F64(if *b == 0.0 {
                f64::NAN
            } else {
                (a / b).floor()
            }))),
        ...
        (Value::F32(a), Value::F32(b)) => {
            let value = match op {
                Intrinsic::DynamicAdd => Value::F32(a + b),
                Intrinsic::DynamicSub => Value::F32(a - b),
                Intrinsic::DynamicMul => Value::F32(a * b),
                Intrinsic::DynamicDiv => Value::F32(a / b),
                Intrinsic::EqFloat => Value::Bool(a == b),
                Intrinsic::NeFloat => Value::Bool(a != b),
                Intrinsic::LtFloat => Value::Bool(a < b),
                Intrinsic::LeFloat => Value::Bool(a <= b),
                Intrinsic::GtFloat => Value::Bool(a > b),
                Intrinsic::GeFloat => Value::Bool(a >= b),
                // `%` / `rem`: truncated remainder via fmod (Rust `%`), sign of
                // the dividend. NOT floor-mod (Issues #9507/#9450, was #1776).
                Intrinsic::SremInt => Value::F32(a % b),
                // Julia's div: floor division (Issue #1849).
                Intrinsic::SdivInt => {
                    Value::F32(if *b == 0.0 { f32::NAN } else { (a / b).floor() })
                }
                _ => return None,
            };
            Some(Ok(value))
        ...
        (Value::F16(a), Value::F16(b)) => {
            ...
            let value = match op {
                Intrinsic::DynamicAdd => Value::F16(half::f16::from_f64(a + b)),
                Intrinsic::DynamicSub => Value::F16(half::f16::from_f64(a - b)),
                Intrinsic::DynamicMul => Value::F16(half::f16::from_f64(a * b)),
                Intrinsic::DynamicDiv => Value::F16(half::f16::from_f64(a / b)),
                Intrinsic::EqFloat => Value::Bool(a == b),
                Intrinsic::NeFloat => Value::Bool(a != b),
                Intrinsic::LtFloat => Value::Bool(a < b),
                Intrinsic::LeFloat => Value::Bool(a <= b),
                Intrinsic::GtFloat => Value::Bool(a > b),
                Intrinsic::GeFloat => Value::Bool(a >= b),
                // `%` / `rem`: truncated remainder via fmod (Issues #9507/#9450).
                Intrinsic::SremInt => Value::F16(half::f16::from_f64(a % b)),
                Intrinsic::SdivInt => Value::F16(half::f16::from_f64(if b == 0.0 {
                    f64::NAN
                } else {
                    (a / b).floor()
                })),
...
/// (Issue #6338), mirroring `julia/base/promotion.jl` and the shared
/// rules in `promotion.rs` / docs/vm/PROMOTION.md
...
/// Int128) == Float32`).
///
/// Only pairs whose dynamic-dispatch semantics and fallback placement are owned
/// by this early promote table are listed here. Pairs that still need later
/// explicit arms (Bool operands, Float16×Int after small-int normalization,
/// unsigned widths, Int128×Int64/F64, BigInt/BigFloat, Char) must keep those
/// arms (see Issue #5966 on why missing pairs must never silently reach the
...
/// Issue #9334 (residual) / #9335: a same-type diagonal parametric candidate
/// — a method whose full `where`-wrapped `core_signature` is structurally
/// `UnionAll{T<:B}(Tuple{T,T})` (both slots the *same* bound type variable,
/// e.g. `(==)(x::T,y::T) where {T<:Number} = x===y`) — is a strict structural
/// subtype of any other matching candidate whose signature it dominates (most
/// notably the Base promote-fallback `op(x::B,y::B) = op(promote(x,y)...)`,
/// i.e. `Tuple{B,B}`): upstream Julia's own subtyping makes `Tuple{T,T} where
/// T<:B` a proper subtype of `Tuple{B,B}` (the diagonal rule forces both
/// slots to bind one concrete type, which `Tuple{B,B}` does not require), so
/// it must dispatch first. `resolve_runtime_core_signature_candidates`'s flat
/// per-slot scoring cannot see this: both patterns fall through to the same
/// generic subtype-fallback tier and score identically, so a tie is broken by
/// registration order — for Base's fallbacks (compiled before user code) that
/// is always the abstract candidate, so a same-type user method (or the
/// upstream same-type Base anchors themselves) could never win and the
/// fallback re-dispatched on the unchanged same-type pair forever ->
/// StackOverflow.
///
/// This is a pure augmentation: it only ever *overrides* the resolver's own
/// winning match with another candidate that (a) is itself a legitimate match
/// for the actual operand pair (passes the same score+signature gate) and (b)
/// is a strict structural subtype of the winner's signature. It can never
/// select a candidate the score-based resolver would not otherwise have
/// matched, so a legitimate concrete/parametric struct method (e.g. a
/// package's `*(::RotMatrix{N,T}, ::RotMatrix{N,T})`) — whose signature is
/// unrelated to (not a supertype of) any diagonal pattern — is never
/// displaced; when no dominating candidate exists the resolver's original
/// winner is returned unchanged.
fn binary_diagonal_dominant_candidate_pos(
    hierarchy: &StructHierarchy,
    candidates: &[RuntimeCoreCandidate<'_, 2>],
    actual_types: &[CoreType; 2],
    winner_pos: usize,
    same_type_pair: bool,
) -> Option<usize> {
    // The diagonal rule this override models — `Tuple{T,T} where {T<:B}` is a
    // proper subtype of `Tuple{B,B}` — is only sound to apply when both slots
    // can actually bind the one type variable, i.e. the runtime operand pair
    // is same-type. The per-slot pattern scorer cannot enforce that
    // consistency (it sees each `T` slot as its bound `B`), so for a MIXED
    // pair (e.g. `BigFloat < Irrational{:π}`) a diagonal anchor such as
    // `<(x::T,y::T) where {T<:Real}` would "dominate" the Base promote
    // fallback `<(x::Real,y::Real)` structurally while being semantically
    // inapplicable — upstream dispatches the promote fallback there. Restrict
    // the override to genuinely same-type operand pairs (judged by the
    // caller from the operands' rendered runtime type names, the same
    // identity `execute_binary_both` uses for its same-type augmentation);
    // every mixed pair keeps the resolver's original winner.
    if !same_type_pair {
        return None;
    }
    let winner = candidates.iter().find(|c| c.idx == winner_pos)?;
    let winner_sig = winner
        .signature
        .cloned()
        .unwrap_or_else(|| CoreType::Tuple(winner.slots.iter().map(|s| (**s).clone()).collect()));

    let engine = CoreSubtypeEngine::with_hierarchy(hierarchy);
    let mut best: Option<(usize, CoreType)> = None;
    for candidate in candidates {
        if candidate.idx == winner.idx {
            continue;
        }
        // Must independently be a legitimate match for the actual operand
        // pair (same gate the resolver itself applies), not merely
        // structurally comparable to the winner in the abstract.
        let mut no_extra_fallback = |_actual: &CoreType, _expected: &CoreType| false;
        if runtime_core_pattern_score(
            hierarchy,
            &candidate.slots,
            actual_types,
            &mut no_extra_fallback,
        )
        .is_none()
        {
            continue;
        }
        let candidate_sig = candidate.signature.cloned().unwrap_or_else(|| {
            CoreType::Tuple(candidate.slots.iter().map(|s| (**s).clone()).collect())
        });
        let dominates_winner = engine.is_subtype(&candidate_sig, &winner_sig)
            && !engine.is_subtype(&winner_sig, &candidate_sig);
        if !dominates_winner {
            continue;
        }
        let is_best_so_far = match &best {
            Some((_, best_sig)) => engine.is_subtype(&candidate_sig, best_sig),
            None => true,
        };
        if is_best_so_far {
            best = Some((candidate.idx, candidate_sig));
        }
    }
    best.map(|(idx, _)| idx)
}
...
impl<R: RngLike> Vm<R> {
            ...
            let derived = self.functions.get(func_index).and_then(|func| {
                let param_types: Option<Vec<crate::types::JuliaType>> =
                    if func.param_julia_types.len() == 2 {
                        ...
                        crate::vm::expanded_param_types_for_call(func, 2)
    ...
    fn try_complex_scalar_array_mul(
        &mut self,
        left: &Value,
        right: &Value,
    ) -> Result<bool, VmError> {
        let Some((scalar_val, arr_val)) = struct_scalar_array_pair(left, right, &self.struct_heap)?
        else {
            return Ok(false);
        };
        let Some(scalar_struct) = complex_scalar_instance(&scalar_val, &self.struct_heap) else {
            return Ok(false);
        };
        let Some((c_re, c_im)) = scalar_struct.as_complex_parts() else {
            return Ok(false);
        };

        use crate::vm::matmul::{scalar_vector_mul_complex, Complex64};
        let scalar = Complex64::new(c_re, c_im);
        match scalar_vector_mul_complex(scalar, &arr_val, &self.struct_heap) {
            Ok(mut result) => {
                if result
    ...
    fn resolve_binary_both_candidate(
        ...
        let runtime_candidates: Vec<RuntimeCoreCandidate<'_, 2>> = candidates
            .iter()
            .enumerate()
            .filter_map(|(pos, &func_index)| {
                ...
                if is_rust_dict_parametric_mismatch(left, left_expected)
                    ...
                    || dedicated_bare_container_arg_mismatch(func, 0, left, &self.struct_heap)
                    || dedicated_bare_container_arg_mismatch(func, 1, right, &self.struct_heap)
                {
                    return None;
                }
        ...
        let resolved = resolve_runtime_core_signature_candidates(
            &self.struct_hierarchy,
            runtime_candidates.iter().copied(),
            &actual_cores,
            |actual, expected| self.check_subtype_core(actual, expected),
        )?;

        let winner_pos = binary_diagonal_dominant_candidate_pos(
            &self.struct_hierarchy,
            &runtime_candidates,
            &actual_cores,
            resolved.0,
            actual_type_names[0] == actual_type_names[1],
        )
        .unwrap_or(resolved.0);
        Some(candidates[winner_pos])
    }
    ...
    pub(super) fn execute_binary_both(
        ...
        // Fast path for same-type primitive pairs (Int64/Int64, Float64/Float64).
        // This is the hottest path in numeric loops; check it first before any
        // special-case handlers for arrays, Complex, Bool/Float mixed semantics,
        // etc. — none of those apply to plain primitive pairs.
        ...
        // Runtime `missing` values reached through `Any`-typed containers must
        // propagate for arithmetic and comparison just like literal `missing`
        // does in the compiler. Keep bitwise three-valued logic out of this
        // broad path; `Bool & missing`/`Bool | missing` need their own truth
        // tables (Issue #10692).
        if let Some(result) = missing_dynamic_binary_result(&left, &right, fallback_intrinsic) {
        ...
        if let Some(result) = bool_float_add_preserve_zero(&left, &right, fallback_intrinsic) {
            self.stack.push(result);
            return Ok(DispatchAction::Continue);
        }

        // Issue #9343: Bool strong-zero multiply (`false * Inf == 0.0`) must be
        // handled before the generic Bool→Int normalization, which would compute
        // `0.0 * Inf == NaN`.
        if let Some(result) = bool_float_strong_zero_mul(&left, &right, fallback_intrinsic) {
            self.stack.push(result);
            return Ok(DispatchAction::Continue);
        }

        // Issue #9125: Rust-level fast path for Complex{Float64} arithmetic.
        // Intercepts before the 23-candidate Julia method resolver; both
        // operands must be Complex with F64 fields.  The result struct_name
        // is an Rc::clone of the left operand's name (0 heap allocs for name).
        // Issue #9198 S6: bypassable for the retirement A/B measurement.
        if !crate::vm::complex_fastpath_gate::complex_fastpath_disabled() {
            if let Some(result) =
                try_complex_f64_binary_op(&left, &right, fallback_intrinsic, &self.struct_heap)
            {
                crate::vm::profiler::record_event("BinaryBothComplexF64FastHit");
                match result {
                    Ok(value) => {
                        self.stack.push(value);
                    }
        ...
        if matches!(fallback_intrinsic, Intrinsic::DynamicMul)
            && self.try_complex_scalar_array_mul(&left, &right)?
        {
            return Ok(DispatchAction::Continue);
        }
        ...
        if let (Some(left_bytes), Some(right_bytes)) = (left.string_bytes(), right.string_bytes()) {
            let result = match fallback_intrinsic {
                Intrinsic::EqFloat | Intrinsic::EqInt => Some(left_bytes == right_bytes),
                Intrinsic::NeFloat | Intrinsic::NeInt => Some(left_bytes != right_bytes),
                Intrinsic::LtFloat | Intrinsic::SltInt => Some(left_bytes < right_bytes),
                Intrinsic::LeFloat | Intrinsic::SleInt => Some(left_bytes <= right_bytes),
                Intrinsic::GtFloat | Intrinsic::SgtInt => Some(left_bytes > right_bytes),
                Intrinsic::GeFloat | Intrinsic::SgeInt => Some(left_bytes >= right_bytes),
                _ => None,
            };
        ...
        {
            let result = match fallback_intrinsic {
                Intrinsic::DynamicAdd => self.dynamic_add(&left, &right)?,
                Intrinsic::DynamicSub => self.dynamic_sub(&left, &right)?,
                Intrinsic::DynamicMul => self.dynamic_mul(&left, &right)?,
                Intrinsic::DynamicDiv => self.dynamic_div(&left, &right)?,
                _ => unreachable!(),
            };
            self.stack.push(result);
            return Ok(DispatchAction::Continue);
        ...
        if let (Value::DataType(l), Value::DataType(r)) = (&left, &right) {
            if matches!(
                fallback_intrinsic,
                Intrinsic::EqFloat | Intrinsic::EqInt | Intrinsic::NeFloat | Intrinsic::NeInt
            ) {
                let is_equal = type_objects_equal(l, r);
                let result = match fallback_intrinsic {
        ...
        // Issue #9335: a cached Base binary-operator fallback (e.g. the
        // `+(x::Number, y::Number) = +(promote(x, y)...)` promote fallback in
        // `promotion.jl`) bakes its `candidates` operand once, at the compile
        // time of *that* method body — which for Base itself is long before
        // any user code has been compiled. The recursive `px + py` inside such
        // a fallback therefore can never see a user method for a user numeric
        // type defined *after* Base, even when the actual runtime operands are
        // both that exact same struct type: the resolver's only visible
        // candidates are the ones that existed when the fallback body itself
        // was compiled, so it re-selects the same fallback and recurses on the
        // (promotion-unchanged) same-type pair forever -> StackOverflow.
        //
        // For two same-type struct operands, additionally consult the LIVE,
        // name-indexed method table (which includes every method defined up to
        // the current point of execution, unlike the frozen `candidates`
        // operand) and union it into the candidate set. This can only ever ADD
        // methods the frozen list could not see — every previously reachable
        // candidate stays reachable — so it is a pure augmentation: the
        // resolver's own scoring (plus the diagonal-dominance check in
        // `resolve_binary_both_candidate`) still decides the winner among the
        // widened set exactly as it would have among any candidate list that
        // happened to already contain the live methods.
        let same_type_struct_operands = matches!(&left, Value::Struct(_) | Value::StructRef(_))
            && matches!(&right, Value::Struct(_) | Value::StructRef(_))
            && left_type_name == right_type_name;
        let augmented_candidates: Option<Vec<usize>> = if same_type_struct_operands {
            let op_name = candidates
                .first()
                .and_then(|&idx| self.functions.get(idx))
                .map(|f| f.name.clone());
            op_name.and_then(|name| {
                let live = self.get_function_indices_by_name(&name);
                if live.iter().any(|i| !candidates.contains(i)) {
                    let mut merged: Vec<usize> = candidates.to_vec();
                    for &i in live {
        ...
        let candidates: &[usize] = augmented_candidates.as_deref().unwrap_or(candidates);
        ...
        if let Some(func_index) = matched {
            ...
            // Issue #10645: try the frame-less `TypedScalarFunctionBlock` path
            // before allocating a frame. This is the same machinery
            // `CallSpecialize` already uses for mixed-type args
            // (`try_execute_typed_scalar_function_call_from_values`); it is
            // NOT Complex-specific — any resolved binary method whose body
            // predecodes (scalar I64/F64/ComplexF64 params, all exits via
            // `Return*`) takes this path. `Complex{Float64}, Complex{Float64}`
            // `+`/`*` are the motivating case (their bodies now predecode
            // since the concrete `Complex{Float64}` overloads were added,
            // making `try_complex_f64_binary_op`'s `*`/`+` arms redundant for
            // this route — see that function's doc comment). Falls straight
            // through to the existing frame path on any bail (wrong shape,
            // non-decodable body, etc.); `left`/`right` are un-consumed on
            // that path since the array below only borrows them.
            let fastpath_args = [left, right];
            if func.type_params.is_empty() && func.kwparams.is_empty() {
                if let Some(result) = self.try_execute_typed_scalar_function_call_from_values(
                    func.code_start,
                    func.code_end,
                    &func.param_slots,
                    &fastpath_args,
            ...
            let left_is_range = matches!(&left, Value::Range(_));
            let right_is_range = matches!(&right, Value::Range(_));

            if bigint_fallback_handles {
                // BinaryBothFallback: bigint-intrinsics [bootstrap] (Issue #4262)
                // Run before small-int normalization so UInt64/UInt128 operands
                // reach `pop_bigint` instead of overflowing through Int64.
                self.stack.push(left);
                self.stack.push(right);
                if let Err(e) =
                    self.execute_intrinsic(bigint_fallback_intrinsic(fallback_intrinsic))
                {
                    self.raise(e)?;
                }
                return Ok(DispatchAction::Continue);
            ...
            if bigfloat_fallback_handles {
                // BinaryBothFallback: bigfloat-intrinsics [bootstrap] (Issue #4262)
                // BigFloat operations and BigInt×float promotion must also see
                // the original operands, not the primitive Int64-normalized form.
                self.stack.push(left);
                self.stack.push(right);
                if let Err(e) =
                    self.execute_intrinsic(bigfloat_fallback_intrinsic(fallback_intrinsic))
                {
                    self.raise(e)?;
                }
                return Ok(DispatchAction::Continue);
            ...
            if matches!(
                ...
                let direct_result = match fallback_intrinsic {
                    ...
                    Intrinsic::DynamicAdd | Intrinsic::DynamicSub
                        if is_array_like_value(&left, &self.struct_heap)
                            && is_array_like_value(&right, &self.struct_heap) =>
                    {
            ...
            // BinaryBothFallback: uint128-arithmetic signed×unsigned +,-,* (Issue #9441).
            // At same width (Int128×UInt128) and for any narrower signed operand
            // paired with UInt128, upstream promotes to UInt128 ("unsigned wins"
            // — `promote_type(Int128, UInt128) == UInt128`) and converts the
            // signed operand via `x % UInt128`: a sign-extend + reinterpret, NOT
            // a checked `convert` (which would throw InexactError on a negative
            // value). `+`, `-`, `*` then wrap in UInt128. The general
            // `has_u128_arith` block below only accepts NON-NEGATIVE signed
            // operands (`to_u128` guards `x >= 0`) and otherwise falls through, so
            // a negative-signed × UInt128 pair previously reached the legacy
            // small-int path and returned the *signed* type. div/rem/mod are
            // intentionally NOT handled here — the mixed Signed×Unsigned methods
            // for those are owned by pure Julia (Issue #9337) and follow the
            // dividend/divisor signedness, so they must keep falling through.
            let is_u128_add_sub_mul = matches!(
                fallback_intrinsic,
                Intrinsic::AddInt
                    | Intrinsic::SubInt
                    | Intrinsic::MulInt
            ...
            if has_u128_arith && !has_float_arith && is_u128_add_sub_mul {
                // Sign-extend + reinterpret every fixed-width integer operand to
                // u128, matching upstream `x % UInt128`. BigInt is left to the
                // bigint path below (returns None here).
                let to_u128_wrap = |v: &Value| -> Option<u128> {
                    match v {
                        Value::U128(x) => Some(*x),
                        Value::U64(x) => Some(u128::from(*x)),
                        Value::U32(x) => Some(u128::from(*x)),
                        ...
                        Value::I128(x) => Some(*x as u128),
                        Value::I64(x) => Some(*x as u128),
                        Value::I32(x) => Some(*x as u128),
                        Value::I16(x) => Some(*x as u128),
                        Value::I8(x) => Some(*x as u128),
                        _ => None,
                    }
                };
                if let (Some(a), Some(b)) = (to_u128_wrap(&left), to_u128_wrap(&right)) {
                    let result = match fallback_intrinsic {
                        Intrinsic::AddInt | Intrinsic::DynamicAdd => a.wrapping_add(b),
                        Intrinsic::SubInt | Intrinsic::DynamicSub => a.wrapping_sub(b),
                        Intrinsic::MulInt | Intrinsic::DynamicMul => a.wrapping_mul(b),
                        // Guarded by `is_u128_add_sub_mul` above.
                        _ => unreachable!(),
                    };
            ...
            if has_u128_arith && !has_float_arith {
                ...
                if let (Some(a), Some(b)) = (to_u128(&left), to_u128(&right)) {
                    let result = match fallback_intrinsic {
                        Intrinsic::DynamicAdd => Value::U128(a.wrapping_add(b)),
                        Intrinsic::DynamicSub => Value::U128(a.wrapping_sub(b)),
                        Intrinsic::DynamicMul => Value::U128(a.wrapping_mul(b)),
                        Intrinsic::DynamicDiv => {
                        ...
                        Intrinsic::DynamicPow => Value::F64((a as f64).powf(b as f64)),
                        _ => {
            ...
            if has_u64_arith && !has_u128_arith && !has_float_arith {
                ...
                if let (Some(a), Some(b)) = (to_u64(&left), to_u64(&right)) {
                    let result = match fallback_intrinsic {
                        Intrinsic::DynamicAdd | Intrinsic::AddInt => {
                            Some(Value::U64(a.wrapping_add(b)))
                        }
                        Intrinsic::DynamicSub | Intrinsic::SubInt => {
                        ...
                        // `^` (DynamicPow) and any other op are not handled here;
            ...
            // BinaryBothFallback: i128-u64-arithmetic (Issue #8893)
            // When one operand is I128 and the other is U64 (or a smaller integer),
            // Julia promotes UInt64 → Int128 (u64::MAX fits in i128 since i128::MAX
            // = 2^127-1 >> u64::MAX = 2^64-1). The has_u64_arith block above falls
            // through because to_u64 does not include I128, and the normalization
            // block below would then try I64::try_from(u64::MAX) which overflows.
            let has_i128_and_u64 = (matches!(&left, Value::I128(_))
                || matches!(&right, Value::I128(_)))
                && has_u64_arith
                && !has_u128_arith
                && !has_float_arith;
            if has_i128_and_u64 {
                let to_i128 = |v: &Value| -> Option<i128> {
                    match v {
                        Value::I128(x) => Some(*x),
                        // U64 is always non-negative and fits in i128
                ...
                if let (Some(a), Some(b)) = (to_i128(&left), to_i128(&right)) {
                    let result = match fallback_intrinsic {
                        Intrinsic::DynamicAdd => Value::I128(a.wrapping_add(b)),
                        Intrinsic::DynamicSub => Value::I128(a.wrapping_sub(b)),
                        Intrinsic::DynamicMul => Value::I128(a.wrapping_mul(b)),
                        Intrinsic::DynamicDiv => Value::F64(a as f64 / b as f64),
                        Intrinsic::SdivInt => {
                            // checked_div: covers b == 0 and the i128::MIN / -1
                            // overflow (DivideError in Julia, Issue #9429).
                            match a.checked_div(b) {
                                Some(q) => Value::I128(q),
                                None => {
                                    self.raise(VmError::DivisionByZero)?;
                        ...
                        Intrinsic::SremInt => {
                            ...
                            // `%` / `rem` is the truncated remainder (sign of the
                            // dividend). `mod` is derived in pure Julia from `%`
                            // with its own floored sign adjustment, so this arm
                            // must NOT pre-floor — the old `((a%b)+b)%b` both
                            // applied floored semantics to rem callers AND
                            // overflowed i128 near typemax (panic in debug,
                            // silent wrap in release), Issue #9520.
                            // wrapping_rem: rem(typemin, -1) == 0 in Julia; plain
                            // `%` panics on i128::MIN % -1 (Issue #9429).
                            Value::I128(a.wrapping_rem(b))
                        }
                        Intrinsic::EqFloat | Intrinsic::EqInt => Value::Bool(a == b),
                        Intrinsic::NeFloat | Intrinsic::NeInt => Value::Bool(a != b),
                        Intrinsic::LtFloat | Intrinsic::SltInt => Value::Bool(a < b),
                        ...
                        Intrinsic::GeFloat | Intrinsic::SgeInt => Value::Bool(a >= b),
                        _ => {
                            self.raise(VmError::unsupported_op("I128-U64", fallback_intrinsic))?;
                            return Ok(DispatchAction::Continue);
                        }
                    };
                    self.stack.push(result);
            ...
            // BinaryBothFallback: u128-float-arithmetic (Issue #8894)
            // When one operand is U128 and the other is a float (F16/F32/F64),
            // the has_u128_arith block above skips (it guards !has_float_arith),
            // and the normalization block below tries U128 → I64 which overflows
            // for values > i64::MAX (e.g. typemax(UInt128)). Julia promotes the
            // U128 to the float type: Float16 + UInt128 → Float16, etc.
            if has_u128_arith && has_float_arith {
                let u128_val = match (&left, &right) {
                    (Value::U128(x), _) => Some(*x),
                    (_, Value::U128(x)) => Some(*x),
                    _ => None,
                };
                if let Some(u128v) = u128_val {
                    // Determine target float type: F16 > F32 > F64 narrowness.
                    ...
                    if has_f16 {
                        let a = fixed_width_value_as_f16(&left)
                            .map(|v| v.to_f64())
                            .unwrap_or(f64::NAN);
                        let b = fixed_width_value_as_f16(&right)
                            .map(|v| v.to_f64())
                            .unwrap_or(f64::NAN);
                        let result = match fallback_intrinsic {
                            Intrinsic::DynamicAdd => a + b,
                            Intrinsic::DynamicSub => a - b,
                            Intrinsic::DynamicMul => a * b,
                            Intrinsic::DynamicDiv => a / b,
                            Intrinsic::SremInt => a % b,
                            _ => f64::NAN,
                        };
                        self.stack.push(Value::F16(half::f16::from_f64(result)));
                        return Ok(DispatchAction::Continue);
                    }
                    let float_val_f64 = match (&left, &right) {
                        (Value::F16(f), _) => f.to_f64(),
                        (_, Value::F16(f)) => f.to_f64(),
                        (Value::F32(f), _) => *f as f64,
                        (_, Value::F32(f)) => *f as f64,
                        (Value::F64(f), _) => *f,
                        (_, Value::F64(f)) => *f,
                        _ => 0.0,
                    };
                    let u128_as_f64 = u128v as f64;
                    // Determine which operand is left/right for non-commutative ops
                    let (a_f64, b_f64) = if matches!(&left, Value::U128(_)) {
                        (u128_as_f64, float_val_f64)
                    } else {
                        (float_val_f64, u128_as_f64)
                    };
                    let result_f64 = match fallback_intrinsic {
                        Intrinsic::DynamicAdd => a_f64 + b_f64,
                        Intrinsic::DynamicSub => a_f64 - b_f64,
                        Intrinsic::DynamicMul => a_f64 * b_f64,
                        Intrinsic::DynamicDiv => a_f64 / b_f64,
                        // `%` / `rem`: truncated remainder (Issue #9338). The
                        // U128 operand is promoted to float first (Julia widens
                        // UInt128 → the float type), then rem(a,b) via Rust `%`
                        // (fmod — exact, and correct for an infinite divisor
                        // where the `a - trunc(a/b)*b` formula is NaN, Issues
                        // #9507/#9450). Without this arm rem/mod fell into the
                        // NaN default below, and the Pure-Julia `mod` (which is
                        // derived from `%`) inherited the NaN. `fld`/`cld` stay
                        // correct because they derive from `/` (DynamicDiv above),
                        // not from `%`.
                        Intrinsic::SremInt => a_f64 % b_f64,
                        _ => f64::NAN, // comparisons handled below
                    };
                    // Comparison ops return Bool regardless of float width
                    if matches!(
                        fallback_intrinsic,
                        Intrinsic::EqFloat
                            | Intrinsic::NeFloat
                        ...
                        let result = match fallback_intrinsic {
                            Intrinsic::EqFloat => a_f64 == b_f64,
                            Intrinsic::NeFloat => a_f64 != b_f64,
                            Intrinsic::LtFloat => a_f64 < b_f64,
                            Intrinsic::LeFloat => a_f64 <= b_f64,
                            Intrinsic::GtFloat => a_f64 > b_f64,
                            Intrinsic::GeFloat => a_f64 >= b_f64,
                            _ => unreachable!(),
                        };
                        self.stack.push(Value::Bool(result));
                        return Ok(DispatchAction::Continue);
                    }
                    // Arithmetic: narrow result back to the float type
                    let result = if has_f16 {
                        Value::F16(half::f16::from_f64(result_f64))
                    } else if has_f32 {
                        Value::F32(result_f64 as f32)
                    } else {
                        Value::F64(result_f64)
                    };
                    self.stack.push(result);
                    return Ok(DispatchAction::Continue);
                }
            ...
            // Convert Bool and small integer values to I64 for arithmetic operations
            //
            // Out-of-range U64/U128 operands must raise a CATCHABLE
            // OverflowError: a bare `Err(...)`/`?` here bypasses the try/catch
            // handler stack (`handle_error` is never consulted) and aborts the
            // whole program, so route through `self.raise` instead (Issue #9422).
            let left = match left {
                ...
                Value::U64(v) => match i64::try_from(v) {
                    Ok(i) => Value::I64(i),
                    Err(_) => {
                        self.raise(VmError::OverflowError(format!(
                ...
                Value::U128(v) => match i64::try_from(v) {
                    Ok(i) => Value::I64(i),
                    Err(_) => {
                        self.raise(VmError::OverflowError(format!(
            ...
            if left_is_primitive && right_is_primitive {
                ...
                // - DynamicDiv: Julia's / always returns Float64, even for integers
                // - DynamicPow: Power should use floating point for proper semantics
                ...
                if has_i128 && !has_f64 && !has_f32 && !has_f16 {
                    ...
                    let result = match fallback_intrinsic {
                        ...
                        Intrinsic::DynamicPow => Value::F64((a as f64).powf(b as f64)),
                        Intrinsic::SdivInt => {
                            // Integer division (÷). checked_div covers b == 0 and
                            // the i128::MIN / -1 overflow (DivideError in Julia,
                            // Issue #9429).
                            match a.checked_div(b) {
                                Some(q) => Value::I128(q),
                                None => {
                    ...
                    // F16×Int reaches this branch. Julia promotes the integer
                    // operand to Float16 first, so boundary integers overflow or
                    // round before the operation (Issue #9717).
                    {
                        // F16 <-> Int: result is F16 (Julia semantics: Float16 + Int -> Float16)
                        let a = match &left {
                            Value::F16(_) | Value::I64(_) | Value::I128(_) => {
                                fixed_width_value_as_f16(&left).map(|v| v.to_f64())
                            }
                            _ => None,
                        };
                        let Some(a) = a else {
                        ...
                        let b = match &right {
                            Value::F16(_) | Value::I64(_) | Value::I128(_) => {
                                fixed_width_value_as_f16(&right).map(|v| v.to_f64())
                            }
                            _ => None,
                        };
                        let Some(b) = b else {
                        ...
                        let result = match fallback_intrinsic {
                            ...
                            Intrinsic::GeFloat => Value::Bool(a >= b),
                            Intrinsic::SremInt => {
                                // `%` / `rem`: truncated remainder via fmod
                                // (Rust `%`), sign of the dividend — NOT
                                // floor-mod (Issues #9507/#9450).
                                let result = a % b;
                                Value::F16(half::f16::from_f64(result))
                            }
                            Intrinsic::SdivInt => {
                                let result = if b == 0.0 { f64::NAN } else { (a / b).floor() };
                                Value::F16(half::f16::from_f64(result))
                            }
                            _ => {
                                self.raise(VmError::unsupported_op(
                    ...
                    // Rust's `%` is fmod, exactly Julia's float `rem`. NOT floor —
                    // that is `mod`, which base/math.jl derives from `%` with its own
                    // sign adjustment (#6895). Do NOT compute `a - trunc(a/b) * b`:
                    // that formula is NaN for a finite `a` and infinite `b`
                    // (`trunc(a/b) * b == 0.0 * Inf`), where Julia gives `a`
                    // (Issues #9507/#9450), and it rounds for huge quotients where
                    // fmod is exact.
                    let result = a % b;
                    ...
                    let actual_intrinsic = if both_int {
                        match fallback_intrinsic {
                            ...
                            Intrinsic::DynamicSub => Intrinsic::SubInt,
                            Intrinsic::DynamicMul => Intrinsic::MulInt,
                            // DynamicDiv stays as DynamicDiv - Julia's / always returns Float64
                            Intrinsic::DynamicDiv => Intrinsic::DynamicDiv,
                            // DynamicPow stays as DynamicPow for proper floating point semantics
                            Intrinsic::DynamicPow => Intrinsic::DynamicPow,
                    ...
                    let (left_val, right_val) = if matches!(
                        actual_intrinsic,
                        Intrinsic::DynamicDiv | Intrinsic::DynamicPow
                    ) && both_int
                    {
                        // For DynamicDiv and DynamicPow with integers, convert both to F64
                        let l = match left {
            ...
            } else if matches!(fallback_intrinsic, Intrinsic::DynamicMul)
                && matches!(
                    (&left, &right),
                    (Value::Str(_), Value::Str(_))
                        | (Value::Str(_), Value::StrBytes(_))
                        | (Value::StrBytes(_), Value::Str(_))
                        | (Value::StrBytes(_), Value::StrBytes(_))
                        | (Value::Str(_), Value::Char(_))
                        | (Value::StrBytes(_), Value::Char(_))
                        | (Value::Char(_), Value::Str(_))
                        | (Value::Char(_), Value::StrBytes(_))
            ...
            } else if left.string_bytes().is_some()
                && right.string_bytes().is_some()
                && matches!(
                    fallback_intrinsic,
                    Intrinsic::LtFloat
                        | Intrinsic::LeFloat
                ...
                )
            {
                ...
                let result = match (left.string_bytes(), right.string_bytes()) {
                    (Some(a), Some(b)) => match fallback_intrinsic {
                ...
                || ((left_is_range || right_is_range) && (left_is_struct || right_is_struct)))
                && matches!(fallback_intrinsic, Intrinsic::EqFloat | Intrinsic::NeFloat)
            {
                // BinaryBothFallback: struct/range equality [compatibility]
                // During the range migration, native `RangeValue` and the
                // faithful `UnitRange`/`StepRange` structs can meet at array and
                // fallback comparison boundaries. Compare through the same
                // range-aware struct helper used by array equality.
                ...
                && matches!(fallback_intrinsic, Intrinsic::EqFloat | Intrinsic::NeFloat)
            {
                ...
                let result = match fallback_intrinsic {
                    ...
                    Intrinsic::DynamicSub | Intrinsic::SubInt => Value::I64(a - b),
                ...
                || (matches!(&left, Value::I64(_)) && matches!(&right, Value::Char(_)))
            {
                ...
                let result = match fallback_intrinsic {
                    ...
                    Intrinsic::DynamicSub | Intrinsic::SubInt if left_is_char => {
                        // Char - Int -> Char
                        let cp = char_val - int_val;
                        Value::Char(char::from_u32(cp as u32).unwrap_or('\0'))
                    }
                    ...
                    // Issue #9343: Char is NOT a Number. Upstream Julia has no
                    // `==`/`isequal` method for a Char × Integer pair, so it
                    // falls to the generic `==(x, y) = x === y`, which is `false`
                    // for values of different types (`'a' == 97` is `false`, not
                    // a codepoint comparison). `!=` is `!(x === y)` → `true`.
                    Intrinsic::EqFloat | Intrinsic::EqInt => Value::Bool(false),
                    Intrinsic::NeFloat | Intrinsic::NeInt => Value::Bool(true),
                    // Ordering comparisons between Char and Integer have no
                    // upstream method (`'a' < 98` raises MethodError); match that
                    // rather than comparing codepoints numerically (Issue #9343).
                    Intrinsic::LtFloat
                    | Intrinsic::SltInt
                    | Intrinsic::LeFloat
                    | Intrinsic::SleInt
                    ...
                    | Intrinsic::GeFloat
                    | Intrinsic::SgeInt => {
                        let (lt, rt) = if left_is_char {
                            ("Char", "Int64")
                        } else {
                            ("Int64", "Char")
                        };
                        self.raise(VmError::no_method_matching_op(lt, rt))?;
                        return Ok(DispatchAction::Continue);
                    }
                    _ => {
            ...
            } else if let Some((a, b)) = if matches!(fallback_intrinsic, Intrinsic::DynamicMul) {
...
mod tests {
    ...
    fn non_numeric_pairs_use_shared_resolver_first() {
        assert_eq!(
            binary_both_fallback_precedence(&Value::str_new("x".to_string()), &Value::I64(2)),
            BinaryBothFallbackPrecedence::SharedResolverFirst
        );
    }
    ...
    #[test]
    fn missing_dynamic_binary_result_propagates_arithmetic_and_comparison_10612() {
        for intrinsic in [
            Intrinsic::DynamicAdd,
            Intrinsic::DynamicSub,
            Intrinsic::DynamicMul,
            ...
            assert!(
                matches!(
                    missing_dynamic_binary_result(&Value::Missing, &Value::I64(1), &intrinsic),
                    Some(Value::Missing)
                ),
                "{intrinsic:?} should propagate a left missing"
            );
            assert!(
                matches!(
                    missing_dynamic_binary_result(&Value::I64(1), &Value::Missing, &intrinsic),
                    Some(Value::Missing)
                ),
                "{intrinsic:?} should propagate a right missing"
            );
        }
    }

    #[test]
    fn missing_dynamic_binary_result_leaves_bitwise_truth_tables_to_dispatch_10692() {
        for intrinsic in [Intrinsic::AndInt, Intrinsic::OrInt, Intrinsic::XorInt] {
            assert!(
                missing_dynamic_binary_result(&Value::Bool(false), &Value::Missing, &intrinsic)
                    .is_none(),
                "{intrinsic:?} needs Bool/Missing-specific three-valued logic"
            );
        }
    }
    ...
    fn promote_numeric_pair_skips_same_type_and_exception_pairs() {
        ...
        .is_none());
        // Pairs owned by later explicit arms stay out of promote_numeric_pair
        // (Issue #5966): Bool, Float16×Int, unsigned, Int128×Int64/F64, BigInt.
        ...
        assert!(promote_numeric_pair(&Value::str_new("a".to_string()), &Value::F64(2.0)).is_none());
    }

    #[test]
    fn same_type_fast_path_f64_extends_trunc_rem_and_div() {
        // `%` / `rem`: truncated remainder (sign of the dividend), fmod — NOT
        // floor-mod. rem(-7.0, 3.0) == -1.0, not 2.0 (Issues #9507/#9450).
        let result =
            match same_type_fast_path(&Intrinsic::SremInt, &Value::F64(-7.0), &Value::F64(3.0)) {
                Some(Ok(value)) => value,
                other => panic!("F64 rem must hit the same-type table, got {other:?}"),
            };
        assert!(matches!(result, Value::F64(v) if v == -1.0));

        // rem with an infinite divisor returns the dividend (fmod), where the
        // old `a - floor(a/b)*b` formula gave NaN (Issue #9507).
        let result = match same_type_fast_path(
            ...
            Some(Ok(value)) => value,
            other => panic!("F64 rem(1.0, Inf) must hit the same-type table, got {other:?}"),
        };
        assert!(matches!(result, Value::F64(v) if v == 1.0));
        ...
        let result =
            match same_type_fast_path(&Intrinsic::DynamicAdd, &Value::F64(1.5), &Value::F64(2.0)) {
                Some(Ok(value)) => value,
                other => panic!("F64 + F64 must hit the fast path, got {other:?}"),
            };
        assert!(matches!(result, Value::F64(v) if v == 3.5));

        // Ops the legacy chain still owns return None (e.g. DynamicPow).
        assert!(
            same_type_fast_path(&Intrinsic::DynamicPow, &Value::F64(2.0), &Value::F64(3.0))
                .is_none()
        );
    }
    ...
    fn same_type_fast_path_handles_f32_and_f16_tables() {
        ...
        let result =
            match same_type_fast_path(&Intrinsic::DynamicAdd, &Value::F32(1.5), &Value::F32(2.0)) {
                Some(Ok(value)) => value,
                other => panic!("F32 + F32 must hit the same-type table, got {other:?}"),
            };
        assert!(matches!(result, Value::F32(v) if v == 3.5));

        // F32 `%` / `rem`: truncated remainder via fmod (Issues #9507/#9450, was
        // #1776) and floor-div (Issue #1849). rem(-7.0f0, 3.0f0) == -1.0f0.
        let result =
            match same_type_fast_path(&Intrinsic::SremInt, &Value::F32(-7.0), &Value::F32(3.0)) {
                Some(Ok(value)) => value,
                other => panic!("F32 rem must hit the same-type table, got {other:?}"),
            };
        assert!(matches!(result, Value::F32(v) if v == -1.0));
        let result =
        ...
        // Unsupported ops stay with the per-pair policy (DynamicPow -> None).
        assert!(
            same_type_fast_path(&Intrinsic::DynamicPow, &Value::F32(2.0), &Value::F32(3.0))
                .is_none()
        );
    }
    ...
    fn narrow_int_arith_result_kind_skips_mixed_and_wide_and_nonarith() {
        ...
        assert!(narrow_int_arith_result_kind(
            &Value::I8(1),
            &Value::I16(2),
            &Intrinsic::DynamicAdd
        )
        ...
        assert!(
            narrow_int_arith_result_kind(&Value::I8(1), &Value::I8(2), &Intrinsic::DynamicDiv)
                .is_none()
        );
    }
