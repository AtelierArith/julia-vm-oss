...
use super::super::util::{bind_value_to_slot, format_float_julia, format_value, Resolved};
        ...
        match instr {
            ...
            Instr::PrintF64NoNewline => {
                ...
                let s = match &val {
                    ...
                    Value::StructRef(_) => {
                        // Deep-resolve the StructRef (and any nested refs in its
                        // fields) against the heap. A dangling index resolves to
                        // the bare value unchanged, matching the prior fallback
                        // (Issue #8642).
                        format_value(&Resolved::new(&val, &self.struct_heap))
                    }
                    // Non-StructRef `val`: `Resolved::new` borrows it unchanged.
                    _ => format_value(&Resolved::new(&val, &self.struct_heap)),
                };
                self.emit_output(&s, false);
                Ok(DispatchAction::Continue)
            }
            ...
            Instr::PrintAnyNoNewline => {
                if let Some(v) = self.stack.pop() {
                    ...
                    let show_func_index = self.user_show_method_for_print(&resolved);
            ...
            Instr::IOPrintlnNewline => {
                ...
                let val = self.stack.pop_value()?;
                self.emit_print_text_to_sink(Some(&val), "\n")?;
                Ok(DispatchAction::Continue)
            }
