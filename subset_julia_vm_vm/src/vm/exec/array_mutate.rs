...
use crate::vm::value::{
    array_wrapper_value_to_array_value, ensure_native_array_value_acyclic, new_memory_ref,
    ArrayElementType, MemoryRefValue, MemoryValue, RangeElementType, RangeValue, RustBigInt,
    StructInstance,
};
...
fn range_value_to_delete_indices(range: &RangeValue) -> Result<Vec<usize>, VmError> {
    let mut indices = Vec::new();
    for index in 1..=range.length() {
        let value = range.get_value(index)?;
        let idx = match value {
...
fn elem_type_supports_inplace_growth(elem_type: &ArrayElementType) -> bool {
    !matches!(
        ...
        ArrayElementType::ComplexF64
            ...
            // Contiguous AoS f64 storage needs the field-packing `push` path,
            // not single-slot in-place growth (Issue #9198 S4).
...
impl<R: RngLike> Vm<R> {
    pub(in crate::vm) fn pop_array_wrapper(
        &mut self,
        value: Value,
    ...
    pub(in crate::vm) fn pop_first_array_wrapper(
        &mut self,
        value: Value,
    ) -> Result<Result<(Value, Value), Value>, VmError> {
        let Value::StructRef(idx) = value else {
    ...
    pub(in crate::vm) fn push_array_wrapper(
        ...
        // Issue #9198 S4: a contiguous `StructInlineF64` backing packs the
        // struct's fields, so resolve a heap `StructRef` item to an inline
        // `Value::Struct` up front — this covers every `push!` grow branch below
        // (in-place set, capacity set, and exact realloc) in one place.
        let item = match &mem_value {
            Value::Memory(m) => self.resolve_struct_ref_for_inline_store(m, item),
            Value::MemoryRef(memref) => {
                self.resolve_struct_ref_for_inline_store(&memref.parent(), item)
            }
            _ => item,
        };
        ...
        match mem_value {
            Value::Memory(mem_ref) => {
                ...
                if owns_whole_front {
                    ...
                    if self.memory_allocation_exceeds_budget(&element_type, new_len) {
    ...
    fn push_array_wrapper_typejoin(
        ...
        let item = if let Value::StructRef(item_idx) = item {
            self.struct_heap
                .get(item_idx)
                .cloned()
                .map(Value::Struct)
                .unwrap_or(Value::StructRef(item_idx))
        } else {
            item
        };
        ...
        if self.memory_allocation_exceeds_budget(&elem_type, new_len) {
            return Err(VmError::OutOfMemory);
        }
        ...
        match instr {
            ...
            Instr::ArrayPush | Instr::ArrayPushTypejoin => {
                ...
                match try_consume_array_value(arr_val) {
                    Ok(arr) => {
                        let val = if matches!(instr, Instr::ArrayPushTypejoin) {
                            ...
                            let elem_type = arr.borrow().element_type();
                            self.resolve_struct_ref_for_array_element_type(&elem_type, val)
                        };
                    ...
                    Err(other) => {
                        let wrapper_result = self.push_array_wrapper(
                            other.clone(),
                            val.clone(),
                            matches!(instr, Instr::ArrayPushTypejoin),
                        );
                        match self.try_or_handle(wrapper_result)? {
                            Some(Ok(array_value)) => {
            ...
            Instr::ArrayPop => {
                ...
                match try_consume_array_value(arr_val) {
                    ...
                    Err(other) => match other {
                        other @ Value::StructRef(_) => {
                            let wrapper_result = self.pop_array_wrapper(other);
            ...
            Instr::ArrayPopFirst => {
                ...
                match array_mutation_target(arr_val) {
                    ...
                    ArrayMutationTarget::Other(other) => {
                        let wrapper_result = self.pop_first_array_wrapper(other);
                        let wrapper_result = match self.try_or_handle(wrapper_result)? {
                            Some(result) => result,
                            None => return Ok(DispatchAction::Continue),
                        };
            ...
            Instr::ArrayDeleteAtIndices => {
                ...
                let mut indices: Vec<usize> =
                    if let Some(range) = value_as_range_value(&inds_val, &self.struct_heap) {
                        range_value_to_delete_indices(&range)?
                    } else {
                        let other = &inds_val;
