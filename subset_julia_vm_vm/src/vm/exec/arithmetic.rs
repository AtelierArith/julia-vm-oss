...
use super::super::intrinsics_exec::{
    apply_unary_float_op_with_heap, apply_unary_rounding_op_named_with_heap, pow_f64,
    remap_numeric_fastpath_error, value_to_f64_with_heap,
};
use super::super::stack_ops::StackOps;
use super::super::value::{get_bigfloat_precision, get_bigfloat_rounding, RustBigFloat, Value};
    ...
    pub(super) fn execute_arithmetic(&mut self, instr: &Instr) -> Result<DispatchAction, VmError> {
        match instr {
            ...
            Instr::SqrtF64 => {
                ...
                if let Value::BigFloat(x) = value {
                    if x.is_negative() {
                        let msg = format!(
                            "sqrt was called with a negative real argument ({}) but will only return a complex result if called with a complex argument. Try sqrt(Complex(x)).",
                            x
                        );
                        let err = self.domain_error_with_val(msg, Value::BigFloat(x));
                        self.raise(err)?;
                        return Ok(DispatchAction::Continue);
                    }
                    self.stack.push(Value::BigFloat(
                        x.sqrt(get_bigfloat_precision(), get_bigfloat_rounding()),
                    ));
                    return Ok(DispatchAction::Continue);
                }
                // A non-numeric operand is a dispatch miss upstream
                // (`sqrt("a")` raises MethodError), not a conversion
                // TypeError (Issue #10481).
                let x = value_to_f64_with_heap(&value, &self.struct_heap).map_err(|err| {
                    let err = remap_numeric_fastpath_error("sqrt", err, &value, &self.struct_heap);
                    self.park_method_error_payload(&err, "sqrt", std::slice::from_ref(&value));
                    err
                })?;
                ...
                if x < 0.0 {
                    ...
                    let err = self.domain_error_with_val(msg, Value::F64(x));
                    self.raise(err)?;
                    return Ok(DispatchAction::Continue);
                }
            ...
            Instr::FloorF64 => {
                let value = self.stack.pop_value()?;
                self.stack.push(apply_unary_rounding_op_named_with_heap(
                    "floor",
                    value,
                    &self.struct_heap,
                    f64::floor,
                    RustBigFloat::floor_at_current_precision,
                )?);
                Ok(DispatchAction::Continue)
            }
            Instr::CeilF64 => {
                ...
                self.stack.push(apply_unary_rounding_op_named_with_heap(
                    ...
                    &self.struct_heap,
                    f64::ceil,
                    RustBigFloat::ceil_at_current_precision,
                )?);
                Ok(DispatchAction::Continue)
            }
            Instr::AbsF64 => {
