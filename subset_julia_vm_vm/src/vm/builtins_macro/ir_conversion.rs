...
impl<R: RngLike> Vm<R> {
    ...
    fn ir_stmt_to_value(&self, stmt: &crate::ir::core::Stmt) -> Result<Value, VmError> {
        use crate::ir::core::{LocalDeclKind, Stmt};
        ...
        match stmt {
            ...
            Stmt::LocalDecl { var, kind, .. } => match kind {
                LocalDeclKind::Explicit | LocalDeclKind::CompilerEnclosing => Ok(Value::Expr(
                    ExprValue::from_head("local", vec![Value::Symbol(SymbolValue::new(var))]),
                )),
            },
            ...
            Stmt::Test {
                ...
                if let Some(msg) = message {
                    args.push(Value::str_new(msg.clone()));
                }
                Ok(Value::Expr(ExprValue::from_head(
                    "macrocall",
                    vec![
            ...
            Stmt::RuntimeNominalDef { .. } => Err(VmError::NotImplemented(
                "converting a runtime nominal definition back to Expr".to_string(),
            )),
            Stmt::Global { names, .. } => {
    ...
    fn ir_expr_to_value(&self, expr: &crate::ir::core::Expr) -> Result<Value, VmError> {
        use crate::ir::core::{BinaryOp, Expr as IrExpr, NumericConvertTarget, UnaryOp};
        ...
        match expr {
            ...
            // #9803): reflects back to the same `Expr(:call, :Int64/:Float64,
            // operand)` shape the equivalent `IrExpr::Call` would have
            // produced, so macro/quote reflection is unaffected by the
            // shared-plan rewrite (this node is normally only reached
            // through the SSA plan, never through quoted/generated source).
            IrExpr::Convert {
                target, operand, ..
            } => {
                ...
                Ok(Value::Expr(ExprValue::from_head(
                    "call",
                    vec![Value::Symbol(SymbolValue::new(name)), operand_val],
                )))
            }
            IrExpr::Call {
                function,
    ...
    fn ir_literal_to_value(&self, lit: &crate::ir::core::Literal) -> Result<Value, VmError> {
        ...
        match lit {
            ...
            Literal::BigInt(s) => Ok(Value::str_new(s.clone())), // Return as string
            Literal::BigFloat(s) => Ok(Value::str_new(s.clone())), // Return as string
            Literal::Float(f) => Ok(Value::F64(*f)),
            Literal::Float32(f) => Ok(Value::F32(*f)),
            Literal::Float16(f) => Ok(Value::F16(*f)),
            Literal::Bool(b) => Ok(Value::Bool(*b)),
            Literal::Str(s) => Ok(Value::str_new(s.clone())),
            Literal::StrBytes(bytes) => Ok(Value::str_from_bytes(bytes.clone())),
            Literal::Char(c) => Ok(Value::Char(*c)),
            Literal::CharMalformed(bits) => Ok(Value::CharMalformed(*bits)),
