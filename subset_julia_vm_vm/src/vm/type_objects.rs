...
use crate::inference_core::{core_type_to_julia_type, CorePrimitive, CoreType, CoreValueParam};
use crate::runtime_types::ParametricStructDef;
...
pub(crate) enum RuntimeTypeObjectKind {
    ...
    UnionAll,
    TypeVar,
    /// A `Union{A, B}` type value. Upstream `Union` is its own first-class
    /// kind — `typeof(Union{Int64, Float64}) === Union`, it is NOT a
    /// `DataType`, and it exposes its branch types through the fields
    /// `a`/`b` (Issues #5335, #10313).
    Union,
}
...
impl RuntimeTypeHandle {
    ...
    fn nominal_base_name(&self) -> Option<&str> {
        self.core().nominal_base_name()
    }

    fn nominal_type_parameters(&self) -> Option<&[CoreType]> {
        self.core().nominal_type_parameters()
    }

    /// Registry lookup key for a nominal type.
    ///
    /// The core parser deliberately treats an unknown bare single-letter name
    /// as a TypeVar. A declaration such as `struct P end` is disambiguated only
    /// by the runtime registry, so retain that exact bare projection as a hint.
    /// Parametric bases and arguments still come exclusively from `CoreType`.
    fn registry_base_name(&self) -> Option<&str> {
        self.nominal_base_name()
            .or_else(|| match self.projection() {
...
impl<'a> RuntimeTypeRegistry<'a> {
    ...
    pub(crate) fn handle(&self, ty: &JuliaType) -> RuntimeTypeHandle {
        let core = CoreType::from_julia_type_preserving_owner(ty);
    ...
    pub(crate) fn supertype_name(&self, type_name: &str) -> String {
        ...
        let (parent, type_params): (Option<String>, Vec<String>) =
            if let Some(def) = self.struct_defs.iter().find(|d| d.name == base_name) {
                (def.parent_type.clone(), Vec::new())
    ...
    fn kind_for(&self, ty: &JuliaType, core: &CoreType) -> RuntimeTypeObjectKind {
        // A `Union{...}` type value is its own first-class kind upstream:
        // `typeof(Union{Int64, Float64}) === Union`, `isa DataType` is false,
        // and `isa Union` is true (Issues #5335, #10313). `Union{}` (Bottom)
        // is deliberately NOT classified here — upstream projects it onto the
        // separate `Core.TypeofBottom` kind (Issue #9741).
        if matches!(ty, JuliaType::Union(_)) {
            return RuntimeTypeObjectKind::Union;
        }
        ...
        if matches!(core, &CoreType::TypeVar(_)) {
            ...
            if self.declares_type_name(ty, core) {
    ...
    /// (Issue #5252).
    fn declares_type_name(&self, ty: &JuliaType, core: &CoreType) -> bool {
        if !matches!(ty, JuliaType::Struct(_) | JuliaType::AbstractUser(_, _)) {
            return false;
        }
        core.nominal_base_name()
            .or_else(|| match ty {
                JuliaType::Struct(name) | JuliaType::AbstractUser(name, _)
                    if !name.contains('{') =>
                {
            ...
            .is_some_and(|base| self.declares_base_name(base))
    }
    ...
    pub(crate) fn is_unionall_like(&self, ty: &JuliaType) -> bool {
        match ty {
            JuliaType::UnionAll { .. }
...
impl RuntimeTypeObject<'_> {
    ...
    pub(crate) fn runtime_type_projection(&self) -> JuliaType {
        match self.kind() {
            ...
            RuntimeTypeObjectKind::Union => JuliaType::Struct("Union".to_string()),
        }
    }

    pub(crate) fn layout(&self) -> RuntimeTypeLayout {
        if let Some(fields) = named_tuple_reflection_fields(self.handle.projection()) {
    ...
    pub(crate) fn field_names(&self) -> Option<Vec<String>> {
        if let Some(fields) = named_tuple_reflection_fields(self.handle.projection()) {
            return Some(fields.into_iter().map(|(name, _)| name).collect());
        }
    ...
    pub(crate) fn field_types(&self) -> Option<Vec<JuliaType>> {
        if let Some(fields) = named_tuple_reflection_fields(self.handle.projection()) {
            return Some(fields.into_iter().map(|(_, ty)| ty).collect());
        }
    ...
    pub(crate) fn parameters_with_values(&self) -> Vec<ReflectionParameter> {
        match self.handle.projection() {
            JuliaType::VectorOf(element) => {
                return vec![
                    ReflectionParameter::Type(element.as_ref().clone()),
            ...
            JuliaType::TypeOf(inner) => {
                return vec![ReflectionParameter::Type(inner.as_ref().clone())];
            }
            _ => {}
        }
    ...
    pub(crate) fn unionall_var(&self) -> Option<JuliaType> {
        match self.handle.projection() {
            JuliaType::UnionAll {
                ...
                let interval = match (lower_bound.as_deref(), bound.as_deref()) {
                    (None, None) => None,
                    (None, Some(upper)) => Some(upper.to_string()),
                    (Some(lower), None) => Some(format!(">:{}", lower.as_str())),
                    (Some(lower), Some(upper)) => {
                        Some(format!("{}<:{var}<:{}", lower.as_str(), upper.as_str()))
                    }
                };
                Some(JuliaType::TypeVar(var.clone(), interval))
            }
            JuliaType::RuntimeUnionAll { var, .. } => Some(var.as_ref().clone()),
    ...
    pub(crate) fn unionall_body(&self) -> Option<JuliaType> {
        match self.handle.projection() {
            JuliaType::UnionAll { body, .. } | JuliaType::RuntimeUnionAll { body, .. } => {
                Some(unwrap_array_alias_body(body.as_ref().clone()))
            }
    ...
    /// The `a`/`b` branch-type fields of a `Union` type value (Issue #10313).
    ///
    /// Upstream `Union` is a struct with fields `a::Any` and `b::Any`;
    /// `jl_type_union` (`julia/src/jltypes.c`) normalizes at construction —
    /// arms are flattened, absorbed, and sorted into the canonical order
    /// (`union_sort_cmp`) — and an n-arm union nests right-associated, so
    /// `Union{A, B, C}.a === A` and `Union{A, B, C}.b === Union{B, C}`.
    /// sjulia's flat `JuliaType::Union` arms already carry that canonical
    /// order (`canonicalize_union`, Issue #5066); re-canonicalizing here keeps
    /// the projection correct even for a projection built elsewhere from
    /// unsorted arms. Returns `None` when this object is not a `Union` or the
    /// field is not `a`/`b`, so callers fall through to their `FieldError`
    /// path.
    pub(crate) fn union_branch_field(&self, field: &str) -> Option<JuliaType> {
        let JuliaType::Union(arms) = self.handle.projection() else {
            return None;
        };
        let JuliaType::Union(arms) = crate::types::canonicalize_union(arms.clone()) else {
            // Canonicalization collapsed the arms (single member / Bottom):
            // such a value is not a `Union` object upstream and has no `a`/`b`.
            return None;
        };
        match field {
            "a" => arms.first().cloned(),
            "b" => {
                let rest: Vec<JuliaType> = arms.into_iter().skip(1).collect();
                match rest.len() {
                    0 => None,
                    // The last two arms terminate the right-nested chain.
                    1 => rest.into_iter().next(),
                    // More than two arms remaining: `b` is the (already
                    // canonically ordered) union of the rest.
                    _ => Some(JuliaType::Union(rest)),
                }
            }
            _ => None,
        }
    ...
    /// Owner-qualified family identity for this type's shared `TypeName`.
    ///
    /// Upstream stores the leaf `name` and declaring `module` as distinct
    /// fields on `jl_typename_t` (`julia/src/julia.h`).  Keeping this key
    /// separate from [`Self::typename_symbol`] lets `A.T.name.name` and
    /// `B.T.name.name` both expose `:T` while their `TypeName` objects remain
    /// distinct; all concrete instantiations of one family still share a key.
    pub(crate) fn typename_identity(&self) -> String {
        if let CoreType::TypeVar(var) = self.handle.core() {
            return var.name.clone();
        }
        let projection = self.handle.projection().name();
        canonical_typename_identity(projection.as_ref())
    }
    ...
    pub(crate) fn is_abstract_type(&self) -> bool {
        let base = self.handle.registry_base_name();
        ...
        // (Issue #5223).
        if base.is_some_and(is_ref_abstract_base_name) {
            return true;
        }
        self.registry
            .abstract_types
            .iter()
            .any(|def| base.is_some_and(|base| def.name == base))
    ...
    fn is_bare_builtin_parametric(&self) -> bool {
        let Some(type_name) = self.handle.nominal_base_name() else {
            return false;
        };
        // Only the bare form (no structural arguments) is a UnionAll upstream.
        if self
            .handle
    ...
    pub(crate) fn is_primitive_type(&self) -> bool {
        ...
        self.handle.core().is_builtin_primitive_datatype()
    }
    ...
    fn parametric_type_params(&self, name: &str) -> Option<Vec<JuliaType>> {
        ...
        self.registry
            ...
            .map(|def| {
                def.type_params
                    .iter()
                    .map(|param| JuliaType::TypeVar(param.name.clone(), param.upper_bound.clone()))
                    .collect()
            })
    }
    ...
    /// `Box{Int64}` forms.
    fn parametric_schema(&self) -> Option<&'_ ParametricStructDef> {
        let base = self.handle.registry_base_name()?;
    ...
    fn parametric_field_types(&self) -> Option<Vec<JuliaType>> {
        let base = self.handle.registry_base_name()?;
        let args: Vec<JuliaType> = match self.handle.projection() {
            // The runtime projection is the identity-bearing structural graph
            // that was supplied to type application. Reuse its parameters for
            // field substitution: converting a bound RuntimeUnionAll through
            // CoreType intentionally alpha-normalizes its binder to a scope ID,
            // which is sufficient for semantics but cannot reconstruct the
            // original reflection object's `===` identity (Issue #10460).
            JuliaType::RuntimeParametric { params, .. } => params.clone(),
            _ => self
                .handle
                .nominal_type_parameters()?
                .iter()
                .map(reflection_julia_type_from_core_total)
                .collect(),
        };
...
fn projection_has_free_typevars(ty: &JuliaType) -> bool {
    match ty {
        JuliaType::TypeVar(..) | JuliaType::RuntimeTypeVar { .. } => true,
        JuliaType::RuntimeParametric { params, .. } => {
            params.iter().any(projection_has_free_typevars)
        }
        ...
        JuliaType::UnionAll { .. } | JuliaType::RuntimeUnionAll { .. } => false,
        JuliaType::Struct(name) => {
...
fn is_type_variable_spelling(name: &str) -> bool {
    ...
    // (e.g. `S2`, `W1`) is a real, concrete type — not a free variable
    // (Issue #9464).
    if crate::types::is_registered_type_name(name) {
        return false;
    }
...
/// upstream `TypeName` is reported. A module-qualified nominal type reports
/// its leaf name because the owner belongs to the enclosing module, not to the
/// `TypeName` symbol (`M.Box{Int}` -> `:Box`, Issue #7958).
...
fn parse_display_type_name(type_name: &str) -> (&str, Vec<&str>) {
    let without_where =
        crate::inference_core::type_core::strip_top_level_where_clauses(type_name.trim());
    let (base, args) = crate::inference_core::parse_parametric_type_name(without_where);
    (base.trim(), args)
}

fn canonical_typename(type_name: &str) -> String {
    let (base, _) = parse_display_type_name(type_name);
    // Classify actual Base aliases before removing an owner. Otherwise a user
    // type such as `M.Vector` loses its provenance and is incorrectly treated
    // as the Base `Vector` alias (Issue #11361).
    match base {
        // `Array{T,1}`/`Vector{T}` and `Array{T,2}`/`Matrix{T}` share the
        // `Array` TypeName upstream; the bare `Vector`/`Matrix` UnionAlls do
        // too. Only the bare and actual `Base` spellings are those aliases.
        ...
        _ => {
            let (owner, leaf) = syntax_aware_nominal_owner_and_leaf(base);
            match (owner, leaf.as_str()) {
                (Some("Base"), "Vector" | "Matrix") => "Array".to_string(),
                (Some("Base"), "DenseVector" | "DenseMatrix") => "DenseArray".to_string(),
                _ => leaf,
            }
        }
    }
...
/// Canonical family key underlying a `TypeName` value.
///
/// Unlike [`canonical_typename`], this preserves a user type's declaring
/// module.  It strips only the implicit top-level `Main` owner and collapses
/// Julia's actual Array/DenseArray aliases, whose different surface spellings
/// share one upstream `jl_typename_t`.
fn canonical_typename_identity(type_name: &str) -> String {
    let (base, _) = parse_display_type_name(type_name);
    let base = base.strip_prefix("Main.").unwrap_or(base);
    let (owner, leaf) = syntax_aware_nominal_owner_and_leaf(base);
    let builtin_owner = owner.is_none() || matches!(owner, Some("Base") | Some("Core"));
    if builtin_owner && matches!(leaf.as_str(), "Array" | "Vector" | "Matrix") {
        return "Array".to_string();
    }
    if owner.is_none_or(|owner| owner == "Base")
        && matches!(leaf.as_str(), "DenseArray" | "DenseVector" | "DenseMatrix")
    {
        return "DenseArray".to_string();
    }
    owner.map_or(leaf.clone(), |owner| format!("{owner}.{leaf}"))
}

/// Return the final owner-qualified nominal segment without treating dots in
/// nested syntax or `var"..."` identifiers as separators (Issue #11360).
/// Parenthesized carriers such as `typeof(Main.M.f)` therefore remain unsplit;
/// their canonical singleton name is the separate Issue #11362.
fn syntax_aware_nominal_owner_and_leaf(name: &str) -> (Option<&str>, String) {
    ...
    (
        owner_end.map(|end| name[..end].trim()),
        decode_var_quoted_identifier(leaf).unwrap_or_else(|| leaf.to_string()),
    )
}

/// Decode the display spelling of a Julia non-standard identifier. These raw
/// strings escape only `\\` and `"`; malformed spellings are left untouched.
fn decode_var_quoted_identifier(leaf: &str) -> Option<String> {
    let content = leaf.strip_prefix("var\"")?.strip_suffix('"')?;
    if content
        .as_bytes()
        .iter()
        .rev()
...
/// Supertype name for a BARE (unparameterized) rank/dim-generic type alias,
/// e.g. `Vector`, `AbstractMatrix`.
///
/// Upstream, `Vector{T} = Array{T,1}` and `AbstractMatrix{T} =
/// AbstractArray{T,2}` etc. are `UnionAll` aliases: written bare, `Vector`
/// literally IS `Array{T,1} where T`. `supertype` recurses through a
/// `UnionAll` by peeling the body, taking ITS supertype, and re-wrapping:
/// `supertype(u::UnionAll) = UnionAll(u.var, supertype(u.body))`
/// (`julia/base/operators.jl`). Because these aliases FIX the rank/dim
/// parameter (1 or 2) but leave the element type free, that recursion is
/// deterministic regardless of which element type the free variable is
/// eventually bound to: either the underlying family's declared parent
/// doesn't depend on the element type at all (`AbstractArray`'s own parent
/// is unconditionally `Any`), or it does and the wrapped result re-collapses
/// to the sibling alias name at the SAME rank one level up/down the
/// hierarchy — never the rank-ERASED family name (`AbstractArray`/
/// `DenseArray`) that a naive "strip the alias, keep the base family" answer
/// would give (Issues #10282, #10283).
///
/// This must be consulted for the bare alias BEFORE any is-a/subtype-style
/// "declared parent" table (`CoreType::direct_builtin_supertype_name_for_julia_name`
/// and friends), because those tables intentionally route `AbstractVector`/
/// `AbstractMatrix`/`Vector`/`Matrix` through their rank-erased family
/// (`AbstractArray`/`DenseArray`) for SUBTYPE hierarchy walking (where that
/// hop is correct: `AbstractVector <: AbstractArray` truly holds) — a
/// different question from what `supertype()` reflects.
fn rank_generic_alias_bare_supertype_name(type_name: &str) -> Option<&'static str> {
    Some(match type_name {
        "Vector" => "DenseVector",
        "Matrix" => "DenseMatrix",
        "DenseVector" => "AbstractVector",
        "DenseMatrix" => "AbstractMatrix",
        "AbstractVector" => "Any",
        "AbstractMatrix" => "Any",
        _ => return None,
    })
...
/// Supertype name for the `BitArray` family, threading the fixed `Bool`
/// element parameter (Issue #10312).
///
/// Upstream declares `mutable struct BitArray{N} <: AbstractArray{Bool,N}`
/// (`julia/base/bitarray.jl`), and `BitVector = BitArray{1}` / `BitMatrix =
/// BitArray{2}` are zero-free-typevar CONCRETE aliases (the element type is
/// always `Bool`), not rank-generic UnionAlls like `Vector{T}`. So
/// `supertype` on them is a plain parametric-instantiation lookup whose
/// result ALWAYS carries `Bool`: `AbstractVector{Bool}` /
/// `AbstractMatrix{Bool}` / `AbstractArray{Bool, N}`. The bare `BitArray`
/// UnionAll recurses through its body (`supertype(u::UnionAll) =
/// UnionAll(u.var, supertype(u.body))`), yielding `AbstractArray{Bool, N}
/// where N`, which upstream displays as `AbstractArray{Bool}`. The
/// context-free builtin table (`builtin_struct_direct_supertype_name`)
/// returns bare `&'static str` family names for subtype-hierarchy walking
/// and cannot thread the `Bool` parameter, so the reflection layer resolves
/// the family here, BEFORE that table. Verified against upstream Julia
/// 1.12.6.
fn bit_array_family_supertype_name(type_name: &str) -> Option<String> {
    match type_name {
        "BitVector" => return Some("AbstractVector{Bool}".to_string()),
        "BitMatrix" => return Some("AbstractMatrix{Bool}".to_string()),
        "BitArray" => return Some("AbstractArray{Bool}".to_string()),
        _ => {}
    }
    ...
    let rank = args.first()?.name();
    Some(match rank.as_ref() {
        "1" => "AbstractVector{Bool}".to_string(),
        "2" => "AbstractMatrix{Bool}".to_string(),
        rank => format!("AbstractArray{{Bool, {rank}}}"),
    })
}

fn dense_array_alias_supertype_name(type_name: &str) -> Option<String> {
    if let Some(parent) = bit_array_family_supertype_name(type_name) {
        return Some(parent);
    }

    if let Some(parent) = rank_generic_alias_bare_supertype_name(type_name) {
    ...
    Some(match base.as_str() {
        ...
        // `AbstractVector`/`AbstractMatrix`'s own declared parent (via the
        // `AbstractArray{T,N} <: Any` declaration) does not depend on the
        // element type, so a concrete-elem instantiation resolves to `Any`
        // exactly like the bare alias (Issues #10282, #10283).
        "AbstractVector" | "AbstractMatrix" => "Any".to_string(),
...
pub(crate) fn core_type_to_reflection_julia_type(core: &CoreType) -> Option<JuliaType> {
    match core {
        CoreType::TypeVar(var) => {
            if let Some(id) = var.rigid_identity {
                return Some(JuliaType::RuntimeTypeVar {
                    id,
                    name: var.name.clone(),
                    lower_bound: Box::new(
                        var.lower_bound
            ...
            return Some(core_type_to_julia_type(core));
        }
        CoreType::UnionAll { var, body } if var.rigid_identity.is_some() => {
            let binder = core_type_to_reflection_julia_type(&CoreType::TypeVar(var.clone()))?;
            return Some(JuliaType::RuntimeUnionAll {
                var: Box::new(binder),
                body: Box::new(reflection_julia_type_from_core_total(body)),
            });
        }
        CoreType::Tuple(elements) => {
            return Some(JuliaType::TupleOf(
                elements
                    .iter()
                    .map(reflection_julia_type_from_core_total)
                    .collect(),
        ...
        CoreType::TypeOf(inner) => {
            return Some(JuliaType::TypeOf(Box::new(
                reflection_julia_type_from_core_total(inner),
            )));
        }
        CoreType::Struct { name, params } if !params.is_empty() => {
            return Some(JuliaType::from_structured_parametric(
                name.clone(),
                params
                    .iter()
                    .map(reflection_julia_type_from_core_total)
...
fn reflection_julia_type_from_core_total(core: &CoreType) -> JuliaType {
    core_type_to_reflection_julia_type(core).unwrap_or_else(|| core_type_to_julia_type(core))
}

/// `Vector`/`Matrix` are aliases for fixed-rank `Array` applications. Their
/// UnionAll wrapper may use the canonical alias projection internally, but
/// Julia exposes the declared `Array{T,N}` body through `.body` and
/// `Base.unwrap_unionall` (Issue #5593). Preserve a structured runtime TypeVar
/// while restoring that raw body spelling.
pub(crate) fn unwrap_array_alias_body(body: JuliaType) -> JuliaType {
    match body {
        JuliaType::VectorOf(element) => JuliaType::RuntimeParametric {
            base: "Array".to_string(),
            params: vec![*element, JuliaType::Struct("1".to_string())],
        },
        JuliaType::MatrixOf(element) => JuliaType::RuntimeParametric {
            base: "Array".to_string(),
            params: vec![*element, JuliaType::Struct("2".to_string())],
        },
        other => other,
    }
}
...
fn named_tuple_reflection_fields(ty: &JuliaType) -> Option<Vec<(String, JuliaType)>> {
    ...
    {
        return Some(parse_concrete_named_tuple_fields(body));
    }
    ...
    let body = name
        ...
        .and_then(|body| body.strip_suffix('}'))?
        .trim();
    let names = parse_names_only_named_tuple_fields(body)?;
    Some(
        names
            .into_iter()
            .map(|name| (name, JuliaType::Any))
            .collect(),
    )
}

fn parse_concrete_named_tuple_fields(body: &str) -> Vec<(String, JuliaType)> {
    if body.trim().is_empty() {
        return Vec::new();
    }

    split_named_tuple_field_body(body)
        .into_iter()
        .filter_map(|field| {
            let (name, ty) = field
                .split_once("::")
                .map_or((field.trim(), JuliaType::Any), |(name, ty)| {
                    (name.trim(), JuliaType::from_name_or_struct(ty.trim()))
                });
            if name.is_empty() {
                None
            } else {
...
fn parse_names_only_named_tuple_fields(body: &str) -> Option<Vec<String>> {
    let names_body = body.strip_prefix('(')?.strip_suffix(')')?.trim();
    if names_body.is_empty() {
        return Some(Vec::new());
    }

    Some(
        split_named_tuple_field_body(names_body)
            .into_iter()
            .filter_map(|field| {
                let name = field.trim();
                if name.is_empty() {
                    ...
                    Some(name.strip_prefix(':').unwrap_or(name).trim().to_string())
                }
            })
            .collect(),
    )
...
fn split_named_tuple_field_body(body: &str) -> Vec<&str> {
...
mod tests {
    ...
    #[test]
    fn runtime_type_handle_reads_owner_and_params_from_core_issue_10460() {
        let registry = RuntimeTypeRegistry::new(None, &[]);
        let ty = JuliaType::RuntimeParametric {
            base: "Owner.Box".to_string(),
            params: vec![JuliaType::Int64, JuliaType::Struct("Val{3}".to_string())],
        };
        let handle = registry.handle(&ty);

        assert_eq!(handle.nominal_base_name(), Some("Owner.Box"));
        assert_eq!(
            handle.nominal_type_parameters(),
            Some(
                [
                    CoreType::Primitive(CorePrimitive::Int64),
                    CoreType::Struct {
                        name: "Val".to_string(),
                        params: vec![CoreType::Value(CoreValueParam::Int(3))],
                    },
                ]
                .as_slice()
            )
        ...
        crate::types::register_type_name("OwnerA10460.HandleBox10460");
        crate::types::register_type_name("OwnerB10460.HandleBox10460");
        let ordinary = registry.handle(&JuliaType::Struct(
            "OwnerA10460.HandleBox10460{Int64}".to_string(),
        ));
        assert_eq!(
            ordinary.nominal_base_name(),
            Some("OwnerA10460.HandleBox10460")
        );
        assert_eq!(
            ordinary.nominal_type_parameters(),
        ...
        let partially_applied = JuliaType::RuntimeParametric {
            base: "Owner.Partial10460".to_string(),
            params: vec![free.clone()],
        };
        assert_eq!(
            registry.object(&partially_applied).parameters_with_values(),
            vec![ReflectionParameter::Type(free)]
        );
    }

    #[test]
    fn reflection_projection_preserves_nested_runtime_unionall_identity_issue_10460() {
        let binder = CoreTypeVar::unscoped("N").with_rigid_identity(104_603);
        let core = CoreType::UnionAll {
            var: binder.clone(),
            body: Box::new(CoreType::Tuple(vec![CoreType::TypeVar(binder)])),
        };

        assert!(matches!(
            core_type_to_reflection_julia_type(&core),
            Some(JuliaType::RuntimeUnionAll { var, body })
                if matches!(
                    var.as_ref(),
                    JuliaType::RuntimeTypeVar { id: 104_603, .. }
                ) && matches!(
                    body.as_ref(),
                    JuliaType::TupleOf(elements)
                        if matches!(
    ...
    #[test]
    fn runtime_type_object_reflects_named_tuple_fields_issue_9540() {
        let registry = RuntimeTypeRegistry::new(None, &[]);

        let concrete = JuliaType::Struct("@NamedTuple{a::Int64, b::String}".to_string());
        let object = registry.object(&concrete);
        assert_eq!(
            object.field_names(),
        ...
        assert_eq!(object.layout().nfields, 2);

        let nested = JuliaType::Struct(
            "@NamedTuple{coords::Tuple{Int64, Float64}, label::String}".to_string(),
        );
        assert_eq!(
            registry.object(&nested).field_types(),
            Some(vec![
        ...
        let names_only = JuliaType::Struct("NamedTuple{(:x, :y)}".to_string());
        let object = registry.object(&names_only);
        assert_eq!(
            object.field_names(),
        ...
        assert_eq!(
            object.field_types(),
            Some(vec![JuliaType::Any, JuliaType::Any])
        );
        assert_eq!(object.layout().nfields, 2);
    }
    ...
    fn canonical_typename_collapses_array_aliases_issue_5106() {
        ...
        assert_eq!(canonical_typename("DenseVector{Int64}"), "DenseArray");
        assert_eq!(canonical_typename("DenseMatrix{Int64}"), "DenseArray");
        assert_eq!(canonical_typename("Base.Vector{Int64}"), "Array");
        assert_eq!(canonical_typename("Base.Matrix{Int64}"), "Array");
        assert_eq!(canonical_typename("Base.DenseVector{Int64}"), "DenseArray");
        assert_eq!(canonical_typename("Base.DenseMatrix{Int64}"), "DenseArray");
        assert_eq!(canonical_typename(r#"Base.var"Vector"{Int64}"#), "Array");
        assert_eq!(
            canonical_typename(r#"Base.var"DenseVector"{Int64}"#),
            "DenseArray"
        );
    }

    #[test]
    fn canonical_typename_identity_keeps_owner_separate_from_leaf_8451() {
        assert_eq!(canonical_typename("OwnerA8451.T"), "T");
        assert_eq!(canonical_typename("OwnerB8451.T"), "T");
        assert_ne!(
            canonical_typename_identity("OwnerA8451.T"),
            canonical_typename_identity("OwnerB8451.T")
        );

        // Parameters vary across DataTypes, but one nominal family shares a
        // single upstream TypeName object.
        assert_eq!(
            canonical_typename_identity("OwnerA8451.Box{Int64}"),
            canonical_typename_identity("OwnerA8451.Box{Float64}")
        );
        assert_eq!(
            canonical_typename_identity("Main.OwnerA8451.Box{Int64}"),
            "OwnerA8451.Box"
        );

        // Real Base aliases share their underlying TypeName; same-spelled user
        // types retain their declaring module identity.
        assert_eq!(
            canonical_typename_identity("Vector{Int64}"),
            canonical_typename_identity("Matrix{Float64}")
        );
        assert_ne!(
            canonical_typename_identity("OwnerA8451.Vector{Int64}"),
            canonical_typename_identity("Vector{Int64}")
        );
        assert_eq!(
            canonical_typename_identity(r#"Base.var"Vector"{Int64}"#),
            canonical_typename_identity("Array{Float64, 1}")
        );
        assert_eq!(
            canonical_typename_identity(r#"OwnerA8451.var"T""#),
            canonical_typename_identity("OwnerA8451.T")
        );
    }

    #[test]
    fn canonical_typename_strips_nominal_owner_without_losing_alias_provenance_7958() {
        assert_eq!(canonical_typename("QPIF7958.Wrapped{Int64}"), "Wrapped");
        assert_eq!(
            canonical_typename("Outer7958.Inner7958.Wrapped{String}"),
            "Wrapped"
        );

        // A user module may define names that spell like Base aliases. Owner
        // removal must happen only after the exact alias provenance is known.
        assert_eq!(canonical_typename("M7958.Vector{Int64}"), "Vector");
        assert_eq!(canonical_typename(r#"M7958.var"Vector"{Int64}"#), "Vector");
        assert_eq!(
            canonical_typename("Outer7958.Inner7958.Vector{Int64}"),
            "Vector"
        );
        assert_eq!(
            canonical_typename("Outer7958.Inner7958.DenseVector{Int64}"),
            "DenseVector"
        );
    }

    #[test]
    fn canonical_typename_respects_quoted_nominal_leaf_11360() {
        // Dots inside a quoted nominal leaf are identifier content, not owner
        // separators (Issue #11360).
        assert_eq!(
            canonical_typename("Edge11360.var\"Foo.Bar\"{Int64}"),
            "Foo.Bar"
        );
        assert_eq!(
            canonical_typename("Outer11360.var\"Owner.With.Dot\".var\"Foo.Bar\"{Int64}"),
            "Foo.Bar"
        );
        assert_eq!(
            canonical_typename(r#"Edge11360.var"Foo\".Bar"{Int64}"#),
            "Foo\".Bar"
        );
        assert_eq!(
            canonical_typename(r#"Edge11360.var"Foo\\Bar"{Int64}"#),
        ...
        assert_eq!(
            canonical_typename(r#"Q11360.var"Foo where Bar"{Int64}"#),
            "Foo where Bar"
        );
        assert_eq!(
            canonical_typename(r#"Q11360.var"Foo{Bar}"{Int64}"#),
            "Foo{Bar}"
        );
    }

    #[test]
    fn canonical_typename_malformed_names_are_total_11360() {
        // Malformed names are diagnostic inputs too. Keep them total and
        // deterministic rather than indexing or unwrapping into a panic.
        assert_eq!(canonical_typename(""), "");
        assert_eq!(canonical_typename("M7958."), "");
        assert_eq!(
            canonical_typename("M7958.var\"unterminated.with.dot"),
            "var\"unterminated.with.dot"
        );
        assert_eq!(canonical_typename("Broken{Int64"), "Broken{Int64");
    }
    ...
    #[test]
    fn supertype_name_recurses_bare_rank_generic_aliases_issue_10282_10283() {
        // Bare `Vector`/`AbstractMatrix`/etc. are UnionAll aliases
        // (`Vector{T} = Array{T,1} where T`). `supertype` on a UnionAll
        // recurses through the body (`supertype(u::UnionAll) =
        // UnionAll(u.var, supertype(u.body))`), which for these fixed-rank
        // aliases deterministically re-collapses to either a fixed name
        // (`Any`) or the sibling alias at the same rank — never the
        // rank-ERASED family (`AbstractArray`/`DenseArray`) that a naive
        // "strip the alias" answer would give. Verified against upstream
        // Julia 1.12.6 (Issues #10282, #10283).
        let registry = RuntimeTypeRegistry::new(None, &[]);
        assert_eq!(registry.supertype_name("AbstractMatrix"), "Any");
        assert_eq!(registry.supertype_name("AbstractVector"), "Any");
        assert_eq!(registry.supertype_name("Vector"), "DenseVector");
        assert_eq!(registry.supertype_name("Matrix"), "DenseMatrix");
        assert_eq!(registry.supertype_name("DenseVector"), "AbstractVector");
        assert_eq!(registry.supertype_name("DenseMatrix"), "AbstractMatrix");
        // The rank-generic bare `Array`/`DenseArray`/`AbstractArray` aliases
        // (no fixed rank) are unaffected and keep their existing behavior.
        assert_eq!(registry.supertype_name("Array"), "DenseArray");
        assert_eq!(registry.supertype_name("DenseArray"), "AbstractArray");
        assert_eq!(registry.supertype_name("AbstractArray"), "Any");
        // A concrete-elem instantiation of AbstractVector/AbstractMatrix
        // resolves to `Any` too, for the same reason (the element type does
        // not affect AbstractArray's own declared parent).
        assert_eq!(registry.supertype_name("AbstractVector{Int64}"), "Any");
        assert_eq!(registry.supertype_name("AbstractMatrix{Int64}"), "Any");
    }

    #[test]
    fn supertype_name_threads_bitarray_bool_param_issue_10312() {
        // The BitArray family are zero-free-typevar concrete aliases
        // (`BitVector = BitArray{1}`, `BitArray{N} <: AbstractArray{Bool,N}`,
        // `julia/base/bitarray.jl`): their supertype always carries the fixed
        // `Bool` element parameter, which the context-free builtin table's
        // bare `&'static str` names cannot thread through. Verified against
        // upstream Julia 1.12.6 (Issue #10312).
        let registry = RuntimeTypeRegistry::new(None, &[]);
        assert_eq!(registry.supertype_name("BitVector"), "AbstractVector{Bool}");
        assert_eq!(registry.supertype_name("BitMatrix"), "AbstractMatrix{Bool}");
        // The bare `BitArray` UnionAll recurses through the body, giving
        // `AbstractArray{Bool, N} where N`, displayed `AbstractArray{Bool}`.
        assert_eq!(registry.supertype_name("BitArray"), "AbstractArray{Bool}");
        // Parametric instantiations resolve rank 1/2 to the display aliases
        // and keep higher ranks explicit, mirroring the Array/DenseArray arms.
        assert_eq!(
            registry.supertype_name("BitArray{1}"),
            "AbstractVector{Bool}"
        );
        assert_eq!(
            registry.supertype_name("BitArray{2}"),
            "AbstractMatrix{Bool}"
        );
        assert_eq!(
            registry.supertype_name("BitArray{3}"),
            "AbstractArray{Bool, 3}"
        );
    }
    ...
    fn single_letter_declared_struct_classifies_as_datatype_issue_5252() {
        // Single uppercase-letter names are nominal unless a declaration scope
        // explicitly marks them as TypeVars. Declared `P` / `N` and undeclared
        // `Z` therefore all avoid the former name-shape TypeVar heuristic.
        let struct_defs = vec![StructDefInfo {
            name: "P".to_string(),
            is_mutable: false,
            fields: vec![
        ...
        // Undeclared single letter -> nominal type name, not a TypeVar.
        let z = registry.object(&JuliaType::Struct("Z".to_string()));
        assert_eq!(z.kind(), RuntimeTypeObjectKind::DataType);
        assert!(!z.is_concrete_type());

        // A parametric schema with a declared type-variable argument is still
        // non-concrete even though its base `P` is declared.
        let p_of_t = registry.object(&JuliaType::Struct("P{T} where T".to_string()));
