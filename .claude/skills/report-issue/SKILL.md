...
Unsupported-Feature Discovery Rule**（`CLAUDE.md`）を強制する。回避策を入れる前に、
まず Issue を立てるのが原則。
...
cursor スキル `@sjulia-report-gap`、修正後の防止策一般化は `@sjulia-bug-prevention`。
...
- Issue を立てる前に回避策を入れない。回避策が不可避なら Issue 番号を
  ...
  （詳細は cursor スキル `@sjulia-document-workaround`）。
