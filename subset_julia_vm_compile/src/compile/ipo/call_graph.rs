...
fn extract_calls_from_stmt(stmt: &Stmt, called: &mut HashSet<String>) {
    match stmt {
        ...
        Stmt::RuntimeNominalDef { .. } => {
            // Runtime nominal templates contain definition metadata, not
            // expression calls executed by the surrounding statement.
        }
        Stmt::Global { .. } => {
...
fn extract_calls_from_expr(expr: &Expr, called: &mut HashSet<String>) {
    match expr {
        Expr::Call {
            ...
            called.insert(function.to_string());
        ...
        // would have, so call-graph-driven analyses see the same call set
        // regardless of whether the shared plan rewrote the node.
        Expr::Convert {
            target, operand, ..
        } => {
            called.insert(
            ...
            body: Block {
                stmts: vec![Stmt::Return {
                    value: Some(Expr::Call {
                        function: called_name.to_string().into(),
                        args: vec![],
                        kwargs: vec![],
                        splat_mask: vec![],
