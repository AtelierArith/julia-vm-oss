...
impl CoreCompiler<'_> {
    ...
    pub(in super::super) fn compile_builtin_hof(
        ...
        match name {
            ...
            "_compose" => {
                // _compose(f, g) - internal boundary for public ∘/compose wrappers
                if args.len() != 2 {
                    return err("_compose requires exactly 2 arguments: _compose(f, g)");
                }
            ...
            "sprint" => {
                ...
                let is_func_ref = match &args[0] {
                    ...
                    Expr::Var(name, _) => self.method_tables.contains_key(name.as_str()),
                    _ => false,
                };
