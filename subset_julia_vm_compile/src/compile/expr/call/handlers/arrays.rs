...
pub(super) fn compile_collect(
    ...
    let is_direct_step_range_len_bridge = is_direct_range_bridge
        && matches!(&first_julia_type, JuliaType::Struct(name) if {
            let base_name = name
                .split('{')
                .next()
    ...
    if matches!(first_type, ValueType::Generator)
        || (matches!(first_type, ValueType::Range)
            && (!is_direct_range_bridge
                || is_direct_step_range_len_bridge
