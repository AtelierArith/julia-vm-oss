...
/// `print` / `println` (and `Base.`-qualified forms) route straight to the
/// builtin I/O compiler.
///
/// Issue #4580: when an argument is only known as Any, method-table dispatch can
/// pick a singleton fallback such as `Nothing` and then try to coerce the
/// runtime expression before printing. Issue #9460 generalized the same failure:
/// a concrete `print(io::IO, ::VersionNumber)` method can be over-selected for
/// unrelated statically-known values. The builtin I/O compiler already owns
/// arbitrary print routing, including the runtime `Base.print(io::IO, ::T)` /
/// `Base.show(io::IO, ::T)` registries.
