...
impl CoreCompiler<'_> {
    ...
    pub(crate) fn coercion_accepts(&self, actual: &ValueType, target: &ValueType) -> bool {
        ...
        match (actual, target) {
            ...
            (ValueType::Struct(actual_id), ValueType::Struct(target_id)) => {
                self.struct_type_ids_are_logically_compatible(*actual_id, *target_id)
            }
            ...
            // A where-clause value type parameter (e.g. `N` in `SVector{N,T}`)
            // is statically typed `DataType` but bound to an `I64` value at
            // runtime by `bind_type_params` (Issues #6625, #8539). Accept the
            // pair and convert dynamically; a genuine type object raises the
            // conversion error at runtime instead of compile time, matching
            // Julia's runtime MethodError timing.
            (ValueType::DataType, ValueType::I64) => true,
            ...
            (ValueType::Set, ValueType::Struct(type_id)) => self
                ...
                .get_struct_name(*type_id)
                .is_some_and(|name| name == "Set" || name.starts_with("Set{")),
            (actual_ty, ValueType::Union(types)) => self
                .union_member_coercion_target(actual_ty, types)
                .is_some(),
            ...
            // Union -> DataType: e.g. `float(::Type{T}) where T<:Integer =
            // T === BigInt ? BigFloat : Float64` infers as
            // `Union{Type{BigFloat}, Type{Float64}}` — a type object is
            // always a `DataType` instance upstream (`Type{X} <: DataType`
            // for every concrete `X`), so every union member is already a
            // valid `DataType` value; like `Union -> Any`, no runtime
            // conversion is needed (Issue #9955).
            (ValueType::Union(_), ValueType::DataType) => true,
            _ => false,
        }
    }
    ...
    pub(crate) fn compile_expr_as(&mut self, expr: &Expr, target: ValueType) -> CResult<()> {
        ...
        if actual != target {
            ...
            match (actual.clone(), target.clone()) {
                ...
                // DataType -> I64: a where-clause value type parameter is
                // statically `DataType` but holds an `I64` at runtime
                // (Issue #8539); DynamicToI64 passes it through.
                ...
                // Cached Base structs and current-program structs can carry
                // distinct internal ids for the same logical parametric type. When
                // the names match, the representation is already correct and no
                // bytecode conversion is needed (Issue #9381).
                (ValueType::Struct(actual_id), ValueType::Struct(target_id))
                    if self.struct_type_ids_are_logically_compatible(actual_id, target_id) => {}
                ...
                (ValueType::Set, ValueType::Struct(type_id))
                    if self
                        ...
                        .get_struct_name(type_id)
                        .is_some_and(|name| name == "Set" || name.starts_with("Set{")) => {}
                // T -> Union{T, ...}: exact alternatives need no conversion;
                // otherwise coerce into the first concrete member that accepts
                // the source type (e.g. Int64 -> Float64 for Union{Nothing, Float64}).
                        ...
                        .is_some() =>
                {
                    if let Some(member_target) =
                        self.union_member_coercion_target(&actual_ty, &types)
                    {
                        if member_target != actual_ty {
                            self.emit_union_member_coercion(&actual_ty, &member_target);
                        }
                    }
                }
                // Union -> Any: no conversion needed (Union is a subtype of Any)
                (ValueType::Union(_), ValueType::Any) => {}
                // Union -> DataType: no conversion needed, mirrors Union -> Any
                // (every member is already a valid DataType value, Issue #9955).
                (ValueType::Union(_), ValueType::DataType) => {}
    ...
    fn struct_type_ids_are_logically_compatible(&self, actual_id: usize, target_id: usize) -> bool {
        if actual_id == target_id {
            return true;
        }
        let Some(actual_name) = self.shared_ctx.get_struct_name(actual_id) else {
            return false;
        };
        let Some(target_name) = self.shared_ctx.get_struct_name(target_id) else {
            return false;
        };
        struct_names_are_logically_compatible(&actual_name, &target_name)
    }

    fn union_member_coercion_target(
        &self,
        actual: &ValueType,
        union_types: &[ValueType],
    ) -> Option<ValueType> {
        if union_types
            .iter()
        ...
        union_types
            ...
            .filter(|candidate| {
                !matches!(
                    candidate,
                    ValueType::Any | ValueType::Nothing | ValueType::Missing | ValueType::Union(_)
                )
            })
            .find(|candidate| self.coercion_accepts(actual, candidate))
            .cloned()
    }

    fn emit_union_member_coercion(&mut self, actual: &ValueType, target: &ValueType) {
        match (actual, target) {
            (ValueType::I64, ValueType::F64) => self.emit(Instr::ToF64),
            (ValueType::F64, ValueType::I64) => self.emit(Instr::ToI64),
            (
            ...
            (ValueType::Bool, ValueType::I64) => self.emit(Instr::BoolToI64),
            (ValueType::Bool, ValueType::F64) => {
                self.emit(Instr::BoolToI64);
                self.emit(Instr::ToF64);
            }
            ...
            (ValueType::I64, ValueType::Bool) => self.emit(Instr::I64ToBool),
            (ValueType::I64, ValueType::BigInt) => {
                self.emit(Instr::CallIntrinsic(Intrinsic::I64ToBigInt));
            }
            (ValueType::F64, ValueType::BigFloat) => {
...
fn struct_names_are_logically_compatible(actual: &str, target: &str) -> bool {
    actual == target
        || matches!(
            (actual, target),
