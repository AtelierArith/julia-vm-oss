...
impl CoreCompiler<'_> {
    pub(in super::super) fn compile_unary_op(
        ...
        if matches!(op, UnaryOp::Neg) {
            ...
            if matches!(operand_julia_ty, JuliaType::Struct(_)) {
                ...
                if let Some((global_index, return_type)) =
                    self.resolve_unary_neg_struct_method(&operand_julia_ty)
                {
        ...
        match (op, &ty) {
            ...
            // For Struct types, dispatch is usually handled before compiling
            // the operand when Julia type inference can identify the struct.
            // Call operands can compile to a concrete struct even when
            // `infer_julia_type` could not recover the Julia struct name before
            // compiling the operand. Recover it from the concrete ValueType and
            // dispatch now, with the operand already on the stack (Issue #9059).
            (UnaryOp::Neg, ValueType::Struct(_)) => {
                let operand_julia_ty = self.value_type_to_julia_type(&ty);
                if let Some((global_index, return_type)) =
                    self.resolve_unary_neg_struct_method(&operand_julia_ty)
                {
                    self.emit(Instr::Call(global_index, 1));
    ...
    fn resolve_unary_neg_struct_method(
        &self,
        operand_ty: &JuliaType,
    ) -> Option<(usize, ValueType)> {
        let table = self.method_tables.get("-")?;
        let arg_types = [operand_ty.clone()];
        let method = table.dispatch(&arg_types).ok()?;
        Some((method.global_index, method.return_type.clone()))
    }
    ...
    pub(in super::super) fn compile_and_expr(
        ...
        let known_any_rank_array_locals_before_right = self.known_any_rank_array_locals.clone();
        ...
        self.known_any_rank_array_locals = known_any_rank_array_locals_before_right;
...
mod tests {
    ...
    fn test_var_is_not_never_returning() {
        let var = Expr::Var("x".to_string().into(), s());
