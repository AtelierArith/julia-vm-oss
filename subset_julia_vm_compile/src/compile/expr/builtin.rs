...
use crate::bytecode::{
    DynamicCallCandidate, GeneratorCallableSpec, Instr, NativeIteratorKind, ScalarRandType,
    ValueType,
};
...
/// Map a concrete bits-numeric / `Bool` type-identifier name to its
/// [`ScalarRandType`] for a scalar typed `rand(T)` / `rand(rng, T)` draw.
/// Returns `None` for anything that is not a full-range-sampleable scalar type,
/// so callers fall through to the array / dimension / user-method paths
/// unchanged (Issue #9265). `Int`/`Int64` and `UInt`/`UInt64` alias to the
/// 64-bit host representation, matching the rest of the `rand` lowering.
pub(in crate::compile) fn scalar_rand_type_from_name(name: &str) -> Option<ScalarRandType> {
    Some(match name {
        "Int8" => ScalarRandType::I8,
        "Int16" => ScalarRandType::I16,
        "Int32" => ScalarRandType::I32,
        "Int" | "Int64" => ScalarRandType::I64,
        "Int128" => ScalarRandType::I128,
        "UInt8" => ScalarRandType::U8,
        "UInt16" => ScalarRandType::U16,
        "UInt32" => ScalarRandType::U32,
        "UInt" | "UInt64" => ScalarRandType::U64,
        "UInt128" => ScalarRandType::U128,
        "Bool" => ScalarRandType::Bool,
        "Float16" => ScalarRandType::F16,
        "Float32" => ScalarRandType::F32,
        "Float64" => ScalarRandType::F64,
        _ => return None,
    })
}

/// Static result [`ValueType`] produced by a scalar typed `rand(T)` draw.
fn scalar_rand_value_type(t: ScalarRandType) -> ValueType {
    match t {
        ScalarRandType::I8 => ValueType::I8,
        ScalarRandType::I16 => ValueType::I16,
        ScalarRandType::I32 => ValueType::I32,
        ScalarRandType::I64 => ValueType::I64,
        ScalarRandType::I128 => ValueType::I128,
        ScalarRandType::U8 => ValueType::U8,
        ScalarRandType::U16 => ValueType::U16,
        ScalarRandType::U32 => ValueType::U32,
        ScalarRandType::U64 => ValueType::U64,
        ScalarRandType::U128 => ValueType::U128,
        ScalarRandType::Bool => ValueType::Bool,
        ScalarRandType::F16 => ValueType::F16,
        ScalarRandType::F32 => ValueType::F32,
        ScalarRandType::F64 => ValueType::F64,
    }
}

/// A single `rand(T)` / `rand(rng, T)` argument list where `T` is the only
/// trailing argument names a concrete scalar type: return that type so the
/// caller can emit the scalar typed draw instead of misreading `T` as an array
/// dimension (Issue #9265).
fn single_scalar_rand_type(rest: &[Expr]) -> Option<ScalarRandType> {
    match rest {
        [Expr::Var(name, _)] => scalar_rand_type_from_name(name),
        _ => None,
    }
}

/// A single `randn(T)` / `randn(rng, T)` argument list where `T` is the only
/// trailing argument names a concrete floating type. Upstream defines typed
/// `randn` only for floating element types, so integer / Bool names deliberately
/// fall through to the existing dimension/error path.
fn single_scalar_randn_type(rest: &[Expr]) -> Option<ScalarRandType> {
    match rest {
        [Expr::Var(name, _)] => match name.as_str() {
            "Float16" => Some(ScalarRandType::F16),
            "Float32" => Some(ScalarRandType::F32),
            "Float64" => Some(ScalarRandType::F64),
            _ => None,
        },
        _ => None,
    }
...
fn emit_randn_scalar_type_conversion(compiler: &mut CoreCompiler<'_>, sty: ScalarRandType) {
    match sty {
        ScalarRandType::F16 => compiler.emit(Instr::DynamicToF16),
        ScalarRandType::F32 => compiler.emit(Instr::DynamicToF32),
        ScalarRandType::F64 => {}
        _ => unreachable!("single_scalar_randn_type only returns floating types"),
    }
}

/// A `rand(T, dims...)` / `rand(rng, T, dims...)` argument list whose first
/// element names a concrete scalar type and is followed by one or more
/// dimensions: return that scalar type together with the dimension expressions,
/// so the caller can emit a faithfully-typed random array instead of misreading
/// `T` as a dimension (which produced a Float64-backed array for `Int` and a
/// runtime "expected numeric, got DataType" error for every other type)
/// (Issue #9328). Returns `None` when the first element is not a scalar type
/// name or has no trailing dimensions (the scalar `rand(T)` case is handled by
/// [`single_scalar_rand_type`]).
pub(in crate::compile) fn leading_scalar_rand_array_type(
    rest: &[Expr],
) -> Option<(ScalarRandType, &[Expr])> {
    match rest {
        [Expr::Var(name, _), dims @ ..] if !dims.is_empty() => {
            scalar_rand_type_from_name(name).map(|sty| (sty, dims))
        }
        _ => None,
    }
}
...
impl CoreCompiler<'_> {
    ...
    fn store_rng_back(&mut self, rng_arg: &Expr) {
        if let Expr::Var(name, _) = rng_arg {
            self.emit(Instr::StoreRng(name.to_string()));
        } else {
            self.emit(Instr::Pop);
        }
    }
    ...
    /// A `rand(a, dims...)` / `randn(a, dims...)` call of two or more arguments
    /// whose first argument's static type is `Any` — so the compiler cannot
    /// distinguish an explicit `Rng` (e.g. one captured inside a
    /// generator/comprehension body, arriving as a `LoadCaptured` value of static
    /// type `Any`) from a leading array dimension. Such calls must branch at
    /// runtime via [`Instr::RandMaybeRng`] instead of the static dimension
    /// lowering, which would coerce the RNG to an array length (Issue #9285).
    ///
    /// A leading *type name* (`rand(Int, n)` / `rand(Float64, n)` /
    /// `rand(UInt32, n)`) is the element-typed array form handled by the
    /// dimension path and is never an RNG, so it is excluded here. A statically
    /// typed RNG first argument is handled by the explicit-`Rng` path above and
    /// also never reaches this predicate. The single-argument `rand(x)` /
    /// `randn(x)` case is handled by [`Instr::RandArg`] / [`Instr::RandnArg`], so
    /// this requires at least two arguments.
    fn is_maybe_rng_first_arg(&mut self, args: &[Expr]) -> bool {
        if args.len() < 2 {
            return false;
        }
        if self.infer_expr_type(&args[0]) != ValueType::Any {
        ...
        if let Expr::Var(name, _) = &args[0] {
            if matches!(name.as_str(), "Int" | "Int64" | "Float64")
                || scalar_rand_type_from_name(name).is_some()
            {
                return false;
            }
        }
    ...
    fn compile_time_isa_result(&self, args: &[Expr]) -> Option<bool> {
        ...
        if self.first_hidden_builtin_type_binding(type_name).is_some() {
            return None;
        }
        let resolved_type_name = self
            .resolve_visible_type_alias(type_name)
            .or_else(|| {
                crate::types::builtin_type_for_compiler(type_name)
                    .map(|builtin_type| builtin_type.name().into_owned())
            })
            .or_else(|| self.resolve_visible_type_object_name(type_name))?;

        let target =
            super::super::narrowing::value_type_for_type_name(&resolved_type_name, |name| {
                self.shared_ctx.get_struct_type_id(name)
            })?;
        let current = self.locals.get(var_name.as_str())?;
    ...
    pub(in super::super) fn compile_builtin_call(
        ...
        match name {
            ...
            "uppercase" | "lowercase" | "titlecase" | "ncodeunits" | "codeunit" | "repeat"
            | "split" | "join" | "_string" | "sprintf" => err(format!(
                "String function {} should be handled by builtin_string",
                name
            )),
            // Type constructors delegated to builtin_types.rs. Fixed-width
            // public constructors such as Int8/Float32 are pure Julia wrappers
            // over underscored `_to_*` conversion boundaries (Issue #8777).
            "Int" | "UInt" | "BigInt" | "BigFloat" | "Int64" | "Float64" => err(format!(
            ...
            "_ref_new" => {
                if args.len() != 1 {
                    return err("_ref_new requires exactly 1 argument");
                }
                self.compile_expr(&args[0])?;
                self.emit(Instr::CallBuiltin(BuiltinId::RefNew, 1));
                Ok(ValueType::Any)
            }
            "_ref_get" => {
                if args.len() != 1 {
                    return err("_ref_get requires exactly 1 argument");
                }
                self.compile_expr(&args[0])?;
                self.emit(Instr::CallBuiltin(BuiltinId::RefUnwrap, 1));
                Ok(ValueType::Any)
            }
            "_weakref_new" => {
                if args.len() != 1 {
                    return err("_weakref_new requires exactly 1 argument");
                }
                self.compile_expr(&args[0])?;
                self.emit(Instr::CallBuiltin(BuiltinId::WeakRefNew, 1));
                Ok(ValueType::Any)
            }
            "_weakref_value" => {
                if args.len() != 1 {
                    return err("_weakref_value requires exactly 1 argument");
                }
                self.compile_expr(&args[0])?;
                self.emit(Instr::CallBuiltin(BuiltinId::WeakRefValue, 1));
                Ok(ValueType::Any)
            }
            "_weakref_set_value!" => {
                if args.len() != 2 {
                    return err("_weakref_set_value! requires exactly 2 arguments");
                }
                self.compile_expr(&args[0])?;
                self.compile_expr(&args[1])?;
                self.emit(Instr::CallBuiltin(BuiltinId::WeakRefSetValue, 2));
                Ok(ValueType::Nothing)
            }
            "_finalizer" => {
                if args.len() != 2 {
                    return err("_finalizer requires exactly 2 arguments");
                }
                self.compile_expr(&args[0])?;
                self.compile_expr(&args[1])?;
                self.emit(Instr::CallBuiltin(BuiltinId::Finalizer, 2));
                Ok(ValueType::Any)
            }
            "_finalize" => {
                if args.len() != 1 {
                    return err("_finalize requires exactly 1 argument");
                }
                self.compile_expr(&args[0])?;
                self.emit(Instr::CallBuiltin(BuiltinId::Finalize, 1));
                Ok(ValueType::Nothing)
            }
            "_gc_collect" => {
                if args.len() > 1 {
                    return err("_gc_collect requires zero or one argument");
                }
                for arg in args {
                    self.compile_expr(arg)?;
                }
                self.emit(Instr::CallBuiltin(BuiltinId::GcCollect, args.len()));
                Ok(ValueType::Nothing)
            }
            "_gc_safepoint" => {
                if !args.is_empty() {
                    return err("_gc_safepoint requires no arguments");
                }
                self.emit(Instr::CallBuiltin(BuiltinId::GcSafepoint, 0));
                Ok(ValueType::Nothing)
            }
            "_gc_in_finalizer" => {
                if !args.is_empty() {
                    return err("_gc_in_finalizer requires no arguments");
                }
                self.emit(Instr::CallBuiltin(BuiltinId::GcInFinalizer, 0));
                Ok(ValueType::Bool)
            }
            "TypeVar" => {
            ...
            "_expand_substitution" => {
                // _expand_substitution(subst, match, regex) - expand SubstitutionString
                // capture references against a single regex match (Issue #10174)
                if args.len() != 3 {
                    return err("_expand_substitution requires exactly 3 arguments: _expand_substitution(subst, match, regex)");
                }
                self.compile_expr(&args[0])?;
                self.compile_expr(&args[1])?;
                self.compile_expr(&args[2])?;
            ...
            "_regex_match_from" => {
                // _regex_match_from(regex, string, byteindex) - findnext(re, str, i)
                // primitive for the multi-pattern replace scan (Issue #10175)
                if args.len() != 3 {
                    return err("_regex_match_from requires exactly 3 arguments: _regex_match_from(regex, string, byteindex)");
                }
                self.compile_expr(&args[0])?;
                self.compile_expr(&args[1])?;
                self.compile_expr(&args[2])?;
                self.emit(Instr::CallBuiltin(BuiltinId::RegexMatchFrom, 3));
                Ok(ValueType::Any) // Union{RegexMatch, Nothing}
            }
            "_sjulia_backtrace" => {
                if !args.is_empty() {
                    return err("_sjulia_backtrace requires no arguments");
                }
                self.emit(Instr::CallBuiltin(BuiltinId::Backtrace, 0));
                Ok(ValueType::Array)
            }
            "_sjulia_catch_backtrace" => {
                if !args.is_empty() {
                    return err("_sjulia_catch_backtrace requires no arguments");
                }
                self.emit(Instr::CallBuiltin(BuiltinId::CatchBacktrace, 0));
                Ok(ValueType::Array)
            }
            "_sjulia_stacktrace" => {
                if args.len() > 1 {
                    return err("_sjulia_stacktrace requires zero or one argument");
                }
                for arg in args {
                    self.compile_expr(arg)?;
                }
                self.emit(Instr::CallBuiltin(BuiltinId::Stacktrace, args.len()));
                Ok(ValueType::Array)
            }
            _ => {
    ...
    pub(in super::super) fn resolve_function_ref_with_arity(
        ...
        // A lowered FunctionRef can denote a private helper whose spelling
        // collides with a Julia-visible source generic. Resolve that exact
        // span/provenance family before the public method table so static HOF
        // instructions never bake the unrelated source index (#9784).
        let helper_candidates = self.lowering_helper_function_ref_candidates(expr);
        if !helper_candidates.is_empty() {
            if let Some(index) = helper_candidates.iter().copied().find(|index| {
                self.shared_ctx
                    .function_ir_by_global_index
                    .get(index)
                    .is_some_and(|function| function.params.len() == preferred_arity)
            }) {
                return Ok(index);
            }
            return Ok(helper_candidates[0]);
        }

        if let Some(table) = self.method_tables.get(name.as_str()) {
    ...
    pub(in super::super) fn resolve_sprint_function_ref(
        ...
        // A lowered FunctionRef may denote a private helper whose spelling is
        // also a public source generic. `sprint` emits the resolved function
        // index directly, so preserve the exact span/provenance family before
        // consulting the public method table (#9784).
        let helper_candidates = self.lowering_helper_function_ref_candidates(func_expr);
        if !helper_candidates.is_empty() {
            let preferred_arity = 1 + extra_args.len();
            if let Some(index) = helper_candidates.iter().copied().find(|index| {
                self.shared_ctx
                    .function_ir_by_global_index
                    .get(index)
        ...
        let table_opt = self.method_tables.get(name.as_str()).cloned();
        ...
        if let Some(table) = table_opt {
            ...
            // `print` has runtime display-method routing in the VM
            // (`Base.print(io::IO, ::T)` preferred, then `Base.show(io::IO, ::T)`).
            // Do not statically bind `sprint(print, x)` to a concrete overload:
            // stale/incomplete compile-time signatures can over-select
            // `print(io::IO, ::VersionNumber)` for unrelated values at `Any`
            // call sites (Issue #9460). The generic print method preserves the
            // runtime semantics for both scalar and vararg sprint calls.
            if name == "print" || name == "Base.print" {
                let preferred_arity = arg_julia_types.len();
                if let Some(method) = table.methods.iter().find(|m| {
                    m.expanded_core_param_types_for_arity(preferred_arity)
                        .is_some_and(|cores| {
                            cores.len() == preferred_arity
                                && cores
                                    .iter()
                                    .skip(1)
                                    .all(|core| matches!(core, CoreType::Any))
                        })
                }) {
                    return Ok(method.global_index);
                }
            ...
            match table.dispatch(&arg_julia_types) {
                ...
                Err(_) => {
                    // Dispatch failed (e.g. arg type Any, no exact compile-time
                    // match). Prefer the generic IO + Any/Vararg{Any} method
                    // instead of the first same-arity method; otherwise adding a
                    // concrete display overload such as
                    // `print(io::IO, ::VersionNumber)` can make
                    // `sprint(print, x::Any)` bind that overload for unrelated
                    // runtime values (Issue #9460).
                    let preferred_arity = arg_julia_types.len();
                    if let Some(method) = table.methods.iter().find(|m| {
                        m.expanded_core_param_types_for_arity(preferred_arity)
                            .is_some_and(|cores| {
        ...
        match name {
            BuiltinOp::Rand => {
                ...
                if args.is_empty() {
                    ...
                    // type from the explicit RNG (Issue #9265). Must precede the
                    // dimension logic so `T` is not misread as an array length.
                    if let Some(sty) = single_scalar_rand_type(rest) {
                        self.compile_expr(&args[0])?; // push Rng
                        self.emit(Instr::RngRandScalarTyped(sty));
                        self.store_rng_back(&args[0]);
                        return Ok(scalar_rand_value_type(sty));
                    }
                    // rand(rng, x) where x is statically untyped: x may resolve to
                    // a concrete scalar type at runtime (scalar typed draw) or to a
                    // dimension (vector). Branch at runtime via RngRandArg so a
                    // `Type` value is not coerced to an array length (Issue #9265).
                    if rest.len() == 1
                        && matches!(
                            self.infer_expr_type(&rest[0]),
                            ValueType::Any | ValueType::DataType
                        )
                    {
                        self.compile_expr(&args[0])?; // push Rng
                        self.compile_expr(&rest[0])?; // push x (uncoerced)
                        self.emit(Instr::RngRandArg);
                        self.store_rng_back(&args[0]);
                        return Ok(ValueType::Any);
                    }
                    // rand(rng, T, dims...) - faithfully-typed array of a concrete
                    // scalar type from the explicit RNG (Issue #9328). Precedes the
                    // untyped-dimension path so a leading type object (Int, Float32,
                    // Bool, …) is materialized with the correct element type rather
                    // than dropped to a Float64 backing / raising a runtime error.
                    if let Some((sty, dims)) = leading_scalar_rand_array_type(rest) {
                        self.compile_expr(&args[0])?; // push Rng
                        for dim in dims {
                            self.compile_expr_as(dim, ValueType::I64)?;
                        }
                        self.emit(Instr::RngRandArrayTyped(sty, dims.len()));
                        self.store_rng_back(&args[0]);
                    ...
                    if rest.is_empty() {
                        ...
                        // rand(rng, dims...) - untyped Float64 array.
                        for dim in rest {
                            self.compile_expr_as(dim, ValueType::I64)?;
                        }
                        self.emit(Instr::RngRandArrayF64(rest.len()));
                        self.store_rng_back(&args[0]);
                        Ok(ValueType::Array)
                    }
                } else if let Some(sty) = single_scalar_rand_type(args) {
                    // rand(T) - scalar draw of a concrete bits-numeric / Bool type
                    // from the global RNG (Issue #9265). Precedes the RandArg /
                    // dimension paths so a type identifier such as `UInt32` or
                    // `Bool` is not misread as a runtime RNG or an array length.
                    self.emit(Instr::RandScalarTyped(sty));
                    Ok(scalar_rand_value_type(sty))
                    ...
                    Ok(ValueType::Any)
                } else if self.is_maybe_rng_first_arg(args) {
                    // rand(a, dims...) where a is statically untyped
                    // (ValueType::Any), e.g. a captured RNG inside a
                    // generator/comprehension body: a may be an explicit Rng or a
                    // leading array dimension. Defer the whole decision to runtime
                    // so the RNG is not coerced to an array length (Issue #9285).
                    // All operands are pushed uncoerced.
                    for arg in args {
                        self.compile_expr(arg)?;
                    }
                    self.emit(Instr::RandMaybeRng {
                        argc: args.len(),
                        is_randn: false,
                    });
                    self.store_rng_back(&args[0]);
                    Ok(ValueType::Any)
                } else if let Some((sty, dims)) = leading_scalar_rand_array_type(args) {
                    // rand(T, dims...) - faithfully-typed array of a concrete
                    // scalar type from the global RNG (Issue #9328). Covers Int,
                    // Float32, Bool, … which previously either dropped to a
                    // Float64 backing (Int) or raised a runtime error (all others).
                    for dim in dims {
                        self.compile_expr_as(dim, ValueType::I64)?;
                    }
                    self.emit(Instr::RandArrayTyped(sty, dims.len()));
                    Ok(ValueType::Array)
                } else {
                    // rand(dims...) - untyped Float64 array from the global RNG.
                    for dim in args {
                        self.compile_expr_as(dim, ValueType::I64)?;
                    }
                    self.emit(Instr::RandArray(args.len()));
                    Ok(ValueType::Array)
                }
            }
            BuiltinOp::Sqrt => {
                let arg_ty = self.infer_expr_type(&args[0]);
                if self.is_struct_type_of(arg_ty.clone(), "Complex") {
                    ...
                    // Static routing of the Complex sqrt method failed at
                    // compile time — the argument is statically known to be a
                    // Complex but its concrete element type is not (e.g. a
                    // Complex-typed variable, whose static type erases the
                    // element type). This must NOT be a fatal, span-less
                    // compilation error that kills the whole program even when
                    // the call is never executed (Issue #9342) — that violates
                    // the dispatch-first and error-span principles.
                    //
                    // Fall back to the upstream promotion path `sqrt(float(z))`:
                    // sjulia's `float(::Complex)` always yields `Complex{Float64}`,
                    // which the concrete `sqrt(::Complex{Float64})` method below
                    // resolves. Emit `float(arg)` and dispatch sqrt on the
                    // resulting `Complex{Float64}`. This mirrors the pure-Julia
                    // `sqrt(z::Complex{<:Union{Integer,Rational}}) = sqrt(float(z))`
                    // methods for the cases where static element-type resolution
                    // is unavailable.
                    if let Some(table) = self.method_tables.get("sqrt") {
                        let complex_f64 = JuliaType::Struct("Complex{Float64}".to_string());
                        if let Ok(method) = table.dispatch(std::slice::from_ref(&complex_f64)) {
                            let float_arg = Expr::Call {
                                function: "float".to_string().into(),
                            ...
                            self.compile_expr(&float_arg)?;
                            self.emit(Instr::Call(method.global_index, 1));
                            return Ok(method.return_type.clone());
                        }
                    }
                    // No concrete Complex{Float64} sqrt method is available:
                    // defer to runtime dispatch over the single-argument `sqrt`
                    // candidates, which raises a span-carrying MethodError if
                    // nothing applies rather than aborting compilation.
                    let mut candidates: Vec<usize> = Vec::new();
                    for (name, table) in self.method_tables.iter() {
                        if name != "sqrt" && !name.ends_with(".sqrt") {
                            continue;
                        }
                        for method in table.methods.iter() {
                            if method.accepts_arity(1)
                                && self
                                    .shared_ctx
                                    .function_ir_by_global_index
                    ...
                    // Not statically known to be Complex. That is not the
                    // same as statically known to be real: an Any/Union
                    // static type, or an unresolved Struct family other than
                    // Complex, must not be treated as "known real" (Issue
                    // #11486/#11481 — mirrors the guard in
                    // `compile_builtin_math`'s `sqrt` arm and `compile_sqrt`'s
                    // `Any` branch, the third of the three sqrt routers).
                    let arg_julia_ty = self.infer_julia_type(&args[0]);
                    if matches!(arg_julia_ty, JuliaType::BigFloat) {
                        self.compile_expr(&args[0])?;
                        self.emit(Instr::CallBuiltin(BuiltinId::Sqrt, 1));
                        return Ok(ValueType::BigFloat);
                    }
                    let exact_primitive = matches!(
                        arg_julia_ty,
                        JuliaType::Int8
                            | JuliaType::Int16
                            | JuliaType::Int32
                    ...
                    if matches!(arg_ty, ValueType::Any | ValueType::Struct(_)) || !exact_primitive {
                        let candidates = self.sqrt_runtime_candidates();
                        self.compile_expr(&args[0])?;
                        self.emit(Instr::CallTypedDispatchOrBuiltin(
                            BuiltinId::Sqrt,
                            "sqrt".to_string(),
            ...
            BuiltinOp::Length => {
                ...
                if let Expr::Var(name, _) = &args[0] {
                    if self.locals.get(name.as_str()) == Some(&ValueType::Dict) {
                        self.emit(Instr::LoadDict(name.to_string()));
            ...
            BuiltinOp::Push => {
                ...
                if let Expr::Var(name, _) = &args[0] {
                    ...
                    let is_set = matches!(self.locals.get(name.as_str()), Some(ValueType::Set));
                    if is_set {
                        // Set: load set, compile value, add to set, store back
                        self.emit(Instr::LoadSet(name.to_string()));
                        self.compile_expr(&args[1])?;
                        self.emit(Instr::SetAdd);
                        self.emit(Instr::StoreSet(name.to_string()));
                        self.emit(Instr::LoadSet(name.to_string()));
                        Ok(ValueType::Set)
                    } else {
                ...
                match args.len() {
                    ...
                    2 | 3 => {
                        ...
                        if let Expr::Var(name, _) = &args[0] {
                            ...
                            self.emit(Instr::StoreDict(name.to_string()));
                ...
                if args.is_empty() {
                    ...
                    if first_ty == ValueType::Rng {
                        ...
                        if rest.is_empty() {
                            ...
                            self.store_rng_back(&args[0]);
                            Ok(ValueType::F64)
                        } else if let Some(sty) = single_scalar_randn_type(rest) {
                            self.emit(Instr::RngRandnF64);
                            self.store_rng_back(&args[0]);
                            emit_randn_scalar_type_conversion(self, sty);
                            Ok(scalar_rand_value_type(sty))
                        } else {
                            for dim in rest {
                                self.compile_expr_as(dim, ValueType::I64)?;
                    ...
                    } else if let Some(sty) = single_scalar_randn_type(args) {
                        // randn(T) - scalar normal draw of a concrete floating
                        // type from the global RNG (Issue #9553). Must precede
                        // the untyped-RNG / dimension paths so `Float32` is not
                        // treated as a runtime RNG carrier or coerced to an array
                        // length.
                        self.emit(Instr::RandnF64);
                        emit_randn_scalar_type_conversion(self, sty);
                        Ok(scalar_rand_value_type(sty))
                        ...
                        // randn(a, dims...) where a is statically untyped
                        // (ValueType::Any), e.g. a captured RNG inside a
                        // generator/comprehension body: a may be an explicit Rng or
                        // a leading array dimension. Defer to runtime so the RNG is
                        // not coerced to an array length (Issue #9285). All operands
                        // are pushed uncoerced.
                        for arg in args {
                            self.compile_expr(arg)?;
                        }
                        self.emit(Instr::RandMaybeRng {
            ...
            BuiltinOp::RangeStep => {
                if args.len() != 1 {
                    return err("_range_step requires exactly 1 argument: _range_step(r)");
                }
                self.compile_expr(&args[0])?;
            ...
            BuiltinOp::DictDelete => {
                ...
                // Get the variable name for in-place modification
                if let Expr::Var(name, _) = &args[0] {
                    let var_type = self.locals.get(name.as_str()).cloned();
            ...
            BuiltinOp::Collect => {
                ...
                if matches!(
                    ...
                    if !collect_tables.is_empty() {
                        ...
                        let mut collect_candidates = Vec::new();
                        for table in &collect_tables {
                            for m in table.methods.iter() {
                                if m.param_count() != 1 {
                                    continue;
                                }
                                let is_user_range_method =
                                    self.is_user_range_collect_method(table, m);
                                ...
                                if Self::method_first_param_matches(
                                    m,
                                ...
                                {
                                    collect_candidates
                                        .push(DynamicCallCandidate::Method(m.global_index));
                                }
                            }
                        }
                            ...
                            })
                        {
                            if !collect_candidates.is_empty() {
                                ...
                                {
                                    ValueType::Array
                                } else if !has_user_range_collect_method {
                                    Self::range_collect_result_type(&arg_ty)
            ...
            BuiltinOp::Generator => {
                ...
                // Issue #9200 S3: the desugared filtered form
                // `Generator(map, Filter(pred, base))` collapses onto the native
                // filtered-generator representation (lazy FilteredFunctionIndex /
                // MakeGeneratorRuntimeFiltered), reusing the proven #9127 / #9271
                // machinery instead of constructing a Generator over a real
                // (eagerly-materialized) Filter value.
                if let Some(value_type) = self.try_compile_generator_over_filter(args)? {
                ...
                let callable = if let Some(julia_type) = Self::generator_type_object(&args[0]) {
                    self.emit(Instr::PushDataType(julia_type.name().to_string()));
                    if args.len() == 2 {
                        self.compile_expr(&args[1])?;
                ...
                } else if matches!(&args[0], Expr::Var(name, _) if self.locals.contains_key(name.as_str()))
                {
                    self.compile_expr(&args[0])?;
                    if args.len() == 2 {
                        self.compile_expr(&args[1])?;
            ...
            BuiltinOp::Eval => {
                // eval(expr) evaluates an Expr at runtime in the module where
                // the call appears — upstream defines a per-module
                // `eval(x) = Core.eval(M, x)`, so a bare `eval` inside module
                // `P.C` installs bindings in `P.C`, not `Main` (Issue #11421).
                // The explicit two-argument form `eval(m, expr)` (`Core.eval` /
                // `Base.eval`) targets the given module value.
                match args.len() {
                    1 => {
                        if let Some(module_path) = self.current_module_path.clone() {
                            self.emit(Instr::PushStr(module_path));
                            self.compile_expr(&args[0])?;
                            self.emit(Instr::CallBuiltin(BuiltinId::Eval, 2));
                        } else {
                            self.compile_expr(&args[0])?;
                    ...
                    _ => {
                        return err("eval requires 1 or 2 arguments: eval(expr) or eval(m, expr)");
                    }
                }
            ...
            BuiltinOp::TestRecordError => {
                // _test_record_error!(msg, detail) - record errored test result (Issue #10093)
                if args.len() != 2 {
                    return err(
                        "_test_record_error! requires exactly 2 arguments: _test_record_error!(msg, detail)",
                    );
                }
                self.compile_expr(&args[0])?; // msg: String (test expression)
                self.compile_expr(&args[1])?; // detail: String (exception / non-Bool description)
                self.emit(Instr::CallBuiltin(BuiltinId::TestRecordError, 2));
                Ok(ValueType::Nothing)
            }
    ...
    fn infer_generator_default_eltype(
        ...
        let return_type =
            self.infer_generator_callable_return_type(func_arg, &[element_type], 0)?;
        Self::generator_default_element_type_from_value_type(&return_type)
    }

    fn infer_generator_callable_return_type(
        &mut self,
        func_arg: &Expr,
        arg_types: &[ValueType],
        depth: usize,
        ...
        // Only a real FunctionRef carries structural helper provenance. Keep
        // that exact callable through empty-generator eltype inference instead
        // of reconstructing a same-named span-0 reference (#9784).
        if matches!(func_arg, Expr::FunctionRef { .. })
            && !self
                .lowering_helper_function_ref_candidates(func_arg)
                .is_empty()
        {
            let func_index = self
                .resolve_function_ref_with_arity(func_arg, arg_types.len())
                .ok()?;
            return self.infer_generator_function_index_return_type(func_index, arg_types, depth);
        }
        self.infer_generator_named_return_type(&func_name, arg_types, depth)
    }

    fn infer_generator_named_return_type(
        &mut self,
        func_name: &str,
        arg_types: &[ValueType],
        depth: usize,
        ...
        let normalized = func_name.strip_prefix("function ").unwrap_or(func_name);
        if arg_types.len() == 1 {
            if matches!(
                normalized,
            ...
            if matches!(normalized, "identity" | "abs" | "abs2" | "-") {
        ...
        self.infer_generator_function_index_return_type(method.global_index, arg_types, depth)
    }

    fn infer_generator_function_index_return_type(
        &mut self,
        func_index: usize,
        arg_types: &[ValueType],
        depth: usize,
        ...
        self.infer_generator_function_return_expr_type(&func_ir, arg_types, depth + 1)
    }

    fn infer_generator_function_return_expr_type(
        &mut self,
        func: &Function,
        arg_types: &[ValueType],
        depth: usize,
        ...
        self.infer_generator_bound_expr_type(expr, &bindings, depth)
            .filter(|ty| !matches!(ty, ValueType::Any))
    }

    fn infer_generator_bound_expr_type(
        &mut self,
        expr: &Expr,
        ...
        match expr {
            ...
            Expr::Call { function, args, .. } => {
                if function == "convert" && args.len() == 2 {
                    if Self::expr_names_type(&args[0], "Any") {
                        return self.infer_generator_bound_expr_type(&args[1], bindings, depth + 1);
                    }
                    if let Some(target) = Self::expr_type_name_value_type(&args[0]) {
                        return Some(target);
                    }
                }
                if let Some(target) = Self::value_type_for_type_name(function) {
                    return Some(target);
                }
                let arg_types: Option<Vec<ValueType>> = args
                    .iter()
                    .map(|arg| self.infer_generator_bound_expr_type(arg, bindings, depth + 1))
                    .collect();
                self.infer_generator_named_return_type(function, &arg_types?, depth + 1)
            }
            Expr::Ternary {
                then_expr,
                else_expr,
                ...
                let then_ty =
                    self.infer_generator_bound_expr_type(then_expr, bindings, depth + 1)?;
                let else_ty =
                    self.infer_generator_bound_expr_type(else_expr, bindings, depth + 1)?;
                if then_ty == else_ty {
                    Some(then_ty)
                } else {
                    Some(ValueType::Union(vec![then_ty, else_ty]))
                }
            }
            _ => Self::infer_simple_bound_expr_type(expr, bindings),
        }
    }
    ...
    fn expr_type_name_value_type(expr: &Expr) -> Option<ValueType> {
        match expr {
            Expr::Var(name, _) | Expr::FunctionRef { name, .. } => {
                Self::value_type_for_type_name(name)
            }
            _ => None,
        }
    }

    fn generator_default_element_type_from_value_type(
        return_type: &ValueType,
    ) -> Option<ArrayElementType> {
        match return_type {
            ValueType::Any => None,
            ValueType::Union(types) => Self::numeric_union_generator_element_type(types),
            other => {
                let element_type = ArrayElementType::from_value_type(other);
                (!matches!(element_type, ArrayElementType::Any)).then_some(element_type)
            }
        }
    }

    fn numeric_union_generator_element_type(types: &[ValueType]) -> Option<ArrayElementType> {
        let mut iter = types.iter();
        let first = ArrayElementType::from_value_type(iter.next()?);
        if !Self::is_numeric_array_element_type(&first) {
            return None;
        }
        let mut joined = first;
        for ty in iter {
            let right = ArrayElementType::from_value_type(ty);
            if !Self::is_numeric_array_element_type(&right) {
                return None;
            }
            joined = Self::join_numeric_array_element_types(&joined, &right)?;
        }
        Some(joined)
    }

    fn join_numeric_array_element_types(
        ...
        if Self::is_integer_array_element_type(left) && Self::is_integer_array_element_type(right) {
            return Some(ArrayElementType::Abstract("Integer".to_string()));
        }
        if Self::is_float_array_element_type(left) && Self::is_float_array_element_type(right) {
            return Some(ArrayElementType::Abstract("AbstractFloat".to_string()));
        }
        if Self::is_numeric_array_element_type(left) && Self::is_numeric_array_element_type(right) {
            return Some(ArrayElementType::Abstract("Real".to_string()));
        }
        None
    }

    fn is_numeric_array_element_type(element_type: &ArrayElementType) -> bool {
        Self::is_integer_array_element_type(element_type)
            || Self::is_float_array_element_type(element_type)
            || matches!(
                element_type,
                ArrayElementType::Abstract(name)
                    if matches!(name.as_str(), "Integer" | "Signed" | "Unsigned" | "AbstractFloat" | "Real")
            )
    }

    fn is_integer_array_element_type(element_type: &ArrayElementType) -> bool {
        match element_type {
            ...
            | ArrayElementType::U128 => true,
            ArrayElementType::Abstract(name) => {
                matches!(name.as_str(), "Integer" | "Signed" | "Unsigned")
            }
            _ => false,
        }
    }

    fn is_float_array_element_type(element_type: &ArrayElementType) -> bool {
        match element_type {
            ArrayElementType::F16 | ArrayElementType::F32 | ArrayElementType::F64 => true,
            ArrayElementType::Abstract(name) => name == "AbstractFloat",
            _ => false,
        }
    }
    ...
    fn value_type_for_type_name(name: &str) -> Option<ValueType> {
        match name {
            ...
            // Issue #9301: `Float16(x)` in a generator body reports Float16 so the
            // collected container narrows to Vector{Float16} like Float32/Float64.
    ...
    fn generator_type_object(expr: &Expr) -> Option<JuliaType> {
        match expr {
            ...
            Expr::Literal(Literal::DataType(type_name), _) => JuliaType::from_name(type_name)
                .or_else(|| Some(JuliaType::Struct(type_name.clone()))),
            _ => None,
        }
    }
    ...
    fn array_element_type_for_generator_type(julia_type: &JuliaType) -> Option<ArrayElementType> {
        match julia_type {
            // Issue #9301: `collect(Float16(i) for i in ...)` narrows to
            // Vector{Float16} (boxed storage tag) like the F32/F64 arms below.
    ...
    fn range_collect_result_type(julia_type: &JuliaType) -> Option<ValueType> {
        let element_type = match julia_type {
            // Legacy non-parametric range carriers default to Int64.
            JuliaType::UnitRange | JuliaType::StepRange => JuliaType::Int64,
            JuliaType::Struct(name) => {
                let (base, params) = crate::compile::types::parse_parametric_call(name)?;
                let base_leaf = base.rsplit('.').next().unwrap_or(base.as_str());
                if !matches!(base_leaf, "UnitRange" | "StepRange" | "StepRangeLen") {
                    return None;
                }
                params.first()?.to_julia_type_lossy()
            }
            _ => return None,
        };
        Self::array_element_type_for_generator_type(&element_type)
            .map(|element| ValueType::ArrayOf(element, Some(1)))
    }
        ...
        ["collect", "Base.collect"]
            ...
            .any(|table| {
                table.methods.iter().any(|method| {
                    method.param_count() == 1 && self.is_user_range_collect_method(table, method)
                })
            })
    }
    ...
    pub(super) fn module_constant_qualified_name(&self, name: &str) -> Option<String> {
...
fn array_element_value_type(element: &ArrayElementType) -> ValueType {
    match element {
        ArrayElementType::F16 => ValueType::F16,
        ...
        ArrayElementType::StructOf(id)
        | ArrayElementType::StructInlineOf(id, _)
        | ArrayElementType::StructInlineF64(id, _) => ValueType::Struct(*id),
        ArrayElementType::Any
        | ArrayElementType::TupleOf(_)
        | ArrayElementType::UnionOf(_)
        | ArrayElementType::Abstract(_)
        | ArrayElementType::Structured(_) => ValueType::Any,
    }
}
