...
use super::shared;
use crate::bytecode::value::{array_element_type_to_julia_type, julia_array_type_for_ndims};
...
use crate::types::{parse_type_args_recursive, JuliaType, TypeExpr};
...
fn shared_ctx_struct_type_id(ctx: &SharedCompileContext, name: &str) -> Option<usize> {
    ctx.struct_table.resolve(name).map(|(_, info)| info.type_id)
}
...
/// Convert only an exact-name lookup or structurally resolved instantiation to
/// a concrete constructor return. A miss remains dynamic (Issue #11436).
pub(super) fn exact_constructor_return_type(type_id: Option<usize>) -> ValueType {
    type_id.map(ValueType::Struct).unwrap_or(ValueType::Any)
}

/// A constructor result is concrete only when every inferred parameter names a
/// concrete Julia type. `Any` and abstract/type-variable projections do not
/// identify the runtime instantiation (Issue #11436).
pub(super) fn constructor_type_args_are_complete(type_args: &[JuliaType]) -> bool {
...
/// (Issue #5922).
///
/// Resolution requires complete inferred type arguments, then uses an exact
/// concrete entry (`Base{A, B}`) or on-demand structural instantiation. It
/// never substitutes another same-base instantiation (Issue #11436).
pub(super) fn infer_value_parametric_struct_ctor(
    ...
    if !constructor_type_args_are_complete(&type_args) {
        return ValueType::Any;
    }
    ...
    if let Some(type_id) = inst.struct_type_id(&concrete_name) {
        return exact_constructor_return_type(Some(type_id));
    }
    // Instantiate on demand so type inference can be precise for arrays.
    exact_constructor_return_type(inst.resolve_instantiation(function, &type_args))
}

/// ValueType-path resolution for the parametric `Rational` constructor
/// (Issues #5922/#11436): only the exact complete spelling may establish
/// concrete identity.
pub(super) fn infer_value_rational_ctor(
    function: &str,
    inst: &dyn StructInstantiation,
) -> ValueType {
    exact_constructor_return_type(inst.struct_type_id(function))
}
...
fn constructor_type_expr_is_complete(
    type_expr: &TypeExpr,
    is_active_type_param: &dyn Fn(&str) -> bool,
) -> bool {
    match type_expr {
        TypeExpr::Concrete(ty) => ty.is_concrete(),
        // The shared parser represents both unresolved nominal names and
        // literal value parameters as TypeVar atoms. Only names bound by the
        // active method are incomplete here.
        TypeExpr::TypeVar(name) => !is_active_type_param(name),
        TypeExpr::Parameterized { params, .. } => params
            .iter()
            .all(|param| constructor_type_expr_is_complete(param, is_active_type_param)),
...
pub(super) fn infer_value_instantiated_ctor(
    ...
    let Some(close) = function.rfind('}').filter(|close| *close > open) else {
        return ValueType::Any;
    };
    let Some(type_exprs) = parse_type_args_recursive(&function[open + 1..close]) else {
        return ValueType::Any;
    };
    if type_exprs.is_empty()
        || !type_exprs
            .iter()
            .all(|arg| constructor_type_expr_is_complete(arg, is_active_type_param))
    {
        return ValueType::Any;
    }
    ...
    let type_args: Vec<JuliaType> = type_exprs
        .iter()
        .map(TypeExpr::to_julia_type_lossy)
        .collect();
    exact_constructor_return_type(inst.resolve_instantiation(resolved_base_name, &type_args))
}

/// Map a constructor-result lattice type to the legacy `ValueType`
/// representation.
///
/// Deliberately bypasses `crate::runtime_types::bridge::lattice_to_value_type(&LatticeType)`: the bridge aliases
...
/// (Issue #5922).
///
/// Unknown argument types must remain dynamic rather than inventing a
/// `Complex{Float64}` identity (Issues #11436/#11468).
...
pub(super) fn is_dict_constructor_name(function: &str) -> bool {
...
fn dict_struct_julia_type_from_lattice(result: &LatticeType) -> Option<JuliaType> {
    ...
    let key_ty = crate::runtime_types::bridge::lattice_to_julia_type(&LatticeType::Concrete(
        (**key).clone(),
    ));
    let value_ty = crate::runtime_types::bridge::lattice_to_julia_type(&LatticeType::Concrete(
        (**value).clone(),
    ));
    Some(JuliaType::Struct(format!(
        "Dict{{{},{}}}",
...
pub(super) fn is_set_constructor_name(function: &str) -> bool {
...
fn set_struct_julia_type_from_lattice(result: &LatticeType) -> Option<JuliaType> {
    ...
    let element_ty = crate::runtime_types::bridge::lattice_to_julia_type(&LatticeType::Concrete(
        (**element).clone(),
    ));
...
fn infer_value_linear_algebra_call(function: &str) -> Option<ValueType> {
    ...
    Some(match &result {
        ...
        other => crate::runtime_types::bridge::lattice_to_value_type(other),
    })
}
...
/// - a concrete `TupleOf` argument collects to a vector of the promoted field
///   type, matching `collect(::Tuple)`,
/// - everything else stays the bare `Array`.
fn collect_tuple_element_type(types: &[JuliaType]) -> JuliaType {
    let Some(first) = types.first() else {
        return JuliaType::Any;
    };
    let mut joined = CoreType::from_julia_type_preserving_owner(first);
    for ty in types.iter().skip(1) {
        joined = joined.typejoin(&CoreType::from_julia_type_preserving_owner(ty));
        if matches!(joined, CoreType::Any | CoreType::Bottom) {
            return JuliaType::Any;
        }
    }
    crate::inference_core::core_type_to_julia_type(&joined)
}
    ...
    F: FnMut(&Expr) -> JuliaType,
{
    ...
    match first {
        ...
        JuliaType::TupleOf(types) => {
            JuliaType::VectorOf(Box::new(collect_tuple_element_type(&types)))
        }
        _ => JuliaType::Array,
    }
}
...
enum FixedFallback {
    ...
    /// Fall back to the given concrete type only when every inferred argument is
    /// an integer; for float/mixed/unknown args, defer to dynamic dispatch
    /// instead of pinning the concrete type. Used by `div`/`rem`/`mod` and
    /// `fld`/`cld`, whose result is `Int64` only for all-integer args —
    /// `mod(::Int64, ::Float64)` is `Float64`, and `fld(::BigFloat, ::BigFloat)`
    /// is `BigFloat`, so an unconditional `Int64` fallback silently coerces a
    /// nested non-integer result (Issues #9528, #8780).
    IntegerArgsElseDefer(ConcreteType),
}

/// Integer-typed argument test for [`FixedFallback::IntegerArgsElseDefer`]
/// (Issues #9528, #8780). Concrete and abstract integer types keep the `Int64`
/// fallback; floats, `Any`, structs and everything else defer to dynamic.
fn julia_type_is_integer_for_divrem(ty: &JuliaType) -> bool {
    matches!(
        ty,
        JuliaType::Int8
            | JuliaType::Int16
...
fn normalized_value_general_tfunc(function: &str) -> Option<(&str, FixedFallback)> {
    ...
    match name {
        ...
        | "chomp" | "chop" | "takestring!" | "sprint" | "sprintf" | "lowercasefirst"
        ...
        // gcd/lcm route through the general tfunc registry (tfunc_gcd) so BigInt
        // args preserve BigInt, and default to Int64 otherwise (Issues #5922,
        // #2383). This is a general name-normalizer entry, not a return-type
        // override: the audited `infer_gcd_lcm` kill-switch that could disable it
        // was retired in Issue #8618 (parent #8608) once the audit confirmed the
        // general inference path already matches upstream julia.
        "gcd" | "lcm" => Some((
            name,
            FixedFallback::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Int64,
...
fn normalized_julia_general_tfunc(function: &str) -> Option<(&str, FixedFallback)> {
    ...
    match name {
        "string" | "uppercase" | "lowercase" | "join" | "repr" | "strip" | "lstrip" | "rstrip"
        | "chomp" | "chop" | "takestring!" | "sprint" | "lowercasefirst" | "uppercasefirst"
        | "escape_string" | "chopprefix" | "chopsuffix" | "lpad" | "rpad" | "bitstring"
        | "ascii" | "unescape_string" => Some((name, FixedFallback::String)),
        ...
        "div" | "rem" | "mod" => Some((
            name,
            FixedFallback::IntegerArgsElseDefer(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Int64,
            ))),
        )),
        "sqrt" | "sin" | "cos" | "exp" | "log" => Some((
        ...
        // Int64-result helpers: hash and date/time accessors keep their legacy
        // unconditional Int64 result (Issue #5922).
        "hash" | "year" | "month" | "day" | "hour" | "minute" | "second" | "dayofweek"
        | "dayofyear" | "week" | "days" => Some((
            name,
            FixedFallback::Concrete(ConcreteType::Core(CoreType::Primitive(
                CorePrimitive::Int64,
            ))),
...
pub(super) fn infer_value_type_object_call(
    ...
    Some(crate::runtime_types::bridge::lattice_to_value_type(&result))
}
    ...
    FS: FnMut(&str) -> Option<usize>,
{
    ...
    let lattice_args = (0..args.len())
        .map(|idx| match name {
            ...
            _ => crate::runtime_types::bridge::value_type_to_lattice(&value_args[idx]),
        })
        .collect::<Vec<_>>();
    ...
    let dims = array_constructor_dims_args(name, args, julia_args);
    Some(ValueType::ArrayOf(element, dims_rank_from_args(dims)))
}
...
enum RandLead {
    /// A leading concrete scalar type object whose random array sjulia
    /// materializes with a faithful element type: `rand(T, dims...)` builds
    /// `Array{T}` (`rand(Int, n)` → `Vector{Int64}`, `rand(Float32, n)` →
    /// `Vector{Float32}`, …). Carries that pinned element type (Issue #9328).
    TypeObject(ArrayElementType),
    /// A leading scalar type object whose random array is NOT element-faithful in
    /// sjulia (`Float16` → `Vector{Any}`, no native array storage): DEFER so
    /// inference never disagrees with the value the VM actually builds
    /// (Issue #9328).
    DeferTypeObject,
...
/// Faithful array element type sjulia materializes for a leading
/// `rand(T, dims...)` type object, or `None` to defer — either `name` is not a
/// scalar type object, or it is `Float16`, whose random array degrades to
/// `Vector{Any}` (no native F16 array storage, see `compile/expr/collection.rs`)
/// so pinning it would disagree with the value the VM builds. `Int`/`UInt` pin
/// the 64-bit host element, matching the `rand` lowering
/// (`builtin.rs::scalar_rand_type_from_name`) — Issue #9328.
fn rand_lead_array_element_type(name: &str) -> Option<ArrayElementType> {
    Some(match name {
...
/// (`builtin.rs::BuiltinOp::Rand` / `leading_scalar_rand_array_type`), so this
/// classification must agree with the instruction that is actually emitted
/// (Issue #7307, extended in Issue #9328).
fn rand_lead_kind(arg: &Expr) -> RandLead {
    let Expr::Var(name, _) = arg else {
        return RandLead::None;
    };
    match rand_lead_array_element_type(name) {
        Some(elem) => RandLead::TypeObject(elem),
        // A scalar type name we cannot pin faithfully (only `Float16`) must
        // defer; any other identifier is a plain leading dimension variable.
        None if crate::compile::expr::builtin::scalar_rand_type_from_name(name).is_some() => {
            RandLead::DeferTypeObject
        }
        None => RandLead::None,
    }
}
...
///
/// The element type is pinned to whatever the VM actually materializes: the
/// dimension-only forms (`rand(n)`, all `randn`) and a leading `Float64` object
/// give `Float64`, and a leading faithfully-typed scalar object gives that type
/// (`rand(Int, n)` → `Vector{Int64}`, `rand(Float32, n)` → `Vector{Float32}`,
/// …) now that the runtime builds element-faithful typed arrays (Issue #9328,
/// via [`rand_lead_array_element_type`]). `rand(Float16, n)` is deliberately
/// deferred — sjulia has no native `Float16` array storage so the runtime
/// degrades to `Vector{Any}`, and pinning `Vector{Float16}` would disagree.
///
/// Returns `None` (defer to the legacy `Array` fallback) for forms whose shape we
/// cannot pin from the call alone: a leading `Float16` type object (see above),
/// an explicit-RNG first argument (`rand(rng, ...)`), or a non-integer first
/// argument such as a collection (`rand(itr)` / `rand([1.0, 2.0])`, whose result
/// is the element type, not an array).
    ...
    F: FnMut(&Expr) -> JuliaType,
{
    ...
    let (dims, element): (&[Expr], JuliaType) = if is_randn {
        (args, JuliaType::Float64)
    } else {
        match rand_lead_kind(&args[0]) {
            RandLead::DeferTypeObject => return None,
            RandLead::TypeObject(elem) => (&args[1..], array_element_type_to_julia_type(&elem)),
            RandLead::None => (args, JuliaType::Float64),
        }
    };

    // `rand(Float64)` / `rand(Int)` (type object only) samples a scalar, not an
    // array — defer.
    ...
    Some(julia_array_type_for_ndims(element, rank))
}
    ...
    F: FnMut(&Expr) -> ValueType,
{
    ...
    let (dims, element): (&[Expr], ArrayElementType) = if is_randn {
        (args, ArrayElementType::F64)
    } else {
        match rand_lead_kind(&args[0]) {
            RandLead::DeferTypeObject => return None,
            RandLead::TypeObject(elem) => (&args[1..], elem),
            RandLead::None => (args, ArrayElementType::F64),
        }
    };
    ...
    let rank = dims_rank_from_args(dims)?;
    Some(ValueType::ArrayOf(element, Some(rank)))
}
    ...
    F: FnMut(&Expr) -> ValueType,
{
    ...
    if let Some(name) = normalized_first_arg_tfunc_name(function) {
        ...
        let first_lattice = crate::runtime_types::bridge::value_type_to_lattice(&first);
        ...
        return Some(crate::runtime_types::bridge::lattice_to_value_type(&result));
    }
    ...
    let result = general_tfunc_result(
        ...
        inferred_args
            .iter()
            .map(crate::runtime_types::bridge::value_type_to_lattice),
        inferred_args.len(),
    );
    Some(result.map_or_else(
        || legacy_fixed_value_type_fallback(fallback),
        |result| crate::runtime_types::bridge::lattice_to_value_type(&result),
    ))
}
...
fn legacy_fixed_value_type_fallback(fallback: FixedFallback) -> ValueType {
    match fallback {
        ...
        FixedFallback::Concrete(concrete)
        | FixedFallback::ConcreteDeferStructAny(concrete)
        | FixedFallback::IntegerArgsElseDefer(concrete) => {
            crate::runtime_types::bridge::lattice_to_value_type(&LatticeType::Concrete(concrete))
        }
    }
}
    ...
    F: FnMut(&Expr) -> JuliaType,
{
    ...
    // Arg-type-dependent fallback (Issue #9528): `div`/`rem`/`mod` only yield
    // `Int64` when every argument is an integer. When the registry tfunc cannot
    // pin a result (`Top`, e.g. mixed `Int64 x Float64`), pinning `Int64` here
    // makes a nested consumer insert `DynamicToI64` on a float result. Defer to
    // dynamic dispatch unless all args are integers.
    if let FixedFallback::IntegerArgsElseDefer(ref concrete) = fallback {
        return Some(match result {
            Some(result) => lattice_to_julia_type(&result)
                .unwrap_or_else(|| julia_type_from_concrete_type(concrete)),
            None => {
                if inferred_args.iter().all(julia_type_is_integer_for_divrem) {
...
pub(super) fn infer_value_type_call_from_value_args(
    function: &str,
    inferred_args: &[ValueType],
) -> Option<ValueType> {
    if let Some(name) = normalized_first_arg_tfunc_name(function) {
        let first = inferred_args.first()?;
        let first_lattice = crate::runtime_types::bridge::value_type_to_lattice(first);
        let result = first_arg_tfunc_result(name, inferred_args.len(), first_lattice.clone())?;
...
fn legacy_fixed_julia_type_fallback(fallback: FixedFallback) -> JuliaType {
    match fallback {
        ...
        FixedFallback::Concrete(concrete)
        | FixedFallback::ConcreteDeferStructAny(concrete)
        | FixedFallback::IntegerArgsElseDefer(concrete) => julia_type_from_concrete_type(&concrete),
    }
}
...
fn julia_type_to_lattice(ty: &JuliaType) -> LatticeType {
    ...
    if tests::should_delegate_pin(ty) {
        return crate::runtime_types::bridge::julia_type_to_lattice(ty);
    }
    match ty {
        ...
        _ => crate::runtime_types::bridge::julia_type_to_lattice(ty),
    }
}
...
fn julia_type_from_concrete_type(ty: &ConcreteType) -> JuliaType {
    match ty {
        ...
        // Memory{T} — report as a named struct so type-reflection and
        // function dispatch see the correct type name (Issue #9009).
        ConcreteType::Memory { element, .. } => {
            let elem_name = julia_type_from_concrete_type(element).name().to_string();
            JuliaType::Struct(format!("Memory{{{}}}", elem_name))
        }
...
mod issue_10460_tests {
    use super::{collect_tuple_element_type, JuliaType};

    #[test]
    fn collect_tuple_typejoin_stays_structural_issue_10460() {
        assert_eq!(
            collect_tuple_element_type(&[JuliaType::Int64, JuliaType::Float64]),
            JuliaType::Real
        );
        assert_eq!(
            collect_tuple_element_type(&[JuliaType::Int64, JuliaType::Bool]),
            JuliaType::Integer
        );
        assert_eq!(
            collect_tuple_element_type(&[JuliaType::Int32, JuliaType::Int64]),
            JuliaType::Signed
        );
        assert_eq!(
            collect_tuple_element_type(&[JuliaType::Int64, JuliaType::String]),
            JuliaType::Any
        );

        crate::types::register_type_name("OwnerA10460.Box10460");
        crate::types::register_type_name("OwnerB10460.Box10460");
        assert_eq!(
            collect_tuple_element_type(&[
                JuliaType::Struct("OwnerA10460.Box10460{Int64}".to_string()),
                JuliaType::Struct("OwnerB10460.Box10460{Int64}".to_string()),
            ]),
            JuliaType::Any
        );
    }
...
mod tests {
    ...
    fn julia_rand_randn_keep_arity_split() {
        ...
        // `rand(Int, n)` now pins `Vector{Int64}` — the typed-array runtime is
        // element-faithful (Issue #9328), so inference agrees with the value the
        // VM builds instead of deferring to the unparameterized Array.
        let int_var = Expr::Var(
            "Int".to_string().into(),
            crate::span::Span::new(0, 0, 1, 1, 1, 1),
        );
        assert_eq!(
        ...
        // `rand(Float32, n, m)` pins a ranked `Matrix{Float32}`.
        let f32_var = Expr::Var(
            "Float32".to_string().into(),
            crate::span::Span::new(0, 0, 1, 1, 1, 1),
        );
        assert_eq!(
            infer_julia_type_call("rand", &[f32_var, lit(), lit()], |arg| {
                if matches!(arg, Expr::Var(n, _) if n == "Float32") {
                    JuliaType::DataType
                } else {
                    JuliaType::Int64
                }
        ...
        // `rand(Float16, n)` stays deferred: sjulia has no native Float16 array
        // storage, so the runtime degrades to `Vector{Any}` and inference must
        // not pin `Vector{Float16}` (Issue #9328).
        let f16_var = Expr::Var(
            "Float16".to_string().into(),
            crate::span::Span::new(0, 0, 1, 1, 1, 1),
        );
        assert_eq!(
            infer_julia_type_call("rand", &[f16_var, lit()], |arg| {
                if matches!(arg, Expr::Var(n, _) if n == "Float16") {
                    JuliaType::DataType
                } else {
                    JuliaType::Int64
                }
    ...
    fn value_rand_randn_rank_aware() {
        ...
        // `rand(Int, n)` pins the faithful ranked element type (Issue #9328).
        let int_var = Expr::Var(
            "Int".to_string().into(),
            crate::span::Span::new(0, 0, 1, 1, 1, 1),
        );
        assert_eq!(
            infer_value_type_call("rand", &[int_var, lit()], |arg| {
                if matches!(arg, Expr::Var(n, _) if n == "Int") {
                    ValueType::DataType
                } else {
                    ValueType::I64
            ...
            Some(ValueType::ArrayOf(ArrayElementType::I64, Some(1)))
        );
    ...
    /// Prevention coverage (Issue #10538, follow-up to #10512): HOF named
    /// callables — `map(<callable>, xs)` resolving `<callable>` to a plain
    /// named function rather than an inline lambda — go through
    /// [`infer_value_type_call_from_value_args`] (called from
    /// `compile::expr::infer::hof::infer_named_map_callable_return_type`),
    /// *not* the Expr-based [`infer_value_type_call`] tested above. That
    /// sibling function has no `legacy_fixed_*_fallback` safety net: if its
    /// `general_tfunc_result` lookup ever declined for one of these
    /// genuinely-fixed-result callables, `map_call_result`
    /// (`compile::tfuncs::hof_ops`) would silently fall back to
    /// `tfunc_map`'s conservative *input-element-type-preserving* rule — the
    /// exact defect shape of #10512 (`map(string, [42])` inferred as
    /// `Vector{Int64}`, so `r[1]` emitted a typed integer consumer for a
    /// `String` value).
    ///
    /// This table locks in, directly against the function the HOF path
    /// actually calls, that every named callable with a result type that is
    /// genuinely independent of the input element type never reports back
    /// that input element type — across a spread of representative element
    /// types (`Int64`, `Float64`, `Bool`, a struct, and `Any`).
    #[test]
    fn hof_value_tfuncs_fixed_string_callables_never_preserve_input_element_type() {
        // Every named callable registered as `FixedFallback::String`
        // (`normalized_value_general_tfunc`) whose result is meaningfully
        // exercised with a single non-string argument by `map`/`broadcast`.
        // (`escape_string`/`ascii`/`chomp`/etc. only accept `AbstractString`
        // in upstream Julia, so a non-string element is not a realistic HOF
        // input for them and they are intentionally left out of this table.)
        const FIXED_STRING_CALLABLES: &[&str] = &["string", "repr", "bitstring"];
        ...
        for &name in FIXED_STRING_CALLABLES {
            for element in &element_types {
                let result =
                    infer_value_type_call_from_value_args(name, std::slice::from_ref(element));
                assert_eq!(
                    result,
                    Some(ValueType::Str),
                    "{name} over {element:?} must sharpen to String"
                );
                assert_ne!(
                    result,
                    Some(element.clone()),
                    "{name} over {element:?} must not fall back to the input element type"
                );
            }
        }
    }

    /// Companion to the String-group table above: `typeof` is registered as
    /// `FixedFallback::Concrete(DataType)` (Issue #10538) — its result is a
    /// `DataType`, never the input element type, so `map(typeof, xs)` must
    /// sharpen to `Vector{DataType}` rather than preserving `xs`'s element
    /// type.
    #[test]
    fn hof_value_tfuncs_typeof_never_preserves_input_element_type() {
        let element_types = [
            ValueType::I64,
            ValueType::F64,
            ValueType::Bool,
        ...
        for element in &element_types {
            let result =
                infer_value_type_call_from_value_args("typeof", std::slice::from_ref(element));
            assert_ne!(
                result,
                Some(element.clone()),
                "typeof over {element:?} must not fall back to the input element type"
            );
        }
    }
    ...
    fn array_constructor_tfuncs_cover_value_type_zeros_and_ones() {
        ...
        assert_eq!(
            ...
            Some(ValueType::ArrayOf(ArrayElementType::I16, Some(1)))
        );
    ...
    fn array_constructor_tfuncs_cover_value_type_fill_and_complex_type_objects() {
        assert_eq!(
            ...
            Some(ValueType::ArrayOf(ArrayElementType::F32, Some(1)))
        );
        ...
        assert_eq!(
            ...
            Some(ValueType::ArrayOf(ArrayElementType::ComplexF64, Some(1)))
        );
    }
    ...
    fn julia_type_adapter_routes_int64_result_helpers() {
        ...
        assert_eq!(
            infer_julia_type_call("fld", &[lit(), lit()], |_| JuliaType::BigFloat),
            None
        );
    ...
    #[test]
    fn julia_type_adapter_keeps_promote_type_result_datatype_for_dispatch_9914() {
        let int_arg = Expr::Var(
            "Int64".to_string().into(),
            crate::span::Span::new(0, 0, 1, 1, 1, 1),
        );
        let float_arg = Expr::Var(
            "Float64".to_string().into(),
            crate::span::Span::new(0, 0, 1, 1, 1, 1),
        );
        let args = [int_arg, float_arg];

        assert_eq!(
            infer_julia_type_call("promote_type", &args, |arg| {
                if matches!(arg, Expr::Var(name, _) if name == "Int64") {
                    type_of(JuliaType::Int64)
                } else {
                    type_of(JuliaType::Float64)
                }
            }),
            Some(JuliaType::DataType)
        );
        assert_eq!(
            infer_julia_type_call("promote_rule", &args, |arg| {
                if matches!(arg, Expr::Var(name, _) if name == "Int64") {
                    type_of(JuliaType::Int64)
                } else {
                    type_of(JuliaType::Float64)
    ...
    #[test]
    fn complex_value_adapter_keeps_unknown_element_dynamic_11468() {
        let ids = StubIds {
            complex_f64: Some(42),
            point: None,
        };
    ...
    #[test]
    fn complex_julia_adapter_keeps_unknown_element_dynamic_11468() {
        let ids = StubIds {
            complex_f64: Some(42),
            point: None,
        };
        assert_eq!(infer_julia_complex_call(&ids), JuliaType::Any);
    ...
    // Issue #11436: constructor return identity is exact-or-Any.
    #[test]
    fn parametric_struct_ctor_requires_complete_identity_11436() {
        ...
        // Sibling registrations in either order cannot replace the requested
        // identity when instantiation fails.
        for exact in [
            vec![("Point{Float64}", 5), ("Point{Bool}", 6)],
            vec![("Point{Bool}", 6), ("Point{Float64}", 5)],
        ] {
            let mut inst = StubInstantiation {
                type_args: Some(vec![JuliaType::Int64]),
                exact,
        ...
        // Missing type arguments stay dynamic even if a bare family entry exists.
        let mut inst = StubInstantiation {
            exact: vec![("Point", 7)],
            ..Default::default()
        };
        ...
        // An inferred `Any` is not a complete runtime parameter. Do not create
        // `Point{Any}` and statically dispatch on that invented identity.
        let mut inst = StubInstantiation {
            type_args: Some(vec![JuliaType::Any]),
            exact: vec![("Point{Any}", 8)],
            instantiation: Some(8),
            ..Default::default()
        };
        assert_eq!(
            infer_value_parametric_struct_ctor("Point", &mut inst, &[JuliaType::Any]),
            ValueType::Any
        );
        assert!(inst.resolved_calls.borrow().is_empty());
    }
    ...
    #[test]
    fn rational_ctor_requires_exact_complete_entry_11436() {
        ...
        let inst = StubInstantiation {
            exact: vec![("Rational{Bool}", 12), ("Rational{Float64}", 13)],
            ..Default::default()
        };
        assert_eq!(
            infer_value_rational_ctor("Rational{Int64}", &inst),
    ...
    #[test]
    fn instantiated_ctor_parses_type_args_without_family_fallback_11436() {
        // Exact entry wins.
        let mut inst = StubInstantiation {
            exact: vec![("Val{2}", 21)],
            ..Default::default()
        ...
        assert_eq!(
            infer_value_instantiated_ctor("Val{2}", "Val", &mut inst, &|_| false),
            ValueType::Struct(21)
        );
        assert!(inst.resolved_calls.borrow().is_empty());
        ...
        assert_eq!(
            infer_value_instantiated_ctor("Point{Int64}", "MyGeometry.Point", &mut inst, &|_| {
                false
            },),
            ValueType::Struct(22)
        );
        ...
        // Instantiation failure stays Any for forward and reverse sibling
        // registration order.
        for exact in [
            vec![("Val{1}", 23), ("Val{2}", 24)],
            vec![("Val{2}", 24), ("Val{1}", 23)],
        ] {
            let mut inst = StubInstantiation {
                exact,
                ..Default::default()
            ...
            assert_eq!(
                infer_value_instantiated_ctor("Val{9}", "Val", &mut inst, &|_| false),
                ValueType::Any
            );
        }
        ...
        let mut inst = StubInstantiation {
            exact: vec![("Val{T}", 25)],
            instantiation: Some(25),
            ..Default::default()
        };
        assert_eq!(
            infer_value_instantiated_ctor("Val{T}", "Val", &mut inst, &|name| name == "T"),
            ValueType::Any
        );
        assert!(inst.resolved_calls.borrow().is_empty());

        let mut inst = StubInstantiation {
            exact: vec![("Outer{Inner{T}}", 26)],
            instantiation: Some(26),
            ..Default::default()
        };
        assert_eq!(
            infer_value_instantiated_ctor("Outer{Inner{T}}", "Outer", &mut inst, &|name| name
                == "T",),
            ValueType::Any
        );
        assert!(inst.resolved_calls.borrow().is_empty());
    ...
    fn julia_type_to_lattice_pins_type_object_and_legacy_edges() {
        ...
        assert_eq!(
            ...
            crate::runtime_types::bridge::julia_type_to_lattice(&tuple)
        );
        assert_eq!(
            julia_type_to_lattice(&tuple),
    ...
    fn julia_type_to_lattice_adopts_canonical_union_handling() {
        ...
        assert_eq!(
            ...
            crate::runtime_types::bridge::julia_type_to_lattice(&union)
        );
        assert!(matches!(
            julia_type_to_lattice(&union),
            LatticeType::Union(_)
    ...
    fn pin_audit_load_bearing_arms_diverge_dead_arms_match() {
        ...
        // pinned is load-bearing EXCEPT the dead arms proven by the audit:
        //  - `TupleOf` (now delegated; element types never surface),
        ...
        //    canonical→Struct{0}), and
        //  - `TypeOf(concrete)` (Issue #9955): the canonical bridge now maps a
        //    `Type{C}` annotation over a concrete `C` to the same
        //    `DataType{name}` singleton the local pin computes (constant
        //    type-object propagation), so the sampled `TypeOf(Int64)` arm no
        //    longer diverges. `TypeOf(TypeVar(_))` (an unbound `where`
        //    parameter, not currently sampled here) is NOT covered by the
        //    canonical arm and would still diverge — computed rather than
        //    hardcoded so a future TypeVar-inner sample is caught correctly.
        let load_bearing = |ty: &JuliaType| -> bool {
            match ty {
                JuliaType::TupleOf(_) => false,
                JuliaType::VectorOf(element) | JuliaType::MatrixOf(element) => {
                    julia_type_to_lattice(element)
                        != crate::runtime_types::bridge::julia_type_to_lattice(element)
                }
                JuliaType::TypeOf(inner) => matches!(inner.as_ref(), JuliaType::TypeVar(_, _)),
                JuliaType::Struct(_)
                | JuliaType::Signed
                | JuliaType::Unsigned
                | JuliaType::Bottom
