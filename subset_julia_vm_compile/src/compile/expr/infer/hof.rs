...
use crate::compile::inference::infer_simple_function_return_type_for_value_args;
...
impl HofLambdaAnalyzer for CoreCompiler<'_> {
    fn map_mapped_element_type(
        ...
        let element_value_types: Vec<ValueType> = input_elements
            .iter()
            .map(crate::runtime_types::bridge::lattice_to_value_type)
            .collect();
        let mapped = self.map_callable_return_value_types(func_expr, &element_value_types)?;
        Some(crate::runtime_types::bridge::value_type_to_lattice(&mapped))
    }

    fn reduce_result_type(&mut self, op_expr: &Expr, element: &LatticeType) -> Option<LatticeType> {
        let element_type = crate::runtime_types::bridge::lattice_to_value_type(element);
        let result = self.infer_reduce_operator_return_type(op_expr, element_type)?;
        Some(crate::runtime_types::bridge::value_type_to_lattice(&result))
    }
}
...
impl CoreCompiler<'_> {
    ...
    pub(in crate::compile) fn infer_map_call_return_type(
        ...
        let arg_types = [
            crate::runtime_types::bridge::value_type_to_lattice(&self.infer_expr_type(func_arg)),
    ...
    pub(in crate::compile) fn infer_reduce_call_return_type(
        ...
        op_arg: &Expr,
        itr_arg: &Expr,
        init_arg: Option<&Expr>,
    ) -> Option<ValueType> {
        let element_type = self.hof_iterable_element_type(itr_arg)?;
        if let Some(init_arg) = init_arg {
            if is_visibly_empty_iterable_literal(itr_arg) {
                let init_type = self.infer_expr_type(init_arg);
                if !matches!(init_type, ValueType::Any) {
    ...
    pub(in crate::compile) fn infer_hof_call_site_return_type(
        ...
        match name {
            ...
            "mapreduce" | "mapfoldl" | "mapfoldr" if args.len() >= 3 => {
                self.infer_mapreduce_call_return_type(&args[0], &args[1], &args[2], args.get(3))
            }
    ...
    fn infer_reduce_operator_return_type(
        ...
        match op_name.as_str() {
            ...
            "+" | "*" | "-" | "/" | "^" => {
                binary_numeric_map_return_type(op_name.as_str(), &element_type, &element_type)
...
impl CoreCompiler<'_> {
    fn hof_iterable_element_type(&mut self, iter_arg: &Expr) -> Option<ValueType> {
        ...
        if let Expr::TypedEmptyArray { element_type, .. } = iter_arg {
            return typed_empty_array_element_value_type(element_type);
        }
    ...
    fn resolve_hof_callable(&self, func_arg: &Expr) -> Option<HofCallable> {
        match func_arg {
            Expr::FunctionRef { name, .. } | Expr::Var(name, _) => {
                Some(HofCallable::Named(name.to_string()))
            }
            Expr::LetBlock { body, .. } => {
    ...
    fn infer_named_map_callable_return_type(
        ...
        if let Some(inferred) = super::expr_tfuncs::infer_value_type_call_from_value_args(
            normalized_func_name,
            std::slice::from_ref(element_type),
        ) {
        ...
        if matches!(&method.return_type, ValueType::Any) {
                ...
                .get(&method.global_index)
            {
                ...
                return infer_simple_function_return_type_for_value_args(
                    func_ir,
                    std::slice::from_ref(element_type),
                );
            }
...
fn value_type_to_array_concrete(
    ...
    match crate::runtime_types::bridge::value_type_to_lattice(element_type) {
...
fn array_result_to_value_type(result: &LatticeType) -> Option<ValueType> {
    match result {
        LatticeType::Concrete(ConcreteType::Array { element, .. }) => {
            let mapped = crate::runtime_types::bridge::lattice_to_value_type(
...
fn map_input_element_value_type(element_type: &ArrayElementType) -> ValueType {
    match element_type {
        ...
        // Issue #9301: Float16 element carries the scalar Float16 value type.
...
fn map_return_element_type(return_type: &ValueType) -> ArrayElementType {
    match return_type {
        ...
        // Issue #9301: `Float16.(xs)` / `map(f16, xs)` narrows to Vector{Float16}.
...
fn binary_numeric_map_return_type(
    ...
    match (func_name, left_element_type, right_element_type) {
        ...
        ("+" | "-" | "*", ValueType::U8, ValueType::U8) => Some(ValueType::U8),
        ("+" | "-" | "*", ValueType::U16, ValueType::U16) => Some(ValueType::U16),
        ("+" | "-" | "*", ValueType::U32, ValueType::U32) => Some(ValueType::U32),
        ("+" | "-" | "*", ValueType::U64, ValueType::U64) => Some(ValueType::U64),
        ...
        ("/", ValueType::F32, ValueType::F32) => Some(ValueType::F32),
        ("/", ValueType::F64, ValueType::F64) => Some(ValueType::F64),
        ("min" | "max", ValueType::I8, ValueType::I8) => Some(ValueType::I8),
        ("min" | "max", ValueType::I16, ValueType::I16) => Some(ValueType::I16),
        ("min" | "max", ValueType::I64, ValueType::I64) => Some(ValueType::I64),
        ("min" | "max", ValueType::I32, ValueType::I32) => Some(ValueType::I32),
        ("min" | "max", ValueType::U8, ValueType::U8) => Some(ValueType::U8),
        ("min" | "max", ValueType::U16, ValueType::U16) => Some(ValueType::U16),
        ("min" | "max", ValueType::U32, ValueType::U32) => Some(ValueType::U32),
        ("min" | "max", ValueType::U64, ValueType::U64) => Some(ValueType::U64),
...
fn is_visibly_empty_iterable_literal(expr: &Expr) -> bool {
    match expr {
        Expr::TypedEmptyArray { .. } => true,
        Expr::ArrayLiteral {
            elements, shape, ..
        } => elements.is_empty() || shape.contains(&0),
        _ => false,
    }
}

fn typed_empty_array_element_value_type(element_type: &str) -> Option<ValueType> {
    match element_type {
        ...
        "UInt" | "UInt64" => Some(ValueType::U64),
