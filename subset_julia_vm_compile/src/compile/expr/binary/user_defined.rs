...
use crate::bytecode::value::is_complex_type_name;
    ...
    pub(in crate::compile) fn compile_user_defined_binary_op(
        ...
        if is_comparison {
                ...
                && !self.is_struct_type_of(method.return_type.clone(), "Complex")
            {
                ...
                //
                // Issue #9169: this MUST use the actual call-site argument
                // types, not the method's declared parameter types. A method
                // declared as `Base.:+(x::Real, z::Complex)` (bare, unparametrized
                // `Complex` — no element type, matching any `Complex{T}`) has
                // `right_param_ty == JuliaType::Struct("Complex")`. Feeding that
                // bare name into `julia_type_to_type_name` fails its
                // `starts_with("Complex{")` check and silently falls back to
                // `"Float64"`, so `promote_complex_operands` always produced
                // `"Float64"` regardless of the real element type, and the
                // `struct_table.get(&promoted_name)` lookup below then missed
                // (the key was `"Float64"`, not a `Complex{...}` name) and hit
                // the hardcoded `"Complex{Float64}"` safe-default — silently
                // promoting every bare-`Complex`-typed operator call to
                // `ComplexF64`, e.g. `z = 2 + 3im; z + z` computed `4.0 + 6.0im`
                // instead of `4 + 6im`. The actual runtime argument types
                // (`self.infer_julia_type(left/right)`, already computed as
                // `left_julia_ty`/`right_julia_ty` by the caller) always carry
                // the concrete element (`Complex{Int64}`), so promoting those
                // instead yields the correct result type for every component
                // type (Int64, Int32, Bool, Float32, ...), not just Float64.
                let left_actual_ty = self.infer_julia_type(left);
                let right_actual_ty = self.infer_julia_type(right);
                let promoted_name = promote_complex_operands(&left_actual_ty, &right_actual_ty);
                if let Some(info) = self.shared_ctx.struct_table.get(&promoted_name) {
                    Ok(ValueType::Struct(info.type_id))
                } else if let Some(element_name) = promoted_name
                    .strip_prefix("Complex{")
                    .and_then(|s| s.strip_suffix('}'))
                {
                    // The promoted `Complex{T}` instantiation is not yet in the
                    // struct table (a less common element type, e.g. `Int32`/
                    // `Int16`, that nothing else has instantiated yet). Resolve
                    // (creating on demand) the real instantiation instead of
                    // silently substituting `Complex{Float64}` — the previous
                    // hardcoded fallback here (issue #9169) widened every
                    // not-yet-registered element type to `Float64`.
                    let element_ty = JuliaType::from_name_or_struct(element_name);
                    match self
                        .shared_ctx
                        .resolve_instantiation("Complex", &[element_ty])
                    {
                        Ok(type_id) => Ok(ValueType::Struct(type_id)),
                        Err(_) => Ok(ValueType::Any),
                    }
                } else {
