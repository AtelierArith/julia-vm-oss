...
/// Re-export shim: the widening policy constants moved with the lattice type
/// definitions into `crate::runtime_types::lattice` (Issue #8557) so the
/// shared type layer does not reach back into `compile` for them; the
/// `compile::lattice::widening::MAX_*` / `compile::lattice::MAX_*` paths stay
/// valid for existing users.
// widening constants moved with `LatticeType` to `subset_julia_vm_types` (Issue #8655);
// re-routed through `crate::runtime_types` which now re-exports them.
pub use crate::runtime_types::{MAX_INFERENCE_ITERATIONS, MAX_UNION_COMPLEXITY, MAX_UNION_LENGTH};
...
pub fn limit_type_size(
    ...
    match t {
        ...
        // PartialStruct (Issue #8544): bound recursive field-fact nesting the
        // same way upstream `limit_type_size` bounds `PartialStruct` — once
        // the nesting budget is exceeded, drop the field facts and limit the
        // widened struct type instead. Within budget, each field fact is
        // limited recursively (field-wise against a same-shape comparison
        // PartialStruct, when available) so loop joins converge.
        LatticeType::PartialStruct {
            ...
            if partial_struct_nesting_depth(t) > max_complexity {
                infer_metrics::record_union_complexity_widening();
...
/// Depth of nested `PartialStruct` field facts (a non-`PartialStruct` value
/// has depth 0). Used to bound recursive constructor-fact growth in
/// [`limit_type_size`] (Issue #8544).
fn partial_struct_nesting_depth(t: &LatticeType) -> usize {
    match t {
        LatticeType::PartialStruct { fields, .. } => {
            1 + fields
                .iter()
                .map(partial_struct_nesting_depth)
                .max()
                .unwrap_or(0)
        }
        _ => 0,
...
fn limit_union(
    ...
    if let Some(c) = compare_to {
        if union_is_derived_from(&trimmed, c) {
            return if trimmed.len() == 1 {
                // `trimmed.len() == 1` was just checked; the `None` arm below
                // is unreachable but falls back to the same empty-union
                // sentinel this function already returns at the top for a
                // genuinely empty `types` set, rather than a raw unwrap
                // (Issue #10905, Phase 1b of #10869).
                match trimmed.into_iter().next() {
                    Some(only) => LatticeType::Concrete(only),
                    None => LatticeType::Bottom,
                }
            } else {
    ...
    if new_count <= max_length && trimmed.len() <= MAX_UNION_LENGTH {
        if trimmed.len() == 1 {
            return match trimmed.into_iter().next() {
                Some(only) => LatticeType::Concrete(only),
                None => LatticeType::Bottom,
            };
        }
...
pub fn concrete_is_derived_from(ct: &ConcreteType, compare_to: &LatticeType) -> bool {
    match compare_to {
        ...
        // A PartialStruct covers its widened struct type plus every field
        // fact's structure (Issue #8544).
        LatticeType::PartialStruct { fields, .. } => {
            concrete_is_derived_from(ct, &compare_to.widen_partial_struct())
                || fields.iter().any(|f| concrete_is_derived_from(ct, f))
        }
    }
}
...
fn find_same_wrapper(ct: &ConcreteType, compare_to: &LatticeType) -> Option<ConcreteType> {
    match compare_to {
        ...
        // Search the widened struct type first, then each field fact
        // (Issue #8544).
        ps @ LatticeType::PartialStruct { fields, .. } => {
            find_same_wrapper(ct, &ps.widen_partial_struct())
                .or_else(|| fields.iter().find_map(|f| find_same_wrapper(ct, f)))
        }
