...
impl AbstractLattice for LatticeType {
    #[inline]
    fn join(&self, other: &Self) -> Self {
        let result = LatticeType::lattice_join(self, other);
        // Attributable widening (Issue #8546): restored in the compile layer
        // after the body moved to `_types` (Issue #8655).
        if matches!(result, LatticeType::Top)
            && !matches!(self, LatticeType::Top)
            && !matches!(other, LatticeType::Top)
        {
            crate::compile::budget_metrics::record_join_top_widening();
        }
        result
    }
    ...
    #[inline]
    fn join_limited(&self, other: &Self, compare_to: &Self) -> Self {
        let result = LatticeType::lattice_join_limited(self, other, compare_to);
        // Attributable widening (Issue #8546): restored in the compile layer.
        if matches!(result, LatticeType::Top)
            && !matches!(self, LatticeType::Top)
            && !matches!(other, LatticeType::Top)
