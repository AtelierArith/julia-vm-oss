...
pub(super) fn merge_with_precompiled_base(program: &Program) -> MergedProgram {
    ...
    let mut base_program = match crate::base_loader::get_base_program() {
        Some(p) => p.clone(),
        None => {
    ...
    let mut chronology = DefinitionOrderCursor::after_program(&base_program);
    chronology.append_fragment(&mut program);
    base_program.mark_structs_as_base_origin();

    {
        ...
        //
        // `Function` is Arc-wrapped (Issue #9140): `.into_iter()` moves the Arcs
        // out of `base_program.functions` (no clone), and the `.extend()` below
        // bumps refcounts instead of deep-cloning each function body.
        let mut merged_functions: Vec<std::sync::Arc<crate::ir::core::Function>> = base_program
            .functions
            .into_iter()
            .filter(|f| {
                let param_types: Vec<String> = f
