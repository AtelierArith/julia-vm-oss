...
/// (Issue #5922).
///
/// Concrete argument-driven inference is handled before this fallback. Without
/// an element type, no exact `Complex{T}` identity exists, so remain `Top`
/// instead of guessing `Complex{Float64}` (Issues #11436/#11468).
...
mod tests {
    ...
    #[test]
    fn test_complex_contextual_rejects_unknown_element_identity() {
        let ids = ComplexIds { f64_entry: true };
        assert_eq!(complex_ctx_result(&ids), LatticeType::Top);
    }

    #[test]
    fn test_complex_contextual_without_complex_structs_returns_top() {
        let ids = ComplexIds { f64_entry: false };
        assert_eq!(complex_ctx_result(&ids), LatticeType::Top);
