...
pub fn tfunc_convert(args: &[LatticeType]) -> LatticeType {
    ...
    // args[0] is T (a DataType literal or type-constructor function), args[1] is the value.
    //
    // Type annotations such as `x::Float64 = f()` lower to `x = convert(Float64, f())`.
    // During inference, `Float64` is looked up as a module-level binding and resolves to
    // `ConcreteType::Function { name: "Float64" }` (a type constructor) rather than
    // `ConcreteType::DataType { name: "Float64" }`.  Both forms encode the same target
    // type, so we extract the name from either variant and call `from_type_name`. (Issue #9121)
    let name = match &args[0] {
        LatticeType::Concrete(ConcreteType::DataType { name }) => name.as_str(),
        LatticeType::Concrete(ConcreteType::Function { name }) => name.as_str(),
...
/// Transfer function for `promote_type(::Type...)`.
///
/// This keeps the result as a type object while preserving the promoted target
/// name when every argument is a known type object. Unknown/non-type arguments
/// retain the historical generic `DataType` fallback (Issue #9914).
pub fn tfunc_promote_type(args: &[LatticeType]) -> LatticeType {
    ...
    for arg in args {
        let LatticeType::Concrete(ConcreteType::DataType { name }) = arg else {
            return tfunc_datatype_result(args);
        };
        if name.is_empty() {
            return tfunc_datatype_result(args);
        }
...
mod tests {
    ...
    #[test]
    fn promote_type_returns_named_datatype_for_known_type_objects_9914() {
        let args = vec![
            LatticeType::Concrete(ConcreteType::DataType {
    ...
    #[test]
    fn promote_type_keeps_datatype_fallback_for_unknown_type_objects_9914() {
        let args = vec![LatticeType::Concrete(ConcreteType::DataType {
            name: String::new(),
        })];
        assert_eq!(tfunc_promote_type(&args), tfunc_datatype_result(&args));
