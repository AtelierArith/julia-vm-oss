...
pub fn tfunc_length(args: &[LatticeType]) -> LatticeType {
    ...
    match &args[0] {
        LatticeType::Union(members) => {
            if members.iter().any(length_member_returns_int64) {
...
fn length_member_returns_int64(member: &ConcreteType) -> bool {
    matches!(
        member,
        ConcreteType::Array { .. }
            | ConcreteType::Tuple { .. }
...
///
/// Type rules:
/// - collect(Range{T}) → Array{T,1}
/// - collect(Generator{T}) → Array{T,1}
///
/// # Examples
/// ```text
/// collect(1:10) → Array{Int64,1}
/// collect(x for x in 1:10) → Array{Int64,1}
/// ```
