    ...
    match (op, lhs, rhs) {
        ...
        ("%", ConstValue::Int64(a), ConstValue::Int64(b)) => {
            if *b != 0 {
                ...
                Some(ConstValue::Int64(a.wrapping_rem(*b)))
            } else {
                None // Division by zero
            }
        }
...
pub fn eval_const_unary(op: &str, operand: &ConstValue) -> Option<ConstValue> {
    match (op, operand) {
        ...
        // Math functions
        ("abs", ConstValue::Int64(v)) => v.checked_abs().map(ConstValue::Int64),
        ("abs", ConstValue::Float64(v)) => Some(ConstValue::Float64(v.abs())),
...
mod tests {
    ...
    fn test_unary_ops() {
        ...
        assert_eq!(
            eval_const_unary("abs", &ConstValue::Int64(-5)),
            Some(ConstValue::Int64(5))
        );
        assert_eq!(eval_const_unary("abs", &ConstValue::Int64(i64::MIN)), None);
    }
}
