...
impl FunctionStabilityReport {
    ...
    fn determine_status(return_type: &LatticeType) -> StabilityStatus {
        match return_type {
            ...
            // A PartialStruct is a single concrete struct type with extra
            // per-field facts — stable (Issue #8544).
            LatticeType::PartialStruct { .. } => StabilityStatus::Stable,
        }
    }
    ...
    fn format_lattice_type(ty: &LatticeType) -> String {
        match ty {
            ...
            // Display as the widened struct name (Issue #8544).
