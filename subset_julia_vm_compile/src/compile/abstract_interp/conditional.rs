...
/// predicate subset — the sjulia realization of upstream Julia's
/// `InterConditional` / `from_interconditional` (Issue #8545). This recognizes
/// one-argument functions whose whole body reduces to a conditional over that
/// argument slot:
///
/// - direct atoms: `isa(x, T)`, `x === nothing`, `x !== nothing`;
/// - `&&` / `||` / `!` compositions of such atoms, where non-narrowing leaves
///   (e.g. `x > 0`) are replaced by neutral placeholders that contribute no
///   refinement in either branch;
/// - calls to further single-argument predicates on the same slot, resolved
///   recursively up to [`MAX_PREDICATE_INLINE_DEPTH`].
///
/// The rewrite substitutes the caller's actual argument expression for the
/// callee's parameter slot (upstream's slot renumbering), so the resulting
/// condition is split directly in the caller's frame.
...
/// Maximum depth for resolving predicates that delegate to further
/// single-argument predicates (`isnil(x) = isnothing(x)` →
/// `isnothing(x) = x === nothing`). The rewrite is purely syntactic (no
/// callee-body re-inference), so this bound — not the #8185 interprocedural
/// work budget — is what keeps self- or mutually-recursive predicate chains
/// from looping; each level costs O(callee body size).
const MAX_PREDICATE_INLINE_DEPTH: usize = 3;

/// Placeholder for a predicate-body leaf that carries no narrowing over the
/// argument slot (e.g. `x > 0`, calls on other variables). The placeholder is
/// only ever consumed by `split_env_by_condition_impl`, whose catch-all arm
/// applies no refinement in either branch — exactly the sound "unknown truth
/// value" semantics a dropped conjunct/disjunct requires. It is never
/// compiled or evaluated.
fn opaque_predicate_leaf(span: crate::span::Span) -> Expr {
    Expr::Var("#8545#opaque_predicate_leaf".to_string().into(), span)
}
...
fn predicate_call_condition(
    ...
    predicate_call_condition_depth(condition, function_table, 0)
}

fn predicate_call_condition_depth(
    condition: &Expr,
    function_table: &HashMap<String, Function>,
    depth: usize,
) -> Option<Expr> {
    if depth >= MAX_PREDICATE_INLINE_DEPTH {
        return None;
    }
    let Expr::Call {
        function,
    ...
    let func = function_table.get(function.as_str())?;
    ...
    rewrite_single_arg_predicate(predicate_expr, param_name, &args[0], function_table, depth)
}
...
fn rewrite_single_arg_predicate(
    expr: &Expr,
    param_name: &str,
    actual: &Expr,
    function_table: &HashMap<String, Function>,
    ...
    match expr {
        ...
        // Compound predicate body: `x isa T && x > 0`, `x isa T || x === nothing`
        // (Issue #8545). Each operand is rewritten independently; a leaf that
        // carries no narrowing over the parameter becomes a neutral placeholder
        // (unknown truth value — refines neither branch), which is sound for
        // both `&&` (the other conjunct still holds when the predicate is true)
        // and `||` (both disjuncts are still false when the predicate is false).
        // At least one leaf must narrow, otherwise the rewrite is pointless.
        Expr::BinaryOp {
            ...
            let left_rewrite =
                rewrite_single_arg_predicate(left, param_name, actual, function_table, depth);
            let right_rewrite =
                rewrite_single_arg_predicate(right, param_name, actual, function_table, depth);
            if left_rewrite.is_none() && right_rewrite.is_none() {
                return None;
            }
            Some(Expr::BinaryOp {
                op: *op,
                left: Box::new(left_rewrite.unwrap_or_else(|| opaque_predicate_leaf(*span))),
                right: Box::new(right_rewrite.unwrap_or_else(|| opaque_predicate_leaf(*span))),
                span: *span,
            })
        }
        // Negated predicate body: `!(x isa T)` (Issue #8545). The operand must
        // itself narrow — negating a neutral leaf carries no information.
        Expr::UnaryOp {
            op: UnaryOp::Not,
            operand,
            span,
            ...
            let inner =
                rewrite_single_arg_predicate(operand, param_name, actual, function_table, depth)?;
            Some(Expr::UnaryOp {
                op: UnaryOp::Not,
        ...
        // Delegation to another single-argument predicate on the same slot:
        // `isnil(x) = isnothing(x)` (Issue #8545). Substitute the caller's
        // actual argument and resolve the inner predicate recursively,
        // bounded by MAX_PREDICATE_INLINE_DEPTH.
        Expr::Call {
            function,
            args,
            kwargs,
            ...
            && is_param_var(&args[0], param_name) =>
        {
            ...
            predicate_call_condition_depth(&delegated, function_table, depth + 1)
        }
        _ => None,
    }
}
...
fn extract_field_object_narrow_path(object: &Expr) -> Option<String> {
    match object {
        Expr::Var(obj_name, _) => Some(obj_name.to_string()),
...
fn handle_nothing_check(
    ...
    if !matches!(val_expr, Expr::Var(_, _)) {
        return SplitEnv {
            then_env: env.clone(),
            else_env: env.clone(),
        };
    ...
    // Upstream inference does not treat repeated field loads guarded by
    // `field !== nothing` as must-alias-equivalent. Keep this narrowing to
    // bare variables; `isa(field, T)` still records field refinements below.
    let path = match extract_narrowable_path(val_expr) {
        Some(p) => p,
        None => {
            // Complex expression: cannot narrow
...
mod tests {
    ...
    fn test_extract_type_from_expr() {
        // Basic types
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Int64".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int64)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Float64".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Float64)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("String".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::String)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("UnknownType".to_string().into(), dummy_span())),
            None
        );
    }
    ...
    fn test_extract_type_from_expr_all_numeric_types() {
        // Signed integers
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Int8".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int8)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Int16".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int16)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Int32".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int32)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Int".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int64)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Int128".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int128)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("BigInt".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::BigInt)
            )))
        );
        ...
        // Unsigned integers
        assert_eq!(
            extract_type_from_expr(&Expr::Var("UInt8".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::UInt8)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("UInt16".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::UInt16)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("UInt32".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::UInt32)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("UInt".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::UInt64)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("UInt64".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::UInt64)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("UInt128".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::UInt128)
            )))
        );
        ...
        // Floating point
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Float32".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Float32)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Float".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Float64)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("BigFloat".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::BigFloat)
            )))
        );
        ...
        // Other types
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Bool".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Bool)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Char".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Char)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Nothing".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Nothing)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Missing".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Missing)
            )))
        );
        assert_eq!(
            extract_type_from_expr(&Expr::Var("Symbol".to_string().into(), dummy_span())),
            Some(LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Symbol)
            )))
        );
    ...
    fn test_split_env_or_operator_either_narrow() {
        ...
        let cond2 = Expr::Builtin {
            ...
            args: vec![
                Expr::Var("val".to_string().into(), dummy_span()),
                Expr::Var("String".to_string().into(), dummy_span()),
            ],
            span: dummy_span(),
        };
        let condition = Expr::BinaryOp {
    ...
    fn test_split_env_typeof_egal_narrows_then_branch() {
        ...
        let condition = Expr::BinaryOp {
            op: BinaryOp::Egal,
            left: Box::new(typeof_call(Expr::Var("x".to_string().into(), dummy_span()))),
    ...
    fn test_split_env_typeof_reversed_operands() {
        ...
        let condition = Expr::BinaryOp {
            op: BinaryOp::Egal,
            left: Box::new(Expr::Var("Int64".to_string().into(), dummy_span())),
            right: Box::new(typeof_call(Expr::Var("x".to_string().into(), dummy_span()))),
            span: dummy_span(),
        };
    ...
    fn test_split_env_typeof_builtin_form() {
        ...
        let condition = Expr::BinaryOp {
            ...
            right: Box::new(Expr::Var("String".to_string().into(), dummy_span())),
            span: dummy_span(),
        };
    ...
    fn test_split_env_typeof_unknown_type_no_narrow() {
        ...
        let condition = Expr::BinaryOp {
            ...
            right: Box::new(Expr::Var(
                "SomeUnknownType".to_string().into(),
                dummy_span(),
            )),
            span: dummy_span(),
        };
    ...
    fn test_extract_narrowable_path_var() {
        let expr = Expr::Var("x".to_string().into(), dummy_span());
    ...
    fn test_extract_narrowable_path_getfield_call() {
        let expr = Expr::Call {
            ...
            args: vec![
                Expr::Var("obj".to_string().into(), dummy_span()),
    ...
    fn test_extract_narrowable_path_index_returns_none_issue_4270() {
        let expr = Expr::Index {
            array: Box::new(Expr::Var("arr".to_string().into(), dummy_span())),
    ...
    fn test_extract_narrowable_path_nested_field_issue_5862() {
        let inner = Expr::FieldAccess {
            object: Box::new(Expr::Var("a".to_string().into(), dummy_span())),
            field: "b".to_string().into(),
            span: dummy_span(),
        };
        let expr = Expr::FieldAccess {
            object: Box::new(inner),
            field: "c".to_string().into(),
            span: dummy_span(),
        };
    ...
    fn test_split_env_nothing_check_field_access() {
        ...
        assert!(split.then_env.get_refinement("obj.value").is_none());
        assert!(split.else_env.get_refinement("obj.value").is_none());
    }
    ...
    fn test_split_env_isa_index_access_does_not_narrow_issue_4270() {
        ...
        let condition = Expr::Builtin {
            ...
            args: vec![
                Expr::Index {
                    array: Box::new(Expr::Var("tup".to_string().into(), dummy_span())),
                    indices: vec![Expr::Literal(Literal::Int(1), dummy_span())],
                    span: dummy_span(),
                },
                Expr::Var("Int64".to_string().into(), dummy_span()),
    ...
    // ====== InterConditional predicate rewriting (Issue #8545) ======

    use crate::ir::core::{Block, Function, TypedParam};

    fn predicate_function(name: &str, param: &str, body_expr: Expr) -> Function {
        Function {
            name: name.to_string(),
    ...
    fn predicate_call(name: &str, arg: &str) -> Expr {
        Expr::Call {
            function: name.to_string().into(),
            args: vec![Expr::Var(arg.to_string().into(), dummy_span())],
            kwargs: vec![],
            splat_mask: vec![false],
            kwargs_splat_mask: vec![],
            span: dummy_span(),
    ...
    fn isa_int64(var: &str) -> Expr {
        Expr::Builtin {
            name: BuiltinOp::Isa,
            args: vec![
                Expr::Var(var.to_string().into(), dummy_span()),
                Expr::Var("Int64".to_string().into(), dummy_span()),
            ],
            span: dummy_span(),
        }
    ...
    #[test]
    fn test_predicate_compound_and_narrows_then_branch_8545() {
        // p(y) = y isa Int64 && y > 0 — the `y > 0` leaf is neutral: the
        // then-branch still narrows to Int64, the else-branch stays unrefined.
        let gt_zero = Expr::BinaryOp {
            op: BinaryOp::Gt,
            left: Box::new(Expr::Var("y".to_string().into(), dummy_span())),
            right: Box::new(Expr::Literal(Literal::Int(0), dummy_span())),
            span: dummy_span(),
        };
        let body = Expr::BinaryOp {
            op: BinaryOp::And,
            left: Box::new(isa_int64("y")),
            right: Box::new(gt_zero),
            span: dummy_span(),
        };
        let mut table = HashMap::new();
        table.insert("p".to_string(), predicate_function("p", "y", body));

        let env = nullable_int_env();
        let split = split_env_by_condition_with_predicates(&env, &predicate_call("p", "x"), &table);
        ...
        assert_eq!(
            ...
            "then-branch must narrow x to Int64"
        );
        // Predicate false does not exclude Int64 (y > 0 may have failed).
        assert_eq!(
            split.else_env.get("x"),
            env.get("x"),
            "else-branch must stay unrefined (neutral leaf may be false)"
        );
    }

    #[test]
    fn test_predicate_compound_or_narrows_else_branch_8545() {
        // p(y) = y isa String || y === nothing — the else-branch subtracts
        // both disjuncts (x::Int64), the then-branch keeps their union.
        let egal_nothing = Expr::BinaryOp {
            op: BinaryOp::Egal,
            left: Box::new(Expr::Var("y".to_string().into(), dummy_span())),
            right: Box::new(Expr::Literal(Literal::Nothing, dummy_span())),
            span: dummy_span(),
        };
        let isa_string = Expr::Builtin {
            name: BuiltinOp::Isa,
            args: vec![
        ...
        let body = Expr::BinaryOp {
            op: BinaryOp::Or,
            left: Box::new(isa_string),
            right: Box::new(egal_nothing),
            span: dummy_span(),
        };
        let mut table = HashMap::new();
        table.insert("p".to_string(), predicate_function("p", "y", body));
        ...
        assert_eq!(
            ...
            "else-branch must subtract both disjuncts"
        );
    }

    #[test]
    fn test_predicate_delegation_resolves_nested_8545() {
        // isnil(y) = isnothing2(y); isnothing2(z) = z === nothing
        let inner_body = Expr::BinaryOp {
            op: BinaryOp::Egal,
            left: Box::new(Expr::Var("z".to_string().into(), dummy_span())),
            right: Box::new(Expr::Literal(Literal::Nothing, dummy_span())),
            span: dummy_span(),
        };
        let mut table = HashMap::new();
        table.insert(
            "isnothing2".to_string(),
            predicate_function("isnothing2", "z", inner_body),
        );
        table.insert(
            "isnil".to_string(),
            predicate_function("isnil", "y", predicate_call("isnothing2", "y")),
        );

        let env = nullable_int_env();
        let split =
            split_env_by_condition_with_predicates(&env, &predicate_call("isnil", "x"), &table);
    ...
    #[test]
    fn test_predicate_negated_body_swaps_branches_8545() {
        // p(y) = !(y isa Int64)
        let body = Expr::UnaryOp {
            op: UnaryOp::Not,
            operand: Box::new(isa_int64("y")),
            span: dummy_span(),
        };
        let mut table = HashMap::new();
        table.insert("p".to_string(), predicate_function("p", "y", body));
    ...
    #[test]
    fn test_predicate_self_recursive_is_depth_bounded_8545() {
        // p(y) = p(y) — must terminate (depth bound) with no narrowing.
        let mut table = HashMap::new();
        table.insert(
            "p".to_string(),
            predicate_function("p", "y", predicate_call("p", "y")),
        );

        let env = nullable_int_env();
        let split = split_env_by_condition_with_predicates(&env, &predicate_call("p", "x"), &table);

        assert_eq!(split.then_env.get("x"), env.get("x"));
        assert_eq!(split.else_env.get("x"), env.get("x"));
    }

    #[test]
    fn test_predicate_all_neutral_body_does_not_narrow_8545() {
        // p(y) = y > 0 — no narrowing atom at all: no refinement either way.
        let body = Expr::BinaryOp {
            op: BinaryOp::Gt,
            left: Box::new(Expr::Var("y".to_string().into(), dummy_span())),
            right: Box::new(Expr::Literal(Literal::Int(0), dummy_span())),
