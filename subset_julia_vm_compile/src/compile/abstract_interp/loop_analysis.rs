...
/// Extracts the element type `T` from an `iterate(...)` return type shaped
/// `Union{Nothing, Tuple{T, State}}` — Julia's stateful iterate-protocol
/// contract (`iterate(itr[, state]) -> Union{Nothing, Tuple{item, state}}`,
/// see `julia/base/essentials.jl`). Also accepts a bare `Tuple{T, State}`
/// return (the `Nothing` branch proven statically unreachable). Returns
/// `None` for any other shape so callers keep their existing conservative
/// fallback (Issue #9124).
pub fn iterate_return_element_lattice(return_ty: &LatticeType) -> Option<LatticeType> {
    fn tuple_first_element(ct: &ConcreteType) -> Option<ConcreteType> {
        match ct {
            ConcreteType::Tuple { elements } if elements.len() == 2 => Some(elements[0].clone()),
            _ => None,
        }
    }
    ...
    match return_ty {
        LatticeType::Union(types) => {
            let has_nothing = types.contains(&ConcreteType::Core(CoreType::Primitive(
            ...
            types
                .iter()
                .find_map(tuple_first_element)
                .map(LatticeType::Concrete)
        }
        LatticeType::Concrete(ct) => tuple_first_element(ct).map(LatticeType::Concrete),
        _ => None,
    }
}
...
mod tests {
    ...
    /// Issue #9124: `iterate(pl::PointList, state=1)` returning
    /// `Union{Nothing, Tuple{Point, Int64}}` should yield `Point`.
    #[test]
    fn test_iterate_return_element_lattice_union_nothing_tuple() {
        let point = ConcreteType::Struct {
            name: "Point".to_string(),
            type_id: 7,
        };
        let mut variants = BTreeSet::new();
        variants.insert(ConcreteType::Core(CoreType::Primitive(
            CorePrimitive::Nothing,
        )));
        variants.insert(ConcreteType::Tuple {
            elements: vec![
                point.clone(),
                ConcreteType::Core(CoreType::Primitive(CorePrimitive::Int64)),
            ],
        });
        let return_ty = LatticeType::Union(variants);

        assert_eq!(
            iterate_return_element_lattice(&return_ty),
            Some(LatticeType::Concrete(point))
        );
    }

    /// A bare `Tuple{T, State}` (the `Nothing` branch proven unreachable)
    /// is also a valid iterate return shape.
    #[test]
    fn test_iterate_return_element_lattice_bare_tuple() {
        let return_ty = LatticeType::Concrete(ConcreteType::Tuple {
            elements: vec![
                ConcreteType::Core(CoreType::Primitive(CorePrimitive::Float64)),
                ConcreteType::Core(CoreType::Primitive(CorePrimitive::Int64)),
            ],
    ...
    /// A union without `Nothing` is not the iterate-protocol shape — must
    /// not be misread as one.
    #[test]
    fn test_iterate_return_element_lattice_rejects_non_iterate_union() {
        let mut variants = BTreeSet::new();
        variants.insert(ConcreteType::Core(CoreType::Primitive(
            CorePrimitive::Int64,
        )));
        ...
        let return_ty = LatticeType::Union(variants);
        assert_eq!(iterate_return_element_lattice(&return_ty), None);
    }

    /// A plain (non-tuple) return type is not the iterate-protocol shape.
    #[test]
    fn test_iterate_return_element_lattice_rejects_non_tuple() {
        let return_ty = LatticeType::Concrete(ConcreteType::Core(CoreType::Primitive(
            CorePrimitive::Int64,
        )));
        assert_eq!(iterate_return_element_lattice(&return_ty), None);
    }
