        ...
        body: Block {
            stmts: vec![Stmt::Return {
                value: Some(Expr::Call {
                    function: "callee_fn".to_string().into(),
...
fn test_issue_5939_function_table_typevar_mismatch_keeps_bare_edge_fallback() {
    ...
    let previous =
        engine.replace_active_context(caller_key.to_string(), caller_key.to_string(), None);
    assert!(
...
/// Issue #8442: precise invalidation needs a real backedge graph, not only
/// per-cache snapshots scanned during mutation. The index should point a
/// mutated callee at exactly the cache keys that depend on it.
#[test]
fn test_issue_8442_method_backedge_index_tracks_transitive_cache_keys() {
    let leaf_name = "leaf_backedge_index_8442";
    let mid_name = "mid_backedge_index_8442";
    let caller_name = "caller_backedge_index_8442";
    let unrelated_name = "unrelated_backedge_index_8442";
    let int_args = [LatticeType::Concrete(ConcreteType::Core(
        CoreType::Primitive(CorePrimitive::Int64),
    ))];
    ...
    let mid = typed_forwarder_function_5603(mid_name, JuliaType::Int64, leaf_name);
    let caller = typed_forwarder_function_5603(caller_name, JuliaType::Int64, mid_name);
    let unrelated = int_identity_function(unrelated_name);
    let mut function_table = HashMap::new();
    function_table.insert(leaf_name.to_string(), leaf);
    function_table.insert(mid_name.to_string(), mid);
    let mut engine = InferenceEngine::with_tables(HashMap::new(), function_table);
    ...
    let caller_key = InferenceCacheKey::new(&inference_cache_function_id(&caller), &int_args);
    let mid_key = InferenceCacheKey::new(&format!("{}({})", mid_name, JuliaType::Int64), &int_args);
    let unrelated_key = InferenceCacheKey::new(&inference_cache_function_id(&unrelated), &int_args);

    let leaf_backedges = engine.method_backedge_keys_for_tests(leaf_name);
    assert!(
        leaf_backedges.contains(&caller_key),
        "transitive caller cache key should be indexed under the mutated leaf"
    );
    assert!(
        leaf_backedges.contains(&mid_key),
        "direct mid cache key should be indexed under the mutated leaf"
    );
    assert!(
        !leaf_backedges.contains(&unrelated_key),
        "unrelated cache key must not appear in the leaf backedge index"
    );

    engine.add_method(leaf_name.to_string(), int_identity_method_sig());

    let leaf_backedges_after = engine.method_backedge_keys_for_tests(leaf_name);
    assert!(
        !leaf_backedges_after.contains(&caller_key),
        "expired caller cache key should be pruned from the leaf backedge index"
    );
    assert!(
        !leaf_backedges_after.contains(&mid_key),
        "expired mid cache key should be pruned from the leaf backedge index"
    );
}
...
/// Issue #5603 → #8739: PartialStruct return facts ride the REGULAR
/// return-type cache (the ConstructorPartial side cache is retired), so they
/// carry the same CodeInstance-style world ranges as any other cached return.
/// A method mutation for an unrelated function must not wipe them wholesale.
#[test]
fn test_issue_5603_unrelated_method_mutation_preserves_partial_struct_cache() {
    let arg_types = [LatticeType::Concrete(ConcreteType::Core(
        CoreType::Primitive(CorePrimitive::Int64),
    ))];
    let make_fn = partial_box_constructor_function_5603("make_partial_box_5603");
    let mut function_table = HashMap::new();
    function_table.insert("make_partial_box_5603".to_string(), make_fn.clone());
    let mut engine = InferenceEngine::with_tables(partial_box_struct_table_5603(), function_table);

    let fact = engine.infer_function_with_arg_types(&make_fn, &arg_types);
    assert_eq!(
        fact.partial_struct_field_by_name("x"),
        Some(&LatticeType::Concrete(ConcreteType::Core(
            CoreType::Primitive(CorePrimitive::Int64)
        ))),
        "constructor return carries the PartialStruct field fact"
    );
    ...
    assert_eq!(
        engine.return_cache_len_for_tests(),
        before,
        "unrelated method mutation must not clear cached PartialStruct returns"
    );
    assert!(
...
/// Issue #5603 → #8739: cached PartialStruct returns carry backedges through
/// the regular return cache. If a cached caller's PartialStruct fact was
/// produced by dispatching to a callee, mutating that callee must retire the
/// caller entry.
#[test]
fn test_issue_5603_callee_method_mutation_invalidates_partial_struct_caller_cache() {
    ...
    let outer_fn = partial_box_forwarder_function_5603("outer_partial_5603", "inner_partial_5603");
    let mut function_table = HashMap::new();
    function_table.insert(
        "inner_partial_5603".to_string(),
        partial_box_constructor_function_5603("inner_partial_5603"),
    );
    function_table.insert("outer_partial_5603".to_string(), outer_fn.clone());
    let mut engine = InferenceEngine::with_tables(partial_box_struct_table_5603(), function_table);

    let fact = engine.infer_function_with_arg_types(&outer_fn, &arg_types);
    assert_eq!(
        fact.partial_struct_field_by_name("x"),
        Some(&LatticeType::Concrete(ConcreteType::Core(
            CoreType::Primitive(CorePrimitive::Int64)
        ))),
        "forwarded constructor return keeps the PartialStruct field fact"
    );
...
/// Issue #5939 → #8739: PartialStruct returns riding the regular cache reuse
/// the same precise function-table method edges as any other return type. If
/// a caller only dispatched to `inner(::Int64)`, adding/replacing
/// `inner(::Float64)` must not retire the caller's PartialStruct fact through
/// a legacy bare callee edge.
#[test]
fn test_issue_5939_partial_struct_callee_method_mutation_preserves_unmatched_caller_cache() {
    ...
    let outer_fn = partial_box_forwarder_function_5603(outer_name, inner_name);
    let mut function_table = HashMap::new();
    function_table.insert(
        inner_name.to_string(),
        partial_box_constructor_function_5603(inner_name),
    );
    function_table.insert(outer_name.to_string(), outer_fn.clone());
    let mut engine = InferenceEngine::with_tables(partial_box_struct_table_5603(), function_table);
    ...
    let outer_key = InferenceCacheKey::new(&inference_cache_function_id(&outer_fn), &arg_types);
    let outer_cached = engine
        .return_type_cache
        .get(&outer_key)
        .expect("outer cached return");
    assert!(
...
/// Issue #5939 → #8739: PartialStruct-carrying return-cache dependencies are
/// transitive under method identity too. If `outer(::Int64)` returns `mid(x)`
/// and `mid(::Int64)` returns `inner(x)`, mutating `inner(::Int64)` must
/// retire the outer PartialStruct fact.
#[test]
fn test_issue_5939_partial_struct_method_edges_propagate_transitively() {
    ...
    let outer_fn = partial_box_forwarder_function_5603(outer_name, mid_name);
    let mut function_table = HashMap::new();
    function_table.insert(
        inner_name.to_string(),
        partial_box_constructor_function_5603(inner_name),
...
/// Issue #5603 → #8739: cached PartialStruct returns also carry global
/// binding reads. A binding change must retire only entries that read the
/// changed binding rather than clearing unrelated cached returns.
#[test]
fn test_issue_5603_global_binding_change_invalidates_partial_struct_cache() {
    let no_args: [LatticeType; 0] = [];
    let global_fn = partial_box_global_function_5603("global_partial_5603", "G_PARTIAL_5603");
    let mut function_table = HashMap::new();
    function_table.insert("global_partial_5603".to_string(), global_fn.clone());
    let mut engine = InferenceEngine::with_tables(partial_box_struct_table_5603(), function_table);
    ...
    let fact = engine.infer_function_with_arg_types(&global_fn, &no_args);
    assert_eq!(
        fact.partial_struct_field_by_name("x"),
        Some(&LatticeType::Concrete(ConcreteType::Core(
            CoreType::Primitive(CorePrimitive::Int64)
        ))),
        "global-reading constructor return keeps the PartialStruct field fact"
    );
