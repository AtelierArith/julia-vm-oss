        ...
        body: Block {
            stmts: vec![Stmt::Return {
                value: Some(Expr::BinaryOp {
                    ...
                    right: Box::new(Expr::Var("y".to_string().into(), dummy_span())),
                    span: dummy_span(),
                }),
                span: dummy_span(),
            }],
...
fn test_interprocedural_analysis_float() {
    ...
    let call_expr = Expr::Call {
        function: "double".to_string().into(),
...
fn test_interprocedural_caches_result() {
    ...
    let call_expr1 = Expr::Call {
        function: "identity".to_string().into(),
        ...
        body: Block {
            stmts: vec![Stmt::Return {
                value: Some(Expr::Call {
                    function: "helper".to_string().into(),
                    args: vec![
                        Expr::Literal(Literal::Int(1), dummy_span()),
                        Expr::Literal(Literal::Int(2), dummy_span()),
                    ],
...
fn test_varargs_call_packs_remaining_args_into_tuple() {
        ...
        body: Block {
            stmts: vec![Stmt::Return {
                value: Some(Expr::Index {
                    array: Box::new(Expr::Var("xs".to_string().into(), dummy_span())),
    ...
    let call = Expr::Call {
        function: "sum_varargs".to_string().into(),
        args: vec![
            Expr::Literal(Literal::Int(1), dummy_span()),
            Expr::Literal(Literal::Int(2), dummy_span()),
...
fn test_call_inference_union_splits_method_matches() {
    ...
    let call = Expr::Call {
        function: "classify".to_string().into(),
...
fn test_ternary_inference_joins_branch_types_4287() {
    ...
    let expr = Expr::Ternary {
        condition: Box::new(Expr::Var("b".to_string().into(), dummy_span())),
...
fn test_call_inference_union_split_budget_matches_julia_4287() {
    ...
    let call = Expr::Call {
        function: "pair_classify".to_string().into(),
        args: vec![
...
fn test_call_inference_large_union_split_bails_with_diagnostic_4287() {
    ...
    let call = Expr::Call {
        function: "wide_classify".to_string().into(),
        args: vec![Expr::Var("x".to_string().into(), dummy_span())],
        kwargs: vec![],
        kwargs_splat_mask: vec![],
        splat_mask: vec![false],
...
fn test_call_inference_skips_method_table_when_arg_is_top() {
    ...
    let call = Expr::Call {
        function: "maybe_struct".to_string().into(),
        args: vec![Expr::Var("x".to_string().into(), dummy_span())],
        kwargs: vec![],
        kwargs_splat_mask: vec![],
        splat_mask: vec![false],
...
fn test_issue_5603_ambiguous_method_table_call_infers_bottom() {
    ...
    let call = Expr::Call {
        function: "ambiguous_5603".to_string().into(),
        args: vec![
            Expr::Var("x".to_string().into(), dummy_span()),
            Expr::Var("y".to_string().into(), dummy_span()),
        ],
...
fn test_issue_7215_declared_return_type_short_circuits_call_site() {
    ...
    let call = Expr::Call {
        function: "declared_ret".to_string().into(),
