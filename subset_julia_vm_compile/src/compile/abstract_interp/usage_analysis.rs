...
fn add_numeric_constraint_for_expr(
    ...
    if let Expr::Var(name, _) = expr {
        if let Some(constraint_list) = constraints.get_mut(name.as_str()) {
            ...
            body: Block {
                stmts: vec![Stmt::Return {
                    value: Some(Expr::Index {
                        ...
                        indices: vec![Expr::Var("i".to_string().into(), dummy_span())],
                        span: dummy_span(),
                    }),
                    span: dummy_span(),
                }],
        ...
        let func = Function {
            ...
            body: Block {
                stmts: vec![Stmt::Return {
                    value: Some(Expr::BinaryOp {
                        op: BinaryOp::And,
                        left: Box::new(Expr::Var("a".to_string().into(), dummy_span())),
                        right: Box::new(Expr::Var("b".to_string().into(), dummy_span())),
                        span: dummy_span(),
                    }),
                    span: dummy_span(),
                }],
