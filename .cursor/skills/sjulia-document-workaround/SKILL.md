...
description: >-
  Document ad-hoc workarounds in SubsetJuliaVM (sjulia): when a temporary
  implementation is allowed, require a GitHub Issue first, add the in-code
  Workaround comment (Rust/Julia/JS), register a full entry in
  docs/vm/WORKAROUNDS.md (detailed section + Summary Table W-ID), and run
  check_workarounds_documented.sh and check_workarounds_sync.sh. Use when
  adding, updating, or removing a workaround, ad-hoc special case, temporary
  shortcut, or compatibility shim — or when the user mentions WORKAROUNDS.md.
---

# Document Ad-Hoc Workarounds in WORKAROUNDS.md

This skill enforces the **Workaround Management** rules from `CLAUDE.md` /
`AGENTS.md` and `docs/vm/CODE_AUDITS.md`. An ad-hoc implementation is allowed
