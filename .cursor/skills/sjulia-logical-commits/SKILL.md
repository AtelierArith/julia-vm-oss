...
description: >-
  Commit work in the SubsetJuliaVM (sjulia) repo as a sequence of logical,
  self-contained commits — one coherent change per commit, never a grab-bag.
  Group related files (the fix + its regression fixture + the matching
  docs/vm update), keep each commit buildable and testable, order foundations
  before consumers, and write message bodies that capture WHY and the Issue
  link rather than restating the diff. Use when finishing a multi-file change
  in sjulia and preparing commits, or when an existing branch has accumulated
  mixed concerns that should be separated before a PR.
---
...
- **Keep each commit buildable.** `cargo build --release --bin sjulia
...
| Audit script | new `scripts/check_*.sh` **+** CI wiring **+** `docs/vm/CODE_AUDITS.md` entry |
...
Read the diff and your recent edit history (the IDE provides recently viewed
files) to recover intent. Identify which files/hunks belong to which logical
unit.
...
```bash
# Whole files belonging to this unit:
git add <file1> <file2> ...

# Or specific hunks when a file spans multiple units:
git add -p <file>
```
...
Subject: imperative mood, ≤72 chars, no trailing period. If it closes an
Issue, prefix with the type (`fix:`, `feat:`, `test:`, `docs:`, `chore:`) only
if the repo already uses conventional prefixes — otherwise plain imperative.
...
## When this fits with the other sjulia skills
...
- `sjulia-dev` covers the PR creation + regular-merge flow after commits land.
