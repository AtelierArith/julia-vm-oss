...
name: sjulia-dev
description: >-
  SubsetJuliaVM (sjulia) development workflows: adding Julia functions by
  reproducing upstream Julia in src/julia/, creating fixture tests with parity
  checks against upstream julia, filing unsupported-feature/bug Issues before
  workarounds, measuring VM performance and dumping bytecode, and running the
  git/PR flow (branch → commit → push → gh pr create → merge). Use when adding
  base/stdlib functions, writing or updating fixture tests, hitting a Julia
  compatibility gap, optimizing VM codegen/runtime, or preparing a PR in the
  SubsetJuliaVM repo.
---

# SubsetJuliaVM Development

This skill encodes the project's mandatory workflows from `CLAUDE.md` /
`AGENTS.md`. Always follow the issue-driven, upstream-first rules below.
...
1. **Upstream first.** Before any design decision or implementation, consult
   ...
   under `subset_julia_vm/src/julia/`. Avoid ad hoc special cases.
...
3. **Issue-driven.** Create a GitHub Issue *before* any workaround or fix for a
   gap/bug. Reference the Issue number in workaround comments, tests, and PRs.
...
5. Add fixture tests (see "Fixture tests" below).
6. Run the relevant implementation checklist in `docs/vm/CHECKLISTS.md` (new
   type, new literal, new operator, AoT op, etc.).
...
- **manifest.toml:** add a `[[tests]]` entry with `name`, `file`, `expected`,
  `description`. End the fixture file with `true`.
...
- **Fast feedback:** `bash scripts/fixture_fast_feedback.sh <fixture.jl>…`
  ...
  sequentially; do not run `cargo build` and `nextest` concurrently
...
run `base_exports_do_not_exceed_upstream` after base/ changes.
