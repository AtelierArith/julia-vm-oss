...
//! The only backend today is the pure-Rust `subset_julia_vm_parser`; the CST
//! node shape is still modeled on the tree-sitter-julia grammar for lineage
//! and compatibility (the crate began as a tree-sitter-julia replacement).
...
impl<'a> CstWalker<'a> {
    ...
    ///
    /// For `Identifier` leaves this strips a `var"..."` non-standard
    /// identifier wrapper (Issue #8754): the parser merges `var"name"` into
    /// an Identifier leaf spanning the full source range (so span-derived
    /// reconstruction of enclosing expressions stays intact), and the name
    /// is recovered here as the quoted content. Ordinary identifier text can
    /// never contain `"`, so this never rewrites a genuine identifier.
    pub fn text(&self, node: &Node<'a>) -> &'a str {
        let raw = node.node.text_from_source(node.source);
        if node.node.kind == subset_julia_vm_parser::NodeKind::Identifier {
            subset_julia_vm_parser::strip_var_quotes(raw)
        } else {
            raw
        }
    }
    ...
    /// Get all named children of a node (excludes punctuation) as an iterator.
    ///
    /// This avoids allocating a `Vec` on every call — `named_children()` is
    /// invoked hundreds/thousands of times throughout lowering, and most call
    /// sites only need the first child or a single forward pass (Issue #10125).
    /// Call sites that genuinely need random access, slicing, or multiple
    /// passes should use [`Self::named_children_vec`] instead.
    pub fn named_children(&self, node: &Node<'a>) -> impl Iterator<Item = Node<'a>> + 'a {
        let source = node.source;
        node.node
            .children
            .iter()
            .filter(|n| n.is_named)
            .map(move |n| Node { node: n, source })
    }

    /// Get all named children of a node as a `Vec` (tree-sitter-compatible).
    ///
    /// Prefer [`Self::named_children`] where only a forward pass or the first
    /// child is needed; use this when the caller needs indexing, slicing, or
    /// multiple passes over the children.
    pub fn named_children_vec(&self, node: &Node<'a>) -> Vec<Node<'a>> {
        self.named_children(node).collect()
    }
