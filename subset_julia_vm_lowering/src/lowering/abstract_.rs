    ...
    for child in walker.named_children(&node) {
        match walker.kind(&child) {
            NodeKind::TypeParameter => {
                ...
                // (Issue #10644): a single `SubtypeConstraint` child with
                // grandchildren `[name, upper, lower]`.
                if named.len() == 1 && walker.kind(&named[0]) == NodeKind::SubtypeConstraint {
                    let inner = walker.named_children_vec(&named[0]);
                    if inner.len() >= 3 {
                        ...
                        params.push(TypeParam::with_both_bounds(param_name, lower, upper));
                        continue;
                    }
                }
                ...
                if let Some(first) = identifiers.first() {
                    ...
                    if named.len() >= 2 {
                        let bound_name = walker.text(&named[1]).to_string();
                        if walker.text(&child).contains(">:") {
                            params.push(TypeParam::with_lower_bound(param_name, bound_name));
                        } else {
                            params.push(TypeParam::with_upper_bound(param_name, bound_name));
                        }
                    } else {
...
mod tests {
    #[test]
    fn abstract_type_lower_bound_is_preserved_issue_10554() {
        let source = "abstract type LowerBounded{T>:Int64} end";
        let mut parser = crate::parser::Parser::new().expect("create parser");
        let parsed = parser.parse(source).expect("parse abstract type");
        let mut lowering = crate::lowering::Lowering::new(source);
        let program = lowering.lower(parsed).expect("lower abstract type");

        let def = program.abstract_types.first().expect("abstract type def");
        assert_eq!(def.type_params.len(), 1);
        assert_eq!(def.type_params[0].name, "T");
        assert_eq!(def.type_params[0].lower_bound.as_deref(), Some("Int64"));
        assert_eq!(def.type_params[0].upper_bound, None);
    }
}
