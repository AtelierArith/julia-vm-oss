...
// Free-variable analysis is owned by `subset_julia_vm_types` (Issue #8656):
// no upward edge from lowering into `compile/`.
use crate::ir::core::{Block, Expr, Function, Literal, Stmt};
use crate::span::Span;
use subset_julia_vm_types::ir::free_vars::analyze_free_variables;

/// True when a `let` block's bindings introduce no user-visible name, so its
/// body is a defining scope exactly like an empty-binding block for boxing.
///
/// This covers the empty-binding `let` (bare `begin`/`@test`/`@testset` body,
/// `@time` result) as well as a `let` whose only bindings are synthetic
/// `__sjulia_let_scope_*` scope markers — injected by `lower_let_expr` for a
/// bindings-less source `let` and by the `@testset` hard-scope expansion
/// (Issue #9312) so the block is a hard scope for the soft-scope loop pass. A
/// `let` with real user bindings returns `false` and is treated as a distinct
/// binding scope.
fn is_scope_marker_let(bindings: &[(crate::ir::core::InternedStr, Expr)]) -> bool {
    bindings
        .iter()
...
pub fn box_captured_reassigned_locals(functions: &mut [Function], main: &mut [Stmt]) {
...
impl BoxCtx<'_> {
    ...
    fn recurse_scopes_stmt(&mut self, stmt: &mut Stmt, global_scope: bool) {
        match stmt {
            ...
            // (Issue #6281). The `@testset` hard-scope expansion (Issue #9312) and
            // a bindings-less source `let` both wrap the body in a `let` whose only
            // binding is a synthetic `__sjulia_let_scope_*` scope marker; that is
            // also a user-binding-free defining scope and must be descended the same
            // way (`is_scope_marker_let`). A `let` with real user bindings is a
            // distinct binding scope and is left alone here.
            Stmt::Expr {
                expr: Expr::LetBlock { bindings, body, .. },
                ..
            } if is_scope_marker_let(bindings) => {
                self.box_scope(&mut body.stmts, &[], global_scope);
            }
            ...
            // A `let` with real user bindings is its own local (hard) scope, even
            // at module top level: a closure defined in its body captures the
            // let-locals rather than reading globals. That is the Base bootstrap
            // pattern (`let counter = 0; global function inc() counter += 1 end;
            // end`), so the let-local a closure mutates must be boxed exactly like
            // a function-scope local (Issue #11015).
            Stmt::Expr {
                expr: Expr::LetBlock { bindings, body, .. },
                ..
            }
            | Stmt::Assign {
                ...
                self.box_let_scope(bindings, body);
            }
            _ => {}
        }
    }

    /// Box captured-and-reassigned locals of a real-binding `let` scope.
    ///
    /// The bindings are not statements, so the scope is analyzed as the virtual
    /// statement list `[x = init, …] ++ body`: `box_in_parent_stmts` then turns
    /// the binding into `x = Ref(init)` (it is the first assignment) and every
    /// body-level rebind into `x[] = …`, matching the function-scope shape. The
    /// rewritten prologue is written back into `bindings` afterwards.
    ///
    /// `global_scope` is `false`: a `let` body is a local scope in any position,
    /// so a closure that assigns a let-local captures it (the #7759 top-level
    /// exception applies to true module globals only).
    fn box_let_scope(
        &mut self,
        bindings: &mut [(crate::ir::core::InternedStr, Expr)],
        body: &mut Block,
    ) {
        if is_scope_marker_let(bindings) {
            self.box_scope(&mut body.stmts, &[], false);
            return;
        }
        let span = body.span;
        let mut scope: Vec<Stmt> = bindings
            .iter()
            ...
            .collect();
        let prologue_len = scope.len();
        scope.append(&mut body.stmts);

        self.box_scope(&mut scope, &[], false);

        let rest = scope.split_off(prologue_len);
        body.stmts = rest;
        for (binding, stmt) in bindings.iter_mut().zip(scope) {
            if let Stmt::Assign { var, value, .. } = stmt {
                debug_assert_eq!(var, binding.0.to_string());
                binding.1 = value;
            }
        }
    }
...
fn collect_inline_funcs_expr<'a>(expr: &'a Expr, out: &mut Vec<&'a Function>) {
    match expr {
        ...
        Expr::UnaryOp { operand, .. } | Expr::Convert { operand, .. } => {
            collect_inline_funcs_expr(operand, out)
        }
...
fn make_deref(v: &str, span: Span) -> Expr {
    Expr::Index {
        array: Box::new(Expr::Var(v.to_string().into(), span)),
        indices: Vec::new(),
        span,
    }
}
...
fn expr_unsafe_for_box(expr: &Expr, v: &str, allow_assignment: bool) -> bool {
    match expr {
        ...
        Expr::UnaryOp { operand, .. } | Expr::Convert { operand, .. } => {
            expr_unsafe_for_box(operand, v, allow_assignment)
        }
        Expr::Call {
            function,
            args,
...
fn collect_function_refs_expr(expr: &Expr, out: &mut HashSet<String>) {
    match expr {
        ...
        Expr::UnaryOp { operand, .. } | Expr::Convert { operand, .. } => {
            collect_function_refs_expr(operand, out)
        }
        Expr::Call { args, kwargs, .. } | Expr::ModuleCall { args, kwargs, .. } => {
            for a in args {
...
fn collect_assigned_names_expr(expr: &Expr, out: &mut HashSet<String>) {
    match expr {
        ...
        Expr::UnaryOp { operand, .. } | Expr::Convert { operand, .. } => {
            collect_assigned_names_expr(operand, out)
        }
        Expr::Call { args, kwargs, .. } | Expr::ModuleCall { args, kwargs, .. } => {
            for arg in args {
