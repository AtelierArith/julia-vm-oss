...
pub fn inject_kwarg_default_prologues(mut func: Function) -> Function {
    ...
    // original body, preserving that order. Upstream materializes ALL omitted
    // defaults before dispatching to the typed inner method, so keep validation
    // self-assignments in a second phase after every guard. Otherwise an early
    // bad annotation can suppress a later default's side effect or exception.
    // The VM recognizes the first store to each annotated optional keyword slot
    // as the final-bound-value assertion (Issue #11135).
    let mut guards: Vec<Stmt> = Vec::new();
    let mut validations: Vec<Stmt> = Vec::new();
    for kwparam in func.kwparams.iter_mut() {
        ...
        if default_needs_body_eval(&kwparam.default) {
            ...
            let condition = Expr::BinaryOp {
                op: BinaryOp::Egal,
                left: Box::new(Expr::Var(name.clone().into(), span)),
        ...
        if kwparam.type_annotation.is_some() && !required {
            validations.push(Stmt::Assign {
                var: name.clone(),
                value: Expr::Var(name.into(), span),
                span,
            });
        }
    }

    if guards.is_empty() && validations.is_empty() {
        return func;
    }

    let mut prologue = guards;
    prologue.extend(validations);
