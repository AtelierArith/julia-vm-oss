...
pub(super) fn parse_where_expression<'a>(
    ...
    let _binder_scope = crate::lowering::type_binder_env::Scope::new(&new_type_params);
...
pub fn parse_type_constraints<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
...
pub fn parse_where_clause_type_params<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
) -> LowerResult<Vec<TypeParam>> {
    let mut type_params = Vec::new();
                ...
                for tp_child in walker.named_children(&where_child) {
                    match walker.kind(&tp_child) {
                        NodeKind::TypeParameter => {
                            ...
                            let tp_children = walker.named_children_vec(&tp_child);
...
pub fn convert_params_with_type_vars(
...
mod tests {
    ...
    #[test]
    fn test_convert_parametric_struct_type_arg_with_where_var_issue_10150() {
        let params = vec![TypedParam::new(
            "r".to_string(),
