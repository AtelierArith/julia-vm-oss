...
use crate::ir::core::{Block, Expr, Function, Literal, Stmt, TypedParam};
use crate::lowering::expr::lower_expr;
use crate::lowering::LowerResult;
use crate::parser::cst::{CstWalker, Node, NodeKind};

pub fn extract_defaults_from_function_def<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
...
pub fn generate_default_arg_stubs(func: &Function, defaults: &[Option<Expr>]) -> Vec<Function> {
    ...
    for num_provided in (first_default_idx..total_params).rev() {
        ...
        let mut call_args: Vec<Expr> = stub_params
            .iter()
            .map(|p| Expr::Var(p.name.clone().into(), func.span))
            .collect();
        ...
        for kwparam in &func.kwparams {
            let value = Expr::Var(kwparam.name.clone().into(), func.span);
            if kwparam.is_varargs {
                kwargs.push((String::new().into(), value));
                kwargs_splat_mask.push(true);
            } else {
                kwargs.push((kwparam.name.clone().into(), value));
        ...
        let mut stub_kwparams = func.kwparams.clone();
        for kwparam in &mut stub_kwparams {
            let required = matches!(kwparam.default, Expr::Literal(Literal::Undef, _));
            if !kwparam.is_varargs && !required {
                // A reduced-arity positional-default stub must preserve whether
                // each keyword was omitted. Forward the same raw `Undef`
                // sentinel already used for call-valued defaults instead of
                // materializing a literal default in the stub and making the
                // full method mistake it for caller-supplied input. The full
                // method owns default materialization and the annotated-default
                // MethodError boundary (Issues #11124, #11135).
...
pub fn extract_defaults_from_short_function<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
) -> LowerResult<Vec<Option<Expr>>> {
    let named = walker.named_children_vec(&node);
