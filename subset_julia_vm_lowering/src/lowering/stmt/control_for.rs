...
pub fn lower_for_stmt_with_body<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
    lambda_ctx: Option<&LambdaContext>,
    body: Block,
...
fn apply_var_type_convert(
    ...
    let convert_call = Expr::Call {
        ...
        args: vec![type_expr, Expr::Var(hidden.clone().into(), span)],
        kwargs: Vec::new(),
...
fn lower_for_binding_impl<'a>(
    ...
    for child in &named {
        match walker.kind(child) {
            ...
            NodeKind::TupleExpression if !binding_found => {
                let tuple_children = walker.named_children_vec(child);
    ...
    if let Some(range_node) = range_node {
        ...
        let range_children = walker.named_children_vec(&range_node);
        ...
        if walker.kind(&range_children[0]) == NodeKind::RangeExpression {
            ...
            let inner_children = walker.named_children_vec(&inner_range);
...
pub fn lower_for_stmt_with_ctx<'a>(
    ...
    if lambda_ctx.in_function_body() {
        ...
        lambda_ctx.with_top_level_for(|| lower_for_stmt_impl(walker, node, Some(lambda_ctx)))
    }
}
