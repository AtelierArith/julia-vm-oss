...
fn try_extract_quote_inner<'a>(walker: &CstWalker<'a>, quote_node: Node<'a>) -> Option<Node<'a>> {
    ...
    let inner_children = walker.named_children_vec(&quote_node);
...
fn try_unquote_assignment<'a>(
    ...
    let assign_children = walker.named_children_vec(&stmt_node);
...
fn try_unquote_return<'a>(
    ...
    let return_children = walker.named_children_vec(&stmt_node);
...
pub fn lower_if_stmt_with_ctx<'a>(
    ...
    if lambda_ctx.in_function_body() {
        ...
        lambda_ctx
            .with_top_level_control_flow(|| lower_if_stmt_impl(walker, node, Some(lambda_ctx)))
    }
}
