...
pub fn process_raw_string_escapes(s: &str) -> String {
...
pub fn is_flattenable_operator(op: &str) -> bool {
    matches!(op, "+" | "*")
}
...
/// Check if an operator can participate in a comparison chain (`a OP b OP c`).
///
/// This is `is_comparison_operator` extended to the *dotted* (broadcast) forms
/// `.<`, `.<=`, `.>`, `.>=`, `.==`, `.!=`, …. Upstream Julia's
/// `expand-compare-chain` (`julia/src/julia-syntax.scm`) treats every
/// comparison-precedence operator uniformly, whether scalar or dotted, so a
/// chain like `0 .<= v .< 1` must be collected and expanded through the
/// broadcast-fused `&` path rather than evaluated left-associatively as
/// `(0 .<= v) .< 1` (which collapses a `Bool` array to all-`false`, Issue #9300).
pub fn is_chainable_comparison_operator(op: &str) -> bool {
    is_comparison_operator(op)
        || (is_broadcast_op(op) && is_comparison_operator(strip_broadcast_dot(op)))
}

/// Check if a node kind represents an operator token.
pub fn is_operator_token(kind: &str) -> bool {
    matches!(
        kind,
        "+" | "-"
            | "*"
...
pub fn make_broadcasted_call(fn_name: &str, args: Vec<Expr>, span: Span) -> Expr {
...
pub fn make_broadcasted_call_with_callee(callee: Expr, args: Vec<Expr>, span: Span) -> Expr {
...
pub fn map_binary_op(op: &str) -> Option<BinaryOp> {
    match op {
        "+" => Some(BinaryOp::Add),
        "-" => Some(BinaryOp::Sub),
        "*" => Some(BinaryOp::Mul),
...
pub fn map_unary_op(op: &str) -> Option<UnaryOp> {
    Some(match op {
        "-" => UnaryOp::Neg,
        "+" => UnaryOp::Pos,
        "!" => UnaryOp::Not,
...
pub fn map_builtin_name(name: &str) -> Option<BuiltinOp> {
    Some(match name {
        "rand" => BuiltinOp::Rand,
        ...
        // Broadcasting control: public Ref is now a Pure Julia wrapper over
        // _ref_new (Issue #8779). Parametric Ref{T} constructors are still
        // handled by the compiler constructor path.
        // Type operations
        ...
        "_test_record_error!" => BuiltinOp::TestRecordError,
...
mod tests {
    ...
    // ── is_chainable_comparison_operator ──────────────────────────────────────

    #[test]
    fn test_is_chainable_comparison_operator_scalar_and_dotted() {
        // Scalar comparisons chain (existing behavior).
        for op in ["<", ">", "<=", ">=", "==", "!=", "<:", ">:"] {
            assert!(
                is_chainable_comparison_operator(op),
                "{op} should be chainable"
            );
        }
        // Dotted (broadcast) comparisons also chain (Issue #9300): `a .< b .< c`.
        for op in [".<", ".>", ".<=", ".>=", ".==", ".!="] {
            assert!(
                is_chainable_comparison_operator(op),
                "{op} should be chainable"
    ...
    #[test]
    fn test_is_chainable_comparison_operator_non_comparison() {
        // Dotted arithmetic / bitwise ops are not comparison chains.
        assert!(!is_chainable_comparison_operator(".+"));
        assert!(!is_chainable_comparison_operator(".*"));
        assert!(!is_chainable_comparison_operator(".&"));
        assert!(!is_chainable_comparison_operator("&&"));
        assert!(!is_chainable_comparison_operator("+"));
