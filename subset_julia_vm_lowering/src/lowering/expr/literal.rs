...
fn parse_hex_float(text: &str) -> Option<f64> {
    ...
    let (mantissa_str, exp_str) = {
        let idx = text.find(['p', 'P'])?;
...
/// Process escape sequences in a string literal to raw BYTES (Issue #8995).
///
/// Julia's `\xNN` and octal `\NNN` escapes denote raw code units, not
/// codepoints: `"\xff"` is a one-byte String that is not valid UTF-8, and
/// `'\xff'` is a malformed one-byte Char. `\u`/`\U` escapes stay
/// codepoint-based (UTF-8 encoded). All other escapes and plain text match
/// [`process_escape_sequences`].
pub fn process_escape_sequences_bytes(content: &str) -> Vec<u8> {
    ...
    while i < bytes.len() {
        ...
        if b == b'\\' && i + 1 < bytes.len() {
            ...
            match next {
                ...
                b'x' => {
                    // Hex escape: \xNN — 1-2 hex digits → ONE raw byte.
                    let start = i + 2;
                    let mut end = start;
                    ...
                    if end > start {
                        if let Ok(n) = u8::from_str_radix(&content[start..end], 16) {
                            result.push(n);
                            i = end;
                            continue;
                        }
                ...
                b'u' | b'U' => {
                    // Codepoint escapes: defer to the char-based processor's
                    // greedy parse by reusing its logic inline.
                    let max = if next == b'u' { 4 } else { 8 };
                    let start = i + 2;
                    let mut end = start;
                    ...
                    if end > start {
                        if let Some(ch) = u32::from_str_radix(&content[start..end], 16)
                            .ok()
                ...
                b'0'..=b'7' => {
                    // Octal escape: \NNN — 1-3 octal digits → ONE raw byte
                    // (0..=0o377, upstream range).
                    let start = i + 1;
                    let mut end = start;
                    while end < bytes.len()
                        && end - start < 3
                    ...
                    if let Ok(n) = u16::from_str_radix(&content[start..end], 8) {
                        if n <= 0xff {
                            result.push(n as u8);
                            i = end;
                            continue;
...
/// Parse a string literal, handling quotes and escape sequences. Escape
/// processing is byte-oriented (Issue #8995): `"\\xff"` produces a one-byte
/// String literal that is not valid UTF-8 (`Literal::StrBytes`); valid UTF-8
/// results stay `Literal::Str`.
fn parse_string_literal(text: &str) -> Literal {
    ...
    match String::from_utf8(process_escape_sequences_bytes(content)) {
        Ok(s) => Literal::Str(s),
        Err(err) => Literal::StrBytes(err.into_bytes()),
    }
}
...
pub fn lower_char_literal<'a>(walker: &CstWalker<'a>, node: Node<'a>) -> LowerResult<Expr> {
    ...
    // Parse the character content: escape-process to raw bytes, then decode
    // exactly one Julia char (Issue #8995). Byte escapes may form a valid
    // scalar ('\x41', '\xe3\x81\x82') or a malformed Char ('\xff').
    let bits = parse_char_literal_bits(content).ok_or_else(|| {
    ...
    Ok(
        match subset_julia_vm_bytecode::value::julia_char_from_bits(bits) {
            Some(ch) => Expr::Literal(Literal::Char(ch), span),
            None => Expr::Literal(Literal::CharMalformed(bits), span),
        },
    )
}

/// Escape-process a char literal's content and decode it as exactly one
/// Julia char, returning its 32-bit pattern — `None` when the content is
/// empty or contains more than one character (Issue #8995).
fn parse_char_literal_bits(content: &str) -> Option<u32> {
    let bytes = process_escape_sequences_bytes(content);
    if bytes.is_empty() {
        return None;
    }
    let (bits, next) = subset_julia_vm_bytecode::value::decode_julia_char(&bytes, 0);
    if next != bytes.len() {
        return None;
    }
    Some(bits)
...
pub fn lower_string_literal<'a>(
    ...
    if !has_interpolation {
        ...
        return Ok(Expr::Literal(
            parse_string_literal(walker.text(&node)),
            span,
        ));
    }
        ...
        if let Some(child) = node.child(i) {
            ...
            match child_kind {
                ...
                "string_interpolation" | "interpolation_expression" => {
                    ...
                    let interp_children = walker.named_children_vec(&child);
...
mod tests {
    ...
    // ── parse_char_literal_bits ────────────────────────────────────────────────────

    #[test]
    fn test_parse_char_content_simple_ascii() {
        assert_eq!(parse_char_literal_bits("a"), Some(0x6100_0000));
        assert_eq!(parse_char_literal_bits("Z"), Some(0x5a00_0000));
        assert_eq!(parse_char_literal_bits("5"), Some(0x3500_0000));
    }

    #[test]
    fn test_parse_char_content_escape_newline() {
        assert_eq!(parse_char_literal_bits("\\n"), Some(0x0a00_0000));
    }

    #[test]
    fn test_parse_char_content_escape_tab() {
        assert_eq!(parse_char_literal_bits("\\t"), Some(0x0900_0000));
    }

    #[test]
    fn test_parse_char_content_escape_backslash() {
        assert_eq!(parse_char_literal_bits("\\\\"), Some(0x5c00_0000));
    }

    #[test]
    fn test_parse_char_literal_bits_malformed_and_multibyte() {
        // '\xff' is a malformed one-byte Char (bits 0xff000000).
        assert_eq!(parse_char_literal_bits("\\xff"), Some(0xff00_0000));
        // Byte escapes forming valid UTF-8 decode to the scalar ('あ').
        assert_eq!(
            parse_char_literal_bits("\\xe3\\x81\\x82"),
            Some(0xe381_8200)
        );
        // Truncated multibyte escape sequence is a two-byte malformed Char.
        assert_eq!(parse_char_literal_bits("\\xe3\\x81"), Some(0xe381_0000));
        // Direct multi-byte character.
        assert_eq!(parse_char_literal_bits("あ"), Some(0xe381_8200));
        // Two characters do not form a char literal.
        assert!(parse_char_literal_bits("ab").is_none());
    }

    #[test]
    fn test_parse_char_content_escape_single_quote() {
        assert_eq!(parse_char_literal_bits("\\'"), Some(0x2700_0000));
    }

    #[test]
    fn test_parse_char_content_hex_escape() {
        assert_eq!(parse_char_literal_bits("\\x41"), Some(0x4100_0000)); // 0x41 = 'A'
    }

    #[test]
    fn test_parse_char_content_unicode_escape_u() {
        assert_eq!(parse_char_literal_bits("\\u0041"), Some(0x4100_0000)); // U+0041 = 'A'
    }

    #[test]
    fn test_parse_char_content_empty_returns_none() {
        assert!(parse_char_literal_bits("").is_none());
    }

    #[test]
    fn test_parse_char_content_invalid_escape_returns_none() {
        // \q is not a valid escape sequence
        assert!(parse_char_literal_bits("\\q").is_none());
    }
}
