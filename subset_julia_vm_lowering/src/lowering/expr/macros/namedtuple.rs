...
//! that canonical string as a static `DataType` literal.
//!
...
pub fn lower_namedtuple_macro_expr<'a>(
    walker: &CstWalker<'a>,
    args: &[Node<'a>],
    ...
    for decl in decls {
        match walker.kind(&decl) {
            ...
            NodeKind::TypedExpression => {
                let children = walker.named_children_vec(&decl);
                if children.len() != 2 {
                    return Err(
                        UnsupportedFeature::new(UnsupportedFeatureKind::MacroCall, span).with_hint(
    ...
    // Build the canonical `@NamedTuple{a::Int64, b::String}` type-object literal,
    // matching the named-tuple type SubsetJuliaVM uses for `typeof((a=1, b="hi"))`.
...
mod tests {
    ...
    fn namedtuple_type_name(source: &str) -> String {
        ...
        fn find_datatype_literal(expr: &Expr) -> Option<String> {
            if let Expr::Literal(Literal::DataType(s), _) = expr {
                return Some(s.clone());
            }
            None
        }
        ...
        for stmt in &program.main.stmts {
            if let Stmt::Expr { expr, .. } = stmt {
                if let Some(s) = find_datatype_literal(expr) {
                    return s;
                }
            }
        }
        panic!("no DataType literal produced for source: {source}");
    }
