...
// Internal re-exports
use expand::{
    expand_base_macro_expr, expand_bundled_package_macro_expr, expand_stdlib_macro_expr,
    expand_user_defined_macro_expr, lower_macroexpand_expr,
};
...
pub use namedtuple::lower_namedtuple_macro_expr;
...
fn doc_target_key_from_node<'a>(walker: &CstWalker<'a>, node: Node<'a>) -> Option<String> {
    match walker.kind(&node) {
        NodeKind::Identifier => Some(walker.text(&node).to_string()),
        NodeKind::CallExpression => {
            walker
                .named_children(&node)
                .find_map(|child| match walker.kind(&child) {
                    NodeKind::Identifier => Some(walker.text(&child).to_string()),
                    _ => None,
                })
        }
        _ => None,
...
fn lower_doc_macro_expr<'a>(
    ...
    if args.len() != 1 {
        return Err(
            UnsupportedFeature::new(UnsupportedFeatureKind::MacroCall, span)
                .with_hint("@doc retrieval requires exactly one target"),
        );
    }
    let Some(key) = doc_target_key_from_node(walker, args[0]) else {
        return Err(
            UnsupportedFeature::new(UnsupportedFeatureKind::MacroCall, span)
                .with_hint("@doc retrieval currently supports simple identifiers"),
        );
    };
    let sanitized: String = key
        ...
        .collect();
    Ok(Expr::Var(format!("__sjulia_doc_{sanitized}").into(), span))
}
...
fn lower_invoke_value_and_type<'a>(
    ...
    if walker.kind(&node) == NodeKind::TypedExpression {
        let typed_children = walker.named_children_vec(&node);
        ...
        let type_expr = Expr::Literal(Literal::DataType(type_name.clone()), span);
...
fn invoke_call_expr(
    ...
    Expr::Call {
        function: "invoke".to_string().into(),
        args: invoke_args,
        kwargs,
        splat_mask: Vec::new(),
        kwargs_splat_mask,
...
fn lower_invoke_call_expression<'a>(
    ...
    let call_children = walker.named_children_vec(&call_node);
    ...
    for arg in walker.named_children(&args_node) {
        match walker.kind(&arg) {
            ...
            NodeKind::SplatExpression if saw_semicolon => {
                let inner_children = walker.named_children_vec(&arg);
...
fn lower_invoke_field_expression<'a>(
    ...
    Ok(invoke_call_expr(
        Expr::FunctionRef {
            name: "getproperty".to_string().into(),
            span,
        },
        vec![object, field],
...
fn lower_invoke_index_expression<'a>(
    ...
    let children = walker.named_children_vec(&index_node);
    if children.len() < 2 {
        return Err(
            UnsupportedFeature::new(UnsupportedFeatureKind::MacroCall, span)
...
fn lower_invoke_field_assignment<'a>(
    ...
    Ok(invoke_call_expr(
        Expr::FunctionRef {
            name: "setproperty!".to_string().into(),
            span,
        },
...
fn invokelatest_call_expr(callee_expr: Expr, mut args: Vec<Expr>, span: crate::span::Span) -> Expr {
    ...
    Expr::Call {
        function: "invokelatest".to_string().into(),
        args,
        kwargs: vec![],
        splat_mask,
...
fn lower_world_macro_call<'a>(
    ...
    Ok(Expr::LetBlock {
        bindings: vec![(world_temp.into(), world_value)],
        body: Block {
            stmts: vec![Stmt::Expr {
    ...
    Ok(Some(Expr::LetBlock {
        ...
        body: Block {
            stmts: vec![
                ...
                Stmt::Assign {
                    var: first_var,
                    value: Expr::Var(first_temp.clone().into(), span),
                    span,
                },
                Stmt::Assign {
                    var: second_var,
                    value: Expr::Var(second_temp.clone().into(), span),
                    span,
                },
                Stmt::Expr {
...
fn lower_inline_policy_macro_expr<'a>(
    ...
    let inner = super::lower_expr_with_ctx(walker, args[0], lambda_ctx)?;
    Ok(Expr::Call {
        function: format!("#__sjulia_{}__", macro_name).into(),
        args: vec![inner],
        kwargs: vec![],
        splat_mask: vec![],
        kwargs_splat_mask: vec![],
...
fn lower_async_macro_expr<'a>(
    ...
    Ok(Expr::LetBlock {
        bindings: vec![(task_var.clone().into(), task_expr)],
        body: Block {
            stmts: vec![
                Stmt::Expr {
                    expr: Expr::Call {
                        function: "schedule".to_string().into(),
                        args: vec![Expr::Var(task_var.clone().into(), span)],
                        kwargs: vec![],
                        splat_mask: vec![],
                        kwargs_splat_mask: vec![],
                        span,
                ...
                Stmt::Expr {
                    expr: Expr::Var(task_var.into(), span),
                    span,
                },
            ],
            span,
...
fn lower_sync_single_async_expr<'a>(
    ...
    let empty_any_vec = Expr::TypedEmptyArray {
        element_type: "Any".to_string().into(),
        span,
    };

    Ok(Expr::LetBlock {
        bindings: vec![(exceptions_var.clone().into(), empty_any_vec)],
        body: Block {
            stmts: vec![
...
pub fn lower_sync_macro_stmt_entry<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
    span: crate::span::Span,
    lambda_ctx: &crate::lowering::LambdaContext,
...
pub fn lower_macro_expr<'a>(walker: &CstWalker<'a>, node: Node<'a>) -> LowerResult<Expr> {
    ...
    match macro_name.as_str() {
        ...
        // Upstream `@nospecialize`/`@specialize` always expand to
        // `Expr(:meta, :nospecialize, vars...)` / `Expr(:meta, :specialize, vars...)`
        // (base/essentials.jl), regardless of position. Evaluating that `:meta`
        // expression at runtime yields `nothing` and never evaluates the wrapped
        // argument (Issue #10323): `x = @nospecialize(f())` does not call `f`.
        // Statement position (`@nospecialize x y`) and argument-annotation position
        // (`f(@nospecialize(x))`) are unwrapped structurally before reaching this
        // expression-macro dispatcher, so this arm only fires in value position.
        "nospecialize" | "specialize" => {
            return Ok(Expr::Literal(Literal::Nothing, span));
        }
        "doc" => return lower_doc_macro_expr(walker, &args, span),
        "eval" => {
    ...
    if crate::lowering::macros_registry::has_base_macro(&macro_name) {
        let temp_ctx = crate::lowering::LambdaContext::new();
...
pub fn lower_macro_expr_with_ctx<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
    lambda_ctx: &crate::lowering::LambdaContext,
) -> LowerResult<Expr> {
    ...
    match macro_name.as_str() {
        "macroexpand" | "macroexpand1" => lower_macroexpand_expr(walker, &args, span, lambda_ctx),
        // @invoke f(x::T, ...) -> invoke(f, Tuple{T, ...}, x, ...)
        ...
        // See the matching arm in `lower_macro_expr` above (Issue #10323): upstream
        // `@nospecialize`/`@specialize` unconditionally expand to a `:meta` `Expr`
        // that evaluates to `nothing` without evaluating its wrapped argument.
        "nospecialize" | "specialize" => Ok(Expr::Literal(Literal::Nothing, span)),
        "doc" => lower_doc_macro_expr(walker, &args, span),
        "eval" => {
            if let Some(expr) = lower_eval_module_macro_expr(walker, &args, span)? {
                Ok(expr)
        ...
        "async" | "spawn" => lower_async_macro_expr(walker, &args, span, lambda_ctx),
        ...
        _ => {
            ...
            if lambda_ctx.has_macro(&macro_name) {
                ...
                .iter()
                .find(|m| {
                    crate::lowering::macros_registry::has_bundled_package_macro(m, &macro_name)
                })
                .cloned()
            {
                ...
                .get_usings()
                .iter()
                .find(|m| crate::lowering::macros_registry::has_stdlib_macro(m, &macro_name))
                .cloned()
            {
                // Stdlib macro (e.g. Test's @test/@testset/@test_throws/@test_broken)
                // in value position -- assignment RHS, binary-operator operand,
                // function argument, etc. Mirrors the statement-position lookup in
                // `lowering::stmt::macros::mod::lower_macro_stmt_with_ctx` so a
                // stdlib macro expands the same way regardless of position
                // (Issue #10293 / #10307).
                expand_stdlib_macro_expr(
                    walker,
                    node,
                    &module_name,
                    &macro_name,
