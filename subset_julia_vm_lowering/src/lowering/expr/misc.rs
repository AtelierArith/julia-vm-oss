...
use crate::lowering::stmt::{
    lower_destructuring_from_targets, lower_stmt, lower_stmt_with_ctx,
    lower_transparent_block_stmts, parse_destructure_targets,
};
            ...
            NodeKind::Assignment | NodeKind::KeywordArgument => {
                ...
                if non_op_children.len() >= 2 {
                    ...
                    if walker.kind(&name_node) == NodeKind::Identifier {
                        ...
                        named_fields.push((name.into(), value));
                    } else {
                        is_named = false;
                    }
                } else {
...
pub fn lower_parameter_list_named_tuple<'a>(
    ...
    let span = walker.span(&node);
    let mut fields: Vec<(crate::ir::core::InternedStr, Expr)> = Vec::new();
    for child in walker.named_children(&node) {
        match walker.kind(&child) {
            NodeKind::Identifier => {
                ...
                fields.push((
                    name.clone().into(),
                    Expr::Var(name.into(), walker.span(&child)),
                ));
            }
            _ => {
    ...
    let callee_expr = match walker.kind(&callee) {
        ...
        NodeKind::ParenthesizedExpression => {
            ...
            if children.len() == 1 && walker.kind(&children[0]) == NodeKind::Operator {
                Expr::FunctionRef {
                    name: walker.text(&children[0]).to_string().into(),
                    span,
                }
            } else {
                return Err(UnsupportedFeature::new(
        ...
        // A dotted call accepts any value expression as its callee, including
        // parametric constructors (`Vector{Int}.(xs)`), field/index lookups,
        // and call results. Lower those through the ordinary expression path
        // instead of restricting broadcast to identifiers/operators (Issue #10475).
        _ => super::lower_expr(walker, callee)?,
    };

    // Remaining children form the argument list.
    // Pure Rust parser uses direct children for `f.(x, y)`. A direct
    // TupleExpression means the user wrote `f.((x, y))`, so it must stay one
    // broadcast operand instead of being unpacked as the dotted call's args.
    let args_node = named
        .iter()
        .skip(1)
        .find(|n| matches!(walker.kind(n), NodeKind::ArgumentList));
        ...
        match walker.kind(&child) {
            NodeKind::LetBindings => {
                ...
                for binding_child in walker.named_children(&child) {
                    let binding = match walker.kind(&binding_child) {
                        NodeKind::Assignment => parse_let_binding(walker, binding_child)?,
                        // A bare `let x` declares an owner without initializing
                        // it. `Literal::Undef` is an internal IR marker here;
                        // the compiler enters the owner but emits no store.
                        NodeKind::Identifier => Some(LetBindingLowering::Simple {
                            var: walker.text(&binding_child).to_string(),
                            value: Expr::Literal(Literal::Undef, walker.span(&binding_child)),
                        }),
                        _ => None,
                    };
            ...
            NodeKind::Block => {
                // Lower the entire body block. `let...end` is a transparent
                // wrapper (executes unconditionally exactly once), so a
                // nested `struct`/`mutable struct` definition is legal here
                // whenever this `let` is itself reachable from top level,
                // matching upstream Julia (Issue #10194).
                let block_children = walker.named_children_vec(&child);
                let block_span = walker.span(&child);
                let stmts = lower_transparent_block_stmts(walker, block_children, None)?;
...
pub fn lower_let_expr_with_ctx<'a>(
    ...
    for child in named {
        match walker.kind(&child) {
            ...
            NodeKind::Block => {
                // See the ctx-agnostic `lower_let_expr` above: `let...end` is
                // a transparent wrapper, so a nested `struct`/`mutable
                // struct` definition is legal here whenever this `let` is
                // itself reachable from top level (Issue #10194) — this is
                // the plain (non-macro) top-level `let ... struct Foo ...
                // end ... end` case.
                let block_children = walker.named_children_vec(&child);
                let block_span = walker.span(&child);
                let stmts =
                    lower_transparent_block_stmts(walker, block_children, Some(lambda_ctx))?;
                body_block = Some(Block {
                    stmts,
                    span: block_span,
                });
...
fn parse_let_binding_impl<'a>(
    ...
    let assign_children = walker.named_children_vec(&node);
    ...
    let lhs = operands[0];
    let rhs = *operands.last().ok_or_else(|| {
        internal_lowering_error(
            walker.span(&node),
            "let-binding operand count checked above",
        )
    })?;
    ...
    match walker.kind(&lhs) {
        ...
        NodeKind::TupleExpression => {
            let targets =
                parse_destructure_targets(walker, &walker.named_children_vec(&lhs), lambda_ctx)?;
