...
impl HygieneContext {
    ...
    /// The set of ORIGINAL names registered for renaming so far. Used by the
    /// standalone Pass-1 query `collect_quote_constructor_introduced_names`
    /// (`handlers.rs`, Issue #10977), which only needs which names the quote
    /// introduces, not this context's gensym choices.
    pub(in crate::lowering) fn registered_names(&self) -> std::collections::HashSet<String> {
