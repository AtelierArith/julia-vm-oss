...
//!
//! Variant of vm_mandelbrot_benchmark that calls a user-defined
//! `sq(x::Float64)::Float64` helper inside the hot loop, exercising the
//! F64FunctionBlock / CallF64Function inline path.
//!
//! Run with:
//!   cargo bench -p subset_julia_vm --bench f64_mandelbrot_function_inline_benchmark
...
const MANDELBROT_SOURCE: &str = include_str!("../../benchmarks/f64_mandelbrot_function_inline.jl");
...
fn bench_vm_f64_mandelbrot(c: &mut Criterion) {
    let compiled = compile_mandelbrot();
    validate_mandelbrot(&compiled);

    let mut group = c.benchmark_group("vm_f64_mandelbrot_function_inline");
...
criterion_group!(benches, bench_vm_f64_mandelbrot);
