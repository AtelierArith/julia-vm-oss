...
"#;
    ...
    assert!(
        ...
        "gif(anim) must emit a Plotly frames animation; got: {data}"
    );
    ...
    // ...and that no frame is empty (the initial trace and every frame must carry
    // data — the regression produced `"traces":[]` and `"data":[]`).
    assert!(
        !data.contains(r#""traces":[]"#) && !data.contains(r#""data":[]"#),
...
"#;
    ...
    assert!(
        ...
        "@gif animation must carry a Plotly frames array; got: {data}"
    );
    assert!(
    ...
    // 0:0.1:5 has 51 elements → 51 frames, named "0".."50".
    let frame_count = data.matches(r#""name":""#).count();
    assert_eq!(
        frame_count, 51,
        "expected 51 animation frames (one per iteration); got {frame_count} in: {data}"
    );
    assert!(
        !data.contains("\"scene\""),
