...
@testset "mapfoldr identity plus typed arrays (#4019, #4620)" begin
    bool_values = mapfoldr(identity, +, Bool[true, false, true])
    @test typeof(bool_values) == Int64
    @test bool_values == 2

    bool_empty = mapfoldr(identity, +, Bool[])
    @test typeof(bool_empty) == Int64
    @test bool_empty == 0

    bool_empty_init = mapfoldr(identity, +, Bool[]; init=false)
    @test typeof(bool_empty_init) == Bool
    @test bool_empty_init == false

    bool_init = mapfoldr(identity, +, Bool[true, false]; init=false)
    @test typeof(bool_init) == Int64
    @test bool_init == 1

    i8 = mapfoldr(identity, +, Int8[1, 2, 3])
    @test typeof(i8) == Int8
    @test i8 == Int8(6)

    i8_empty = mapfoldr(identity, +, Int8[])
    @test typeof(i8_empty) == Int8
    @test i8_empty == Int8(0)

    i8_init = mapfoldr(identity, +, Int8[1, 2, 3]; init=Int8(4))
    @test typeof(i8_init) == Int8
    @test i8_init == Int8(10)

    i16 = mapfoldr(identity, +, Int16[1, 2, 3])
    @test typeof(i16) == Int16
    @test i16 == Int16(6)

    i16_empty = mapfoldr(identity, +, Int16[])
    @test typeof(i16_empty) == Int16
    @test i16_empty == Int16(0)

    u8 = mapfoldr(identity, +, UInt8[1, 2, 3])
    @test typeof(u8) == UInt8
    @test u8 == UInt8(6)

    u8_empty = mapfoldr(identity, +, UInt8[])
    @test typeof(u8_empty) == UInt8
    @test u8_empty == UInt8(0)

    u8_init = mapfoldr(identity, +, UInt8[1, 2, 3]; init=UInt8(4))
    ...
    f32 = mapfoldr(identity, +, Float32[1, 2, 3])
    ...
    f32_init = mapfoldr(identity, +, Float32[1, 2, 3]; init=Float32(4))
    @test typeof(f32_init) == Float32
    @test f32_init == Float32(10)

    i64_empty = mapfoldr(identity, +, Int64[])
    @test typeof(i64_empty) == Int64
    @test i64_empty == 0

    f64_empty = mapfoldr(identity, +, Float64[])
