...
@testset "rmlines & rmdocs" begin
    ...
    @test !any(x -> MacroTools.isline(x), MacroTools.striplines(ex1).args)
    @test !any(x -> MacroTools.isline(x), MacroTools.striplines(ex2).args)
...
@testset "prettify" begin
    @test prettify(:($sin(2))) == :(sin(2))
    @test prettify(:($cos(x))) == :(cos(x))
end

@testset "gatherwheres" begin
    fcall, params = MacroTools.gatherwheres(:(f(a::T) where T))
    @test fcall == :(f(a::T))
    @test params == (:T,)

    fcall_braced, params_braced = MacroTools.gatherwheres(:(f(a::T) where {T}))
    @test fcall_braced == :(f(a::T))
    @test params_braced == (:T,)
end
