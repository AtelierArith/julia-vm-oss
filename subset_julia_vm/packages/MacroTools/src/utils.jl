...
function shortdef1(ex)
  @match ex begin
    ...
    function f_(args__)::rtype_ where T__ body_ end => Expr(:(=), Expr(:where, Expr(:(::), Expr(:call, f, args...), rtype), T...), Expr(:block, body.args...))
...
function splitarg(arg_expr)
    default = nothing
    if isexpr(arg_expr, :(=)) && length(arg_expr.args) == 2
      arg_expr2, default = arg_expr.args
...
function resyntax(ex)
	  prewalk(ex) do x
	    @match x begin
	      ...
	      setfield!(x_, :f_, x_.f_ + v_) => Expr(:(+=), Expr(:., x, QuoteNode(f)), v)
	      setfield!(x_, :f_, v_) => Expr(:(=), Expr(:., x, QuoteNode(f)), v)
	      getindex(x_, i__) => :($x[$(i...)])
	      tuple(xs__) => :($(xs...),)
	      adjoint(x_) => :($x')
      _ => x
