use super::{
    base_type_name, builtin_abstract_name_is_subtype_of, nominal_family_name, substitute_typevars,
    CoreType,
};
use crate::types::{StructHierarchy, StructHierarchyEntry};
...
pub(super) fn registered_struct_parent_in(
    ...
    registered_struct_parent_template_in(hierarchy, name)
        .map(|entry| nominal_family_name(&entry.parent).to_string())
}
...
pub fn registered_struct_parent_family_decision_in(
    ...
    let target = nominal_family_name(target);
    let mut current = nominal_family_name(name).to_string();
    ...
    for _ in 0..32 {
        ...
        if parent == target || builtin_abstract_name_is_subtype_of(&parent, target) {
            return Some(true);
        }
        current = parent;
    }
...
pub(super) fn registered_struct_is_subtype_of_in(
    ...
    let target = nominal_family_name(target);
    let mut current = base_type_name(name).to_string();
    for _ in 0..64 {
        let Some(parent) = registered_struct_parent_in(hierarchy, &current) else {
            return false;
        };
        ...
        if registered_struct_parent_in(hierarchy, &parent).is_none() {
            return builtin_abstract_name_is_subtype_of(&parent, target);
        }
        current = parent;
    }
    false
...
pub fn registered_instantiated_struct_supertype_in(
    ...
    let target_family = nominal_family_name(target_family);
    let mut current = CoreType::Struct {
        name: nominal_family_name(name).to_string(),
    ...
    for _ in 0..64 {
        ...
        if nominal_family_name(cur_name) == target_family {
            return Some(current);
        }
...
fn registered_struct_parent_template_in(
    ...
    hierarchy
        .entry(nominal_family_name(name))
        .and_then(registered_parent_from_entry)
}
