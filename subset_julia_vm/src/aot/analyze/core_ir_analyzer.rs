//! Core IR analysis for AoT compilation.
//!
//! The `CoreIrAnalyzer` discovers functions, builds call graphs,
//! detects recursion, and identifies entry points in persisted Core IR programs.

use super::super::call_graph::CallGraph;
use super::super::AotResult;
use super::loader::load_ir_bytes;
...
pub struct FunctionInfo {
    /// Function name
    pub name: String,
    /// Function index in the lowered program
    pub offset: usize,
...
/// Result of persisted Core IR analysis.
#[derive(Debug)]
...
/// Core IR analyzer
///
/// This analyzer provides functionality to analyze persisted Core IR files (.sjir) for:
/// - Function discovery and enumeration
/// - Call graph construction
/// - Recursion detection (direct and mutual)
/// - Entry point identification
///
/// The analyzer works with the Core IR representation stored in persisted files.
#[derive(Debug)]
pub struct CoreIrAnalyzer {
...
impl CoreIrAnalyzer {
    ...
    /// Analyze persisted Core IR bytes.
    ///
    /// This is the main entry point for Core IR analysis. It loads the persisted IR,
    /// discovers functions, builds a call graph, and detects recursion.
    ///
    /// # Arguments
    ///
    /// * `ir_bytes` - The raw Core IR bytes to analyze
    ///
    /// # Returns
    ///
    /// Returns the analysis result containing function info, call relationships,
    /// and recursion detection.
    pub fn analyze(&mut self, ir_bytes: &[u8]) -> AotResult<AnalysisResult> {
        // Step 1: Load the program from persisted Core IR
        let program = load_ir_bytes(ir_bytes)?;
    ...
    /// without going through the persisted Core IR loading step.
...
impl Default for CoreIrAnalyzer {
    fn default() -> Self {
        Self::new()
    }
}
