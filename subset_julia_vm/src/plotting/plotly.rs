...
//!
//! All Plots series — 2D (`:line`, `:scatter`, `:bar`, `:heatmap`) and 3D
//! (`:path3d`, `:scatter3d`, `:surface`) — render through Plotly.js. 2D
//! line/scatter series become `scatter` traces (`mode:"lines"` /
//! `mode:"markers"`), histograms use `bar`, heatmaps use `heatmap`, and 3D
//! series become `scatter3d` / `surface` traces. The host
...
struct SeriesData {
    ...
    z: Vec<f64>,
    /// Only for matrix-valued `:surface` and `:heatmap`.
...
fn extract_series(series_val: &Value, heap: &[StructInstance]) -> Vec<SeriesData> {
    ...
    arr.iter()
        .filter_map(|elem| {
            ...
            match kind {
                SeriesKind::Surface | SeriesKind::Heatmap => {
...
pub fn generate_plotly_animation_json(
    ...
    // Render each frame's series to a `[trace, ...]` JSON array, plus its title.
    ...
    let mut frame_trace_arrays: Vec<String> = Vec::new();
    ...
    for (frame_index, plot_val) in frame_plots.iter().enumerate() {
        ...
        let traces: Vec<String> = series_list
            .iter()
            .enumerate()
            .map(|(i, sd)| render_trace(sd, COLORS[i % COLORS.len()]))
            .collect();
        frame_trace_arrays.push(format!("[{}]", traces.join(",")));
        frame_titles.push(extract_title(plot));
    }
    if frame_trace_arrays.is_empty() {
        return None;
    }
    ...
    let sliders = render_animation_slider(frame_trace_arrays.len(), frame_duration);
