...
use super::promotion::promote_type;
...
fn type_env_from_locals(
    ...
    for (name, ty) in locals {
        let lattice =
            crate::compile::bridge::value_type_to_lattice_with_struct_table(ty, struct_table);
...
fn literal_rhs_value_type(lit: &Literal, struct_table: &HashMap<String, StructInfo>) -> ValueType {
    match lit {
    ...
    match value {
        ...
        Expr::Var(name, _)
            if is_pi_name(name)
                ...
                && !global_types.contains_key(name) =>
        {
            ValueType::F64
        }
...
fn assign_rhs_value_type_via_engine(
    ...
    crate::compile::bridge::lattice_to_value_type(&lattice)
}
        ...
        match stmt {
            ...
            Stmt::For {
                ...
                if !protected.contains(var) {
                    ...
                    let elem_ty = crate::compile::bridge::lattice_to_value_type(&elem_lattice);
            ...
            Stmt::ForEach {
                ...
                // `ValueType` that was then re-lifted via `LatticeType::from`).
                if !protected.contains(var) {
                    let env = type_env_from_locals(locals, struct_table);
                    let eng = loop_inference_engine(engine, struct_table, global_types);
...
fn visit_outermost_letblocks<'e>(
    expr: &'e Expr,
    f: &mut dyn FnMut(&'e [(String, Expr)], &'e Block),
) {
...
fn contains_letblock(expr: &Expr) -> bool {
    match expr {
        Expr::LetBlock { .. } => true,
        Expr::Ternary {
            condition,
...
fn collect_global_types_for_inference_impl(
    ...
    for stmt in stmts {
        match stmt {
            Stmt::Assign { var, value, .. } => {
                // Issue #6603: type the RHS through the shared engine's own
                // expression inference. The accumulating `globals` map already
                // holds the types of previously seen global assignments, so it
                // serves as the engine's expression environment (the global
                // lookup) for this statement. The engine carries no function
                // table (built via `build_shared_inference_engine_empty`),
                // matching the capability of the legacy pre-scan; the result is
                // bridged back to a `ValueType` via `lattice_to_value_type`.
                let ty = {
                    let env = type_env_from_locals(globals, struct_table);
                    // The engine is seeded once with the struct table; the live,
                    // per-statement global types flow through `env`, not the
                    // engine's fixed `global_types`.
                    let eng = loop_inference_engine(engine, struct_table, &HashMap::new());
                    let lattice = eng.infer_expr_result(value, &env).ty;
                    crate::compile::bridge::lattice_to_value_type(&lattice)
                };
...
pub(crate) fn build_shared_inference_engine_owned(
    struct_table: &HashMap<String, StructInfo>,
    global_types: &HashMap<String, ValueType>,
