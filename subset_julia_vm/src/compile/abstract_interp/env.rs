...
/// Field-value information for an immutable struct value tracked outside the
/// main lattice.
///
/// This is a deliberately small PartialStruct-style side table. It preserves
/// constructor argument types for local immutable struct bindings, but does
/// not make partial structs first-class lattice values. As a result, this info
/// is not stored in return-type caches and does not survive arbitrary value
/// passing through function returns.
#[derive(Clone, Debug, PartialEq)]
pub struct PartialStructFields {
    pub struct_name: String,
    pub fields: HashMap<String, LatticeType>,
    /// Field names in declaration / constructor order.
    ///
    /// Retained so integer-index field access (`getfield(s, i::Int)`) on a
    /// local immutable-struct binding can resolve positionally, mirroring
    /// upstream `Core.PartialStruct`'s positional `fields` vector and
    /// `_getfield_fieldindex` (Issue #4269). May be empty when order is
    /// unknown, in which case index access conservatively falls back to the
    /// declared field type.
...
pub struct TypeEnv {
    ...
    /// Local immutable struct field-value info keyed by root variable.
    partial_structs: HashMap<String, PartialStructFields>,
...
impl TypeEnv {
    ...
    pub(crate) fn bindings(&self) -> impl Iterator<Item = (&String, &LatticeType)> {
        self.bindings.iter()
    }
    ...
    pub fn set(&mut self, name: &str, ty: LatticeType) {
        self.partial_structs.remove(name);
        self.field_path_aliases.remove(name);
        self.bindings.insert(name.to_string(), ty);
    }

    /// Records PartialStruct-style field info for an immutable struct binding.
    pub fn set_partial_struct_fields(
        &mut self,
        var: &str,
        struct_name: String,
    ...
    /// Gets precise field info for an immutable struct binding, if tracked.
    pub fn get_partial_struct_field(&self, var: &str, field: &str) -> Option<&LatticeType> {
        self.partial_structs
            .get(var)
            .and_then(|partial| partial.fields.get(field))
    }

    pub(crate) fn get_partial_struct(&self, var: &str) -> Option<&PartialStructFields> {
        self.partial_structs.get(var)
    }
    ...
    /// the whole group. PartialStruct immutable-constructor field facts are
    /// copied across the alias because they describe the value's concrete
    /// fields and remain valid through a pure alias binding.
    ///
    ...
    pub fn alias_root(&mut self, dst: &str, src: &str) {
        ...
        if let Some(partial) = self.partial_structs.get(src).cloned() {
            self.partial_structs.insert(dst.to_string(), partial);
        }
    ...
    pub fn merge(&mut self, other: &TypeEnv) {
        ...
        self.merge_partial_structs(other);
    ...
    pub fn merge_changed(&mut self, other: &TypeEnv) -> bool {
        ...
        let partial_changed = self.merge_partial_structs(other);
    ...
    fn merge_partial_structs(&mut self, other: &TypeEnv) -> bool {
        let before = self.partial_structs.clone();
        let mut merged = HashMap::new();

        for (var, lhs) in &self.partial_structs {
            if let Some(rhs) = other.partial_structs.get(var) {
                if lhs.struct_name == rhs.struct_name && lhs.fields.len() == rhs.fields.len() {
                    let mut fields = HashMap::new();
                    let mut compatible = true;
                    for (field, lhs_ty) in &lhs.fields {
                        if let Some(rhs_ty) = rhs.fields.get(field) {
                            fields.insert(field.clone(), lhs_ty.join_limited(rhs_ty, lhs_ty));
                        } else {
                            compatible = false;
                            break;
                        }
                    ...
                    if compatible {
                        merged.insert(
                            var.clone(),
                            PartialStructFields {
                                struct_name: lhs.struct_name.clone(),
                                fields,
                                // Same struct shape on both sides, so the
                                // positional field order is identical; keep it
                                // so integer-index access survives the merge
                                // (Issue #4269).
                                field_order: lhs.field_order.clone(),
                            },
                        );
                    }
                }
            } else if !other.bindings.contains_key(var) {
                merged.insert(var.clone(), lhs.clone());
            }
        }

        for (var, rhs) in &other.partial_structs {
            if !self.bindings.contains_key(var) && !merged.contains_key(var) {
                merged.insert(var.clone(), rhs.clone());
            }
        }

        self.partial_structs = merged;
        self.partial_structs != before
    }
    ...
    pub fn restore(&mut self, snapshot: TypeEnv) {
        ...
        self.partial_structs = snapshot.partial_structs;
    ...
    pub fn invalidate_var_paths(&mut self, var: &str) {
        ...
        self.partial_structs.remove(var);
    ...
    pub fn invalidate_field_path(&mut self, obj: &str, field: &str) {
        ...
        for object_path in object_paths {
            ...
            let should_remove_partial = if let Some(partial) = self.partial_structs.get_mut(&root) {
                partial.fields.remove(field);
                partial.fields.is_empty()
            } else {
                false
            };
            if should_remove_partial {
                self.partial_structs.remove(&root);
            }
        }
...
mod tests {
    ...
    #[test]
    fn test_alias_root_still_copies_partial_struct_fields() {
        // PartialStruct immutable-constructor value facts describe the value's
        // concrete fields and remain valid through a pure alias binding, so
        // they continue to be copied across the alias (Issue #4844).
        let mut env = TypeEnv::new();
        let mut fields = HashMap::new();
        fields.insert(
            "value".to_string(),
        ...
        env.set_partial_struct_fields("x", "Box".to_string(), fields, vec!["value".to_string()]);

        env.alias_root("y", "x");

        assert_eq!(
            env.get_partial_struct_field("y", "value"),
            Some(&LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int64)
            )))
        );
        // The positional field order is copied across the alias too, so
        // integer-index access stays precise through `y = x` (Issue #4269).
        assert_eq!(
            env.get_partial_struct("y")
                .map(|p| p.field_order.as_slice()),
            Some(["value".to_string()].as_slice())
        );
    }

    #[test]
    fn test_partial_struct_field_order_retained() {
        // The declaration order is retained alongside the by-name field facts so
        // integer-index field access (`getfield(s, i::Int)`) can resolve
        // positionally with the same precision as named access (Issue #4269).
        let mut env = TypeEnv::new();
        let mut fields = HashMap::new();
        fields.insert(
            "x".to_string(),
        ...
        let partial = env.get_partial_struct("s").expect("partial recorded");
        assert_eq!(partial.field_order, vec!["x".to_string(), "y".to_string()]);
        // Positional facts line up with the declared order.
        assert_eq!(
            partial.fields.get(&partial.field_order[0]),
            Some(&LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Int64)
            )))
        );
        assert_eq!(
            partial.fields.get(&partial.field_order[1]),
            Some(&LatticeType::Concrete(ConcreteType::Core(
                CoreType::Primitive(CorePrimitive::Float64)
            )))
        );
