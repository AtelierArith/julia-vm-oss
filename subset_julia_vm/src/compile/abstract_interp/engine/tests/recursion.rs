    ...
    let recurse_poly = Function {
        ...
        body: Block {
            stmts: vec![
                ...
                Stmt::Expr {
                    expr: Expr::Call {
                        function: "recurse_poly_7357".to_string(),
        ...
        body: Block {
            stmts: vec![Stmt::Return {
                value: Some(Expr::Call {
                    function: "f_mix".to_string(),
                    args: vec![Expr::Var("n".to_string(), dummy_span())],
                    kwargs: vec![],
                    kwargs_splat_mask: vec![],
                    splat_mask: vec![false],
                    span: dummy_span(),
...
fn test_recursive_constructor_walker_partial_struct_terminates_7186() {
    ...
    //   end
    //
    // Recovering `walk`'s `ConstructorPartial` re-analyses its body, whose
    // returned constructor argument is itself a `walk(...)` call — so
    // PartialStruct-return inference re-enters `(walk, [Int64])`. Before the
    // Issue #7186 fix there was no in-flight guard and no negative caching on
    // that path, so the re-analysis fanned out without bound and `using`-time
    // inference of a recursive constructor walker (the Symbolics `_deriv`/`_mk*`
    // family) hung. With the recursion guard + negative cache the analysis
    // terminates; this test reaching its assertions at all is the regression
    // guard against the hang.
    let walk = Function {
            ...
            stmts: vec![
                Stmt::If {
                    ...
                    then_branch: Block {
                        stmts: vec![Stmt::Return {
                            value: Some(Expr::Call {
                                function: "PartialBox5603".to_string(),
