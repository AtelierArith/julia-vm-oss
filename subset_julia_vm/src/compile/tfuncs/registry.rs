...
pub trait StructIdLookup {
    ...
    /// Deterministically pick an instantiation `base_name{...}` and return its
    /// full name plus `type_id`. Implementations must be deterministic (e.g.
    /// smallest name) so inference does not depend on hash-map iteration
    /// order.
...
mod tests {
    ...
    impl StructIdLookup for StubStructIds {
        ...
        fn instantiation_of(&self, base_name: &str) -> Option<(String, usize)> {
            (base_name == "Complex").then(|| ("Complex{Float64}".to_string(), 11))
        }
    }
    ...
    fn test_struct_table_doubles_as_struct_id_lookup() {
        ...
        // Deterministic: smallest instantiation name wins.
        assert_eq!(
            ids.instantiation_of("Wrapper"),
            Some(("Wrapper{Bool}".to_string(), 5))
        );
