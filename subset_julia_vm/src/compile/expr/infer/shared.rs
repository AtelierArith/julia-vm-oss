...
impl InferredExpr {
    pub(super) fn value_type(&self) -> ValueType {
        ValueType::from(&self.lattice_type)
    }

    pub(super) fn julia_type(&self) -> JuliaType {
        JuliaType::from_name_or_struct(&self.core_type.to_julia_name())
    }
}
...
pub(super) fn infer_scalar_literal(lit: &Literal) -> Option<InferredExpr> {
    ...
    let lattice_type = LatticeType::from(&value_type);
...
mod tests {
    ...
    fn const_index_only_matches_integer_literals() {
        ...
        assert_eq!(const_tuple_index(&Expr::Var("i".to_string(), span())), None);
        assert_eq!(
