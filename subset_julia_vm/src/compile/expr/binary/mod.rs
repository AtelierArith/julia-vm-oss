...
use crate::ir::core::{BinaryOp, BuiltinOp, Expr};
...
/// (`compile_builtin_binary_op`, which resolves typed instructions through
/// [`typed_instr_for_intrinsic`]) and the runtime argument-type specializer
...
/// to one silently missed the other (this footgun surfaced in #8183 / PR #8189).
/// Both paths now resolve the instruction here.
///
...
pub(crate) fn typed_scalar_binary_instr(op: BinaryOp, result_is_float: bool) -> Option<Instr> {
    Some(match op {
...
pub(super) fn typed_instr_for_intrinsic(intrinsic: Intrinsic) -> Option<Instr> {
    let (op, result_is_float) = match intrinsic {
        ...
        Intrinsic::AddFloat => (BinaryOp::Add, true),
        Intrinsic::SubFloat => (BinaryOp::Sub, true),
        Intrinsic::MulFloat => (BinaryOp::Mul, true),
        Intrinsic::DivFloat => (BinaryOp::Div, true),
...
fn is_irrational_dispatch_type(ty: &JuliaType) -> bool {
    match ty {
        JuliaType::Struct(name) => name == "Irrational" || name.starts_with("Irrational{"),
...
fn is_vm_known_irrational_type(ty: &JuliaType) -> bool {
    match ty {
        JuliaType::Struct(name) => matches!(name.as_str(), "Irrational{:π}" | "Irrational{:ℯ}"),
        _ => false,
    }
}
...
impl CoreCompiler<'_> {
    ...
    pub(in crate::compile) fn compile_binary_op(
        ...
        // equal. Route such a mixed comparison through the same `isequal`
        // array-equality builtin the all-native-array fallback uses, so the
        // actual runtime values are compared element-wise. Restricted to the
                    ...
                    if !matches!(result_ty, ValueType::I64 | ValueType::F64) {
                        ...
                        match (op, &operand_ty) {
                            ...
                            (BinaryOp::Add, ValueType::F64) => {
                                self.emit_typed_intrinsic_or_call(Intrinsic::AddFloat)
                            }
                            (BinaryOp::Sub, ValueType::F64) => {
                                self.emit_typed_intrinsic_or_call(Intrinsic::SubFloat)
                            }
                            (BinaryOp::Mul, ValueType::F64) => {
                                self.emit_typed_intrinsic_or_call(Intrinsic::MulFloat)
                            }
                            (BinaryOp::Div, ValueType::F64) => {
                                self.emit_typed_intrinsic_or_call(Intrinsic::DivFloat)
                            }
            ...
            if left_is_irrational || right_is_irrational {
                if left_is_irrational && right_is_irrational {
                        ...
                        && is_vm_known_irrational_type(&right_julia_ty)
                    {
                        ...
                        self.emit(Instr::Pop);
                        self.emit(Instr::Pop);
                        let same_singleton_type = match (&left_julia_ty, &right_julia_ty) {
                            (JuliaType::Struct(left_name), JuliaType::Struct(right_name)) => {
        ...
        if matches!(
                ...
                || is_vm_known_irrational_type(&right_julia_ty)
            {
                ...
                let result_ty = match op {
                    BinaryOp::Lt | BinaryOp::Le | BinaryOp::Gt | BinaryOp::Ge => ValueType::Bool,
                    _ if result_is_f32 => {
                        self.emit(Instr::DynamicToF32);
                        ValueType::F32
                    }
        ...
        // Keep Any/String cases ahead of method-table probing so Base string/char `*`
        // signatures do not force an Any operand through `Union{Char,String}` coercion
        // before runtime value handling (Issue #6268).
        ...
        // Uses isequal builtin which handles element comparison without lowering to operator().
        //
...
mod tests {
    ...
    fn typed_instr_for_intrinsic_maps_hot_int_and_float_ops() {
        ...
        assert!(matches!(
            typed_instr_for_intrinsic(Intrinsic::MulFloat),
            Some(Instr::MulF64)
        ));
        assert!(matches!(
    ...
    fn typed_instr_for_intrinsic_delegates_to_shared_table_8192() {
        ...
        assert_instr!(
            typed_instr_for_intrinsic(Intrinsic::AddFloat),
            Instr::AddF64
        );
        assert_instr!(
            typed_instr_for_intrinsic(Intrinsic::SubFloat),
            Instr::SubF64
        );
        assert_instr!(
            typed_instr_for_intrinsic(Intrinsic::MulFloat),
        ...
        assert_instr!(
            typed_instr_for_intrinsic(Intrinsic::DivFloat),
            Instr::DivF64
        );
