...
pub(crate) fn cst_to_macro_arg_constructor<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
) -> LowerResult<Expr> {
...
fn matrix_row_constructor<'a>(
    ...
    let children = walker.named_children(&row);
    if children.len() == 1 {
...
fn matrix_constructor<'a>(walker: &CstWalker<'a>, node: Node<'a>) -> LowerResult<Expr> {
    ...
    if rows.len() == 1 {
        let row_children = walker.named_children(&rows[0]);
        ...
        if !call_children.is_empty() {
            ...
            if func_name == "esc" && call_children.len() >= 2 {
                ...
                let escaped = if walker.kind(&actual_arg) == NodeKind::Identifier {
                    Expr::Var(walker.text(&actual_arg).to_string(), span)
                } else {
...
pub(crate) fn cst_to_expr_constructor<'a>(
    walker: &CstWalker<'a>,
    node: Node<'a>,
) -> LowerResult<Expr> {
    let span = walker.span(&node);
    ...
    match walker.kind(&node) {
            ...
            if text.starts_with('$') && !children.is_empty() {
                ...
                if preserve_macro_dollar() {
                    if inner_kind == NodeKind::CallExpression {
                        let call_children = walker.named_children(inner);
                ...
                if inner_kind == NodeKind::CallExpression {
                    ...
                    if let Some(callee) = call_children.first() {
                        ...
                        if walker.kind(callee) == NodeKind::Identifier {
                            args.push(Expr::Var(
                                walker.text(callee).to_string(),
                                walker.span(callee),
                            ));
                        } else if walker.kind(callee) == NodeKind::ParenthesizedExpression {
                            // `$(esc(:f))(args)` / `$(expr)(args)`: the parser
                            // binds `$` greedily, so the interpolated callee landed
                            // here as a parenthesized group. It is the `$(...)`
                            // payload — interpolate it (handling `esc`), do not
                            // quote it literally (Issue #8066).
                ...
                // Handle `$(esc(expr))` (escape marker preserved), `$(p...)`
                // (splat) and the normal `$(expr)` interpolation via the shared
                // helper (also reused for interpolated short-form function names,
                // Issue #8066).
        ...
        NodeKind::ComprehensionExpression => {
            ...
            let mut generator_args = vec![
                Expr::Builtin {
                    name: BuiltinOp::SymbolNew,
                    args: vec![Expr::Literal(Literal::Str("generator".to_string()), span)],
                    span,
                },
                cst_to_expr_constructor(walker, children[0])?,
            ];
                    ...
                    NodeKind::ForClause => {
                        for binding in walker.named_children(child) {
                            if walker.kind(&binding) == NodeKind::ForBinding {
                                generator_args
                                    .push(for_binding_constructor(walker, binding, span)?);
                            }
                        }
                    }
                    _ => {
                        return Err(UnsupportedFeature::new(
                            UnsupportedFeatureKind::UnsupportedExpression(format!(
                                "quote of comprehension: unsupported clause {}",
                                child.kind()
                            )),
                            span,
                        ));
        ...
        NodeKind::FieldExpression => {
            ...
            let field = match walker.kind(&field_node) {
                ...
                NodeKind::QuoteExpression if walker.named_children(&field_node).is_empty() => {
