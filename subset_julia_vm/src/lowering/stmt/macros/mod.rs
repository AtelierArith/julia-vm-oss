...
use crate::lowering::{contains_macro_call, LambdaContext};
...
fn lower_inbounds_index_assignment<'a>(
    ...
    let mut call_args = vec![
        Expr::Var(array_name, walker.span(&lhs)),
        ...
        && walker.kind(&second_lhs) == NodeKind::Identifier
    {
        ...
        return Ok(Some(Stmt::Block(Block {
            stmts: vec![
                ...
                Stmt::Assign {
                    var: first_var,
                    value: Expr::Var(first_temp, span),
                    span,
                },
                Stmt::Assign {
                    var: second_var,
                    value: Expr::Var(second_temp, span),
                    span,
                },
            ],
            span,
    ...
    Ok(Some(Stmt::Block(Block {
        stmts: vec![
            ...
            inbounds_setindex_stmt(
                first_array,
                first_indices,
                Expr::Var(first_temp, span),
                span,
            ),
            inbounds_setindex_stmt(
                second_array,
                second_indices,
                Expr::Var(second_temp, span),
                span,
            ),
        ],
        span,
...
fn lower_eval_function_definition<'a>(
    ...
    let def_node = args[0];
    let funcs = if walker.kind(&def_node) == NodeKind::FunctionDefinition {
        if contains_macro_call(walker, def_node) {
            function::lower_function_all_with_ctx(walker, def_node, lambda_ctx)?
        } else {
            function::lower_function_all(walker, def_node)?
        }
    } else if function::is_short_function_definition(walker, def_node) {
        if contains_macro_call(walker, def_node) {
            function::lower_short_function_all_with_ctx(walker, def_node, lambda_ctx)?
        } else {
            function::lower_short_function_all(walker, def_node)?
        }
    } else {
        return Ok(None);
    };
    ...
    Ok(Some(if stmts.len() == 1 {
        stmts.pop().expect("single eval function stmt")
    } else {
...
fn lower_meta_annotation_compat<'a>(
    ...
    if macro_name == "boundscheck" {
        ...
        return Ok(Stmt::If {
            condition: Expr::Call {
                function: "#__sjulia_boundscheck_enabled__".to_string(),
                args: vec![],
                kwargs: vec![],
                splat_mask: vec![],
                kwargs_splat_mask: vec![],
...
fn lower_generated_function_compat<'a>(
    ...
    let (mut funcs, mark_generated) = if walker.kind(&def_node) == NodeKind::FunctionDefinition {
        let mut funcs = function::lower_function_all(walker, def_node)?;
        ...
        let mut funcs = function::lower_short_function_all(walker, def_node)?;
        let mut unquoted = false;
        let mut unquoted_requires_generated_frame = false;
        let children = walker.named_children(&def_node);
...
fn is_generated_expr_value(expr: &Expr, expr_locals: &HashSet<String>) -> bool {
    match expr {
        ...
        Expr::Var(name, _) => expr_locals.contains(name),
        _ => false,
    }
}
...
pub fn lower_macro_with_ctx<'a>(
    ...
    match macro_name.as_str() {
        ...
        "task" | "async" | "invoke" | "invokelatest" | "views" | "__dot__" | "." => {
        ...
        "generated" => {
            return lower_generated_function_compat(walker, node, args_node, &direct_args, span);
        }
