...
/// its `Vector`/`Matrix` aliases), ignoring any module qualifier and type
/// parameters. Single source of truth for the byte-identical copies that used
/// to live in `compile/`, `vm/exec/`, and `vm/specialize/` (Issue #6830).
pub(crate) fn is_array_wrapper_struct_name(name: &str) -> bool {
...
pub(crate) fn array_wrapper_shape_and_offset(size: &Value) -> Option<(Vec<usize>, usize)> {
...
pub(crate) fn array_wrapper_shape_from_tuple(dims_tuple: &TupleValue) -> Option<Vec<usize>> {
    dims_tuple
        .elements
        .iter()
        .map(|dim| match dim {
...
pub(crate) fn array_wrapper_value_from_array_value(
    arr: ArrayValue,
    array_type_id: usize,
...
pub(crate) fn array_wrapper_value_from_array_value_inline(
    arr: ArrayValue,
    array_type_id: usize,
) -> Result<Value, VmError> {
...
mod tests {
    use super::*;
    use crate::vm::value::{
        new_memory_ref, ArrayElementType, MemoryRefValue, MemoryValue, TupleValue,
    };
    ...
    fn complex_memoryref_wrapper_materializes_interleaved_array() -> Result<(), VmError> {
        ...
        memory.set(1, Value::StructRef(0))?;
        memory.set(2, Value::StructRef(1))?;
    ...
    fn is_array_wrapper_struct_name_matches_aliases_and_qualifiers() {
        ...
        // Unrelated struct names do not match.
        for name in ["Dict", "Tuple", "MyArrayWrapper", "ArrayLike", "Set{Int64}"] {
            assert!(
