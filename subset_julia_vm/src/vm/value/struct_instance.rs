...
use super::super::field_indices::{
    RATIONAL_DENOMINATOR_FIELD_INDEX, RATIONAL_NUMERATOR_FIELD_INDEX,
};
...
pub struct StructInstance {
    ...
    ///
    /// Stored as `Box<str>` rather than `String` (Issue #7976): the name is
    /// immutable once constructed, so dropping the `String` capacity word shrinks
    /// `StructInstance` from 56 to 48 bytes and lowers the `Value` enum ceiling
    /// from 64 to 56 (`Box<str>` derefs to `str`, so name predicates/display are
    /// unchanged). Full elimination of this redundant-with-`type_id` field is a
    /// larger follow-up (it needs name reconstruction from `struct_defs` at the
    /// predicate sites that have no VM context).
...
impl StructInstance {
    ...
    pub fn with_name(type_id: usize, struct_name: String, values: Vec<Value>) -> Self {
        Self {
            type_id,
            struct_name: struct_name.into_boxed_str(),
            values,
        }
    }
    ...
    pub fn complex(type_id: usize, re: f64, im: f64) -> Self {
        Self {
            type_id,
            struct_name: Box::from(COMPLEX_STRUCT_NAME),
...
mod tests {
    use super::*;
    use crate::vm::value::{
        native_array_value_from_array, ArrayElementType, ArrayValue, TupleValue,
    };
