...
pub struct SpecializationCounters {
    /// `LoadAny`/`StoreAny`/`ReturnAny` dynamic-value traffic.
...
pub fn specialization_counters_from_results(results: &[(String, u64)]) -> SpecializationCounters {
    SpecializationCounters {
        boxed_value_accesses: sum_exact(results, &["LoadAny", "StoreAny", "ReturnAny"]),
...
mod tests {
    ...
    fn test_instruction_name_splits_call_dynamic_binary_both_by_kind() {
        assert_eq!(
            instruction_name(&Instr::CallDynamicBinaryBoth(Intrinsic::AddFloat, vec![1])),
            "CallDynamicBinaryBoth::AddFloat/1"
        );
    }
...
mod specialization_counter_tests {
    ...
    fn specialization_counters_derive_issue_5095_ratios() {
        ...
        assert_eq!(counters.boxed_value_accesses, 3);
        ...
        assert_eq!(counters.boxing_rate(), 0.25);
