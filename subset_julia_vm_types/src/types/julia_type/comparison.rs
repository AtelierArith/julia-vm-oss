...
use super::parsing::{parse_parametric_name, unbounded_vararg_element};
use super::{strip_base_type_prefix, struct_owners_compatible, JuliaType};
...
impl JuliaType {
    ...
    fn is_subtype_of_with_lookup(
        ...
        if let Some(alias) = self.canonical_generic_unionall_alias() {
            return alias.is_subtype_of_with_lookup(other, hierarchy);
        }
        if let Some(alias) = other.canonical_generic_unionall_alias() {
            return self.is_subtype_of_with_lookup(&alias, hierarchy);
        }
        ...
        match other {
            ...
            JuliaType::Struct(other_name) => {
                ...
                if let JuliaType::Struct(self_name) = self {
                    if !struct_owners_compatible(self_name, other_name) {
                        return false;
                    }
    ...
    pub fn extract_type_bindings(
        ...
        self.extract_type_bindings_with_lookup(pattern, type_params, None)
    }

    pub fn extract_type_bindings_in(
        &self,
        pattern: &JuliaType,
        type_params: &[TypeParam],
        hierarchy: &StructHierarchy,
    ) -> Option<std::collections::HashMap<String, JuliaType>> {
        self.extract_type_bindings_with_lookup(pattern, type_params, Some(hierarchy))
    }

    fn extract_type_bindings_with_lookup(
        &self,
        pattern: &JuliaType,
        type_params: &[TypeParam],
        hierarchy: Option<&StructHierarchy>,
    ) -> Option<std::collections::HashMap<String, JuliaType>> {
        if let Some(bindings) = self.extract_type_bindings_direct(pattern, type_params, hierarchy) {
            return Some(bindings);
        }

        // A trailing-`UnionAll` value — the under-applied builtin/parametric
        // family form `Array{Float64, N} where N` (Issue #10586) — binds a
        // `::Type{Array{T}}` pattern exactly like its concrete body does: strip
        // the outer binder(s) and match the body, leaving the still-free trailing
        // parameters (`N`) unmatched because the pattern has fewer parameters.
        // Applied only as a fallback (direct matching already returned `None`).
        //
        // Guard: the match must NOT bind a method `where` parameter to one of the
        // stripped binder variables (`Array{Float64}` matching `::Type{Array{T,
        // K}}` would bind `K = N`, an existential rank upstream does not match).
        // Rejecting such matches keeps the fallback to genuine *prefix* patterns
        // (`Array{T}`), so it only turns a previously-unbound dispatch into a
        // correctly-bound one. Skipped when the pattern is itself a `UnionAll`
        // (a structural match already handled by `_direct`).
        if !matches!(
            pattern,
            ...
            loop {
                match body {
                    JuliaType::UnionAll {
                        ...
                        stripped_vars.push(JuliaType::TypeVar(var.clone(), None));
                        body = inner.as_ref();
                    }
                    JuliaType::RuntimeUnionAll { var, body: inner } => {
                        stripped_vars.push(var.as_ref().clone());
                        body = inner.as_ref();
                    }
                    _ => break,
                }
            ...
            if !stripped_vars.is_empty() {
                if let Some(bindings) =
                    body.extract_type_bindings_with_lookup(pattern, type_params, hierarchy)
                {
                    let consumes_stripped_binder = bindings.values().any(|bound| {
                        stripped_vars.iter().any(|stripped| match stripped {
                            JuliaType::RuntimeTypeVar { id, .. } => {
                                bound.references_runtime_typevar(*id)
                            }
                            JuliaType::TypeVar(name, _) => references_source_typevar(bound, name),
                            _ => bound == stripped,
                        })
                    });
    ...
    fn extract_type_bindings_direct(
        &self,
        pattern: &JuliaType,
        type_params: &[TypeParam],
        hierarchy: Option<&StructHierarchy>,
        ...
        let pattern_array_projection = if pattern_uses_abstract_array_projection(pattern) {
            abstract_array_projection(pattern)
        } else {
            array_projection(pattern)
        };
        if let (Some((self_elem, self_dim)), Some((pattern_elem, pattern_dim))) =
            (self_array_projection, pattern_array_projection)
        {
            ...
            if let Some(bound) = anonymous_upper_bound_pattern(&pattern_elem, type_params) {
                if self_elem.is_subtype_of_parametric(&bound, type_params) {
                    return Some(bindings);
                }
                return None;
            }
            if julia_type_mentions_type_params_local(&pattern_elem, type_params) {
                if let Some(nested) = self_elem.extract_type_bindings_with_lookup(
                    &pattern_elem,
                    type_params,
                    hierarchy,
                ) {
        ...
        if let JuliaType::Union(members) = pattern {
            for member in members {
                if let Some(extracted) =
                    self.extract_type_bindings_with_lookup(member, type_params, hierarchy)
                {
        ...
        // Identity-bearing parametric actuals are the structured counterpart
        // of the legacy `Struct("Family{T}")` method-signature image. Match
        // them positionally, allowing a prefix pattern such as
        // `Type{SubArray{T}}` to match a trailing-UnionAll family value. The
        // caller strips those binders before reaching this branch (Issue #10861).
        if let (JuliaType::RuntimeParametric { base, params }, JuliaType::Struct(pattern_name)) =
            (self, pattern)
        {
            let (pattern_base, pattern_param_names) = parse_parametric_name(pattern_name);
            let pattern_params: Vec<_> = pattern_param_names
                .into_iter()
                .map(JuliaType::from_name_or_struct)
                .collect();
            let both_qualified = base.contains('.') && pattern_base.contains('.');
            let same_family = if both_qualified {
                base == pattern_base
            } else {
                strip_module_prefix(base) == strip_module_prefix(pattern_base)
            };
            if !same_family || params.len() < pattern_params.len() {
                return None;
            }
            for (actual_param, pattern_param) in params.iter().zip(pattern_params.iter()) {
                if let Some(tp) = type_param_for_pattern(pattern_param, type_params) {
                    if let Some(upper) = tp.get_upper_bound() {
                        if let Some(bound) = JuliaType::from_name(upper) {
                            if !actual_param.is_subtype_of_with_lookup(&bound, hierarchy) {
                                return None;
                            }
                        } else if !actual_param.unresolved_bound_is_satisfied(upper, hierarchy) {
                            return None;
                        }
                    }
                    if let Some(lower) = tp.lower_bound.as_deref() {
                        let bound = JuliaType::from_name_or_struct(lower);
                        if !julia_type_mentions_type_params_local(&bound, type_params)
                            && !bound.is_subtype_of_with_lookup(actual_param, hierarchy)
                        {
                            return None;
                        }
                    }
                    merge_type_binding(&mut bindings, &tp.name, actual_param.clone())?;
                } else if julia_type_mentions_type_params_local(pattern_param, type_params) {
                    let nested = actual_param.extract_type_bindings_with_lookup(
                        pattern_param,
                        type_params,
                        hierarchy,
                    )?;
                    merge_tuple_bindings(&mut bindings, nested)?;
                } else if !actual_param.type_eq(pattern_param) {
                    return None;
                }
            }
            return Some(bindings);
        ...
        // Struct-to-parametric-abstract matching. A concrete user struct can
        // bind a method's `where` variables through its declared abstract
        // supertype, e.g. ConM{2,2,Float64} against AbsM{2,2,T}.
        if let (JuliaType::Struct(sn), JuliaType::AbstractUser(abstract_name, _)) = (self, pattern)
        {
            let projected =
                project_struct_to_abstract_pattern(sn, abstract_name, hierarchy, type_params)?;
            return projected.extract_type_bindings_with_lookup(
                &JuliaType::Struct(abstract_name.clone()),
                type_params,
                hierarchy,
            );
        }
        ...
        if let (JuliaType::Struct(sn), JuliaType::Struct(pn)) = (self, pattern) {
            ...
            for (s, p) in sa.iter().zip(pa.iter()) {
                ...
                if let Some(tp) = type_params.iter().find(|tp| tp.name == *p) {
                    let bt =
                        JuliaType::from_name(s).unwrap_or_else(|| JuliaType::Struct(s.to_string()));
                    if let Some(bn) = tp.get_upper_bound() {
                        if let Some(b) = JuliaType::from_name(bn) {
                            if !bt.is_subtype_of_with_lookup(&b, hierarchy) {
                                return None;
                            }
                        } else if !bt.unresolved_bound_is_satisfied(bn, hierarchy) {
                            return None;
                        }
                    }
                    if let Some(lower) = tp.lower_bound.as_deref() {
                        let bound = JuliaType::from_name_or_struct(lower);
                        if !julia_type_mentions_type_params_local(&bound, type_params)
                            && !bound.is_subtype_of_with_lookup(&bt, hierarchy)
                        {
                            return None;
                        }
                    }
                    ...
                    merge_type_binding(&mut bindings, &tp.name, bt)?;
                } else {
                    let s_ty = JuliaType::from_name_or_struct(s);
                    let p_ty = parse_array_projection_param(p);
                    if julia_type_mentions_type_params_local(&p_ty, type_params) {
                        // Nested parametric slot (e.g. `Wrap{T}` in `Box{Wrap{T}}`):
                        // extract bindings recursively so `T` is bound in the
                        // method body (Issue #8853). Merge with conflict
                        // detection (Issue #11231) rather than a raw `extend`.
                        let nested =
                            s_ty.extract_type_bindings_with_lookup(&p_ty, type_params, hierarchy)?;
                        merge_tuple_bindings(&mut bindings, nested)?;
        ...
        if let (JuliaType::VectorOf(se), JuliaType::VectorOf(pe)) = (self, pattern) {
            return se.extract_type_bindings_with_lookup(pe, type_params, hierarchy);
        }
        ...
        if let (JuliaType::TupleOf(st), JuliaType::TupleOf(pt)) = (self, pattern) {
            if let Some((lead, vararg_elem)) = split_trailing_vararg(pt) {
                ...
                for (se, pe) in st.iter().zip(lead.iter()) {
                    let eb = se.extract_type_bindings_with_lookup(pe, type_params, hierarchy)?;
                    merge_tuple_bindings(&mut bindings, eb)?;
                }
                let joined = join_types(&st[lead.len()..]);
                if let Some(elem) = joined {
                    let eb = elem.extract_type_bindings_with_lookup(
                        &vararg_elem,
                        type_params,
                        hierarchy,
                    )?;
        ...
        if let JuliaType::TypeVar(vn, Some(bound)) = pattern {
            if !type_params.iter().any(|p| &p.name == vn) {
                ...
                return self.extract_type_bindings_with_lookup(
                    &bound_pattern,
                    type_params,
                    hierarchy,
                );
        ...
        if let JuliaType::TypeOf(inner) = pattern {
            ...
            if let JuliaType::TypeOf(arg_inner) = self {
                if let Some(extracted) =
                    arg_inner.extract_type_bindings_with_lookup(inner, type_params, hierarchy)
                {
                    return Some(extracted);
                }
            }
        ...
        if let Some(pattern_family) = concrete_array_wrapper_family(pattern) {
            if concrete_array_wrapper_family(self) != Some(pattern_family) {
                return None;
            }
        }

        if self.is_subtype_of_with_lookup(pattern, hierarchy) {
            Some(bindings)
        } else {
            None
        }
...
fn merge_type_binding(
    ...
    match acc.entry(name.to_string()) {
        std::collections::hash_map::Entry::Occupied(entry) => {
            (entry.get() == &bound_type).then_some(())
        }
...
fn references_source_typevar(ty: &JuliaType, name: &str) -> bool {
    match ty {
        JuliaType::TypeVar(var, _) => var == name,
        JuliaType::RuntimeTypeVar { .. } => false,
        JuliaType::Struct(rendered) => super::struct_name_mentions_param(rendered, name),
        JuliaType::RuntimeParametric { params, .. }
        | JuliaType::TupleOf(params)
        | JuliaType::Union(params) => params
            .iter()
            .any(|param| references_source_typevar(param, name)),
        JuliaType::VectorOf(inner) | JuliaType::MatrixOf(inner) | JuliaType::TypeOf(inner) => {
            references_source_typevar(inner, name)
        }
        JuliaType::UnionAll { var, body, .. } => {
            var != name && references_source_typevar(body, name)
        }
        JuliaType::RuntimeUnionAll { var, body } => {
            references_source_typevar(var, name) || references_source_typevar(body, name)
        }
        _ => false,
    }
}
...
fn merge_tuple_bindings(
    ...
    for (name, bt) in extracted {
        merge_type_binding(acc, &name, bt)?;
    }
    Some(())
}
...
fn abstract_array_projection(ty: &JuliaType) -> Option<(JuliaType, Option<usize>)> {
    array_projection(ty)
        .or_else(|| {
            ...
            abstract_array_struct_projection(name).or_else(|| array_wrapper_struct_projection(name))
        })
        .or_else(|| range_projection(ty))
}

fn pattern_uses_abstract_array_projection(ty: &JuliaType) -> bool {
    matches!(ty, JuliaType::AbstractRange)
        || matches!(ty, JuliaType::Struct(name)
            if abstract_array_struct_projection(name).is_some()
                || abstract_range_struct_projection(name).is_some())
}

fn abstract_range_struct_projection(name: &str) -> Option<(JuliaType, Option<usize>)> {
    let (base, params) = parse_parametric_name(name);
    if base.rsplit('.').next().unwrap_or(base) != "AbstractRange" {
        return None;
    }
    let elem = params
        .first()
...
fn array_wrapper_struct_projection(name: &str) -> Option<(JuliaType, Option<usize>)> {
    let (base, params) = parse_parametric_name(name);
    if !matches!(
...
fn concrete_array_wrapper_family(ty: &JuliaType) -> Option<&str> {
    let JuliaType::Struct(name) = ty else {
        return None;
    };
    ...
    match base.rsplit('.').next().unwrap_or(base) {
        ...
        "ReshapedArray" => Some("ReshapedArray"),
        "MatrixView" => Some("MatrixView"),
        _ => None,
    }
}

fn project_struct_to_abstract_pattern(
    struct_name: &str,
    abstract_name: &str,
    hierarchy: Option<&StructHierarchy>,
    type_params: &[TypeParam],
    ...
    let pattern = crate::inference_core::CoreType::from_julia_name(abstract_name);
    let crate::inference_core::CoreType::Struct {
        name: pattern_family,
    ...
    if pattern_params.is_empty()
        && !julia_type_mentions_type_params_local(
            &JuliaType::Struct(abstract_name.to_string()),
            type_params,
        )
    {
        return None;
    ...
    let actual = crate::inference_core::CoreType::from_julia_name(struct_name);
    let (actual_family, actual_params) = match &actual {
        crate::inference_core::CoreType::Struct { name, params } => {
            (name.as_str(), params.as_slice())
        }
        crate::inference_core::CoreType::Named(name) => (name.as_str(), &[][..]),
        _ => return None,
    };

    let projected = if strip_module_prefix(actual_family) == strip_module_prefix(pattern_family) {
        crate::inference_core::CoreType::Struct {
            name: actual_family.to_string(),
            params: actual_params.to_vec(),
        }
    } else {
        crate::inference_core::registered_instantiated_struct_supertype_in(
            hierarchy,
            actual_family,
            actual_params,
            strip_module_prefix(pattern_family),
        )?
    };

    Some(crate::inference_core::core_type_to_julia_type(&projected))
}
...
fn parse_array_projection_param(param: &str) -> JuliaType {
    ...
    if let Some((name, bound)) = param.split_once(">:") {
        let name = name.trim();
        let name = if name.is_empty() { "_" } else { name };
        let bound = JuliaType::from_name_or_struct(bound.trim())
            .name()
            .to_string();
        return JuliaType::TypeVar(name.to_string(), Some(format!(">:{}", bound)));
    }
...
fn parametric_slot_matches(actual: &str, pattern: &str, type_params: &[TypeParam]) -> bool {
    ...
    if type_param_for_pattern(&pattern_ty, type_params).is_some() {
        return true;
    }
    if let Some(bound) = anonymous_upper_bound_pattern(&pattern_ty, type_params) {
        return actual_ty.is_subtype_of_parametric(&bound, type_params);
    }
    if let Some(bound) = anonymous_lower_bound_pattern(&pattern_ty, type_params) {
        return bound.is_subtype_of_parametric(&actual_ty, type_params);
    }
    if !julia_type_mentions_type_params_local(&pattern_ty, type_params) {
        return actual_ty == pattern_ty;
    }
    actual_ty
        .extract_type_bindings(&pattern_ty, type_params)
        .is_some()
}
...
fn array_elem_matches_parametric(
    ...
    // Array element slots are invariant: Vector{String} does NOT match a
    // Vector{Any} parameter (Julia arrays are invariant in their element type).
    ...
    if let Some(bound) = anonymous_upper_bound_pattern(pattern_elem, type_params) {
        return self_elem.is_subtype_of_parametric(&bound, type_params);
    }
    if let Some(bound) = anonymous_lower_bound_pattern(pattern_elem, type_params) {
        return bound.is_subtype_of_parametric(self_elem, type_params);
    }
    if !julia_type_mentions_type_params_local(pattern_elem, type_params) {
        return self_elem == pattern_elem;
    }
...
fn anonymous_upper_bound_pattern(
    pattern: &JuliaType,
    type_params: &[TypeParam],
) -> Option<JuliaType> {
    if let JuliaType::TypeVar(name, Some(bound)) = pattern {
        if type_params.iter().all(|p| p.name != *name) && !bound.starts_with(">:") {
            return Some(JuliaType::from_name_or_struct(bound));
        }
    }
    None
}

fn anonymous_lower_bound_pattern(
    pattern: &JuliaType,
    type_params: &[TypeParam],
) -> Option<JuliaType> {
    if let JuliaType::TypeVar(name, Some(bound)) = pattern {
        if type_params.iter().all(|p| p.name != *name) {
            let lower = bound.strip_prefix(">:")?;
            return Some(JuliaType::from_name_or_struct(lower));
        }
    }
    None
}

fn julia_type_mentions_type_params_local(ty: &JuliaType, type_params: &[TypeParam]) -> bool {
    match ty {
        JuliaType::TypeVar(name, _) | JuliaType::Struct(name) => {
            ...
            if let JuliaType::Struct(name) = ty {
                let (_, params) = parse_parametric_name(name);
                return params.iter().any(|param| {
                    let nested = parse_array_projection_param(param);
                    julia_type_mentions_type_params_local(&nested, type_params)
                });
            }
            false
        }
        JuliaType::VectorOf(inner) | JuliaType::MatrixOf(inner) | JuliaType::TypeOf(inner) => {
            julia_type_mentions_type_params_local(inner, type_params)
        }
        JuliaType::TupleOf(elems) | JuliaType::Union(elems) => elems
            .iter()
            .any(|elem| julia_type_mentions_type_params_local(elem, type_params)),
        JuliaType::RuntimeParametric { params, .. } => params
            .iter()
            .any(|param| julia_type_mentions_type_params_local(param, type_params)),
        JuliaType::UnionAll { var, body, .. } => {
            type_params.iter().any(|p| p.name == *var)
                || julia_type_mentions_type_params_local(body, type_params)
        }
        _ => false,
    }
}
