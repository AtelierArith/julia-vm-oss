...
use crate::ir::core::{
    decode_tuple_comprehension_binding, Block, Expr, Function, LocalDeclKind, Stmt,
};
...
pub fn analyze_free_variables(
    ...
    for kwparam in &func.kwparams {
        local_vars.insert(kwparam.name.clone());
    }
    ...
    for name in &soft_assigned {
        if outer_scope_vars.contains(name)
            && !func.params.iter().any(|param| &param.name == name)
            && !func.kwparams.iter().any(|param| &param.name == name)
        {
...
pub fn collect_referenced_names(func: &Function) -> HashSet<String> {
...
fn collect_referenced_names_expr(expr: &Expr, names: &mut HashSet<String>) {
    match expr {
        ...
        Expr::Convert { operand, .. } => collect_referenced_names_expr(operand, names),
...
pub fn collect_block_local_bindings(block: &Block) -> HashSet<String> {
    let mut local_vars = HashSet::new();
...
fn collect_function_local_bindings_stmt(stmt: &Stmt, local_vars: &mut HashSet<String>) {
    match stmt {
        ...
        Stmt::LocalDecl { var, kind, .. } => match kind {
            LocalDeclKind::Explicit | LocalDeclKind::CompilerEnclosing => {
                local_vars.insert(var.clone());
            }
        },
        _ => {}
...
fn collect_function_local_bindings_expr(expr: &Expr, local_vars: &mut HashSet<String>) {
    match expr {
        ...
        Expr::Convert { operand, .. } => {
            collect_function_local_bindings_expr(operand, local_vars);
        }
        Expr::Var(_, _)
        | Expr::Literal(_, _)
        | Expr::FunctionRef { .. }
...
/// Collect assignment-expression bindings that execute in the expression's
/// current lexical scope. Nested hard scopes are deliberately not descended.
pub fn collect_expr_local_bindings(expr: &Expr, local_vars: &mut HashSet<String>) {
    collect_function_local_bindings_expr(expr, local_vars);
}
...
fn analyze_expr_free_vars(
    ...
    match expr {
        Expr::Var(name, _) => {
            ...
            if !local_vars.contains(name.as_str()) && outer_scope_vars.contains(name.as_str()) {
        ...
        Expr::Call {
            ...
            // `function` is the callee name. When it names a variable from the
            // enclosing scope rather than a local or a global function, it is a
            // captured free variable holding a callable — e.g. the `f` in
            // `makeapply(f) = x -> f(x)`. Without capturing it, the closure body
            // errors with "Unknown function: f" (Issue #5723). Explicit type
            // application remains a call on its base binding: `t{Int}(x)` must
            // capture an outer `t`, not look for an outer variable literally
            // named `t{Int}` (Issue #11373). Globals and user functions are not
            // in `outer_scope_vars`, so they are excluded.
            let callee_binding = parse_parametric_call(function)
                .map(|(base, _)| base)
                .unwrap_or_else(|| function.to_string());
            if !local_vars.contains(callee_binding.as_str())
        ...
        Expr::Comprehension {
            ...
            if let Some(tuple_vars) = decode_tuple_comprehension_binding(var) {
                ...
                comp_local.insert(var.to_string());
            }
        ...
        Expr::Generator {
            ...
            gen_local.insert(var.to_string());
            analyze_expr_free_vars(iter, outer_scope_vars, local_vars, free_vars);
        ...
        Expr::LetBlock { bindings, body, .. } => {
            ...
            for (name, value) in bindings {
                ...
                let_local.insert(name.to_string());
            }
        ...
        Expr::AssignExpr { value, var, .. } => {
            if !local_vars.contains(var.as_str()) && outer_scope_vars.contains(var.as_str()) {
                free_vars.insert(var.to_string());
            }
            analyze_expr_free_vars(value, outer_scope_vars, local_vars, free_vars);
        }
        Expr::ReturnExpr { value, .. } => {
...
mod tests {
    use super::*;
    use crate::ir::core::{BinaryOp, KwParam, Literal, Stmt, TypedParam};
    ...
    fn make_func_with_kwparams(params: &[&str], kwparams: &[&str], body: Vec<Stmt>) -> Function {
        let mut func = make_func(params, body);
        func.kwparams = kwparams
            .iter()
            .map(|name| KwParam::new(name.to_string(), int_lit(1), None, span()))
            .collect();
        func
    }
    ...
    /// A keyword parameter with the same name as an outer variable also shadows it.
    #[test]
    fn test_kwparam_shadows_outer_var() {
        // fn(x; y=1) -> return x + y   -- y is a kwparam, not a free var
        let func = make_func_with_kwparams(
            &["x"],
            &["y"],
            vec![Stmt::Return {
        ...
        assert!(
            result.is_empty(),
            "Expected no free vars (y is a kwparam), got {:?}",
            result
        );
    }
    ...
    #[test]
    fn parametric_call_captures_its_base_binding_issue_11373() {
        let func = make_func(
            &[],
            vec![Stmt::Return {
                value: Some(Expr::Call {
                    function: "Dict{Int}".into(),
                    args: vec![var_expr("x")],
                    kwargs: vec![],
        ...
        let result = analyze_free_variables(&func, &outer(&["Dict", "x"]));
        assert!(result.contains("Dict"));
        assert!(result.contains("x"));
        assert!(!result.contains("Dict{Int}"));
    }
