...
use crate::types::{
    explicit_sibling_nominal_family_conflict, nominal_family_names_compatible, StructHierarchy,
};
...
pub fn core_signature_match_with_bindings(
    ...
    core_signature_match_with_bindings_impl(param_types, arg_types, type_vars, None)
}

/// Hierarchy-aware form of [`core_signature_match_with_bindings`].
///
/// Static dispatch owns the program's [`StructHierarchy`] and must use it for
/// explicit `where` bounds after structured `Type{W{T}}` binding. The
/// hierarchy-free wrapper remains available for callers whose type universe is
/// limited to built-in or self-describing CoreType relations.
pub fn core_signature_match_with_bindings_with_hierarchy(
    param_types: &[CoreType],
    arg_types: &[CoreType],
    type_vars: &[CoreTypeVar],
    hierarchy: &StructHierarchy,
) -> Option<usize> {
    core_signature_match_with_bindings_impl(param_types, arg_types, type_vars, Some(hierarchy))
}

fn core_signature_match_with_bindings_impl(
    param_types: &[CoreType],
    arg_types: &[CoreType],
    type_vars: &[CoreTypeVar],
    hierarchy: Option<&StructHierarchy>,
) -> Option<usize> {
    let mut bindings = LexicalTypeBindings::new();
    let normalized_params: Vec<CoreType> = param_types
        .iter()
        .map(|param| embed_declared_type_vars(param, type_vars))
        .collect();

    for (param_ty, arg_ty) in normalized_params.iter().zip(arg_types.iter()) {
        if !core_dispatch_pattern_matches(param_ty, arg_ty, type_vars, &mut bindings, hierarchy) {
            return None;
        }
    }

    if !bindings.is_empty() && !check_diagonal_rule_for_params(&normalized_params, &bindings) {
        return None;
    }
    Some(bindings.len())
}

fn embed_declared_type_vars(core: &CoreType, type_vars: &[CoreTypeVar]) -> CoreType {
    embed_declared_type_vars_scoped(core, type_vars, &mut Vec::new())
}

fn embed_declared_type_vars_scoped(
    core: &CoreType,
    type_vars: &[CoreTypeVar],
    ...
    match core {
        ...
        {
            find_type_var(type_vars, &var.name)
                .cloned()
                .map(CoreType::TypeVar)
                .unwrap_or_else(|| core.clone())
        }
        CoreType::Named(name) if !shadowed.contains(name) => find_type_var(type_vars, name)
            .cloned()
            .map(CoreType::TypeVar)
            .unwrap_or_else(|| core.clone()),
        CoreType::Struct { name, params } => CoreType::Struct {
            ...
            params: params
                .iter()
                .map(|param| embed_declared_type_vars_scoped(param, type_vars, shadowed))
                .collect(),
        },
        CoreType::Tuple(elems) => CoreType::Tuple(
            elems
                .iter()
                .map(|elem| embed_declared_type_vars_scoped(elem, type_vars, shadowed))
                .collect(),
        ),
        CoreType::Union(arms) => CoreType::Union(
            arms.iter()
                .map(|arm| embed_declared_type_vars_scoped(arm, type_vars, shadowed))
                .collect(),
        ),
        CoreType::Vararg(inner) => CoreType::Vararg(Box::new(embed_declared_type_vars_scoped(
            inner, type_vars, shadowed,
        ))),
        CoreType::VarargLen { element, len } => CoreType::VarargLen {
            element: Box::new(embed_declared_type_vars_scoped(
                element, type_vars, shadowed,
            )),
            len: Box::new(embed_declared_type_vars_scoped(len, type_vars, shadowed)),
        },
        CoreType::TypeOf(inner) => CoreType::TypeOf(Box::new(embed_declared_type_vars_scoped(
            inner, type_vars, shadowed,
        ))),
        CoreType::NamedTuple(fields) => CoreType::NamedTuple(
            fields
                ...
                .map(|(name, ty)| {
                    (
                        name.clone(),
                        embed_declared_type_vars_scoped(ty, type_vars, shadowed),
                    )
                })
                .collect(),
        ),
        CoreType::UnionAll { var, body } => {
            ...
            let body = Box::new(embed_declared_type_vars_scoped(body, type_vars, shadowed));
            shadowed.pop();
            CoreType::UnionAll {
                var: var.clone(),
                body,
...
/// helper. Score equality with the legacy scorer over the whole Base corpus
/// is pinned by
...
pub fn typed_vararg_where_bonus_core(
    param_types: &[CoreType],
    type_vars: &[CoreTypeVar],
    vararg_param_index: Option<usize>,
) -> u32 {
    if vararg_param_mentions_type_vars_core(param_types, type_vars, vararg_param_index) {
        super::VARARG_TYPE_PARAM_BINDING_BONUS
    } else {
        0
    }
}

pub fn vararg_param_mentions_type_vars_core(
    param_types: &[CoreType],
    type_vars: &[CoreTypeVar],
    vararg_param_index: Option<usize>,
) -> bool {
    ...
    param_types
        .get(vararg_idx)
        .is_some_and(|param| core_mentions_type_vars(param, type_vars))
}
...
fn core_dispatch_pattern_matches(
        ...
        if matches!(param_elems.last(), Some(CoreType::VarargLen { .. })) {
            ...
            let leads_ok =
                param_elems[..lead_count]
                    ...
                    .zip(arg_elems.iter())
                    .all(|(param, arg)| {
                        core_dispatch_pattern_matches(param, arg, type_vars, bindings, hierarchy)
                    });
            if !leads_ok {
                return false;
            }
            return arg_elems[lead_count..].iter().all(|arg| {
                core_dispatch_pattern_matches(vararg_elem, arg, type_vars, bindings, hierarchy)
            });
        } else {
    ...
    // Type-variable parameter. Explicit `TypeVar` nodes bind when they are
    // declared by the method `where` scope or carry anonymous/bounded syntax;
    // an undeclared unbounded node keeps the #5314 struct-leaf guard below.
    if let CoreType::TypeVar(var) = param_ty {
        ...
        if declared.is_none() && var.upper_bound.is_none() && base != "_" {
            ...
            if base == "_" {
                ...
                return upper.is_none_or(|bound| {
                    engine_is_subtype_with_optional_hierarchy(arg_ty, bound, hierarchy)
                }) && lower.is_none_or(|bound| {
                    engine_is_subtype_with_optional_hierarchy(bound, arg_ty, hierarchy)
                });
            }
            if let Some(bound_pattern) = parametric_type_var_bound_pattern(upper, type_vars) {
                ...
                return core_dispatch_pattern_matches(
                    bound_pattern,
                    arg_ty,
                    type_vars,
                    bindings,
                    hierarchy,
                ) && bind_or_check_type_var(base, None, None, arg_ty, bindings, hierarchy);
            }
            let upper = covariant_applicability_upper_bound(upper, type_vars);
            return bind_or_check_type_var(base, upper, None, arg_ty, bindings, hierarchy);
        }
    }

    // A `Named` parameter that names a method `where` variable.
        ...
        if let Some(declared) = find_type_var(type_vars, name) {
            ...
            if let Some(bound_pattern) = parametric_type_var_bound_pattern(upper, type_vars) {
                return core_dispatch_pattern_matches(
                    ...
                    bindings,
                    hierarchy,
                ) && bind_or_check_type_var(name, None, None, arg_ty, bindings, hierarchy);
            }
            let upper = covariant_applicability_upper_bound(upper, type_vars);
            return bind_or_check_type_var(name, upper, None, arg_ty, bindings, hierarchy);
        }
        if core_arg_is_dispatch_primitive(arg_ty) {
            return false;
        }
    ...
    if let CoreType::TypeOf(inner_param) = param_ty {
        if let CoreType::TypeOf(inner_arg) = arg_ty {
            ...
            if core_mentions_type_vars(inner_param, type_vars) {
                ...
                if let Some(extracted) = extract_type_bindings(inner_arg, inner_param, type_vars) {
                    return extracted.into_iter().all(|(name, bound_ty)| {
                        let declared = find_type_var(type_vars, &name);
                        bind_or_check_type_var(
                            &name,
                            declared.and_then(|var| var.upper_bound.as_deref()),
                            declared.and_then(|var| var.lower_bound.as_deref()),
                            &bound_ty,
                            bindings,
                            hierarchy,
                        )
            ...
            if matches!(inner_param.as_ref(), CoreType::Any) {
                return matches!(inner_arg.as_ref(), CoreType::Any);
            }
        ...
        && core_is_subtype_of_parametric(arg_ty, param_ty, type_vars)
    {
        if let Some(extracted) = extract_type_bindings(arg_ty, param_ty, type_vars) {
            return extracted.into_iter().all(|(name, bound_ty)| {
                ...
                bind_or_check_type_var(&name, upper, None, &bound_ty, bindings, hierarchy)
            });
        }
        return !same_family_parametric_binding_definitely_rejected(param_ty, arg_ty, type_vars);
    }
...
fn same_family_parametric_binding_definitely_rejected(
    ...
    param_params
        ...
        .zip(arg_params.iter())
        .any(|(param, arg)| {
            let Some(declared) = exact_type_var_for_pattern(param, type_vars) else {
                return false;
            };
            let Some(upper) = declared.upper_bound.as_deref() else {
                return false;
            };
            if bound_is_undecidable_user_type(upper) {
                return false;
            }
            !core_is_subtype_full(arg, upper)
        })
}
...
fn engine_is_subtype(actual: &CoreType, expected: &CoreType) -> bool {
    dispatch_core_is_subtype(actual, expected)
}

fn engine_is_subtype_with_optional_hierarchy(
    actual: &CoreType,
    expected: &CoreType,
    hierarchy: Option<&StructHierarchy>,
) -> bool {
    hierarchy.map_or_else(
        || engine_is_subtype(actual, expected),
        |hierarchy| dispatch_core_is_subtype_with_hierarchy(actual, expected, hierarchy),
    )
}

pub(super) fn dispatch_core_is_subtype(actual: &CoreType, expected: &CoreType) -> bool {
    !dispatch_has_explicit_sibling_owner_conflict(actual, expected)
        && CoreSubtypeEngine::new().is_subtype(actual, expected)
}

pub(super) fn dispatch_core_is_subtype_with_hierarchy(
    actual: &CoreType,
    expected: &CoreType,
    hierarchy: &StructHierarchy,
) -> bool {
...
/// Reject only the information loss that dispatch's owner-preserving bridge
/// can observe but the legacy global subtype relation intentionally cannot.
/// Different qualified families with the same bare tail are distinct nominal
/// declarations; bare-vs-qualified spellings remain compatible aliases.
pub(super) fn dispatch_has_explicit_sibling_owner_conflict(
    actual: &CoreType,
    expected: &CoreType,
) -> bool {
    fn nominal_name(ty: &CoreType) -> Option<&str> {
        match ty {
            ...
            | CoreType::Named(name)
            | CoreType::AbstractUser { name, .. } => Some(name),
            _ => None,
        }
    }

    if let (Some(actual_name), Some(expected_name)) = (nominal_name(actual), nominal_name(expected))
    {
        if explicit_sibling_nominal_family_conflict(actual_name, expected_name) {
            return true;
        }
    }
    ...
    match (actual, expected) {
        ...
        ) if nominal_family_names_compatible(actual_name, expected_name) => actual_params
            .iter()
            .zip(expected_params)
            .any(|(actual, expected)| {
                dispatch_has_explicit_sibling_owner_conflict(actual, expected)
...
fn core_is_subtype_fallback(actual: &CoreType, expected: &CoreType) -> bool {
    match expected {
        ...
        CoreType::Struct {
            ...
            // Legacy builtin-family shortcuts below intentionally strip module
            // prefixes. Reject an explicitly-owned sibling before reaching
            // them, so legal user types such as `A.Array` and `B.Array` do not
            // enter Base's Array lattice (Issue #11076).
            let actual_nominal_name = match actual {
                CoreType::Struct { name, .. }
                | CoreType::Named(name)
                | CoreType::AbstractUser { name, .. } => Some(name.as_str()),
                _ => None,
            };
            if actual_nominal_name.is_some_and(|actual_name| {
                actual_name.contains('.')
                    && expected_name.contains('.')
                    && !nominal_family_names_compatible(actual_name, expected_name)
            }) {
                return false;
            }
            ...
            if core_maps_to_julia_struct(actual) {
                ...
                {
                    if nominal_family_names_compatible(actual_name, expected_name)
                        && !expected_params.is_empty()
                        && actual_params.len() >= expected_params.len()
                    {
                        return actual_params.iter().zip(expected_params.iter()).all(
                            |(actual_param, expected_param)| {
                                core_parametric_slot_matches(
                                    actual_param,
                                    expected_param,
                                    &type_vars_from_pattern(expected_params),
                                )
                            },
                        );
                    }
            ...
            if let CoreType::Named(actual_name) = actual {
                return nominal_family_names_compatible(actual_name, expected_name)
                    && !expected_params.is_empty();
            }
            false
        }
        CoreType::Named(expected_name) => {
            ...
            // A bare/qualified alias can name the same family, but two
            // explicitly-qualified sibling owners cannot (Issue #11076).
            ...
            // `Named("Distributions.Categorical")`.
            if let CoreType::Named(actual_name) = actual {
                return nominal_family_names_compatible(actual_name, expected_name);
            }
            false
...
fn type_vars_from_pattern(params: &[CoreType]) -> Vec<CoreTypeVar> {
    let mut vars = Vec::new();
    collect_type_vars_from_pattern(params, &mut vars);
    vars
}

fn collect_type_vars_from_pattern(params: &[CoreType], vars: &mut Vec<CoreTypeVar>) {
    for param in params {
        match param {
            CoreType::TypeVar(var)
                if var.name != "_" && !vars.iter().any(|existing| existing.name == var.name) =>
            {
                vars.push(var.clone());
            }
            CoreType::Struct { params, .. } | CoreType::Tuple(params) | CoreType::Union(params) => {
                collect_type_vars_from_pattern(params, vars)
            }
            CoreType::Vararg(inner) | CoreType::TypeOf(inner) => {
                collect_type_vars_from_pattern(std::slice::from_ref(inner.as_ref()), vars)
            }
            CoreType::VarargLen { element, len } => {
                collect_type_vars_from_pattern(std::slice::from_ref(element.as_ref()), vars);
                collect_type_vars_from_pattern(std::slice::from_ref(len.as_ref()), vars);
            }
            CoreType::NamedTuple(fields) => {
                for (_, field_ty) in fields {
                    collect_type_vars_from_pattern(std::slice::from_ref(field_ty), vars);
                }
            }
            CoreType::UnionAll { var, body } => {
                ...
                collect_type_vars_from_pattern(std::slice::from_ref(body.as_ref()), vars);
            }
            _ => {}
        }
    }
...
fn core_is_subtype_of_parametric(
    ...
    // an exact `tp.name == sn` lookup; the bridge images unbound names as
    // `Named` until the method type-parameter scope is applied).
...
/// Port of `array_elem_matches_parametric`: array element slots are invariant
/// unless the pattern explicitly uses a method `where` variable or an
/// anonymous upper-bound shorthand.
fn array_elem_matches_parametric(
    ...
    // Array element slots are invariant: a `Vector{Any}` pattern does NOT accept
    // a `Vector{String}` actual (Julia arrays are invariant in their element type).
    // The erased-element workaround (Issue #8848) has been removed; Base methods
    // that previously relied on Vector{Any} as a broad receiver have been updated
    // to use AbstractVector instead.
    if let CoreType::TypeVar(var) = pattern_elem {
        if exact_type_var_for_pattern(pattern_elem, type_vars).is_none() {
            if let Some(upper) = var.upper_bound.as_deref() {
                return bound_unknown_to_from_name(upper)
                    || core_is_subtype_of_parametric(actual_elem, upper, type_vars);
            }
        }
    }
    if !core_type_mentions_type_vars(pattern_elem, type_vars) {
        return actual_elem == pattern_elem;
    }
    actual_elem == pattern_elem
        || extract_type_bindings(actual_elem, pattern_elem, type_vars).is_some()
}

fn core_type_mentions_type_vars(ty: &CoreType, type_vars: &[CoreTypeVar]) -> bool {
    match ty {
        CoreType::TypeVar(var) => {
            type_vars.iter().any(|declared| declared.name == var.name)
                || var
                    .lower_bound
                    .as_deref()
                    .is_some_and(|bound| core_type_mentions_type_vars(bound, type_vars))
                || var
                    .upper_bound
                    .as_deref()
                    .is_some_and(|bound| core_type_mentions_type_vars(bound, type_vars))
        }
        CoreType::Named(name) => type_vars.iter().any(|declared| declared.name == *name),
        CoreType::AbstractUser { parent, .. } => parent
            .as_deref()
            .is_some_and(|parent| core_type_mentions_type_vars(parent, type_vars)),
        CoreType::Struct { params, .. } | CoreType::Tuple(params) | CoreType::Union(params) => {
            params
                .iter()
                .any(|param| core_type_mentions_type_vars(param, type_vars))
        }
        CoreType::Vararg(inner) | CoreType::TypeOf(inner) => {
            core_type_mentions_type_vars(inner, type_vars)
        }
        CoreType::VarargLen { element, len } => {
            core_type_mentions_type_vars(element, type_vars)
                || core_type_mentions_type_vars(len, type_vars)
        }
        CoreType::NamedTuple(fields) => fields
            .iter()
            .any(|(_, field_ty)| core_type_mentions_type_vars(field_ty, type_vars)),
        CoreType::UnionAll { var, body } => {
            type_vars.iter().any(|declared| declared.name == var.name)
                || var
                    .lower_bound
                ...
                || var
                    ...
                    .as_deref()
                    .is_some_and(|bound| core_type_mentions_type_vars(bound, type_vars))
                || core_type_mentions_type_vars(body, type_vars)
        }
        _ => false,
    }
}
...
fn core_parametric_slot_matches(
    ...
    if exact_type_var_for_pattern(pattern, type_vars).is_some() {
        return true;
    }
    if let CoreType::TypeVar(var) = pattern {
        if exact_type_var_for_pattern(pattern, type_vars).is_none() {
            if let Some(upper) = var.upper_bound.as_deref() {
                return bound_unknown_to_from_name(upper)
                    || core_is_subtype_of_parametric(actual, upper, type_vars);
            }
        }
    }
    if !core_type_mentions_type_vars(pattern, type_vars) {
        return false;
    }
    extract_type_bindings(actual, pattern, type_vars).is_some()
}
...
fn extract_type_bindings(
    ...
    pattern: &CoreType,
    type_vars: &[CoreTypeVar],
) -> Option<LexicalTypeBindings> {
    let mut bindings = LexicalTypeBindings::new();

    // A partially applied type object is a trailing UnionAll. Match its body
    // against a shorter prefix pattern, but never let a method `where`
    // parameter bind to one of the still-existential trailing variables. This
    // is the CoreType counterpart of JuliaType's guarded partial-UnionAll
    // extraction and is required by typed-dispatch call sites such as
    // `similar(::Type{Array{T}}, ...)` (Issue #11232).
    if !matches!(pattern, CoreType::UnionAll { .. }) {
        let mut body = actual;
        let mut stripped_vars = Vec::new();
        while let CoreType::UnionAll { var, body: inner } = body {
            stripped_vars.push(var.clone());
            body = inner.as_ref();
        }
        if !stripped_vars.is_empty() {
            if let Some(extracted) = extract_type_bindings(body, pattern, type_vars) {
                let consumes_stripped_binder = extracted
                    .values()
                    .any(|bound| core_type_mentions_type_vars(bound, &stripped_vars));
                if !consumes_stripped_binder {
    ...
    {
        if !nominal_family_names_compatible(actual_name, pattern_name)
        ...
        for (actual_param, pattern_param) in actual_params.iter().zip(pattern_params.iter()) {
            if let Some(declared) = exact_type_var_for_pattern(pattern_param, type_vars) {
                ...
                merge_single_binding(&mut bindings, &declared.name, actual_param.clone())?;
            } else if core_type_mentions_type_vars(pattern_param, type_vars) {
                // Nested parametric slot (e.g. `Wrap{T}` inside `Box{Wrap{T}}`):
                // extract bindings recursively so `T` is visible in the method
                // body (Issue #8853). `core_parametric_slot_matches` only returns
                // a bool and discards the extracted bindings, so we call
                // `extract_type_bindings` directly and propagate them here.
                // Merge with conflict detection (Issue #11231) rather than a
                // raw `extend`.
                let nested = extract_type_bindings(actual_param, pattern_param, type_vars)?;
                merge_tuple_bindings(&mut bindings, nested)?;
    ...
    // A `Named` leaf in `where` position.
...
/// Merge a single `where`-parameter binding into the accumulator, rejecting a
/// second, conflicting binding for the same parameter (Issue #11231): every
/// occurrence of one method `where` parameter must unify with the same type,
/// e.g. `Pair{T,T}` must not match `Pair{Int,String}`.
fn merge_single_binding(
    acc: &mut LexicalTypeBindings,
    name: &str,
    bound_ty: CoreType,
) -> Option<()> {
    match acc.entry(name.to_string()) {
        std::collections::hash_map::Entry::Occupied(existing) => {
            (existing.get() == &bound_ty).then_some(())
        }
...
fn merge_tuple_bindings(
    acc: &mut LexicalTypeBindings,
    extracted: LexicalTypeBindings,
) -> Option<()> {
    for (name, bound_ty) in extracted {
        merge_single_binding(acc, &name, bound_ty)?;
    }
    Some(())
}
...
fn bind_or_check_type_var(
    ...
    if let Some(upper) = upper {
        if !engine_is_subtype_with_optional_hierarchy(arg_ty, upper, hierarchy) {
            return false;
        }
    }
    if let Some(lower) = lower {
        if !engine_is_subtype_with_optional_hierarchy(lower, arg_ty, hierarchy) {
            return false;
        }
    }
...
fn type_object_inner_family(ty: &CoreType) -> Option<TypeObjectInnerFamily> {
    match ty {
        ...
        // A partial application keeps its nominal family in the UnionAll body;
        // the binder only describes unapplied trailing parameters.
        CoreType::UnionAll { body, .. } => type_object_inner_family(body),
...
mod tests {
    ...
    #[test]
    fn core_match_type_object_with_trailing_unionall_extracts_prefix_issue_11232() {
        let trailing_subarray = CoreTypeVar::unscoped("S").with_rigid_identity(112_320);
        let partial_subarray = CoreType::UnionAll {
            var: trailing_subarray.clone(),
            body: Box::new(CoreType::Struct {
                name: "SubArray".to_string(),
                params: vec![
                    CoreType::from(&JuliaType::Int64),
                    CoreType::TypeVar(trailing_subarray),
                ],
            }),
        };
        let trailing_rank = CoreTypeVar::unscoped("N").with_rigid_identity(112_321);
        let actual = CoreType::TypeOf(Box::new(CoreType::UnionAll {
            var: trailing_rank.clone(),
            body: Box::new(CoreType::Struct {
                name: "Array".to_string(),
                params: vec![partial_subarray, CoreType::TypeVar(trailing_rank)],
            }),
        }));
        let pattern = CoreType::TypeOf(Box::new(CoreType::Struct {
            name: "Array".to_string(),
            params: vec![CoreType::TypeVar(CoreTypeVar::unscoped("T"))],
        }));
        let type_vars = vec![CoreTypeVar::unscoped("T")];

        assert_eq!(
            core_signature_match_with_bindings(&[pattern], &[actual], &type_vars),
            Some(1)
        );
    }
    ...
    #[test]
    fn core_match_nested_type_object_enforces_parametric_lower_bound_issue_11233() {
        let pattern = cores(&[JuliaType::TypeOf(Box::new(JuliaType::from_name_or_struct(
            "SubArray{T}",
        )))]);
        let type_vars = vars(&[TypeParam::with_lower_bound(
            "T".to_string(),
            "LowerBoundMWE{Int64}".to_string(),
        )]);
        let matches = |inner: &str| {
            core_signature_match_with_bindings(
                &pattern,
                &cores(&[JuliaType::TypeOf(Box::new(JuliaType::from_name_or_struct(
                    inner,
                )))]),
                &type_vars,
            )
        ...
        assert!(!matches("SubArray{Int8}"));
        assert!(matches("SubArray{LowerBoundMWE{Int64}}"));
    }

    #[test]
    fn core_match_nested_type_object_lower_bound_uses_user_hierarchy_issue_11233() {
        let mut hierarchy = StructHierarchy::new();
        hierarchy.insert("LBAbs", Some("Any".to_string()), Vec::new());
        hierarchy.insert("LBLeaf", Some("LBAbs".to_string()), Vec::new());
        hierarchy.insert("Unrelated", Some("Any".to_string()), Vec::new());
        hierarchy.insert("W", Some("Any".to_string()), vec!["T".to_string()]);

        let pattern = cores(&[JuliaType::TypeOf(Box::new(JuliaType::from_name_or_struct(
            "W{T}",
        )))]);
        let matches = |inner: &str, type_vars: &[CoreTypeVar]| {
            let actual = cores(&[JuliaType::TypeOf(Box::new(JuliaType::from_name_or_struct(
                inner,
            )))]);
            core_signature_match_with_bindings_with_hierarchy(
                &pattern, &actual, type_vars, &hierarchy,
            )
            .is_some()
        };

        let lower_vars = vars(&[TypeParam::with_lower_bound(
            "T".to_string(),
            "LBLeaf".to_string(),
        )]);
        assert!(!matches("W{Int8}", &lower_vars));
        assert!(matches("W{LBLeaf}", &lower_vars));
        assert!(matches("W{LBAbs}", &lower_vars));

        let upper_vars = vars(&[TypeParam::with_upper_bound(
            "T".to_string(),
            "LBAbs".to_string(),
        )]);
        assert!(matches("W{LBLeaf}", &upper_vars));
        assert!(matches("W{LBAbs}", &upper_vars));
        assert!(!matches("W{Unrelated}", &upper_vars));
    }
    ...
    fn core_match_ntuple_param_matches_homogeneous_tuple_issue_6495() {
        ...
        let bounded_params = cores(&[JuliaType::Struct("NTuple{N, <:Number}".to_string())]);
        assert_eq!(
            core_signature_match_with_bindings(
                &bounded_params,
                &cores(&[JuliaType::TupleOf(vec![
