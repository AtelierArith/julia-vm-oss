...
//! (`compile::lattice`, `promotion`, `compile::tfuncs`) and the
...
// `selection` and `specificity` are part of the public API of this crate so
// the main `subset_julia_vm` crate can access them through the `inference_core`
// re-export. Changing from `pub(crate)` to `pub` (Issue #8655).
pub mod selection;
pub mod specificity;
pub mod subtype;
pub mod type_core;
...
// These type_core helpers are accessed by the main crate via `crate::inference_core::`.
pub use type_core::parse_parametric_type_name;
pub use type_core::{
    core_type_to_julia_type, core_type_var_to_type_param, instantiate_unionall_typevars,
    CoreTypeSubstitution,
};
pub use type_core::{
    ...
    CoreAbstract, CorePrimitive, CoreType, CoreTypeVar, CoreTypeVarId, CoreValueParam,
};
