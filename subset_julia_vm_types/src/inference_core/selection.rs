...
pub fn unique_dominant_index(
    candidate_count: usize,
    mut eligible: impl FnMut(usize) -> bool,
    mut dominates: impl FnMut(usize, usize) -> bool,
) -> Option<usize> {
    let mut contender: Option<usize> = None;
    for i in 0..candidate_count {
        if !eligible(i) {
            continue;
        }
        if contender.is_none_or(|current| dominates(i, current)) {
            contender = Some(i);
        }
    }
    let contender = contender?;
    let dominates_all =
        (0..candidate_count).all(|other| contender == other || dominates(contender, other));
    if !dominates_all {
        return None;
    }

    // Strict dominance is asymmetric in production. Keep the helper
    // conservative for inconsistent injected relations as well: no second
    // eligible candidate may dominate the selected contender.
    let uniquely_dominant = (0..candidate_count)
        .filter(|other| *other != contender && eligible(*other))
        .all(|other| !dominates(other, contender));
    uniquely_dominant.then_some(contender)
}
...
pub fn pick_max_score<T, S: PartialOrd + Copy>(
...
/// gates.
pub fn signature_is_broad_wrapper_fence(slots: &[String]) -> bool {
...
pub fn wrapper_fence_name_channel_repair<T>(
...
/// function name index.
pub fn select_typed_dispatch_candidate(
