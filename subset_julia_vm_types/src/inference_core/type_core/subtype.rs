use super::{
    ...
    resolve_named_word_alias, struct_is_subtype_of_abstract, struct_is_subtype_of_abstract_in,
    struct_params_are_subtype, struct_params_are_subtype_in, substitute_typevar_bound,
    tuple_elements_match, tuple_elements_match_in, tuple_params_match, tuple_params_match_in,
    CoreAbstract, CoreType,
};
use crate::types::{
    nominal_family_names_compatible, nominal_type_names_compatible, StructHierarchy,
};
...
impl CoreType {
    ...
    fn is_subtype_of_with_lookup(&self, other: &Self, hierarchy: Option<&StructHierarchy>) -> bool {
        ...
        match (self, other) {
            ...
            (
                ...
                nominal_type_names_compatible(name, other_name)
                    ...
                    || (nominal_family_names_compatible(name, other_name)
            ...
            (Self::AbstractUser { name, parent }, Self::Abstract(other_abstract)) => {
                let canonical_name_matches = match Self::from_julia_name(name) {
                    Self::Abstract(abstract_type) => {
                        abstract_is_subtype_of(&abstract_type, other_abstract)
                    }
                    _ => false,
                };
                canonical_name_matches
                    || parent
                        .as_deref()
                        .is_some_and(|parent| parent.is_subtype_of_with_lookup(other, hierarchy))
                    || hierarchy.is_some_and(|hierarchy| {
                        struct_is_subtype_of_abstract_in(hierarchy, name, &[], other_abstract)
                    })
            }
            (
            ...
            (
                ...
                if nominal_family_names_compatible(child, parent)
                ...
                match hierarchy {
                    Some(hierarchy) => {
                        registered_instantiated_struct_parent_in(hierarchy, child, &[]).is_some_and(
                            |mapped| mapped.is_subtype_of_with_lookup(other, Some(hierarchy)),
                        ) || (params.is_empty()
            ...
            (Self::Struct { name, .. }, Self::AbstractUser { name: parent, .. }) => {
                if parent.contains('{') {
                    return self
                        .is_subtype_of_with_lookup(&Self::from_julia_name(parent), hierarchy);
                }
                (nominal_family_names_compatible(name, parent)
            ...
            (Self::Named(child), Self::AbstractUser { name: parent, .. }) => {
                if parent.contains('{') {
                    let pattern = Self::from_julia_name(parent);
                    return match hierarchy {
                        Some(hierarchy) => {
                            registered_instantiated_struct_parent_in(hierarchy, child, &[])
                                .is_some_and(|mapped| {
                                    mapped.is_subtype_of_with_lookup(&pattern, Some(hierarchy))
                                })
                        }
                        None => false,
                    };
                }
                (nominal_family_names_compatible(child, parent)
                    && base_type_name(child) == base_type_name(parent))
                    || match hierarchy {
                        Some(hierarchy) => {
                            registered_struct_is_subtype_of_in(hierarchy, child, parent)
    ...
    /// Strict dominance on method signatures:
    /// `self` is `<:` `other` but `other` is not `<:` `self`, plus the
    /// diagonal tuple fragment where upstream `type_morespecific_` is stricter
    /// than the subtype partial order (Issue #8439).
    ///
    /// This is still only a subset of upstream `type_morespecific_`, but it
    /// includes the canonical case-H tuple/diagonal relation pinned by #8439.
    #[allow(dead_code)]
    pub fn strict_subtype_dominates(&self, other: &Self) -> bool {
        self.strict_subtype_dominates_with_lookup(other, None)
    }
    ...
    pub fn strict_subtype_dominates_with_hierarchy(
        ...
        self.strict_subtype_dominates_with_lookup(other, Some(hierarchy))
    }

    fn strict_subtype_dominates_with_lookup(
        &self,
        other: &Self,
        hierarchy: Option<&StructHierarchy>,
    ) -> bool {
        if self.is_subtype_of_with_lookup(other, hierarchy)
            && !other.is_subtype_of_with_lookup(self, hierarchy)
        {
            return true;
        }
        tuple_dominates_diagonal_unionall(self, other, hierarchy)
    }
}

fn tuple_dominates_diagonal_unionall(
    candidate: &CoreType,
    other: &CoreType,
    hierarchy: Option<&StructHierarchy>,
) -> bool {
    let CoreType::Tuple(candidate_elems) = candidate else {
        return false;
    };
    let Some((var, diagonal_elems)) = diagonal_unionall_tuple(other) else {
        return false;
    };
    if candidate_elems.len() != diagonal_elems.len() {
        return false;
    }

    let upper_bound = var.upper_bound.as_deref().unwrap_or(&CoreType::Any);
    let mut strict = false;
    for elem in candidate_elems {
        if !elem.is_subtype_of_with_lookup(upper_bound, hierarchy) {
            return false;
        }
        if !upper_bound.is_subtype_of_with_lookup(elem, hierarchy) {
            strict = true;
        }
    }
    strict
...
fn diagonal_unionall_tuple(pattern: &CoreType) -> Option<(&super::CoreTypeVar, &[CoreType])> {
    let CoreType::UnionAll { var, body } = pattern else {
        return None;
    };
    let CoreType::Tuple(elems) = body.as_ref() else {
        return None;
    };
    if elems.len() < 2 {
        return None;
    ...
    elems
        .iter()
        .all(|elem| matches!(elem, CoreType::TypeVar(elem_var) if elem_var.name == var.name))
        .then_some((var, elems.as_slice()))
}
