use super::{
    ...
    normalize_union, CoreType, CoreTypeVar, CoreTypeVarId,
};
...
impl CoreType {
    ...
    pub fn type_intersect(&self, other: &Self) -> Self {
        ...
        if let Some(expanded) = builtin_unionall_alias_pattern(self) {
            return expanded.type_intersect(other);
        }
        if let Some(expanded) = builtin_unionall_alias_pattern(other) {
            return self.type_intersect(&expanded);
        }
...
fn diagonal_unionall_tuple_intersect(pattern: &CoreType, actual: &CoreType) -> Option<CoreType> {
    ...
    if !pattern_elements
        .iter()
        .all(|element| tuple_element_is_bound_var(element, var))
    {
        return None;
    }
    ...
    let mut narrowed_reference = narrowed_var.clone();
    narrowed_reference.lower_bound = None;
    narrowed_reference.upper_bound = None;
    let narrowed_body = CoreType::Tuple(vec![
        CoreType::TypeVar(narrowed_reference);
        pattern_elements.len()
    ]);
    Some(CoreType::UnionAll {
        var: narrowed_var,
        body: Box::new(narrowed_body),
    })
}

fn tuple_element_is_bound_var(element: &CoreType, var: &CoreTypeVar) -> bool {
    match element {
        CoreType::TypeVar(element_var) => same_bound_typevar_identity(element_var, var),
        CoreType::Named(name) => {
            matches!(var.typevar_id(), CoreTypeVarId::Unresolved) && name == &var.name
        }
        _ => false,
    }
}

fn bound_var_for_param<'a>(param: &CoreType, vars: &[&'a CoreTypeVar]) -> Option<&'a CoreTypeVar> {
    vars.iter().copied().find(|candidate| match param {
        CoreType::TypeVar(var) => same_bound_typevar_identity(var, candidate),
        CoreType::Named(name) => {
            matches!(candidate.typevar_id(), CoreTypeVarId::Unresolved) && name == &candidate.name
        }
        _ => false,
    })
}

fn same_bound_typevar_identity(left: &CoreTypeVar, right: &CoreTypeVar) -> bool {
    match (left.typevar_id(), right.typevar_id()) {
        (CoreTypeVarId::Rigid(left_id), CoreTypeVarId::Rigid(right_id)) => left_id == right_id,
        (CoreTypeVarId::Rigid(_), _) | (_, CoreTypeVarId::Rigid(_)) => false,
...
fn mixed_diagonal_unionall_tuple_intersect(
    ...
    for (idx, (pattern_element, actual_element)) in pattern_elements
        ...
        .enumerate()
    {
        if tuple_element_is_bound_var(pattern_element, var) {
            direct_occurrences.push((idx, actual_element.clone()));
            continue;
        }

        let invariant_candidate =
            struct_typevar_invariant_candidate(pattern_element, actual_element, var)?;
...
fn struct_typevar_invariant_candidate(
    ...
    for (pattern_param, actual_param) in params.iter().zip(actual_params.iter()) {
        if tuple_element_is_bound_var(pattern_param, var) {
            saw_var = true;
            match &candidate {
                Some(existing)
...
fn unionall_parametric_container_intersect(
    ...
    for (idx, pparam) in pparams.iter().enumerate() {
        ...
        if let Some(bound_var) = bound_var_for_param(pparam, &vars) {
            ...
            if let Some((_, prior)) = assigned
                .iter()
                .find(|(name, _)| *name == bound_var.name.as_str())
            {
                if !(prior.is_subtype_of(candidate) && candidate.is_subtype_of(prior)) {
                    return Some(CoreType::Bottom);
                }
            } else {
                assigned.push((bound_var.name.as_str(), candidate));
            }
            substituted.push(candidate.clone());
        } else if !core_type_contains_typevar(pparam) {
            substituted.push(pparam.clone());
        } else {
            return None;
        }
    }
...
fn builtin_unionall_alias_pattern(ty: &CoreType) -> Option<CoreType> {
    let name = match ty {
        CoreType::Named(name) => name.as_str(),
        CoreType::Struct { name, params } if params.is_empty() => name.as_str(),
        _ => return None,
    };

    match name {
        "Vector" => Some(unary_container_unionall("T", "Vector")),
        "Matrix" => Some(unary_container_unionall("T", "Matrix")),
        "Set" => Some(unary_container_unionall("T", "Set")),
        "Dict" => Some(binary_container_unionall("K", "V", "Dict")),
        "Array" => Some(binary_container_unionall("T", "N", "Array")),
        "DenseArray" => Some(binary_container_unionall("T", "N", "DenseArray")),
        _ => None,
    }
}

fn unary_container_unionall(var: &str, name: &str) -> CoreType {
    let typevar = unbounded_typevar(var);
    CoreType::UnionAll {
        var: typevar.clone(),
        body: Box::new(CoreType::Struct {
            name: name.to_string(),
            params: vec![CoreType::TypeVar(typevar)],
        }),
    }
}

fn binary_container_unionall(left_var: &str, right_var: &str, name: &str) -> CoreType {
    let left = unbounded_typevar(left_var);
    let right = unbounded_typevar(right_var);
    CoreType::UnionAll {
        var: left.clone(),
        body: Box::new(CoreType::UnionAll {
            ...
            body: Box::new(CoreType::Struct {
                name: name.to_string(),
                params: vec![CoreType::TypeVar(left), CoreType::TypeVar(right)],
            }),
        }),
    }
}

fn unbounded_typevar(name: &str) -> CoreTypeVar {
