...
use self::r#match::{
    core_type_matches_pattern, core_type_matches_pattern_in, TypeVarBindingState, TypeVarScope,
    TypeVarVariance,
};
pub use convert::{core_type_to_julia_type, core_type_var_to_type_param};
use registry::{
...
pub use repr::{
    is_core_builtin_function_name, is_core_builtin_singleton_type_name,
    is_core_function_binding_name, CoreAbstract, CorePrimitive, CoreType, CoreTypeVar,
    CoreTypeVarId, CoreValueParam, CORE_BUILTIN_FUNCTION_NAMES,
};
...
/// Memo for the pure rendered-name-to-[`CoreType`] parser.
///
/// The complete rendered input is a lexical parse key, not semantic identity.
...
thread_local! {
    ...
    static FROM_JULIA_NAME_CACHE: RenderedTypeParseCache =
        std::cell::RefCell::new(HashMap::new());

    /// Dispatch signatures need the same structured parse while preserving
    /// explicit user/package owners on nominal families (Issue #11076).
    /// Keep a distinct cache because the ordinary semantic parser deliberately
    /// canonicalizes those families to their bare names.
    static FROM_JULIA_DISPATCH_NAME_CACHE: RenderedTypeParseCache =
        std::cell::RefCell::new(HashMap::new());
}
...
impl CoreType {
    ...
    /// e.g. `f(x::T, y::T)`) or nested inside an invariant parameter are
    /// preserved untouched, so genuinely distinct methods are never merged. The
    /// exception is a whole parametric family schema whose every parameter is an
    /// unbounded `where` variable: upstream treats `Vector{T} where T` /
    /// `Pair{K,V} where {K,V}` as the bare family, so those collapse for
    /// last-definition-wins dedup.
    pub fn canonicalize_signature_for_dedup(&self) -> Self {
        ...
        let peeled_bounds: HashMap<&str, Self> = peeled_vars
            ...
            .collect();
        let erasable_schema_vars: HashMap<&str, bool> = peeled_vars
            .iter()
            .map(|v| (v.name.as_str(), core_typevar_is_unbounded_for_schema(v)))
            .collect();
        ...
        let collapsed = Self::Tuple(
            elements
                .iter()
                .map(|elem| {
                    canonicalize_signature_element_for_dedup(
                        elem,
                        &counts,
                        &peeled_bounds,
                        &erasable_schema_vars,
                    )
                })
                .collect(),
        );
    ...
    pub fn typejoin(&self, other: &Self) -> Self {
        ...
        // Julia's `typejoin` returns the closest nominal common supertype, not
        // the set-theoretic `Union` used by inference merges. Walk the canonical
        // built-in parent relation that also backs subtype/reflection, so this
        // operation cannot drift into a second numeric/container hierarchy
        // (Issue #10049). User-defined parents require a `StructHierarchy`; when
        // this context-free API cannot recover one, Julia's nominal `typejoin`
        // falls back to `Any`, not the more precise set-theoretic `Union` used by
        // inference value merges (Issue #10460).
        let mut left_chain = vec![self.clone()];
        let mut current = self.clone();
        for _ in 0..32 {
            let Some(parent_name) = current.direct_builtin_supertype_name() else {
                break;
            };
            let parent = Self::from_julia_name(parent_name);
            if parent == current {
                break;
            }
            left_chain.push(parent.clone());
            current = parent;
        }

        let mut right_chain = vec![other.clone()];
        ...
        for _ in 0..32 {
            ...
            right_chain.push(parent.clone());
            current = parent;
        }
        if let Some(common) = left_chain
            .into_iter()
    ...
    /// Owner-qualified nominal base stored by the structural representation.
    ///
    /// Unlike [`Self::nominal_family_name`], this preserves the module owner and
    /// does not inspect a rendered Julia spelling. `CoreType::Struct` stores its
    /// parameters separately, so callers that need registry identity can use
    /// this together with [`Self::nominal_type_parameters`] without a
    /// `to_julia_name()` / suffix-parsing round trip (Issue #10460).
    pub fn nominal_base_name(&self) -> Option<&str> {
        match self.unwrap_unionall_ref() {
            Self::Struct { name, .. }
            | Self::AbstractUser { name, .. }
            | Self::Named(name)
    ...
    /// Parameters of the unwrapped nominal type, when the core node carries
    /// them structurally.
    pub fn nominal_type_parameters(&self) -> Option<&[Self]> {
        match self.unwrap_unionall_ref() {
            Self::Struct { params, .. } => Some(params),
            _ => None,
        }
    }

    /// Borrow the body below every leading `UnionAll` wrapper.
    pub fn unwrap_unionall_ref(&self) -> &Self {
        let mut current = self;
        while let Self::UnionAll { body, .. } = current {
            current = body;
        }
        current
    }
    ...
    pub fn builtin_field_metadata(&self) -> Option<Vec<(&'static str, Self)>> {
        ...
        // `Base.Generator{I,F}` (Issue #11382): `f::F` then `iter::I`, matching
        // `julia/base/generator.jl`'s `struct Generator{I,F}`. Matched on the
        // module/type-param-stripped bare name so both the bare `Generator`
        // spelling and a fully parametric `Base.Generator{UnitRange{Int64},
        // typeof(f)}` (what sjulia's `typeof` reports for a live generator)
        // resolve the same fieldnames/fieldcount.
        if bare == "Generator" {
            return Some(vec![("f", Self::Any), ("iter", Self::Any)]);
        }
        // `RegexMatch{S}` (Issue #11382): `match::SubString{S}`,
        // `captures::Vector{Union{Nothing,SubString{S}}}`, `offset::Int`,
        // `offsets::Vector{Int}`, `regex::Regex`, matching
        // `julia/base/regex.jl`'s
        // `struct RegexMatch{S<:AbstractString} <: AbstractMatch`. sjulia only
        // ever produces `String`-backed matches, so `S` is reported as
        // `String` rather than the unmodeled type parameter.
        if bare == "RegexMatch" {
            return Some(vec![
                (
                    "match",
                    Self::Struct {
                        ...
                        params: vec![Self::Primitive(CorePrimitive::String)],
                    },
                ),
                (
                    "captures",
                ...
                ("offset", Self::Primitive(CorePrimitive::Int64)),
                (
                    "offsets",
                    Self::Struct {
                        name: "Vector".to_string(),
                        params: vec![Self::Primitive(CorePrimitive::Int64)],
                    },
                ),
                (
                    "regex",
        ...
        Some(match name.as_str() {
            ...
            "Core.Binding" | "Binding" => vec![
                (
                    "globalref",
                    Self::Struct {
                        name: "GlobalRef".to_string(),
                        params: vec![],
                    },
                ),
                ("value", Self::Any),
                (
                    "partitions",
                    Self::Struct {
                        name: "Core.BindingPartition".to_string(),
                        params: vec![],
                    },
                ),
                (
                ...
                ("flags", Self::Primitive(CorePrimitive::UInt8)),
            ],
            _ => return None,
        })
    }
    ...
    pub fn from_julia_name(name: &str) -> Self {
        Self::from_julia_name_cached(name, false)
    }

    /// Parse a rendered Julia type for use as a method-dispatch identity.
    ///
    /// Ordinary type operators historically canonicalize user-defined nominal
    /// families to their bare name. Dispatch must retain an explicitly
    /// qualified owner so unrelated sibling definitions such as `A.Box` and
    /// `B.Box` do not collapse into one signature. Bare spellings remain bare
    /// aliases, and Base/Core carriers keep their historical canonical names.
    pub fn from_julia_name_for_dispatch(name: &str) -> Self {
        // Preserve the legacy cache-observation point as well as its parsed
        // image. Existing type-operator paths historically shared this call
        // with dispatch, and declaration-aware name classification can depend
        // on when a family is first observed. The dispatch cache below stores
        // a separate owner-preserving result without changing that legacy
        // cache's value (Issue #11076).
        let legacy = Self::from_julia_name(name);
        if !crate::types::has_qualified_nominal_family_collision(name) {
            return legacy;
        }
        Self::from_julia_name_cached(name, true)
    }

    fn from_julia_name_cached(name: &str, preserve_user_owner: bool) -> Self {
        // Qualification becomes identity-bearing only after a sibling family
        // is registered. Do not memoize a pre-collision bare projection and
        // make the result depend on declaration/cache order (#11076).
        if preserve_user_owner && name.contains('.') {
            return Self::from_julia_name_uncached(name, preserve_user_owner);
        }
        let parse = |cache: &RenderedTypeParseCache| {
            if let Some(cached) = cache.borrow().get(name) {
                return cached.clone();
            }
            let parsed = Self::from_julia_name_uncached(name, preserve_user_owner);
        ...
        if preserve_user_owner {
            FROM_JULIA_DISPATCH_NAME_CACHE.with(parse)
        } else {
            FROM_JULIA_NAME_CACHE.with(parse)
        }
    }

    /// Rewrite every `Named`/empty-`Struct`/`AbstractUser`/`Primitive`/`Abstract`/`Any`/
    /// `Module` leaf whose name is a `where` binder into a `TypeVar`, so a binder
    /// shadows a registered global type of the same name (Issue #9464,
    /// extended to builtin primitive/abstract names by Issue #10100 / epic
    /// #10049: a binder can shadow `Int64`, `Real`, etc. just as it can
    /// shadow a user struct or an unresolved name — `JuliaType::from_name`
    /// resolves those to a dedicated `CoreType::Primitive`/`CoreType::Abstract`/
    /// `CoreType::Any` variant rather than `Named`/`Struct`, so without a
    /// matching arm the shadowed occurrence stayed structurally
    /// indistinguishable from an unrelated concrete use of the same name in
    /// `<:`/`==`/`isa`). Recurses through compound types; a nested `UnionAll`
    /// that rebinds the same name shadows it back out.
    ///
    /// Every leaf variant recognized by the JuliaType-level keep-check
    /// (`builtins_types.rs::julia_type_references_typevar`'s generic
    /// `other.name() == var_name` arm) needs a matching arm HERE, or the kept
    /// `UnionAll`'s body retains the raw unrebound leaf: `<:` may still pass
    /// via the mutual-subtype fallback, but structural `==` against the
    /// canonical `Vector{T} where T<:Real` shape fails. `CoreType::Module` was
    /// the second concrete instance of this gap after `Any` (Issue #10299:
    /// `Vector{Module} where Module<:Real == Vector{T} where T<:Real` returned
    /// false). The remaining leaves are unreachable from a binder-name
    /// collision: `Bottom` renders as `Union{}` and `Value` holds literals —
    /// neither is a legal identifier a `where` binder could be spelled as.
    fn rebind_where_binders(&mut self, binder_names: &[String]) {
        let is_binder = |n: &str| binder_names.iter().any(|b| b == n);
        match self {
            CoreType::Any if is_binder("Any") => {
                *self = CoreType::TypeVar(CoreTypeVar::unscoped("Any"));
            }
            CoreType::Module(n) if is_binder(n) => {
                *self = CoreType::TypeVar(CoreTypeVar::unscoped(n.clone()));
            }
            CoreType::Named(n) if is_binder(n) => {
                *self = CoreType::TypeVar(CoreTypeVar::unscoped(n.clone()));
            }
            CoreType::Primitive(p) if is_binder(p.julia_name()) => {
                *self = CoreType::TypeVar(CoreTypeVar::unscoped(p.julia_name()));
            }
            CoreType::Abstract(a) if is_binder(a.julia_name()) => {
                *self = CoreType::TypeVar(CoreTypeVar::unscoped(a.julia_name()));
            }
            CoreType::Struct { name, params } if params.is_empty() && is_binder(name) => {
                *self = CoreType::TypeVar(CoreTypeVar::unscoped(name.clone()));
            }
            CoreType::AbstractUser { name, parent } if is_binder(name) => {
                let _ = parent;
                *self = CoreType::TypeVar(CoreTypeVar::unscoped(name.clone()));
            }
            CoreType::Struct { params, .. } => {
                for p in params {
                    p.rebind_where_binders(binder_names);
                }
            }
            CoreType::AbstractUser {
                ...
                parent.rebind_where_binders(binder_names);
            }
            CoreType::AbstractUser { parent: None, .. } => {}
            CoreType::Tuple(elems) | CoreType::Union(elems) => {
                for e in elems {
                    e.rebind_where_binders(binder_names);
                }
            }
            CoreType::Vararg(inner) | CoreType::TypeOf(inner) => {
                inner.rebind_where_binders(binder_names);
            }
            CoreType::VarargLen { element, len } => {
                element.rebind_where_binders(binder_names);
                len.rebind_where_binders(binder_names);
            }
            CoreType::NamedTuple(fields) => {
                for (_, ty) in fields {
                    ty.rebind_where_binders(binder_names);
                }
            }
            CoreType::UnionAll { var, body } => {
                // The inner binder re-shadows: drop any outer binder it masks.
                if binder_names.iter().any(|b| b == &var.name) {
                    let filtered: Vec<String> = binder_names
                        .iter()
                        .filter(|b| **b != var.name)
                        .cloned()
                        .collect();
                    body.rebind_where_binders(&filtered);
                } else {
                    body.rebind_where_binders(binder_names);
                }
            }
            _ => {}
        }
    ...
    /// Rebind one explicitly declared source `where` binder throughout this
    /// structural body before it is projected back to a runtime type object.
    ///
    /// Parsing happens before a value-position `UnionAll` constructor receives
    /// its binder, so a source binder that shadows a builtin name can initially
    /// appear as a nominal leaf (`Int64`, `Module`, ...). The constructor knows
    /// the binder declaration and can use this structural operation to recover
    /// the lexical TypeVar without comparing display strings (Issue #10460).
    pub fn rebind_source_where_binder(&mut self, binder_name: &str) {
        self.rebind_where_binders(&[binder_name.to_string()]);
    }

    /// Whether this graph contains a nominal leaf with `binder_name` that is
    /// already a builtin or registered user type. Such a leaf is distinct from
    /// an unresolved lexical `Struct("T")`: rebinding it is source shadowing
    /// and must preserve the fresh UnionAll object's identity (Issue #10460).
    pub fn contains_registered_nominal_leaf_named(
        &self,
        binder_name: &str,
        hierarchy: &StructHierarchy,
    ) -> bool {
        match self {
            CoreType::Any => binder_name == "Any",
            CoreType::Module(name) => name == binder_name,
            CoreType::Primitive(primitive) => primitive.julia_name() == binder_name,
            CoreType::Abstract(abstract_ty) => abstract_ty.julia_name() == binder_name,
            CoreType::Named(name) => name == binder_name && hierarchy.contains_name(binder_name),
            CoreType::Struct { name, params } => {
                (params.is_empty() && name == binder_name && hierarchy.contains_name(binder_name))
                    || params.iter().any(|param| {
                        param.contains_registered_nominal_leaf_named(binder_name, hierarchy)
                    })
            }
            CoreType::AbstractUser { name, parent } => {
                (name == binder_name && hierarchy.contains_name(binder_name))
                    || parent.as_ref().is_some_and(|parent| {
                        parent.contains_registered_nominal_leaf_named(binder_name, hierarchy)
                    })
            }
            CoreType::Tuple(elements) | CoreType::Union(elements) => elements
                .iter()
                .any(|ty| ty.contains_registered_nominal_leaf_named(binder_name, hierarchy)),
            CoreType::Vararg(inner) | CoreType::TypeOf(inner) => {
                inner.contains_registered_nominal_leaf_named(binder_name, hierarchy)
            }
            CoreType::VarargLen { element, len } => {
                element.contains_registered_nominal_leaf_named(binder_name, hierarchy)
                    || len.contains_registered_nominal_leaf_named(binder_name, hierarchy)
            }
            CoreType::NamedTuple(fields) => fields
                .iter()
                .any(|(_, ty)| ty.contains_registered_nominal_leaf_named(binder_name, hierarchy)),
            CoreType::UnionAll { var, body } => {
                var.name != binder_name
                    && body.contains_registered_nominal_leaf_named(binder_name, hierarchy)
            }
            CoreType::Bottom | CoreType::TypeVar(_) | CoreType::Value(_) => false,
        }
    }

    fn from_julia_name_uncached(name: &str, preserve_user_owner: bool) -> Self {
        let parse = |name: &str| Self::from_julia_name_cached(name, preserve_user_owner);
        if let Some(named_tuple) = parse_named_tuple_type_name(name, preserve_user_owner) {
            return named_tuple;
        }
        ...
        if let Some((body, var)) = split_trailing_where(name) {
            if let Some(vars) = parse_where_var_list(var) {
                let mut result = parse(body);
                // A `where T` binder SHADOWS a global type of the same name
                // (Issue #9464): when `struct T`/`struct S2` is registered,
                // The body is parsed before the binder is known, so binder
                // spellings land as nominal `Named("T")` / `Struct{T}` leaves.
                // Rebind those leaves so `Tuple{T,T} where T` keeps
                // diagonal-`where` semantics even when `T` is also a declared
                // type name (Issues #9464/#9563).
                let binder_names: Vec<String> = vars.iter().map(|v| v.name.clone()).collect();
                result.rebind_where_binders(&binder_names);
            ...
            let var = parse_where_var(var);
            let mut body_ty = parse(body);
            body_ty.rebind_where_binders(std::slice::from_ref(&var.name));
        ...
        match name {
            ...
            "Int" => parse(crate::types::native_int_type_name()),
            "UInt" => parse(crate::types::native_uint_type_name()),
            ...
            "OrdinalRange" => Self::Abstract(CoreAbstract::OrdinalRange),
            ...
            _ => {
                if let Some((var_name, upper_bound)) = split_top_level_subtype_bound(name) {
                    ...
                    return Self::TypeVar(CoreTypeVar::with_bounds(
                        ...
                        None,
                        (!upper_bound.is_empty()).then(|| Box::new(parse(upper_bound))),
                    ));
                }
                if let Some((var_name, lower_bound)) = split_top_level_supertype_bound(name) {
                    let var_name = var_name.trim();
                    let lower_bound = lower_bound.trim();
                    return Self::TypeVar(CoreTypeVar::with_bounds(
                        if var_name.is_empty() {
                            "_".to_string()
                        } else {
                        ...
                        (!lower_bound.is_empty()).then(|| Box::new(parse(lower_bound))),
                        None,
                    ));
                }
                ...
                if params.is_empty() {
                    ...
                    if base == "Tuple" && name != base {
                        ...
                        //
                        // Use `callable_singleton_struct_name` (NOT bare
                        // `base_type_name`): the inner function name can be
                        // MODULE-QUALIFIED (`typeof(M.f#anon)`, Issue #10214 —
                        // a module-body nested function value now carries a
                        // module-qualified name). `base_type_name` on the whole
                        // string splits on the LAST `.`, which for such a name
                        // is the module dot INSIDE the parens, yielding a
                        // mangled `f#anon)` that drops the `typeof(...)` wrapper
                        // and is no longer recognized by
                        // `is_callable_singleton_type_name` as `<: Function`
                        // (breaking `findfirst(f::Function, v)` and other HOF
                        // dispatch on such a value).
                        Self::Struct {
                            name: callable_singleton_struct_name(name),
                            params: vec![],
                        }
                    } else if (preserve_user_owner
                        && name.contains('.')
                        && !matches!(name.split('.').next(), Some("Base" | "Core")))
                        || is_known_struct_family(name)
                        || is_callable_singleton_type_name(name)
                    {
                        Self::Struct {
                            // The dispatch parser preserves an explicit owner;
                            // the ordinary parser retains its historical bare
                            // canonical form. Base/Core spellings are bare in
                            // both modes (e.g. Base.Generator; #11076).
                            name: canonical_core_nominal_family(name, preserve_user_owner)
                                .to_string(),
                            params: vec![],
                        }
                    } else {
                    ...
                    let parsed_params: Vec<Self> = params.iter().map(|p| parse(p)).collect();
                    match base {
                        ...
                        base if is_parametric_container_abstract_name(base_type_name(base)) => {
                            Self::Struct {
                                name: canonical_core_nominal_family(base, preserve_user_owner)
                                    .to_string(),
                        ...
                        _ => core_abstract_from_name(base_type_name(base)).map_or_else(
                            || Self::Struct {
                                // Preserve explicit user owners only for the
                                // dispatch image. Ordinary type operators keep
                                // their historical bare image, and Base/Core
                                // remain canonical bare carriers (#11076).
                                name: canonical_core_nominal_family(base, preserve_user_owner)
                                    .to_string(),
                                params: parsed_params,
                            },
...
fn core_type_inner_is_datatype(inner: &CoreType) -> bool {
    match inner {
        ...
        // User-defined nominal types lower to `Named`. Free type variables are
        // represented explicitly as `CoreType::TypeVar`, never inferred from the
        // identifier spelling (Issue #9563).
...
fn abstract_direct_supertype_name(abstract_ty: &CoreAbstract) -> Option<&'static str> {
    ...
    Some(match abstract_ty {
        ...
        A::OrdinalRange => "AbstractRange",
        A::AbstractUnitRange => "OrdinalRange",
        A::DataType => "Type",
...
/// Substitute every occurrence named `var.name` inside `body` with the
/// bound-carrying `var` itself, returning the rewritten type. This realizes the
/// forall-LEFT "fresh RIGID variable confined to its bounds" step: a body from
/// `from_julia_name` may now spell the bound occurrence as a nominal `Named`
/// until the surrounding `UnionAll` establishes scope (Issue #9563), while older
/// structured callers may still carry an unbounded `TypeVar`. Rewriting either
/// form lets the declared bound flow into the subtype check.
///
...
fn substitute_typevar_bound(body: &CoreType, var: &CoreTypeVar) -> CoreType {
    match body {
        ...
        CoreType::Named(name) if name == &var.name => CoreType::TypeVar(var.clone()),
        CoreType::TypeVar(_)
        | CoreType::Bottom
        | CoreType::Any
        | CoreType::Primitive(_)
...
/// One capture-aware substitution for a declared `UnionAll` binder.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct CoreTypeSubstitution {
    pub variable: CoreTypeVar,
    pub value: CoreType,
}

impl CoreTypeSubstitution {
    pub fn new(variable: CoreTypeVar, value: CoreType) -> Self {
        Self { variable, value }
    }
}

fn same_typevar_identity(left: &CoreTypeVar, right: &CoreTypeVar) -> bool {
    match (left.typevar_id(), right.typevar_id()) {
        (CoreTypeVarId::Unresolved, _) | (_, CoreTypeVarId::Unresolved) => left.name == right.name,
        (left_id, right_id) => left_id == right_id,
    }
}

fn substitution_for_var<'a>(
    var: &CoreTypeVar,
    substitutions: &'a [CoreTypeSubstitution],
) -> Option<&'a CoreType> {
    substitutions
        .iter()
        .find(|substitution| same_typevar_identity(var, &substitution.variable))
        .map(|substitution| &substitution.value)
}

fn substitution_for_unresolved_name<'a>(
    name: &str,
    substitutions: &'a [CoreTypeSubstitution],
) -> Option<&'a CoreType> {
    substitutions
...
fn substitute_typevars(body: &CoreType, substitutions: &[CoreTypeSubstitution]) -> CoreType {
    match body {
        CoreType::TypeVar(var) => substitution_for_var(var, substitutions)
            .cloned()
            .unwrap_or_else(|| {
                ...
                var.lower_bound = var
                    .lower_bound
                    .as_deref()
                    .map(|bound| Box::new(substitute_typevars(bound, substitutions)));
                var.upper_bound = var
                    .upper_bound
                    .as_deref()
                    .map(|bound| Box::new(substitute_typevars(bound, substitutions)));
        ...
        CoreType::Named(name) => substitution_for_unresolved_name(name, substitutions)
            .cloned()
        ...
        | CoreType::AbstractUser { .. }
        | CoreType::Value(_)
        | CoreType::Module(_) => body.clone(),
        CoreType::Struct { name, params } if params.is_empty() => {
            substitution_for_unresolved_name(name, substitutions)
                .cloned()
                .unwrap_or_else(|| body.clone())
        }
        CoreType::Struct { name, params } => CoreType::Struct {
        ...
        CoreType::UnionAll { var, body } => {
            let inner_substitutions = substitutions
                .iter()
                .filter(|substitution| !same_typevar_identity(var, &substitution.variable))
                .cloned()
                .collect::<Vec<_>>();
            CoreType::UnionAll {
                var: var.clone(),
...
/// Instantiate selected outer `UnionAll` binders while preserving every
/// unbound binder. Constructor dispatch uses this to apply the implicit
/// `Type{Foo{...}}` self bindings before matching the ordinary value
/// parameters (Issue #10959).
pub fn instantiate_unionall_typevars(
    signature: &CoreType,
    substitutions: &[CoreTypeSubstitution],
) -> CoreType {
    let mut binders = Vec::new();
    let mut body = signature;
    while let CoreType::UnionAll {
        var,
        body: inner_body,
    } = body
    {
    ...
    let mut instantiated = substitute_typevars(body, substitutions);
    for mut binder in binders.into_iter().rev() {
        if substitution_for_var(&binder, substitutions).is_none() {
            // A surviving method-local binder can depend on a constructor-self
            // binder that was just instantiated (`B<:A` in
            // `Foo{A}(x::B) where {A,B<:A}`).  Re-wrapping the original bound
            // leaves it pointing at a removed `A`, so substitute both bounds
            // through the same environment as the signature body (Issue
            // #10959).
            binder.lower_bound = binder
                .lower_bound
                .as_deref()
                .map(|bound| Box::new(substitute_typevars(bound, substitutions)));
            binder.upper_bound = binder
                .upper_bound
                .as_deref()
                .map(|bound| Box::new(substitute_typevars(bound, substitutions)));
            instantiated = CoreType::UnionAll {
                var: binder,
                body: Box::new(instantiated),
            };
        }
    }
    instantiated
...
fn canonicalize_signature_element_for_dedup(
    elem: &CoreType,
    counts: &HashMap<String, usize>,
    peeled_bounds: &HashMap<&str, CoreType>,
    erasable_schema_vars: &HashMap<&str, bool>,
) -> CoreType {
    match elem {
        CoreType::TypeVar(var) if counts.get(&var.name).copied() == Some(1) => peeled_bounds
            .get(var.name.as_str())
            .cloned()
        ...
        CoreType::Struct { name, params }
            if !params.is_empty()
                && params
                    .iter()
                    .all(|param| is_erasable_schema_typevar(param, erasable_schema_vars)) =>
        {
            CoreType::Struct {
                name: name.clone(),
                params: Vec::new(),
...
fn is_erasable_schema_typevar(
    param: &CoreType,
    erasable_schema_vars: &HashMap<&str, bool>,
) -> bool {
    let (name, local_unbounded) = match param {
        CoreType::TypeVar(var) => (var.name.as_str(), core_typevar_is_unbounded_for_schema(var)),
        CoreType::Named(name) => (name.as_str(), true),
        _ => return false,
    };
    local_unbounded && erasable_schema_vars.get(name).copied().unwrap_or(false)
}

fn core_typevar_is_unbounded_for_schema(var: &CoreTypeVar) -> bool {
    let lower_unbounded = var
        .lower_bound
        .as_deref()
        .is_none_or(|bound| matches!(bound, CoreType::Bottom));
    let upper_unbounded = var
        .upper_bound
        .as_deref()
        .is_none_or(|bound| matches!(bound, CoreType::Any));
...
fn builtin_struct_direct_supertype_name(name: &str, is_parametric: bool) -> Option<&'static str> {
    Some(match (base_type_name(name), is_parametric) {
        ...
        ("Dict", _) => "AbstractDict",
        ("Set", _) => "AbstractSet",
        ("UnitRange", true) => "AbstractUnitRange",
        ("StepRange", true) => "OrdinalRange",
        ("UnitRange" | "OneTo", false) => "AbstractUnitRange",
        ("StepRange", false) => "OrdinalRange",
        ("StepRangeLen" | "LinRange", false) => "AbstractRange",
...
fn direct_builtin_subtype_names(parent_name: &str) -> Vec<&'static str> {
    match base_type_name(parent_name) {
        ...
        "AbstractSet" => vec!["Set"],
        "AbstractRange" => vec!["OrdinalRange", "StepRangeLen", "LinRange"],
        "OrdinalRange" => vec!["AbstractUnitRange", "StepRange"],
        "AbstractUnitRange" => vec!["UnitRange", "OneTo"],
        "IO" => vec!["IOBuffer", "Pipe"],
        "Type" => vec!["DataType"],
        _ => vec![],
    }
}
...
fn core_abstract_from_name(name: &str) -> Option<CoreAbstract> {
    Some(match base_type_name(name) {
        ...
        "OrdinalRange" => CoreAbstract::OrdinalRange,
...
fn core_typeof_inner_subtype_with_lookup(
    ...
    match other_inner {
        CoreType::TypeVar(var) if var.is_rigid() => inner == other_inner,
...
fn struct_is_subtype_of_abstract_with_lookup(
    ...
    builtin_struct_is_subtype_of_abstract(name, params, abstract_ty)
        || declared_struct_is_subtype_of_abstract(name, params, abstract_ty, hierarchy)
}

/// Built-in (Rust-declared) family membership: whether `name` is itself one of
/// the built-in Base type names whose place in the abstract lattice is defined
/// in Rust name tables rather than by a registered `<:` declaration.
///
/// These tables are the genuinely primitive roots left after Issue #8560:
/// built-in Base types (`Array`, `UnitRange`, `Dict`, `IOBuffer`, `typeof(f)`
/// singletons, ...) are VM-native — they are not (fully) declared in
/// `boot.jl`, so the program's `StructHierarchy` cannot answer for them and
/// their declared supertypes live in `array_family_*` /
/// `range_family_name_subtype_allowed` / the name matches below. User and
/// bundled-package types must NOT be added here; they are resolved from their
/// declared supertypes by [`declared_struct_is_subtype_of_abstract`].
fn builtin_struct_is_subtype_of_abstract(
    name: &str,
    params: &[CoreType],
    abstract_ty: &CoreAbstract,
) -> bool {
    use CoreAbstract as A;
    match abstract_ty {
        A::AbstractArray => array_family_dim(name).is_some(),
        // `DenseArray` is more specific than `AbstractArray`, so an array-family
        // name only qualifies when it is a (non wrapper) `DenseArray` or more
        // concrete.
        A::DenseArray => {
        ...
        A::AbstractVector => array_family_struct_has_abstract_rank(name, params, 1),
        A::AbstractMatrix => array_family_struct_has_abstract_rank(name, params, 2),
        A::AbstractDict => matches!(base_type_name(name), "Dict"),
        ...
        A::OrdinalRange => range_family_name_subtype_allowed(name, "OrdinalRange"),
        A::AbstractUnitRange => range_family_name_subtype_allowed(name, "AbstractUnitRange"),
        // Built-in function singleton types `typeof(f)`.
        ...
        A::IO => matches!(base_type_name(name), "IOBuffer" | "Pipe"),
        _ => false,
    }
}

/// Declared-supertype derivation (Issue #8560): decide `name{params} <:
/// abstract_ty` by climbing the *registered* declared-parent chain (`struct S
/// <: P` / `abstract type A <: P` entries in the supplied `StructHierarchy`,
/// including the `boot.jl` built-in abstract declarations) until it reaches
/// the target built-in abstract or runs out. This is what makes user-defined
/// and bundled-package hierarchies (`struct Mine <: MyAbstract`, functor
/// structs `<: Function`, `KeySet <: AbstractSet`, Distributions-style chains)
/// participate in `<:`/`isa`/dispatch without per-family Rust arms.
fn declared_struct_is_subtype_of_abstract(
    name: &str,
    params: &[CoreType],
    abstract_ty: &CoreAbstract,
    hierarchy: Option<&StructHierarchy>,
    ...
    use CoreAbstract as A;
    match abstract_ty {
        // Array-family targets walk the *instantiated* parent chain
        // (parameter-substituting, Issue #7787) instead of the nominal chain:
        // the declared rank lives in a parent's parameter list (`<:
        // AbstractArray{T,2}`), which a purely nominal walk would drop, and
        // `boot.jl` declares `AbstractRange <: Any` (parser compatibility), so
        // the nominal chain also cannot see the range-family's
        // `AbstractVector` ancestry.
        A::AbstractArray => user_struct_array_ancestor(name, params, hierarchy).is_some(),
        // `AbsContainer <: AbstractArray{...}` stays NOT `<: DenseArray`: the
        // ancestor must itself be a (non wrapper) `DenseArray` or more concrete.
        A::DenseArray => {
            user_struct_array_ancestor(name, params, hierarchy).is_some_and(|(ancestor, _)| {
                !array_family_is_wrapper(&ancestor)
                    && array_family_abstractness(&ancestor)
                        <= array_family_abstractness("DenseArray")
            })
        }
        A::AbstractVector => user_struct_array_ancestor_has_rank(name, params, 1, hierarchy),
        A::AbstractMatrix => user_struct_array_ancestor_has_rank(name, params, 2, hierarchy),
        // `Core.Builtin` membership is pinned to the genuine built-in function
        // singletons (Issue #5129); no declaration can create a new builtin,
        // so there is deliberately no hierarchy walk for it.
        A::Builtin => false,
        // Everything else — the numeric lattice (Issue #5157), containers,
        // strings/chars, `Function`, `IO`, ... — is answered uniformly by the
        // nominal declared-parent walk to the target's Julia name. `Type` /
        // `DataType` cannot appear as declared parents, so they simply never
        // resolve here.
        _ => abstract_type_name(abstract_ty).is_some_and(|target| {
            registered_struct_is_subtype_of_with_lookup(name, target, hierarchy)
        }),
    }
}
...
fn user_struct_array_ancestor(
    ...
    for _ in 0..64 {
        ...
        let (parent_name, parent_params) = match parent {
            ...
            CoreType::Abstract(ref ab) => (abstract_array_family_name(ab)?.to_string(), Vec::new()),
            _ => return None,
        };
...
fn is_builtin_or_bare_nominal_name(name: &str) -> bool {
    let family = name.split('{').next().unwrap_or(name);
    !family.contains('.')
        || family
            .split('.')
            .next()
            .is_some_and(|root| matches!(root, "Base" | "Core"))
}
...
fn struct_params_are_subtype_with_lookup(
    ...
    let builtin_pair =
        is_builtin_or_bare_nominal_name(name) && is_builtin_or_bare_nominal_name(other_name);
    ...
    if builtin_pair
        && array_family_dim(name_base).is_some()
        && array_family_dim(other_base).is_some()
    {
    ...
    if crate::types::nominal_family_names_compatible(name, other_name) && name_base == other_base {
...
fn array_family_dim(name: &str) -> Option<Option<i64>> {
    match base_type_name(name) {
        ...
        | "OrdinalRange" | "AbstractUnitRange" | "UnitRange" | "StepRange" | "StepRangeLen"
        | "LinRange" | "OneTo" | "LogRange" => Some(Some(1)),
...
fn array_family_abstractness(name: &str) -> u8 {
    match base_type_name(name) {
        ...
        | "BitMatrix" | "SubArray" | "ReshapedArray" | "AbstractRange" | "OrdinalRange"
...
fn range_family_dim(name: &str) -> Option<i64> {
    match base_type_name(name) {
        "AbstractRange" | "OrdinalRange" | "AbstractUnitRange" | "UnitRange" | "StepRange"
        | "StepRangeLen" | "LinRange" | "OneTo" | "LogRange" => Some(1),
        _ => None,
    }
}
...
fn range_family_name_subtype_allowed(name: &str, other_name: &str) -> bool {
    ...
    match name {
        ...
        "OrdinalRange" => matches!(
            other_name,
            "AbstractRange" | "AbstractVector" | "AbstractArray"
        ),
        "AbstractUnitRange" => matches!(
            other_name,
            "OrdinalRange" | "AbstractRange" | "AbstractVector" | "AbstractArray"
        ),
...
fn array_family_pattern_params_match_with_lookup(
    ...
    // NOTE: a bare array-family name with no params (e.g. `Matrix` with
    // `pattern_params = []`) still carries an implied rank from
    // `array_family_element_and_rank` (Matrix → rank 2). The early-return
    // `if pattern_params.is_empty() { return true; }` that previously appeared
    // here bypassed the rank check and caused `Vector{Int64} <: Matrix` to be
    // true inside Tuple matching (Issue #8804).  Removed: the `(_, None) => true`
    // arm below handles the no-element-constraint case correctly, and the rank
    // check rejects Vector-vs-Matrix and Matrix-vs-Vector mismatches.
...
/// Preserve explicit user/package owners only in the dispatch CoreType image,
/// while keeping ordinary semantic parsing and Base/Core spellings on the VM's
/// historical bare carriers.
fn canonical_core_nominal_family(name: &str, preserve_user_owner: bool) -> &str {
    let root = name.split('.').next().unwrap_or(name);
    // A USER module's Array/Vector/Matrix declaration must never canonicalize
    // into the Base-owned native array family: genuine wrapper carriers spell
    // these bare, so stripping a non-builtin owner here made `Faux.Array`
    // parse AS the native `Array` family and leak `isa(v, Base.Array)`.
    // Module-local bare annotations no longer rely on this collapse — the
    // signature side qualifies shadowing declarations (Issues #11388/#11395).
    if name.contains('.')
        && !matches!(root, "Base" | "Core" | "Main")
        && matches!(base_type_name(name), "Array" | "Vector" | "Matrix")
    {
        return name;
    }
    if !preserve_user_owner
        || matches!(root, "Base" | "Core")
        || !crate::types::has_qualified_nominal_family_collision(name)
    {
        base_type_name(name)
    } else {
        name
...
/// Canonicalize a callable-singleton type name `typeof(<maybe-qualified fn>)` by
/// stripping only the MODULE qualifier from the INNER function name while keeping
/// the `typeof(...)` wrapper (Issue #10214).
///
/// A module-body nested function value's name is module-qualified
/// (`typeof(M.outer#anon)`). Running `base_type_name` on the whole string splits
/// on the last `.`, which lands on the module dot INSIDE the parens and produces
/// a mangled `outer#anon)` — no longer recognized as a callable singleton
/// (`is_callable_singleton_type_name` requires the `typeof(` prefix), so it is no
/// longer `<: Function`. That broke higher-order-function dispatch
/// (`findfirst(f::Function, v)` etc.) on such a value. Stripping the module from
/// the inner name only, and re-wrapping, keeps the singleton recognizable while
/// preserving the existing module-normalization (`typeof(Base.sin)` and
/// `typeof(sin)` canonicalize to the same singleton).
fn callable_singleton_struct_name(name: &str) -> String {
    match name
        .strip_prefix("typeof(")
        .and_then(|inner| inner.strip_suffix(')'))
    {
        Some(inner) => format!("typeof({})", base_type_name(inner)),
        None => base_type_name(name).to_string(),
    }
}
...
/// Whether a rendered type name has a TOP-LEVEL trailing ` where ` clause
/// (i.e. it is the surface form of a `UnionAll`, not a plain parametric
/// application). A ` where ` nested inside braces/parens/brackets/quotes does
/// NOT count (`Vector{Vector{T} where T}` is a plain application).
///
/// String-level type plumbing that pattern-matches on name shape (prefix
/// arms like `Vector{...}`, space-stripping normalizers) must consult this
/// first and leave such names intact for `CoreType::from_julia_name`, which
/// owns the full `where`-chain parse. Splitting `Vector{T} where {S<:Real,
/// T<:S}` at the `Vector{`/`}` markers yields a garbage element type and
/// inverted `isa` answers (Issue #10410).
pub fn has_top_level_trailing_where(name: &str) -> bool {
    // Fast pre-filter: the overwhelmingly common case has no `where` at all.
    name.contains(" where ") && split_trailing_where(name).is_some()
}

/// Remove every trailing top-level ` where ` clause from a rendered type
/// name, leaving nested keywords and quoted nominal content intact.
pub fn strip_top_level_where_clauses(mut name: &str) -> &str {
    while let Some((body, _)) = split_trailing_where(name) {
        name = body;
    }
    name
}
...
fn split_top_level_subtype_bound(name: &str) -> Option<(&str, &str)> {
    split_top_level_bound(name, '<')
}

fn split_top_level_supertype_bound(name: &str) -> Option<(&str, &str)> {
    split_top_level_bound(name, '>')
}

fn split_top_level_bound(name: &str, op: char) -> Option<(&str, &str)> {
    let bytes = name.as_bytes();
    let mut brace_depth = 0usize;
    let mut paren_depth = 0usize;
    let mut bracket_depth = 0usize;
...
fn parse_where_var(spec: &str) -> CoreTypeVar {
    parse_where_var_with_prior(spec, &[])
}

fn parse_where_var_with_prior(spec: &str, prior_binders: &[String]) -> CoreTypeVar {
    ...
    if let Some((name, lower)) = split_top_level_supertype_bound(spec) {
        let name = name.trim();
        let lower = lower.trim();
        return CoreTypeVar::with_bounds(
            if name.is_empty() {
                "_".to_string()
            } else {
                name.to_string()
    ...
    let (name, lower_bound, upper_bound) = match parts.as_slice() {
        ...
        // the already-declared bounded type variable `S<:U` (for example
        // `where {S<:Real, T<:S<:Real}`). Use the enclosing where-list scope,
        // not the spelling of `T`/`S`, to choose that interpretation.
        [name, upper_name, upper] if prior_binders.iter().any(|b| b == upper_name) => {
            let upper_var = parse_where_var_with_prior(&format!("{upper_name}<:{upper}"), &[]);
            (*name, None, Some(Box::new(CoreType::TypeVar(upper_var))))
        }
        // `L <: T <: U`
        [lower, name, upper] => (
            *name,
    ...
    for (idx, ch) in inner.char_indices() {
        match ch {
            ...
            ',' if depth == 0 => {
                ...
                if !var.is_empty() {
                    let parsed = parse_where_var_with_prior(var, &binder_names);
                    binder_names.push(parsed.name.clone());
                    vars.push(parsed);
                }
...
pub fn parse_parametric_type_name(name: &str) -> (&str, Vec<&str>) {
    parse_parametric_name(name)
}

fn parse_parametric_name(name: &str) -> (&str, Vec<&str>) {
    let Some(start) = find_top_level_parametric_open(name) else {
        return (name, vec![]);
    };
    if !name.ends_with('}') {
        return (name, vec![]);
...
/// Find the opening brace of a top-level parametric application. Braces in a
/// quoted nominal identifier (for example `var"Foo{Bar}"`) or nested inside a
/// parenthesized/bracketed carrier are content, not the application boundary.
fn find_top_level_parametric_open(name: &str) -> Option<usize> {
    let mut paren_depth = 0usize;
    let mut bracket_depth = 0usize;
    let mut quote: Option<char> = None;
    let mut escaped = false;
    ...
    for (idx, ch) in name.char_indices() {
        ...
        match ch {
            ...
            '[' => bracket_depth += 1,
            ']' => bracket_depth = bracket_depth.saturating_sub(1),
            '{' if paren_depth == 0 && bracket_depth == 0 => return Some(idx),
            _ => {}
        }
    }
    None
...
fn parse_named_tuple_type_name(name: &str, preserve_user_owner: bool) -> Option<CoreType> {
    let (base, params) = parse_parametric_name(name);
    match base {
        "@NamedTuple" => {
            let parsed_fields = params
                .iter()
                .map(|field| parse_concrete_named_tuple_field(field, preserve_user_owner))
                .collect::<Option<Vec<_>>>()?;
            Some(CoreType::NamedTuple(parsed_fields))
        }
        "NamedTuple" => parse_typelevel_concrete_named_tuple(&params, preserve_user_owner),
        _ => None,
    }
}

fn parse_concrete_named_tuple_field(
    field: &str,
    ...
    if let Some((name, ty)) = trimmed.split_once("::") {
        ...
        (!name.is_empty()).then(|| {
            (
                name.to_string(),
                CoreType::from_julia_name_cached(ty.trim(), preserve_user_owner),
            )
        })
    } else {
...
fn parse_typelevel_concrete_named_tuple(
    params: &[&str],
    preserve_user_owner: bool,
) -> Option<CoreType> {
    ...
    let CoreType::Tuple(field_types) =
        CoreType::from_julia_name_cached(types_param.trim(), preserve_user_owner)
    else {
        return None;
    };
...
mod tests {
    ...
    #[test]
    fn nominal_metadata_is_structural_and_owner_qualified_issue_10460() {
        let binder = CoreTypeVar::unscoped("T").with_scope_id(10460);
        let wrapped = CoreType::UnionAll {
            var: binder.clone(),
        ...
        assert_eq!(wrapped.nominal_base_name(), Some("Owner.Box"));
        assert_eq!(
            wrapped.nominal_type_parameters(),
            Some(
                [CoreType::TypeVar(
                    CoreTypeVar::unscoped("T").with_scope_id(10460)
                )]
                .as_slice()
            )
        );
        assert!(matches!(
            wrapped.unwrap_unionall_ref(),
            CoreType::Struct { name, .. } if name == "Owner.Box"
        ));

        crate::types::register_type_name("OwnerA10460.CoreBox10460");
        crate::types::register_type_name("OwnerB10460.CoreBox10460");
        let qualified = CoreType::from_julia_type_preserving_owner(&JuliaType::Struct(
            "OwnerA10460.CoreBox10460{Int64}".to_string(),
        ));
        assert_eq!(
            qualified.nominal_base_name(),
            Some("OwnerA10460.CoreBox10460")
        );
        assert_eq!(
            qualified.nominal_type_parameters(),
            Some([CoreType::Primitive(CorePrimitive::Int64)].as_slice())
        );

        let sibling = CoreType::from_julia_type_preserving_owner(&JuliaType::Struct(
            "OwnerB10460.CoreBox10460{Int64}".to_string(),
        ));
        assert!(!qualified.is_subtype_of(&sibling));
        assert!(!sibling.is_subtype_of(&qualified));

        let mut hierarchy = StructHierarchy::new();
        hierarchy.insert(
            "OwnerA10460.CoreBox10460",
            Some("OwnerB10460.CoreBox10460{Int64}".to_string()),
            vec!["T".to_string()],
        );
        assert!(qualified.is_subtype_of_with_hierarchy(&sibling, &hierarchy));

        let abstract_value = |value: i64| CoreType::AbstractUser {
            name: format!("OwnerA10460.Abs10460{{{value}}}"),
            parent: None,
        };
        assert!(!abstract_value(1).is_subtype_of(&abstract_value(2)));
        assert!(abstract_value(1).is_subtype_of(&CoreType::AbstractUser {
            name: "Abs10460{1}".to_string(),
            parent: None,
        }));
    }

    #[test]
    fn instantiate_unionall_substitutes_surviving_dependent_bounds_issue_10959() {
        let a = CoreTypeVar::unscoped("A");
        let b =
            CoreTypeVar::with_bounds("B", None, Some(Box::new(CoreType::Named("A".to_string()))));
        let signature = CoreType::UnionAll {
            var: a.clone(),
            body: Box::new(CoreType::UnionAll {
                var: b.clone(),
                body: Box::new(CoreType::Tuple(vec![CoreType::TypeVar(b)])),
            }),
        };
        let substitutions = vec![CoreTypeSubstitution::new(
            a.clone(),
            CoreType::Abstract(CoreAbstract::Number),
        )];
        let instantiated = instantiate_unionall_typevars(&signature, &substitutions);
        let expected_b = CoreTypeVar::with_bounds(
            "B",
            None,
            Some(Box::new(CoreType::Abstract(CoreAbstract::Number))),
        );
        assert_eq!(
            instantiated,
            CoreType::UnionAll {
                var: expected_b.clone(),
                body: Box::new(CoreType::Tuple(vec![CoreType::TypeVar(expected_b)])),
            }
        );
    }

    #[test]
    fn instantiate_unionall_does_not_capture_same_name_scoped_binder_issue_10436() {
        let outer = CoreTypeVar::unscoped("T").with_scope_id(1);
        let inner = CoreTypeVar::unscoped("T").with_scope_id(2);
        let signature = CoreType::UnionAll {
            var: outer.clone(),
            body: Box::new(CoreType::UnionAll {
                var: inner.clone(),
                body: Box::new(CoreType::Tuple(vec![
                    CoreType::TypeVar(outer.clone()),
                    CoreType::TypeVar(inner.clone()),
                ])),
            }),
        };
        let substitutions = vec![CoreTypeSubstitution::new(
        ...
        assert_eq!(
            instantiate_unionall_typevars(&signature, &substitutions),
            CoreType::UnionAll {
                var: inner.clone(),
                body: Box::new(CoreType::Tuple(vec![
                    CoreType::Abstract(CoreAbstract::Number),
                    CoreType::TypeVar(inner),
                ])),
            }
        );
    }
    ...
    #[test]
    fn core_typevar_id_projects_legacy_identity_fields_issue_10459() {
        assert_eq!(
            CoreTypeVar::unscoped("T").typevar_id(),
            CoreTypeVarId::Unresolved
        );
        assert_eq!(
            CoreTypeVar::unscoped("T").with_scope_id(42).typevar_id(),
            CoreTypeVarId::Scoped(42)
        );
        assert_eq!(
            CoreTypeVar::unscoped("T")
                .with_scope_id(42)
                .with_rigid_identity(7)
                .typevar_id(),
            CoreTypeVarId::Rigid(7)
        );
    }
    ...
    fn typeof_subtype_is_invariant_for_concrete_inner_issue_5068() {
        ...
        let type_le_number =
            CoreType::TypeOf(Box::new(CoreType::TypeVar(CoreTypeVar::with_bounds(
                "_",
                None,
                Some(Box::new(CoreType::Abstract(CoreAbstract::Number))),
            ))));
        ...
        let type_le_integer =
            CoreType::TypeOf(Box::new(CoreType::TypeVar(CoreTypeVar::with_bounds(
                "_",
                None,
                Some(Box::new(CoreType::Abstract(CoreAbstract::Integer))),
            ))));
    ...
    #[test]
    fn contravariant_type_arg_parses_as_lower_bound_issue_9468() {
        let vector_ge_int = CoreType::from_julia_name("Vector{>:Int64}");
        assert_eq!(
            vector_ge_int,
            CoreType::Struct {
                name: "Vector".to_string(),
                params: vec![CoreType::TypeVar(CoreTypeVar::with_bounds(
                    "_",
                    Some(Box::new(CoreType::Primitive(CorePrimitive::Int64))),
                    None,
                ))],
            }
        );

        assert!(CoreType::from_julia_name("Vector{Real}").is_subtype_of(&vector_ge_int));
        assert!(CoreType::from_julia_name("Vector{Number}").is_subtype_of(&vector_ge_int));
        assert!(CoreType::from_julia_name("Vector{Any}").is_subtype_of(&vector_ge_int));
        assert!(CoreType::from_julia_name("Vector{Int64}").is_subtype_of(&vector_ge_int));
        assert!(!CoreType::from_julia_name("Vector{Int32}").is_subtype_of(&vector_ge_int));

        let lower_where = CoreType::from_julia_name("Vector{T} where T>:Int64");
        assert!(matches!(
            lower_where,
            CoreType::UnionAll {
                var: CoreTypeVar {
    ...
    #[test]
    fn typevar_bounds_survive_core_juliatype_round_trip_issue_9627() {
        let cases = [
            ("Vector{<:Real}", "Vector{<:Real}"),
            ("Vector{>:Int64}", "Vector{>:Int64}"),
            ("Vector{T} where T<:Real", "Vector{T} where T<:Real"),
            ("Vector{T} where T>:Int64", "Vector{T} where T>:Int64"),
            (
                "Vector{T} where Int64<:T<:Real",
                "Vector{T} where Int64<:T<:Real",
            ),
        ];

        for (spelling, rendered) in cases {
            let core = CoreType::from_julia_name(spelling);
            let julia_type = core_type_to_julia_type(&core);
            assert_eq!(
                julia_type.to_string(),
                rendered,
                "{spelling} should preserve canonical TypeVar rendering"
            );
            assert!(
                CoreType::from(&julia_type).is_semantically_equal(&core),
                "{spelling} should preserve TypeVar bounds through CoreType -> JuliaType -> CoreType"
            );
        }

        let direct_double_bounded = CoreType::TypeVar(CoreTypeVar::with_bounds(
            "T",
            Some(Box::new(CoreType::Primitive(CorePrimitive::Int64))),
            Some(Box::new(CoreType::Abstract(CoreAbstract::Real))),
        ));
        let rendered = core_type_to_julia_type(&direct_double_bounded);
        assert_eq!(
            CoreType::from(&rendered),
            direct_double_bounded,
            "direct TypeVar conversion should preserve both lower and upper bounds"
        );
    }
    ...
    #[test]
    fn parse_parametric_name_ignores_delimiters_inside_quoted_nominal_11360() {
        let (base, params) = parse_parametric_type_name(r#"Q.var"Foo where Bar"{Int64}"#);
        assert_eq!(base, r#"Q.var"Foo where Bar""#);
        assert_eq!(params, vec!["Int64"]);

        let (base, params) = parse_parametric_type_name(r#"Q.var"Foo{Bar}"{Int64}"#);
        assert_eq!(base, r#"Q.var"Foo{Bar}""#);
        assert_eq!(params, vec!["Int64"]);
    }
    ...
    fn value_parameter_subtyping_handles_array_aliases_and_ntuple_issue_3885() {
        ...
        assert!(!CoreType::from_julia_name("Tuple{Int64, Int64}")
            .is_subtype_of(&CoreType::from_julia_name("NTuple{3, Int64}")));
        assert!(CoreType::from_julia_name("Tuple{Int64, Int64}")
            .is_subtype_of(&CoreType::from_julia_name("NTuple{N, Int64} where N")));
    }
    ...
    fn abstract_lattice_subtyping_is_transitive_issue_5615() {
        ...
        assert!(CoreType::from_julia_name("OrdinalRange")
            .is_subtype_of(&CoreType::from_julia_name("AbstractRange")));
        assert!(CoreType::from_julia_name("OrdinalRange")
            .is_subtype_of(&CoreType::from_julia_name("AbstractVector")));
        assert!(CoreType::from_julia_name("AbstractUnitRange")
            .is_subtype_of(&CoreType::from_julia_name("OrdinalRange")));
        assert!(CoreType::from_julia_name("AbstractUnitRange")
            .is_subtype_of(&CoreType::from_julia_name("AbstractVector")));
        ...
        assert!(CoreType::from_julia_name("UnitRange{Int64}")
            .is_subtype_of(&CoreType::from_julia_name("AbstractVector{Int64}")));
        assert!(CoreType::from_julia_name("StepRange{Int64, Int64}")
            .is_subtype_of(&CoreType::from_julia_name("OrdinalRange")));
        assert!(CoreType::from_julia_name("UnitRange{Int64}")
            .is_subtype_of(&CoreType::from_julia_name("AbstractArray{Int64,1}")));
        assert!(!CoreType::from_julia_name("UnitRange{Int64}")
    ...
    #[test]
    fn parametric_name_parser_keeps_unbound_names_nominal_issue_9563() {
        ...
        assert!(
            matches!(
                ...
                CoreType::Struct {
                    ref params,
                    ..
                } if matches!(params.as_slice(), [CoreType::Named(name)] if name == "T")
            ),
            "unbound names must stay nominal; TypeVar-ness comes from where/type-parameter scope"
        );
        assert_eq!(core.specificity(), 5);

        let where_core = CoreType::from_julia_name("Complex{T} where T");
        assert!(matches!(
            where_core,
            CoreType::UnionAll {
                ref body,
    ...
    #[test]
    fn typeexpr_typevar_projects_to_explicit_core_typevar_issue_9563() {
        let expr = crate::types::TypeExpr::TypeVar("S2".to_string());
        assert!(matches!(
            CoreType::from(&expr),
            CoreType::TypeVar(var) if var.name == "S2"
        ));
    }
    ...
    fn dispatch_pattern_score_preserves_runtime_ordering() {
        let typevar = |name: &str| CoreType::TypeVar(CoreTypeVar::unscoped(name));
        let rational_t = CoreType::Struct {
            name: "Rational".to_string(),
            params: vec![typevar("T")],
        };
        let two_param_matrix_t = CoreType::Struct {
            name: "TwoParamMatrixIssue".to_string(),
            params: vec![typevar("T")],
        };
        let array_t = CoreType::Struct {
            name: "Array".to_string(),
            params: vec![typevar("T")],
        };
        let type_pair_kv = CoreType::TypeOf(Box::new(CoreType::Struct {
            name: "Pair".to_string(),
            params: vec![typevar("K"), typevar("V")],
        }));
        ...
        assert_eq!(
            rational_t.dispatch_pattern_score(&CoreType::from_julia_name("Rational{Int64}")),
            3
        );
        assert_eq!(
            two_param_matrix_t.dispatch_pattern_score(&CoreType::from_julia_name(
        ...
        assert_eq!(
            array_t.dispatch_pattern_score(&CoreType::from_julia_name("Array{Int64, 1}")),
            3
        );
        assert_eq!(
            array_t.dispatch_pattern_score(&CoreType::from_julia_name("Vector{Int64}")),
            3
        );
        assert_eq!(
    ...
    fn unionall_rhs_binds_typevars_with_bounds() {
        let vector_of_t_integer = CoreType::UnionAll {
            var: CoreTypeVar {
                scope_id: CoreTypeVar::UNRESOLVED_SCOPE_ID,
                rigid_identity: None,
                name: "T".to_string(),
                lower_bound: None,
    ...
    #[test]
    fn dependent_unionall_bounds_use_earlier_actual_bindings_10613() {
        let pattern = CoreType::from_julia_name("RTChainNeg{A,B,C} where {A,B<:A,C<:B}");
        let positive = CoreType::from_julia_name("RTChainNeg{Real,Integer,Int64}");
        let negative = CoreType::from_julia_name("RTChainNeg{Real,Int64,Integer}");

        assert!(positive.is_subtype_of(&pattern));
        assert!(!negative.is_subtype_of(&pattern));
    }

    #[test]
    fn nested_same_name_unionall_typevars_have_distinct_scope_identity_issue_10279() {
        let pattern = CoreType::from_julia_name("Tuple{T, Vector{T} where T} where T");

        assert!(CoreType::from_julia_name("Tuple{Int64, Vector{Float64}}").is_subtype_of(&pattern));
        assert!(CoreType::from_julia_name("Tuple{String, Vector{Int64}}").is_subtype_of(&pattern));

        let outer_reused = CoreType::from_julia_name("Tuple{T, T, Vector{T} where T} where T");
        assert!(
            CoreType::from_julia_name("Tuple{Int64, Int64, Vector{Float64}}")
                .is_subtype_of(&outer_reused)
        );
        assert!(
            !CoreType::from_julia_name("Tuple{Int64, Float64, Vector{Int64}}")
                .is_subtype_of(&outer_reused)
        );
    }
    ...
    #[test]
    fn scoped_named_unionall_occurrences_get_declared_bounds_issue_9563() {
        let bounded_vector = |upper_bound: CoreType| CoreType::UnionAll {
            var: CoreTypeVar::with_bounds("T", None, Some(Box::new(upper_bound))),
            body: Box::new(CoreType::Struct {
                name: "Vector".to_string(),
                params: vec![CoreType::Named("T".to_string())],
            }),
        };

        let vector_real = bounded_vector(CoreType::Abstract(CoreAbstract::Real));
        let vector_integer = bounded_vector(CoreType::Abstract(CoreAbstract::Integer));

        assert!(vector_integer.is_subtype_of(&vector_real));
        assert!(!vector_real.is_subtype_of(&vector_integer));
        assert_eq!(vector_real.type_intersect(&vector_integer), vector_integer);
    }
    ...
    #[test]
    fn strip_top_level_where_clauses_preserves_quoted_and_nested_content_11360() {
        assert_eq!(
            strip_top_level_where_clauses("Tuple{T, S} where S where T"),
            "Tuple{T, S}"
        );
        assert_eq!(
            strip_top_level_where_clauses(r#"Q.var"Foo where Bar"{Int64}"#),
            r#"Q.var"Foo where Bar"{Int64}"#
        );
        assert_eq!(
            strip_top_level_where_clauses("Tuple{Foo where Bar}"),
            "Tuple{Foo where Bar}"
        );
    }

    #[test]
    fn has_top_level_trailing_where_detects_unionall_surface_forms_issue_10410() {
        // UnionAll surface forms — chained and brace-list spellings alike.
        assert!(has_top_level_trailing_where("Vector{T} where T"));
        assert!(has_top_level_trailing_where("Vector{T} where T<:Real"));
        assert!(has_top_level_trailing_where(
            "Vector{T} where {S<:Real, T<:S}"
        ));
        assert!(has_top_level_trailing_where("Tuple{T, S} where S where T"));
        // Plain applications, including a `where` nested inside braces.
        assert!(!has_top_level_trailing_where("Vector{Int64}"));
        assert!(!has_top_level_trailing_where("Vector{Vector{T} where T}"));
        assert!(!has_top_level_trailing_where("Int64"));
    }

    /// Issue #10410: the chained different-name `where` bound
    /// (`Vector{T} where T<:S where S<:Real`) must accept a member whose
    /// element type satisfies the transitive bound (`Float64 <: S <: Real`)
    /// and reject one that does not — pinned at the engine level so the
    /// string-plumbing fixes upstream cannot mask an engine regression.
    #[test]
    fn chained_where_bound_subtype_resolves_outer_binder_issue_10410() {
        let pattern = CoreType::from_julia_name("Vector{T} where {S<:Real, T<:S}");
        assert!(CoreType::from_julia_name("Vector{Float64}").is_subtype_of(&pattern));
        assert!(CoreType::from_julia_name("Vector{Int64}").is_subtype_of(&pattern));
        assert!(!CoreType::from_julia_name("Vector{Any}").is_subtype_of(&pattern));
        assert!(!CoreType::from_julia_name("Vector{String}").is_subtype_of(&pattern));
    }
    ...
    fn parse_where_var_handles_bound_spellings() {
        ...
        assert!(upper.lower_bound.is_none());

        let lower = parse_where_var("T>:Int64");
        assert_eq!(lower.name, "T");
        assert_eq!(
            lower.lower_bound.as_deref(),
            Some(&CoreType::Primitive(CorePrimitive::Int64))
        );
        assert!(lower.upper_bound.is_none());

        let double_bounded = parse_where_var("Int64<:T<:Real");
        assert_eq!(double_bounded.name, "T");
        assert_eq!(
            double_bounded.lower_bound.as_deref(),
            Some(&CoreType::Primitive(CorePrimitive::Int64))
        );
        assert_eq!(
            double_bounded.upper_bound.as_deref(),
            Some(&CoreType::Abstract(CoreAbstract::Real))
        );
    ...
    fn vararg_typevar_patterns_share_tuple_semantics() {
        let tuple_vararg_t = CoreType::from_julia_name("Tuple{Vararg{T}} where T");
    ...
    fn core_builtin_singleton_type_distinction_issue_5129() {
        ...
        assert!(is_core_function_binding_name("getfield"));
        assert!(is_core_function_binding_name("sizeof"));
        assert!(is_core_function_binding_name("memoryref"));
        assert!(is_core_function_binding_name("finalizer"));
        assert!(is_core_function_binding_name("include"));
        assert!(!is_core_function_binding_name("map"));
    ...
    #[test]
    fn named_struct_subtypes_parametric_abstract_parent_issue_9472() {
        let mut hierarchy = StructHierarchy::new();
        hierarchy.insert("Container", Some("Any".to_string()), vec!["T".to_string()]);
        hierarchy.insert("IntBox", Some("Container{Int64}".to_string()), Vec::new());
        hierarchy.insert(
            "FloatBox",
            Some("Container{Float64}".to_string()),
            Vec::new(),
        );

        let int_box = CoreType::Named("IntBox".to_string());
        let int_container = CoreType::from_julia_name("Container{Int64}");
        let float_container = CoreType::from_julia_name("Container{Float64}");

        assert!(int_box.is_subtype_of_with_hierarchy(&int_container, &hierarchy));
        assert!(!int_box.is_subtype_of_with_hierarchy(&float_container, &hierarchy));
        assert!(CoreType::Named("FloatBox".to_string())
            .is_subtype_of_with_hierarchy(&float_container, &hierarchy));
    }

    /// Issue #8560: membership in the built-in abstract families is derived
    /// from *declared* supertypes (the registered `<:` chain), not per-family
    /// Rust arms. User and bundled-package types reach `AbstractSet`,
    /// `AbstractRange`/`AbstractUnitRange`, `Function`, `IO`,
    /// `AbstractString`, and `AbstractChar` through their declaration chain
    /// exactly as upstream Julia resolves them (verified against
    /// `julia --startup-file=no` 1.12.6).
    #[test]
    fn declared_supertype_chain_derives_builtin_abstract_families_issue_8560() {
        let mut hierarchy = StructHierarchy::new();
        // The boot.jl built-in abstract declarations relevant here (the range
        // abstracts enter AbstractVector through the Rust array-family tables).
        hierarchy.insert("AbstractRange", Some("Any".to_string()), Vec::new());
        hierarchy.insert(
            "OrdinalRange",
            Some("AbstractRange".to_string()),
            Vec::new(),
        );
        hierarchy.insert(
            "AbstractUnitRange",
            Some("OrdinalRange".to_string()),
            Vec::new(),
        );
        // User / bundled-package declarations.
        hierarchy.insert(
            "KeySet",
            Some("AbstractSet{K}".to_string()),
            vec!["K".to_string()],
        );
        hierarchy.insert(
            "Fix1",
        ...
        hierarchy.insert("MyFunctorKind", Some("Function".to_string()), Vec::new());
        hierarchy.insert("MyFunctor", Some("MyFunctorKind".to_string()), Vec::new());
        hierarchy.insert("MyIO", Some("IO".to_string()), Vec::new());
        hierarchy.insert("MyStr", Some("AbstractString".to_string()), Vec::new());
        hierarchy.insert("MyChar", Some("AbstractChar".to_string()), Vec::new());
        hierarchy.insert(
            "MyRange",
            Some("AbstractRange{Int64}".to_string()),
            Vec::new(),
        );
        hierarchy.insert(
            "MyUnitRange",
            Some("AbstractUnitRange{Int64}".to_string()),
            Vec::new(),
        );

        let st = |l: &str, r: &str| {
            CoreType::from_julia_name(l)
                .is_subtype_of_with_hierarchy(&CoreType::from_julia_name(r), &hierarchy)
        };

        // AbstractSet family (KeySet is Base's own bundled declaration).
        assert!(st("KeySet{Int64}", "AbstractSet"));
        assert!(st("KeySet{Int64}", "AbstractSet{Int64}"));
        assert!(!st("KeySet{Int64}", "AbstractDict"));
        // Functor structs are `<: Function`, including through an intermediate
        // user abstract; `typeof(f)` singleton membership is unchanged.
        assert!(st("Fix1{typeof(sin), Int64}", "Function"));
        assert!(st("MyFunctor", "Function"));
        assert!(st("typeof(sin)", "Function"));
        assert!(!st("MyIO", "Function"));
        // IO / string / char families.
        assert!(st("MyIO", "IO"));
        assert!(st("MyStr", "AbstractString"));
        assert!(st("MyChar", "AbstractChar"));
        assert!(!st("MyStr", "AbstractChar"));
        assert!(!st("MyChar", "AbstractString"));
        // Range families: user range types resolve through their declaration,
        // reach the array families through the parameter-substituting walk,
        // and the directional lattice is preserved (`AbstractRange` is NOT
        // `<: AbstractUnitRange`; `LogRange` stays out of `AbstractRange`).
        assert!(st("MyRange", "AbstractRange"));
        assert!(st("MyRange", "AbstractVector"));
        assert!(st("MyRange", "AbstractArray"));
        assert!(!st("MyRange", "OrdinalRange"));
        assert!(!st("MyRange", "AbstractUnitRange"));
        assert!(!st("MyRange", "AbstractMatrix"));
        assert!(!st("MyRange", "DenseArray"));
        assert!(st("MyUnitRange", "AbstractUnitRange"));
        assert!(st("MyUnitRange", "OrdinalRange"));
        assert!(st("MyUnitRange", "AbstractRange"));
        assert!(st("MyUnitRange", "AbstractVector"));
        assert!(!st("LogRange{Float64}", "AbstractRange"));
        // `Core.Builtin` stays pinned to genuine builtin singletons: no
        // declared chain can mint one.
        assert!(!st("MyFunctor", "Core.Builtin"));
        assert!(!st("Fix1{typeof(sin), Int64}", "Core.Builtin"));
    }
    ...
    #[test]
    fn bare_parametric_family_matches_existential_parent_issue_5615() {
        let mut hierarchy = StructHierarchy::new();
        hierarchy.insert("Wrapper5615", None, vec!["S".to_string()]);
        hierarchy.insert(
            "MyVec5615",
            Some("Wrapper5615{T}".to_string()),
            vec!["T".to_string()],
        );

        let bare = CoreType::Named("MyVec5615".to_string());
        assert!(bare.is_subtype_of_with_hierarchy(
            &CoreType::from_julia_name("Wrapper5615{S} where S"),
            &hierarchy
        ));
        assert!(!bare.is_subtype_of_with_hierarchy(
            &CoreType::from_julia_name("Wrapper5615{Real}"),
            &hierarchy
        ));
    }
    ...
    fn unionall_pattern_with_explicit_hierarchy_uses_supplied_parents() {
        ...
        let typevar = || CoreType::TypeVar(CoreTypeVar::unscoped("T"));
        let bounded_var = CoreTypeVar::with_bounds(
            "T",
            None,
            Some(Box::new(CoreType::Named("Animal".to_string()))),
        );
    ...
    fn canonicalize_signature_for_dedup_collapses_single_use_covariant_typevar() {
        let tv = |name: &str, ub: Option<CoreType>| {
            CoreType::TypeVar(CoreTypeVar::with_bounds(name, None, ub.map(Box::new)))
        };
        ...
        let var = |name: &str, ub: Option<CoreType>| {
            CoreTypeVar::with_bounds(name, None, ub.map(Box::new))
        };
        ...
        // Bounded invariant-nested use (`Vector{T}`) is preserved —
        // `Vector{T} where T<:Number != Vector{Number}`, so the type variable stays.
        let nested = unionall(
            var("T", Some(number)),
        ...
        // An unbounded schema whose whole parametric family is quantified is the
        // bare family upstream: `Vector{T} where T == Vector`.
        let vector_schema = unionall(
            var("T", None),
            CoreType::Tuple(vec![CoreType::Struct {
                name: "Vector".to_string(),
                params: vec![tv("T", None)],
        ...
        assert_eq!(
            vector_schema.canonicalize_signature_for_dedup(),
            CoreType::Tuple(vec![CoreType::Struct {
                name: "Vector".to_string(),
                params: Vec::new(),
            }])
        ...
        // Same for rank-unconstrained `Array{T} where T == Array`, fixing
        // runtime last-definition-wins between `f(::Array)` and
        // `f(::Array{T}) where T`.
        let array_schema = unionall(
            var("T", None),
            CoreType::Tuple(vec![CoreType::Struct {
                name: "Array".to_string(),
                params: vec![tv("T", None)],
        ...
        assert_eq!(
            array_schema.canonicalize_signature_for_dedup(),
            CoreType::Tuple(vec![CoreType::Struct {
                name: "Array".to_string(),
                params: Vec::new(),
            }])
        ...
        // Multiple unbounded parameters collapse together only when they all
        // belong to the same family schema.
        let pair_schema = unionall(
            var("K", None),
            unionall(
                var("V", None),
                CoreType::Tuple(vec![CoreType::Struct {
                    name: "Pair".to_string(),
                    params: vec![tv("K", None), tv("V", None)],
                }]),
            ),
        );
        assert_eq!(
            pair_schema.canonicalize_signature_for_dedup(),
            CoreType::Tuple(vec![CoreType::Struct {
                name: "Pair".to_string(),
                params: Vec::new(),
            }])
    ...
    fn direct_builtin_supertype_names_cover_reflection_families() {
        ...
        assert_eq!(
            CoreType::from_julia_name("OrdinalRange").direct_builtin_supertype_name(),
            Some("AbstractRange")
        );
        assert_eq!(
            CoreType::from_julia_name("AbstractUnitRange").direct_builtin_supertype_name(),
            Some("OrdinalRange")
        );
        assert_eq!(
    ...
    fn direct_builtin_subtype_names_follow_shared_parents() {
        ...
        assert_eq!(
            CoreType::from_julia_name("AbstractRange").direct_builtin_subtype_names(),
            Some(vec!["OrdinalRange", "StepRangeLen", "LinRange"])
        );
        assert_eq!(
            CoreType::from_julia_name("OrdinalRange").direct_builtin_subtype_names(),
            Some(vec!["AbstractUnitRange", "StepRange"])
        );
        assert_eq!(
            CoreType::from_julia_name("AbstractUnitRange").direct_builtin_subtype_names(),
            Some(vec!["UnitRange", "OneTo"])
        );
        assert_eq!(
    ...
    fn typejoin_uses_existing_supertype_when_available() {
        ...
        assert_eq!(
            CoreType::Primitive(CorePrimitive::Int64)
                .typejoin(&CoreType::Primitive(CorePrimitive::Float64)),
            CoreType::Abstract(CoreAbstract::Real)
        );

        let owner_a = CoreType::Struct {
            name: "OwnerA10460.Box10460".to_string(),
            params: vec![CoreType::Primitive(CorePrimitive::Int64)],
        };
        let owner_b = CoreType::Struct {
            name: "OwnerB10460.Box10460".to_string(),
            params: vec![CoreType::Primitive(CorePrimitive::Int64)],
        };
        assert_eq!(owner_a.typejoin(&owner_b), CoreType::Any);
    }
    ...
    #[test]
    fn diagonal_typeintersect_updates_bound_occurrences_issue_10460() {
        let diagonal = CoreType::from_julia_name("Tuple{T, T} where T<:Real");
        let actual = CoreType::from_julia_name("Tuple{Integer, Real}");
        let intersection = diagonal.type_intersect(&actual);

        let CoreType::UnionAll { var, body } = intersection else {
            panic!("expected a bounded diagonal UnionAll result");
        };
        assert_eq!(
            var.upper_bound.as_deref(),
            Some(&CoreType::Abstract(CoreAbstract::Integer))
        );
        let mut reference = var.clone();
        reference.lower_bound = None;
        reference.upper_bound = None;
        assert_eq!(
            body.as_ref(),
            &CoreType::Tuple(vec![
                CoreType::TypeVar(reference.clone()),
                CoreType::TypeVar(reference),
            ])
        );
    }

    #[test]
    fn diagonal_typeintersect_does_not_capture_same_name_rigid_var_issue_10460() {
        ...
        let free = JuliaType::RuntimeTypeVar {
            id: 104_601,
            name: "T".to_string(),
            lower_bound: Box::new(JuliaType::Bottom),
            upper_bound: Box::new(JuliaType::String),
        };
        let runtime_pattern = JuliaType::RuntimeUnionAll {
            var: Box::new(bound.clone()),
            body: Box::new(JuliaType::TupleOf(vec![free, bound])),
        };
        let pattern = CoreType::from_julia_type_preserving_owner(&runtime_pattern);

        let CoreType::UnionAll { var, body } = &pattern else {
            panic!("runtime conversion must preserve the UnionAll binder");
        };
        assert!(matches!(var.typevar_id(), CoreTypeVarId::Scoped(_)));
        let CoreType::Tuple(elements) = body.as_ref() else {
            panic!("runtime conversion must preserve the tuple body");
        };
        let [CoreType::TypeVar(free), CoreType::TypeVar(bound)] = elements.as_slice() else {
            panic!("runtime conversion must preserve both TypeVars");
        };
        assert_eq!(free.typevar_id(), CoreTypeVarId::Rigid(104_601));
        assert_eq!(bound.typevar_id(), var.typevar_id());

        assert_eq!(
            pattern.type_intersect(&CoreType::from_julia_name("Tuple{Integer, Real}")),
            CoreType::Bottom
        );
    }
    ...
    // NOTE: aot_static_type_bridge_reaches_same_numeric_core test moved to
    // the main subset_julia_vm crate with the aot bridge impl (Issue #8655).
}
