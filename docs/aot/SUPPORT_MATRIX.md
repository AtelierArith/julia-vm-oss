...
| Complex | 一部対応 | monomorphic Rust `Complex` は `Float64` field layout のみ。`Complex` / `Complex{Float64}` / `ComplexF64` の basic construction/add/sub/mul surface は対応。Julia global `im` は lowercase Rust const として出し、local `im` binding は通常の lexical shadowing に任せる (Issue #6966)。upstream に存在しない旧 `Complex64` alias は受け付けない (Issue #9695)。`Complex{Float32}` / `Complex{Int64}` など parameterized non-`Float64` arithmetic は parameterized Complex codegen まで diagnostic gate (Issue #6965) |
