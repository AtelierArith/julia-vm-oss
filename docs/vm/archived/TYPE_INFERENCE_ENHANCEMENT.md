...
> **Archive note (2026-06-11):** この設計計画は履歴として保持します。
...
現在の型推論は `compile/expr/infer.rs` と `compile/inference.rs` に実装されている：

```rust
// compile/expr/infer.rs - 式の型推論
...
// compile/inference.rs - 関数返り値の型推論
...
| 課題 | 説明 | 影響 |
|------|------|------|
| **Any への過度なフォールバック** | 型が決定できない場合すぐに `Any` になる | 最適化機会の損失 |
| **ループ変数の型推論欠如** | `for x in arr` の `x` が常に `Any` | 配列操作の型情報損失 |
| **関数間型伝播なし** | 呼び出し先の返り値型が不明 | メソッドチェーンで型情報損失 |
| **条件分岐での型絞り込み欠如** | `if x isa Int` 後も `x` の型が不変 | 型ガード無効 |
| **Union 型の簡易実装** | Union 型の適切な結合・簡約がない | 型情報の不正確さ |
