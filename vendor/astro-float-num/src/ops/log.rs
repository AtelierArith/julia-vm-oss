...
use crate::defs::Sign;
use crate::defs::ZIV_REFINEMENT_EXTRA_BOUND;
...
impl BigFloatNumber {
    ...
    pub fn ln(&self, p: usize, rm: RoundingMode, cc: &mut Consts) -> Result<Self, Error> {
        ...
        loop {
            ...
            // VENDOR PATCH (#6794): bound the Ziv refinement so `ln` of a
            // boundary value cannot loop forever; round-to-nearest the best
            // approximation (still at working precision because
            // `try_set_precision` leaves it unrounded when it cannot prove the
            // rounding).
