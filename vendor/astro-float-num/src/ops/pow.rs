...
use crate::{
    common::consts::ONE,
    defs::{Error, WORD_BIT_SIZE, WORD_SIGNIFICANT_BIT, ZIV_REFINEMENT_EXTRA_BOUND},
    num::BigFloatNumber,
    RoundingMode, Sign,
};
...
impl BigFloatNumber {
    ...
    pub fn exp(&self, p: usize, rm: RoundingMode, cc: &mut Consts) -> Result<Self, Error> {
        ...
        loop {
            ...
            // VENDOR PATCH (#6794): bound the Ziv refinement. On the
            // table-maker's dilemma (e.g. `exp(ln 2) = 2` from `BigFloat(4)^0.5`)
            // correct rounding can never be proven and this loop would run
            // forever. Once the working precision is far past the target,
            // round-to-nearest the best approximation (`ret` is still at the
            // working precision because `try_set_precision` leaves it unrounded
            // when it cannot prove the rounding); for a boundary value this
            // yields the exact result.
            if p_wrk >= p.saturating_add(ZIV_REFINEMENT_EXTRA_BOUND) {
    ...
    pub fn pow(
        ...
        match || -> Result<Self, Error> {
            loop {
                ...
                // VENDOR PATCH (#6794): bound the Ziv refinement for
                // `self^n = e^(n·ln self)`. When the result lands exactly on a
                // rounding boundary (e.g. `BigFloat(4)^0.5 = 2`), correct
                // rounding can never be proven and this loop would run forever.
                // Once the working precision is far past the target,
                // round-to-nearest the best approximation (still at working
                // precision); for a boundary value this yields the exact result.
                if p_wrk >= p.saturating_add(ZIV_REFINEMENT_EXTRA_BOUND) {
                    ret.set_precision(p, effective_rm)?;
                    ret.set_inexact(ret.inexact() | self.inexact() | n.inexact());
                    return Ok(ret);
                }
