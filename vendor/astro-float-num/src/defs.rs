...
/// VENDOR PATCH (AtelierArith/ailujsoi #6794): extra working-precision bound for
/// the Ziv correct-rounding refinement loops in `exp` / `ln`. When the result
/// sits exactly on a rounding boundary (the table-maker's dilemma, e.g.
/// `exp(ln 2) = 2` from `BigFloat(4)^0.5`), the loop can never prove correct
/// rounding and would run forever. Once the working precision exceeds the target
/// by this many bits, the loops accept the best value computed so far (already
/// rounded to the target precision), which is exact for the boundary cases and
/// at worst one ULP off for the (astronomically rare) genuinely-hard-to-round
/// non-exact value. This makes those operations always terminate.
///
/// 512 extra bits: large enough that any genuinely round-able value converges
/// first (correct rounding almost always needs only a few guard words; a
/// non-exact value within 2^-768 of a boundary is astronomically unlikely), and
/// small enough that the boundary (table-maker's-dilemma) cases bail quickly
/// instead of refining to many thousands of bits.
pub const ZIV_REFINEMENT_EXTRA_BOUND: usize = 512;
